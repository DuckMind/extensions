...
- Direct revival takes an exclusive cross-process lease on the canonical session file until the new child finishes. A concurrent attempt fails before DM is spawned and identifies the owning revived run; dead-owner leases are reclaimed only when staleness can be proved.
...
`steer` waits up to three seconds for a correlated child-DM input acceptance and returns a request id with `delivered`, `scheduled`, `pending`, `partial`, `recovered`, or `failed` plus per-child states. The receipt also has `deliveryStatus: "delivered" | "queued"`. Delivery means DM accepted the user message, not model compliance. A pending indexed child returns `scheduled`.
...
Herdr project panes are peer DM sessions opened by this DM session:
...
Intentionally unsupported: native DM child options such as model override, structured output, acceptance/agent contract, tool budgets, fast mode, fork context, skills, or native DM tools unless the runner explicitly implements them. Foreground/clarify, steer/resume/interrupt-as-pause, nested subagents, and fallback models are also unsupported.
