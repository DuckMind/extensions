...
An agent is a markdown file: YAML frontmatter on top, a system prompt below. The frontmatter defines the specialist that runs in the child DM process.
...
| Scope | Path |
|-------|------|
| Builtin | `~/.dm/agent/extensions/subagent/agents/` |
| Installed package | `package.json` `dm-subagents.agents` or `dm.subagents.agents` |
| User | `~/.dm/agent/agents/**/*.md` |
| Project | Project config `agents/**/*.md` (`.dm/agents/**/*.md` in standard DM) |
...
- Installed DM packages can expose agent directories from either `{"dm-subagents":{"agents":["./agents"]}}` or `{"dm":{"subagents":{"agents":["./agents"]}}}` in their package manifest. Package agents load above builtins and below user/project agents.
...
Builtins load at the lowest priority, so a user or project agent with the same name overrides them. They do not pin a provider model; they inherit your current DM default model unless you set `subagents.defaultModel` or `subagents.agentOverrides.<name>.model` (see [models.md](models.md)).
...
When `surf-cli` is installed and loaded, Surf can expose a `gpt-pro` package agent through the `surf-oracle` external-job provider. It starts through the same `subagent({ agent: "gpt-pro" })` mental model as any other agent, but Surf owns the package agent and provider. Surf maps `model: pro` to ChatGPT GPT-5.6 Sol Pro web mode. dm-subagents does not own that model mapping.
...
The DM async run remains the source of truth for status, artifacts, wake/wait, mission attachment, retention, and diagnostics.

### Advisory runner data boundary

External CLI agents use their own runner contract. Do not pass native DM child options such as model override, structured output, acceptance/agent contract, tool budgets, fast mode, fork context, skills, or native DM tools unless the adapter explicitly implements them.
...
The adapter validates `codex --version` and `codex exec --help` only when a run launches. Discovery, list, status, and native DM launches do not probe Codex. JSONL, stderr, and stdout are untrusted. A run succeeds only after bounded valid JSONL contains one `turn.completed` event and the bounded final-message artifact is present.

Maintainers can collect real smoke evidence without making it part of the normal test suite:

```bash
DM_SUBAGENTS_CODEX_EXEC_SMOKE=1 \
DM_SUBAGENTS_CODEX_EXEC_SMOKE_REPORT=/tmp/dm-subagents-codex-exec-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
  --test test/integration/codex-exec-smoke.test.ts

DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE=1 \
DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE_REPORT=/tmp/dm-subagents-codex-exec-writer-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
...
The adapter validates `claude --version` and `claude --help` only when a run launches. Discovery, list, status, and native DM launches do not probe Claude Code or authentication. JSONL, stderr, and stdout are untrusted. A run succeeds only after bounded valid JSONL contains exactly one successful terminal `result` with non-empty final text. Missing or revoked local authentication, limit stops, malformed JSON, duplicate terminal results, and EOF before a terminal result fail closed.

Maintainers can opt in to separate read-only and writer canaries:

```bash
DM_SUBAGENTS_CLAUDE_CODE_SMOKE=1 \
DM_SUBAGENTS_CLAUDE_CODE_SMOKE_REPORT=/tmp/dm-subagents-claude-code-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
  --test test/integration/claude-code-smoke.test.ts

DM_SUBAGENTS_CLAUDE_CODE_WRITER_SMOKE=1 \
DM_SUBAGENTS_CLAUDE_CODE_WRITER_SMOKE_REPORT=/tmp/dm-subagents-claude-code-writer-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
...
Launch preflight validates `cursor-agent --version` and `cursor-agent --help` only when a run starts. Discovery, list, status, and native DM launches do not execute Cursor or probe authentication. A run succeeds only when bounded valid JSONL ends with one successful `result` event that has non-empty final text. Error events, failed results, malformed JSON, output after the terminal event, and EOF before a result fail closed.
...
```bash
export DM_SUBAGENTS_CURSOR_SMOKE_WORKSPACE=/tmp/dm-subagents-cursor-smoke-workspace
export DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT=/tmp/dm-subagents-cursor-smoke-state
export DM_SUBAGENTS_CURSOR_SMOKE_DISPOSABLE=1
mkdir -p "$DM_SUBAGENTS_CURSOR_SMOKE_WORKSPACE" "$DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT"
mkdir -p "$DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT/external-0.cursor-prompt"
```

Do not place a file at `dm-subagents-cursor-write-canary.txt` in the workspace or any file, including `handoff.txt`, in the prompt directory. The harness refuses the pre-existing canary and any non-empty prompt directory. It does not delete the workspace, state root, or operator-owned prompt directory. It removes only its canary and private handoff file.

Maintainers can then run separate read-only and writer canaries:

```bash
DM_SUBAGENTS_CURSOR_AGENT_SMOKE=1 \
DM_SUBAGENTS_CURSOR_AGENT_SMOKE_REPORT=/tmp/dm-subagents-cursor-agent-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
  --test test/integration/cursor-agent-smoke.test.ts

DM_SUBAGENTS_CURSOR_AGENT_WRITER_SMOKE=1 \
DM_SUBAGENTS_CURSOR_AGENT_WRITER_SMOKE_REPORT=/tmp/dm-subagents-cursor-agent-writer-smoke.json \
node --experimental-strip-types --import ./test/support/register-loader.mjs \
...
Native `oracle` runs inside DM and can use its configured read tools. The Claude profiles send the assembled prompt to the local Claude Code CLI through stdin. An external-job agent sends the assembled prompt to its registered provider. Provider options and a prompt digest are persisted in DM run state. The prompt text is delivered through the local host bridge to the provider and is not stored in the public result payload. Do not place secrets in advisory prompts unless the target provider is approved to receive them.
...
| Provider artifact path | External provider | provider-defined terminal artifact | Provider returns `artifactPath`, or DM writes returned text to `external-job-<index>.result.md` | Provider reports failure or no result | Existing artifact path is retained in `status.json` | Missing artifact with no text output returns a terminal message instead of inventing content |

The `researcher` builtin uses `web_search`, `fetch_content`, and `get_search_content`. Those require [dm-web-access](https://github.com/nicobailon/dm-web-access):

```bash
dm install npm:dm-web-access
```
...
- User: `~/.dm/agent/settings.json`
- Project: project config settings file (`.dm/settings.json` in standard DM)
...
- Use `tools: "inherit"` on a builtin when that one role should omit its bundled tool allowlist and receive DM's normal builtins and ambient extensions. This keeps strict tools as the default for other builtins.
...
Subagents are narrow by default. Custom agents start with a clean system prompt and only the context you intentionally give them. They do not automatically inherit DM's whole base prompt, project instruction files, or discovered skills catalog.

Use these fields when an agent should see more:

| Field | Effect |
|-------|--------|
| `systemPromptMode: append` | Append the agent prompt to DM's normal base prompt. |
| `inheritProjectContext: true` | Keep inherited repository instructions from files like `AGENTS.md` and `CLAUDE.md`. |
| `inheritGlobalContext: true` | Also keep the operator's global context file from the DM config agent directory (such as `~/.dm/agent/AGENTS.md`). Defaults to `false`. |
| `inheritSkills: true` | Let the child see DM's discovered skills catalog. |
...
| `tools` | Strict child tool allowlist. Named extension tools must also have their provider loaded. `mcp:` entries select direct MCP tools when `dm-mcp-adapter` is installed. |
...
| `subagentOnlyExtensions` | Extension paths loaded only in spawned child sessions for this agent. Tools registered there are unavailable to the main agent unless also installed through normal DM extension configuration. |
...
| `systemPromptMode` | `replace` by default; `append` keeps DM's base prompt. |
| `inheritProjectContext` | Keeps or strips inherited repository instruction blocks. |
| `inheritGlobalContext` | Keeps or strips the operator's global context file from the DM config agent directory (e.g. `~/.dm/agent/AGENTS.md`). It has an effect only when `inheritProjectContext` is `true`; otherwise all context files are already disabled. Defaults to `false`. |
| `inheritSkills` | Keeps or strips DM's discovered skills catalog. |
...
This is independent of DM's own parent/session/project memory system and writes nothing to it. Memory lives under a dedicated `agent-memory/` namespace so the two never collide.
...
- Project: resolves under `<project>/.dm/agent-memory/<path>` and travels with the repo.
- User: resolves under `~/.dm/agent/agent-memory/<path>` and is shared across projects for that agent.
...
- The accepted overlay is stored at `.dm/subagents/refinements/<agent>.md` with revision metadata and snapshots. Each `refine` or `refine.rollback` adds a snapshot, and `refine.rollback` restores the previous revision.
- At launch, the current overlay is injected into that agent's child system prompt as a `<dm-subagents-refinement>` block scoped to this project. The base agent definition is never modified.
...
- `tools` omitted: `dm-subagents` does not pass `--tools`, so the child gets DM's normal builtin tools.
...
An allowlisted name does not load the extension that registers it. Load that provider through normal DM extension discovery, `extensions`, `subagentOnlyExtensions`, or a path-like `tools` entry.
...
Direct MCP tools require [dm-mcp-adapter](https://github.com/nicobailon/dm-mcp-adapter). Subagents only receive direct MCP tools when `mcp:` entries are listed in their frontmatter; global `directTools: true` in `mcp.json` is not enough by itself. The generic `mcp` proxy tool can still be used for discovery when available. The adapter caches tool metadata at startup, so after connecting a new MCP server for the first time, restart DM before relying on direct tools. Server `includeTools` and `excludeTools` policies are enforced while resolving cached metadata for children: both accept exact names and `*`/`?` glob patterns against raw, generated-resource, and server/short/none-prefixed names, with `excludeTools` taking precedence. An `mcp:` entry named `subagent` does not authorize nested fanout; declare the builtin `subagent` tool or set `allowNestedSubagents: true`. If a resolved direct MCP name is missing from the child registry, dm-subagents keeps the launch failed under the strict allowlist and identifies the condition as a host/dm-mcp-adapter registration problem; verify that the adapter registers the selected tools before child startup.
...
When `extensions` is present, normal discovered extensions are disabled. The listed extensions, path-like `tools` entries, required dm-subagents runtime extensions, and `subagentOnlyExtensions` still load.
...
1. Project config `skills/{name}/SKILL.md` (`.dm/skills/{name}/SKILL.md` in standard DM)
2. Project packages and project settings packages via `package.json -> dm.skills`
3. Current task cwd package via `package.json -> dm.skills`
4. Project config `settings.json -> skills`
5. `~/.dm/agent/skills/{name}/SKILL.md`
6. User packages and user settings packages via `package.json -> dm.skills`
7. `~/.dm/agent/settings.json -> skills`
...
Agent-local `skillPath` candidates never enter DM's parent/global skills catalog. Pair `inheritSkills: false` with explicit `skills` and `skillPath` when a child should receive only its selected private skills.

## The bundled dm-subagents skill

The package bundles a `dm-subagents` skill that is automatically available to the parent agent when the extension is installed. It is for the orchestrating parent only: child subagents never receive it, and their context is explicitly filtered to strip parent-only orchestration instructions.
