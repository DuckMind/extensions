...
Builtin agents inherit your current DM default model. This keeps new installs from depending on a provider you may not have configured. From there you can layer defaults and overrides:
...
In `~/.dm/agent/settings.json` (user) or the project config settings file (`.dm/settings.json` in standard DM; project wins):
...
Set `subagents.maxThinking` to enforce a hard maximum for every native DM child. The supported levels, from least to most thinking, are `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`:
...
- Absent: preserves DM's normal ambient extension discovery.
...
To see what `dm-subagents` has actually loaded right now:
...
That reports the live runtime mapping, which can differ from settings on disk until you reload DM.
...
Registry ids that themselves contain `/` (Hugging Face `owner/name`) resolve the same way as DM's main agent: `thinkingmachines/Inkling` becomes `huggingface/thinkingmachines/Inkling` when that id is unique or offered by the current session provider. A first path segment that matches a registered provider still means `provider/id`.
...
```text
~/.dm/agent/profiles/dm-subagents/
```

Provider model catalogs are cached under:

```text
~/.dm/agent/profiles/dm-subagents/providers/
```
