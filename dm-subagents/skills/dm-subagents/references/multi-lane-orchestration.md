...
Use this reference when several independent tasks need coordinated workers, worktrees, or repositories. It defines lane ownership; use the other dm-subagents references for run controls, prompts, and mission details. The parent remains the final decision-maker.
...
For DM extension repositories, keep lane worktrees outside auto-discovered extension directories such as `~/.dm/agent/extensions`. A stale extension worktree there can auto-load duplicate tools and shortcuts. Remove or move it only after its handoff is durable, the worktree is clean, and no run owns it.
