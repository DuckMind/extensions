# DM Subagents: Execution Controls

This file is a detailed reference loaded from `skills/dm-subagents/SKILL.md`.
...
Project settings resolve from the nearest parent directory containing `.dm` or `.agents` by default. In monorepos or git worktrees where an incidental nested `.dm` directory should not shadow the repository config, set `subagents.projectRootResolution: "git-root"` in the repository root `.dm/settings.json`; a nested project can opt back with `"nearest"` in its own settings.

## Running Subagents

### External CLI profiles

An agent may set `runner.type: external-cli` with a non-empty `command`, optional string `args`, and `promptDelivery: stdin` (the default). The command runs with `shell: false`, inherits the resolved cwd and environment, and receives the combined agent instructions and task through stdin. It must already be installed; dm-subagents adds no CLI dependency.

External CLI profiles are async-only and one-shot. They support lifecycle artifacts, stdout/stderr logs, timeout, and stop. Full stdout and stderr are retained in their log files, while the final stdout response and stderr error kept in memory are each limited to their last 64 KiB. They do not support native DM child options such as model override, structured output, acceptance/agent contract, tool budgets, fast mode, fork context, skills, or native DM tools unless the runner explicitly implements them. Foreground/clarify, steer/resume/interrupt-as-pause, nested subagents, fallbacks, and sessions are also unsupported.

### External job profiles

An agent may set `runner.type: external-job` with a non-empty `provider` and optional JSON `options`. When `surf-cli` is installed and loaded, Surf can optionally expose a `gpt-pro` package agent through provider `surf-oracle`. Surf maps `model: pro` to its configured pro web mode. dm-subagents does not own that package agent or model mapping. Remove any old `agentOverrides.gpt-pro.disabled` workaround before using Surf's package agent. The provider must be registered in the host DM process through `dm-subagents/external-job-provider`; the async runner talks to that parent-owned registry through a local operation bridge.

External job profiles are async-only. The provider owns the remote job and DM owns the async run record. Status persists provider name, provider job id, prompt digest, provider options, handle/conversation URLs when supplied, result artifact path, last known state, and provider failure code/message. Recovery uses existing provider job metadata to call `reattach` and `result`; it refuses to redispatch a prompt when the persisted provider job does not match the prompt digest.

External job profiles do not support foreground/clarify, steer/resume, DM models/tools/extensions/skills, tool budgets, structured output, native child permissions, fallbacks, or DM child sessions. Capacity conflicts fail closed and include the blocking provider job id when the provider supplies it.
...
If `runs.all` is missing in a running session, reload or update `dm-subagents` before retrying. The current runtime supports `runs.all`; `await Promise.all([runs.run(...)])` is also supported for advanced dynamic fanout.
...
Do not end your turn immediately after launching an async child if you promised to keep working. Continue the local inspection, synthesis, or validation prep, then check the async run when its result is needed. If no safe independent work remains, return control and let DM wake the session; do not convert the child to foreground.

In an ordinary interactive chat, normally return control after launching or
triaging useful async work and let DM wake the session on completion; ordinary
...
Providers are discovered through the `dm-subagents/background-work` registry and
...
fleet. If `DM_SUBAGENT_WAIT_TOOL_ENABLED` disables blocking, direct waits return
...
- Direct revival holds an exclusive cross-process lease on the canonical child session file until the new child finishes. Concurrent attempts fail before DM starts and identify the owning revived run; stale ownership is reclaimed only when the recorded process is demonstrably gone or reused.
...
A cooperating terminal runtime can register read-only external records through `dm-subagents/external-runs`. Records include the source, session, state, optional report path, and completion reason. They are observations only: dm-subagents does not start, stop, steer, or otherwise own the foreign process. Run unattended raw terminal agents in an explicit isolated cwd or worktree; do not use a live project checkout as disposable review space.

### Scheduled subagent runs

Schedules are durable project records under `.dm/subagents/schedules/`. They are enabled by default; set `{ "scheduledRuns": { "enabled": false } }` in `~/.dm/agent/extensions/subagent/config.json` to disable them. Only schedule explicit work the user asked for.
...
generated `.dm/subagents/` / temp artifacts do not trigger review. Writing children
...
such as `node_modules` may be absent. The clean-check ignores dm-subagents'
own `.dm/subagents/` runtime state, including default mission records, but still
rejects ordinary source/config changes. `dm-subagents` attempts to symlink the
...
`dm-subagents` includes native supervisor coordination. Child agents can use `contact_supervisor` to ask the exact parent session that spawned them; messages are scoped by parent session id and should not appear in other DM sessions. Parents inspect or reply with `subagent_supervisor`. This path does not require `dm-intercom`.
