...
const previousEnv = process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN;

afterEach(() => {
	for (const directory of directories.splice(0)) fs.rmSync(directory, { recursive: true, force: true });
	for (const directory of externalDirectories.splice(0)) fs.rmSync(directory, { recursive: true, force: true });
	if (previousEnv === undefined) delete process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN;
	else process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN = previousEnv;
});
...
describe("run fan-out budget", () => {
	it("resolves environment over config and falls back to 64", () => {
		delete process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN;
		...
		process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN = "7";
	...
	it("degrades status projection when persisted budget state is removed", () => {
		...
		const asyncDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-fanout-status-"));
	...
	it("keeps nested status projection when persisted budget state is removed", () => {
		...
		const asyncRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-fanout-nested-root-"));
	...
	it("fails closed when descriptor identity or managed-root containment is invalid", () => {
		...
		const outside = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-fanout-outside-"));
