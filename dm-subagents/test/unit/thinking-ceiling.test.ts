...
describe("thinking ceilings", () => {
	beforeEach(() => {
		home = fs.mkdtempSync(path.join(os.tmpdir(), "dm-thinking-ceiling-home-"));
		project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-thinking-ceiling-project-"));
	...
	afterEach(() => {
		...
		if (previousAgentDir === undefined) delete process.env.DM_CODING_AGENT_DIR; else process.env.DM_CODING_AGENT_DIR = previousAgentDir;
	...
	it("loads maxThinking with project precedence and attaches it to discovered agents", () => {
		writeJson(path.join(home, ".dm", "agent", "settings.json"), { subagents: { maxThinking: "max" } });
		fs.mkdirSync(path.join(project, ".dm"), { recursive: true });
		writeJson(path.join(project, ".dm", "settings.json"), { subagents: { maxThinking: "xhigh" } });
