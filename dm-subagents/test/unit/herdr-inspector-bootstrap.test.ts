...
describe("Herdr inspector bootstrap", () => {
	it("uses ordinary Node for the inspector pane command", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-bootstrap-command-"));
		try {
	...
	it("uses PATH node when DM is a standalone executable", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-bootstrap-standalone-"));
		const originalExecPath = process.execPath;
		try {
			writeCompletedRun(root);
			process.execPath = path.join(root, process.platform === "win32" ? "dm.exe" : "dm");
			const calls: string[][] = [];
			const client: HerdrClient = {
				run: async <T>(args: string[]) => {
					calls.push(args);
			...
			assert.doesNotMatch(command, /(?:^|[\\/])dm(?:\.exe)?'/);
		} finally {
	...
	it("starts the packaged bootstrap with ordinary Node", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-bootstrap-run-"));
		try {
			...
			assert.match(launched.stdout, /dm-subagents inspector for run-123/);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
