...
import { DM_SUBAGENT_PI_BINARY_ENV } from "../../src/runs/shared/dm-spawn.ts";
...
describe("Herdr inspector", () => {
	...
	it("opens one raw inspector pane, persists its binding, and closes only that pane", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-inspector-"));
		try {
			const { asyncDir } = writeRun(root);
			const sessionRoot = path.join(root, "sessions");
			const missionDir = path.join(root, "other-project", ".dm/subagents", "missions");
	...
	it("passes live parent-owned custom session roots to standalone inspectors", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-custom-session-"));
		try {
	...
	it("refuses direct run directories outside trusted roots", async () => {
		const trustedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-trusted-"));
		const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-outside-"));
		try {
	...
	it("renders mission context and reuses existing steer/stop control records", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-controls-"));
		try {
			const { asyncDir, status } = writeRun(root);
			...
			assert.throws(() => submitInspectorControl({ asyncDir, runId: "run-123", refreshMs: 1_500 }, "reply decision-1 yes"), /parent DM session/);
	...
	it("renders trusted child session fallback and refuses untrusted roots", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-session-"));
		try {
			const { asyncDir, status } = writeRun(root);
			const sessionRoot = path.join(root, "sessions");
	...
	it("opens, reports, and closes a project-owned Herdr pane", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm=herdr project pane-"));
		const previousPiBinary = process.env[DM_SUBAGENT_PI_BINARY_ENV];
		process.env[DM_SUBAGENT_PI_BINARY_ENV] = path.join(root, "dm-bin");
		try {
			...
			const client: HerdrClient = {
				run: async <T>(args: string[]) => {
					...
					if (args[0] === "pane" && args[1] === "get") return { ok: true, data: { pane: {
						pane_id: "w1:p10", agent: "dm", agent_status: "idle", cwd: projectRoot,
						foreground_cwd: projectRoot, focused: false, terminal_title_stripped: "DM · project",
					} } as T };
					return { ok: true, data: {} as T };
				},
			};
			...
			fs.writeFileSync(path.join(ownerRoot, ".dm/subagents/project-panes/herdr-roots.json"), JSON.stringify({ schemaVersion: 1, kind: "herdr-project-pane-roots", projectRoots: [123] }));
			assert.throws(() => listHerdrProjectPaneRoots(ownerRoot), /Invalid Herdr project pane root index/);
		} finally {
			if (previousPiBinary === undefined) delete process.env[DM_SUBAGENT_PI_BINARY_ENV];
			else process.env[DM_SUBAGENT_PI_BINARY_ENV] = previousPiBinary;
			fs.rmSync(root, { recursive: true, force: true });
		}
	});

	it("fails closed when an idle-only project pane close sees active work", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-project-pane-busy-"));
		try {
			const projectRoot = fs.realpathSync(root);
			const calls: string[][] = [];
			let opened = false;
			const client: HerdrClient = {
				run: async <T>(args: string[]) => {
					...
					if (args[0] === "pane" && args[1] === "get" && opened) return { ok: true, data: { pane: {
						pane_id: "w1:p11", agent: "dm", agent_status: "working", cwd: projectRoot,
					} } as T };
					return { ok: true, data: {} as T };
				},
			};
	...
	it("preserves legacy model-facing recovery for transient and opaque pane inspection results", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-project-pane-legacy-"));
		try {
			let splitCount = 0;
			...
			fs.writeFileSync(path.join(root, ".dm/subagents", "project-panes", "herdr.json"), JSON.stringify({ ...legacyBinding, startupMessage: 42 }));
	...
	it("rejects project pane targets that are not directories", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-herdr-project-pane-missing-"));
		try {
