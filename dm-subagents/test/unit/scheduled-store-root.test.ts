...
const AGENT_DIR_ENV = "DM_CODING_AGENT_DIR";
const HOME_ENV = "HOME";
...
beforeEach(() => {
	...
	agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-schedule-store-root-"));
	homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-schedule-home-"));
...
describe("config.scheduledRuns.storeRoot", () => {
	it("accepts an absolute path and expands a ~/ path", () => {
		const absolute = path.join(os.tmpdir(), "dm-subagents-schedules");
		assert.equal(resolveScheduledStoreRoot(absolute), path.normalize(absolute));
		assert.equal(resolveScheduledStoreRoot("~/dm-subagents/schedules"), path.join(homeDir, "dm-subagents", "schedules"));
	});
	...
	it("preserves a configured store root", () => {
		const storeRoot = path.join(os.tmpdir(), "dm-subagents-schedules");
