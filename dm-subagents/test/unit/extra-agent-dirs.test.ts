...
const MANAGED_ENV = ["DM_CODING_AGENT_DIR", "HOME", "USERPROFILE", EXTRA_AGENT_DIRS_ENV];
...
describe("DM_SUBAGENT_EXTRA_AGENT_DIRS discovery", () => {
	beforeEach(() => {
		for (const key of MANAGED_ENV) saved[key] = process.env[key];
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-extra-agent-dirs-"));
