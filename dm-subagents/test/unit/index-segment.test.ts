...
describe("bounded index segments", () => {
	...
	it("hashes Windows session file paths that stay under the byte limit", () => {
		const sessionId = String.raw`C:\Users\theap\.dm\agent\sessions\--C--Users-theap--\2026-08-17T07-29-51-353Z_01a00ea0-72f9-754f-82ca-2b74ded94746.jsonl`;
	...
	it("indexes and releases active runs with oversized provider tool-call ids", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-active-index-long-tool-call-"));
		try {
	...
	it("keeps the authoritative marker when optional tool-call indexing fails", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-active-index-optional-alias-"));
