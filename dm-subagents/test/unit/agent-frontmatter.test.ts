...
import { buildPiArgs } from "../../src/runs/shared/dm-args.ts";
...
function createNestedLinkedWorktree(): { repo: string; worktree: string } {
	const repo = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-linked-worktree-"));
	tempDirs.push(repo);
...
function writeLinkedWorktreePackage(repo: string): { packageAgentName: string; packageAgentPath: string; settingsPath: string } {
	...
	const packageRoot = path.join(repo, ".dm", "vendor", "workflow");
	const packageAgentPath = path.join(packageRoot, "agents", "reviewer.md");
	const settingsPath = path.join(repo, ".dm", "settings.json");
...
function withTempHome<T>(fn: (home: string) => T): T {
	const home = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-package-home-"));
	tempDirs.push(home);
	const oldHome = process.env.HOME;
	const oldUserProfile = process.env.USERPROFILE;
	const oldPiCodingAgentDir = process.env.DM_CODING_AGENT_DIR;
	const oldExtraAgentDirs = process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
	process.env.HOME = home;
	process.env.USERPROFILE = home;
	delete process.env.DM_CODING_AGENT_DIR;
	delete process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
	try {
		return fn(home);
	} finally {
		...
		if (oldPiCodingAgentDir === undefined) delete process.env.DM_CODING_AGENT_DIR;
		else process.env.DM_CODING_AGENT_DIR = oldPiCodingAgentDir;
		if (oldExtraAgentDirs === undefined) delete process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
		else process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS = oldExtraAgentDirs;
	}
}
...
describe("agent definition directory inspection", () => {
	it("distinguishes absent, empty, candidates, unreadable, and non-directory paths", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-inspection-"));
		tempDirs.push(root);
	...
	it("reports cached builtin and configured project definition paths without synthesizing a root", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-report-"));
		tempDirs.push(project);
		writeJson(path.join(project, ".dm", "settings.json"), { subagents: { projectRootResolution: "nearest" } });
		const discovered = discoverAgents(project, "project");
		assert.ok(discovered.directories.some((entry) => entry.source === "builtin"));
		assert.deepEqual(discovered.directories.filter((entry) => entry.source === "project").map((entry) => entry.path), [path.join(project, ".agents"), path.join(project, ".dm", "agents")]);
	}));
});
...
describe("agent runner frontmatter", () => {
	it("parses and serializes an external-cli runner", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-runner-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "external.md"), `---
name: external
...
Review carefully.`.replace(" description:", "description:"));
	...
	it("keeps external-cli stop code-owned while refusing capability widening", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-runner-capabilities-"));
		tempDirs.push(project);
		const agentPath = path.join(project, ".dm", "agents", "external.md");
	...
	it("parses and serializes an external-job runner", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-external-job-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "gpt-pro.md"), `---
name: gpt-pro
...
Review carefully.`);
	...
	it("rejects invalid and DM-only external runner fields", () => withTempHome(() => {
		const invalidCases = [
			"type: unknown\n  command: node",
			"type: external-cli\n  command: ''",
			"type: external-cli\n  command: node\n  args: nope",
		...
		for (const [index, runner] of invalidCases.entries()) {
			const project = fs.mkdtempSync(path.join(os.tmpdir(), `dm-subagents-invalid-runner-${index}-`));
			tempDirs.push(project);
		...
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-runner-dm-only-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "external.md"), `---\nname: external\ndescription: External\nrunner:\n  type: external-cli\n  command: node\nmodel: provider/model\n---\nBody`);
		assert.match(discoverAgents(project, "project").agentDiagnostics?.[0]?.error ?? "", /unsupported DM-only fields: model/);
	}));

	it("keeps valid agents executable when another agent is malformed", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-invalid-agent-isolation-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "broken.md"), "---\nname: broken\ndescription: Broken\nrunner:\n  type: unknown\n---\nBody");
		writeAgent(path.join(project, ".dm", "agents", "working.md"), "---\nname: working\ndescription: Working\n---\nBody");
	...
	it("follows symlinked agent directories once without hiding diagnostics", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-symlinked-agent-dir-"));
		tempDirs.push(project);
	...
	it("keeps a lower-priority agent available when a project override is malformed", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-invalid-agent-shadow-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "reviewer.md"), "---\nname: reviewer\ndescription: Broken reviewer\nrunner:\n  type: unknown\n---\nBody");
	...
	it("records the runtime name for malformed packaged agents", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-invalid-packaged-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "code-analysis.zeta-worker.md"), "---\nname: zeta-worker\npackage: code-analysis\ndescription: Broken packaged worker\nrunner:\n  type: unknown\n---\nBody");
...
describe("agent skillPath frontmatter", () => {
	it("parses and serializes comma-separated paths", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-skill-path-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
...
describe("agent aliases", () => {
	it("parses and serializes agent aliases", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-alias-agent-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
...
body`);
	...
	it("reports management alias collisions as ambiguous", () => withTempHome(() => {
		const project = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-alias-collision-"));
		tempDirs.push(project);
		writeAgent(path.join(project, ".dm", "agents", "review-agent.md"), `---
name: review-agent
...
describe("agent simple-scalar list frontmatter", () => {
	it("discovers newline block lists for all list fields and routes MCP tools", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-block-list-frontmatter-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
tools:
  - read-only
...
`);
		...
		assert.deepEqual(worker?.extensions, [path.join(dir, ".dm", "agents", "extension-one.ts"), path.join(dir, ".dm", "agents", "extension-two.ts")]);
		assert.deepEqual(worker?.subagentOnlyExtensions, [path.join(dir, ".dm", "agents", "child-only.ts"), path.join(dir, ".dm", "agents", "child-helper.ts")]);
	});

	it("preserves MCP-only tools as an explicit empty builtin allowlist", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-mcp-only-frontmatter-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".dm", "agents", "mcp-only.md"), `---
name: mcp-only
...
`);
	...
	it("preserves comma-separated syntax across all list fields", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-comma-list-frontmatter-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
...
describe("agent permission frontmatter", () => {
	it("parses and preserves explicit non-bash permission rules", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-permission-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
...
`, "utf-8");
	...
	it("rejects bash permission rules and conflicting aliases", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-permission-invalid-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---\nname: worker\ndescription: Worker\npermission:\n  bash: deny\n---\n`, "utf-8");
		assert.match(discoverAgents(dir, "project").agentDiagnostics?.[0]?.error ?? "", /dm-guard/);
...
describe("agent frontmatter defaultContext", () => {
	...
	it("parses defaultContext from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-default-context-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
`, "utf-8");
	...
	it("loads packaged worker and oracle with fork defaultContext and advisor alias", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-default-context-"));
		tempDirs.push(dir);
	...
	it("keeps bundled agent definitions from module load during package file updates", () => {
		const fixture = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-hot-update-"));
...
describe("agent frontmatter launch defaults", () => {
	it("parses, serializes, and validates outputMode defaults", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-output-mode-"));
		tempDirs.push(dir);
		const filePath = path.join(dir, ".dm", "agents", "worker.md");
		writeAgent(filePath, `---
name: worker
description: Worker
...
`);
	...
	it("serializes and discovers single-agent launch defaults", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-launch-defaults-"));
		tempDirs.push(dir);
		const filePath = path.join(dir, ".dm", "agents", "worker.md");
		const agent: AgentConfig = {
			name: "worker",
	...
	it("parses scalar acceptance defaults and rejects invalid policies", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-acceptance-defaults-"));
		tempDirs.push(dir);
		const filePath = path.join(dir, ".dm", "agents", "worker.md");
		writeAgent(filePath, `---
name: worker
...
`);
	...
	it("parses, serializes, and validates acceptance roles", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-acceptance-role-"));
		tempDirs.push(dir);
		const filePath = path.join(dir, ".dm", "agents", "explorer.md");
		writeAgent(filePath, `---
name: explorer
...
`);
	...
	it("rejects invalid launch defaults instead of silently ignoring them", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-invalid-launch-defaults-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
...
`);
	...
	it("rejects oversized toolTimeoutMs frontmatter at discovery", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-oversized-tool-timeout-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".dm", "agents", "worker.md"), `---
name: worker
description: Worker
...
describe("chain discovery", () => {
	it("prefers same-scope .chain.json over .chain.md for the same runtime name", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-chain-format-precedence-"));
		tempDirs.push(dir);
		const chainsDir = path.join(dir, ".dm", "chains");
...
describe("package-provided agents and chains", () => {
	it("discovers package agents and chains from installed package manifests", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-package-discovery-"));
		tempDirs.push(dir);
		const workflowRoot = path.join(dir, ".dm", "npm", "node_modules", "my-dm-workflow");
		const chainsRoot = path.join(dir, ".dm", "npm", "node_modules", "@scope", "chain-workflow");
...
`);
		...
		assert.equal(packagedAgent.packageSourceName, "my-dm-workflow");
	...
	it("loads packages referenced from DM settings", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-settings-package-"));
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".dm", "vendor", "workflow");
		writeJson(path.join(dir, ".dm", "settings.json"), {
			packages: [{ source: "file:./vendor/workflow" }],
		});
		writeJson(path.join(packageRoot, "package.json"), {
...
`);
	...
	it("loads bare HTTPS git packages from project DM settings", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-settings-bare-git-package-"));
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".dm", "git", "github.com", "user", "repo");
		writeJson(path.join(dir, ".dm", "settings.json"), {
...
`);
	...
	it("loads bare HTTP git packages from user DM settings", () => withTempHome((home) => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-user-settings-bare-git-package-"));
		tempDirs.push(dir);
		const agentDir = path.join(home, ".dm", "agent");
...
`);
	...
	it("discovers project package agents when cwd is nested below the project root", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-nested-package-discovery-"));
		tempDirs.push(dir);
		const nested = path.join(dir, "packages", "app", "src");
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "nested-workflow");
...
`);
	...
	it("keeps nearest project root discovery by default when a nested .dm exists", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-nearest-root-default-"));
		tempDirs.push(dir);
		const nested = path.join(dir, "packages", "app", "src");
		const nestedConfigDir = path.join(dir, "packages", "app", ".dm");
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "outer-workflow");
		fs.mkdirSync(nested, { recursive: true });
...
`);
	...
	it("can resolve project packages and overrides from the git root", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-git-root-resolution-"));
		tempDirs.push(dir);
		const nested = path.join(dir, "packages", "app", "src");
		const nestedConfigDir = path.join(dir, "packages", "app", ".dm");
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "outer-workflow");
...
`);
		...
		assert.equal(reviewer?.override?.path, path.join(dir, ".dm", "settings.json"));
	}));

	it("keeps git-root discovery stable when a nested linked worktree creates incidental .dm state", () => withTempHome(() => {
		...
		fs.mkdirSync(path.join(worktree, ".dm", "todos"), { recursive: true });
	...
	it("lets a linked worktree opt back into nearest-root discovery", () => withTempHome(() => {
		...
		const worktreeSettingsPath = path.join(worktree, ".dm", "settings.json");
	...
	it("does not register legacy skill files from broad package agent roots", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-broad-package-skills-"));
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "broad-workflow");
		writeJson(path.join(packageRoot, "package.json"), {
			name: "broad-workflow",
			"dm-subagents": {
				agents: ["."],
...
`);
	...
	it("prunes repo internals and nested project roots from broad package discovery", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-broad-package-prune-"));
		tempDirs.push(dir);
...
Ignored
`);
		fs.mkdirSync(path.join(dir, "packages", "app", ".dm"), { recursive: true });
...
`);
	...
	it("keeps package definitions below user and project overrides", () => withTempHome((home) => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-package-precedence-"));
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "override-workflow");
		writeJson(path.join(packageRoot, "package.json"), {
...
Package chain.
`);
		writeAgent(path.join(home, ".dm", "agent", "agents", "scout.md"), `---
name: scout
...
User scout.
`);
		writeAgent(path.join(dir, ".dm", "agents", "scout.md"), `---
name: scout
...
Project scout.
`);
		writeAgent(path.join(home, ".dm", "agent", "chains", "shared.chain.md"), `---
name: shared
...
User chain.
`);
		writeAgent(path.join(dir, ".dm", "chains", "shared.chain.md"), `---
name: shared
...
`);
	...
	it("does not allow management updates to package agents", () => withTempHome(() => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-package-readonly-"));
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".dm", "npm", "node_modules", "readonly-workflow");
		writeJson(path.join(packageRoot, "package.json"), {
...
describe("agent frontmatter completionGuard", () => {
	...
	it("parses completionGuard from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-completion-guard-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter maxSubagentDepth", () => {
	...
	it("parses maxSubagentDepth from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter allowNestedSubagents", () => {
	it("serializes and parses explicit nested fanout authorization", () => {
		...
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-nested-fanout-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter thinking", () => {
	it("coerces frontmatter false strings to disabled thinking", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-thinking-false-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`, "utf-8");
	...
	it("preserves supported frontmatter thinking strings", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-thinking-levels-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter fallbackModels", () => {
	...
	it("parses fallbackModels from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-fallback-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter systemPromptMode", () => {
	...
	it("parses systemPromptMode from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-prompt-mode-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter prompt inheritance flags", () => {
	...
	it("parses inheritProjectContext and inheritSkills from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-prompt-inheritance-frontmatter-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter subagentOnlyExtensions", () => {
	it("serializes subagentOnlyExtensions into agent frontmatter", () => {
		const agent: AgentConfig = {
			...
			source: "project",
			filePath: "/tmp/worker.md",
			subagentOnlyExtensions: ["./tools/child-search.ts", "/opt/dm/child-only.ts"],
		};

		const serialized = serializeAgent(agent);
		assert.match(serialized, /subagentOnlyExtensions: \.\/tools\/child-search\.ts, \/opt\/dm\/child-only\.ts/);
	});

	it("parses subagentOnlyExtensions from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-child-ext-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
`, "utf-8");
		...
		const result = discoverAgents(dir, "project");
		const worker = result.agents.find((agent) => agent.name === "worker");
		assert.deepEqual(worker?.subagentOnlyExtensions, [path.join(agentsDir, "tools", "child-search.ts"), "/opt/dm/child-only.ts"]);
	});
});
...
describe("agent frontmatter fast mode", () => {
	it("parses and serializes fast mode", () => {
		...
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-fast-mode-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
describe("agent frontmatter prompt assembly defaults", () => {
	it("defaults ordinary agents to replace mode with no inherited context or skills", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-default-prompt-settings-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "worker.md"), `---
...
`, "utf-8");
	...
	it("builtin agents inherit project context by default", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-default-prompt-settings-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-default-home-"));
		tempDirs.push(dir);
	...
	it("bundled agents all have explicit tool allowlists", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-tools-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-tools-home-"));
		tempDirs.push(dir);
		tempDirs.push(homeDir);
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("worker and delegate include the child-facing supervisor tool", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-supervisor-tool-"));
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-builtin-supervisor-tool-home-"));
		tempDirs.push(dir);
		tempDirs.push(homeDir);
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("defaults delegate to append mode with inherited project context", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-agent-delegate-default-prompt-settings-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("packaged agent and chain discovery", () => {
	it("recursively discovers nested project agents while keeping chain files separate", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-recursive-agent-discovery-"));
		tempDirs.push(dir);
		const nestedDir = path.join(dir, ".dm", "agents", "code-analysis", "deep");
		const nestedChainDir = path.join(dir, ".dm", "chains", "code-analysis", "deep");
...
`, "utf-8");
	...
	it("registers packaged agents by runtime name and serializes local name plus package", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-packaged-agent-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
		fs.writeFileSync(path.join(agentsDir, "scout.md"), `---
...
`, "utf-8");
	...
	it("recursively discovers packaged chains by runtime name and preserves package on serialize", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-packaged-chain-"));
		tempDirs.push(dir);
		const nestedDir = path.join(dir, ".dm", "chains", "flows");
		fs.mkdirSync(nestedDir, { recursive: true });
		const content = `---
name: review-flow
package: code-analysis
...
`;
	...
	it("keeps packaged and un-packaged runtime names distinct while preserving un-packaged precedence", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-packaged-collisions-"));
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".agents"), { recursive: true });
		fs.mkdirSync(path.join(dir, ".dm", "agents"), { recursive: true });
...
Legacy
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "agents", "scout.md"), `---
name: scout
description: Project scout
---
...
Project
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "agents", "packaged.md"), `---
name: scout
package: code-analysis
...
`, "utf-8");
		...
		assert.equal(unqualified?.filePath, path.join(dir, ".dm", "agents", "scout.md"));
...
`, "project", "/tmp/review.chain.md");
	...
	it("normalizes package frontmatter consistently for agents and chains", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-package-normalize-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		const chainsDir = path.join(dir, ".dm", "chains");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`, "utf-8");
	...
	it("skips invalid package frontmatter that cannot be normalized", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-invalid-package-"));
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".dm", "agents");
		const chainsDir = path.join(dir, ".dm", "chains");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("project agent directory discovery", () => {
	it("discovers project agents from both .agents and .dm/agents", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-agent-dirs-"));
		tempDirs.push(dir);
...
Legacy prompt
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "agents", "canonical.md"), `---
name: canonical
...
Canonical prompt
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "agents", "SKILL.md"), `---
...
`, "utf-8");
		...
		assert.ok(result.agents.find((agent) => agent.name === "canonical" && agent.filePath === path.join(dir, ".dm", "agents", "canonical.md")));
		assert.ok(result.agents.find((agent) => agent.name === "skill-named-agent" && agent.filePath === path.join(dir, ".dm", "agents", "SKILL.md")));
		assert.equal(result.projectAgentsDir, path.join(dir, ".dm", "agents"));
	});

	it("does not register legacy project skill files as agents", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-skills-not-agents-"));
		tempDirs.push(dir);
...
`);
	...
	it("does not register user SKILL.md files as agents", () => withTempHome((home) => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-user-skills-not-agents-"));
		tempDirs.push(dir);
...
`);
	...
	it("prefers .dm/agents over .agents on project agent name collisions", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-agent-collision-"));
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".agents"), { recursive: true });
		fs.mkdirSync(path.join(dir, ".dm", "agents"), { recursive: true });
...
Legacy prompt
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "agents", "shared.md"), `---
name: shared
...
`, "utf-8");
		...
		assert.ok(shared);
		assert.equal(shared.filePath, path.join(dir, ".dm", "agents", "shared.md"));
	...
	it("uses the project root for the canonical project agent dir even when only .agents exists", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-agent-root-"));
		tempDirs.push(dir);
		...
		assert.equal(result.projectDir, path.join(dir, ".dm", "agents"));
	});

	it("discovers project chains from .dm/chains", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-chain-dirs-"));
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".dm", "agents"), { recursive: true });
		fs.mkdirSync(path.join(dir, ".dm", "chains", "flows"), { recursive: true });
		fs.writeFileSync(path.join(dir, ".dm", "agents", "ignored.chain.md"), `---
name: ignored-chain
...
Ignore
`, "utf-8");
		fs.writeFileSync(path.join(dir, ".dm", "chains", "flows", "canonical.chain.md"), `---
...
`, "utf-8");
		...
		assert.ok(result.chains.find((chain) => chain.name === "canonical-chain" && chain.filePath === path.join(dir, ".dm", "chains", "flows", "canonical.chain.md")));
		assert.equal(result.projectDir, path.join(dir, ".dm", "agents"));
		assert.equal(result.projectChainDir, path.join(dir, ".dm", "chains"));
	});

	it("prefers project .dm/chains over user chains on name collisions", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-chain-collision-"));
		const home = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-user-chain-home-"));
		...
		process.env.USERPROFILE = home;
		try {
			const userChainsDir = path.join(home, ".dm", "agent", "chains");
			fs.mkdirSync(userChainsDir, { recursive: true });
			fs.mkdirSync(path.join(dir, ".dm", "chains"), { recursive: true });
...
Inspect user
`, "utf-8");
			fs.writeFileSync(path.join(dir, ".dm", "chains", "shared.chain.md"), `---
name: shared-chain
description: Project chain
---
...
`, "utf-8");
			...
			assert.ok(shared);
			assert.equal(shared.filePath, path.join(dir, ".dm", "chains", "shared.chain.md"));
