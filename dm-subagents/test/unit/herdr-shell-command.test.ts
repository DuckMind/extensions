...
describe("formatShellCommand on POSIX shells", () => {
	...
	it("keeps a shell-safe executable bare so Nushell parses it as a command", () => {
		assert.equal(
			formatShellCommand("/usr/bin/node", ["/home/u/.dm/inspector-runner.mjs", "--async-dir", "/tmp/run-1|fc_abc"], platform),
			"/usr/bin/node '/home/u/.dm/inspector-runner.mjs' '--async-dir' '/tmp/run-1|fc_abc'",
		);
	});

	it("keeps bare command names bare", () => {
		assert.equal(formatShellCommand("dm", ["review the diff"], platform), "dm 'review the diff'");
	});
	...
	it("executes an equals-sign path through the shell wrapper", { skip: process.platform === "win32" }, () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm herdr shell="));
		try {
