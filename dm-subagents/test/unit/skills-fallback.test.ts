...
function makeProjectPackageSkill(cwd: string, packageName: string, name: string, body: string): void {
	const packageRoot = path.join(cwd, ".dm", "npm", "node_modules", packageName);
...
function makePackageSkill(packageRoot: string, name: string, body: string, packageName = `${name}-pkg`): void {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ name: packageName, version: "1.0.0", dm: { skills: ["./skills"] } }, null, 2),
		"utf-8",
	);
...
describe("skills filesystem fallback", () => {
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-skills-fallback-"));
		clearSkillCache();
	});
	...
	it("discovers project skills nested below grouping directories", () => {
		writeSkillFile(
			path.join(tempDir, ".dm", "skills", "shell", "issue-262-nested-skill"),
	...
	it("stops recursive project skill discovery at the first SKILL.md anchor", () => {
		const groupedRoot = path.join(tempDir, ".dm", "skills", "group");
	...
	it("keeps direct markdown skills from explicit settings roots after parent recursion", () => {
		...
		fs.writeFileSync(
			path.join(tempDir, ".dm", "settings.json"),
	...
	it("keeps nested skills from higher-priority explicit settings roots after parent recursion", () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "fixture", version: "1.0.0", dm: { skills: ["./skills"] } }, null, 2),
			"utf-8",
		);
	...
	it("does not expose dm-subagents as a child-injectable skill", () => {
		makeProjectSkill(tempDir, "dm-subagents", "Parent orchestration only.");
		makeProjectSkill(tempDir, "safe-bash", "Use safe bash.");

		const available = discoverAvailableSkills(tempDir).map((skill) => skill.name);
		assert.equal(available.includes("dm-subagents"), false);
		assert.equal(available.includes("safe-bash"), true);

		const { resolved, missing } = resolveSkills(["dm-subagents", "safe-bash"], tempDir);
		assert.deepEqual(missing, ["dm-subagents"]);
		assert.deepEqual(resolved.map((skill) => skill.name), ["safe-bash"]);

		const agentDir = path.join(tempDir, "agent");
		writeSkillFile(path.join(agentDir, "skills", "dm-subagents"), "Still parent-only.");
		const local = resolveSkills(["dm-subagents"], tempDir, ["./skills"], agentDir);
		assert.deepEqual(local.resolved, []);
		assert.deepEqual(local.missing, ["dm-subagents"]);
	});
	...
	it("discovers skills from project settings packages", () => {
		const packageRoot = path.join(tempDir, ".dm", "packages", "local-skill-pkg");
	...
	it("discovers skills from project settings npm package sources", () => {
		const packageRoot = path.join(tempDir, ".dm", "npm", "node_modules", "@scope", "skill-package");
		makePackageSkill(
			packageRoot,
...
`, "utf-8");
	...
	it("discovers skills from user settings packages", async () => {
		...
		const userAgentDir = path.join(fakeHome, ".dm", "agent");
	...
	it("does not read malformed global settings when every selected local skill resolves", () => {
		...
		fs.mkdirSync(path.join(tempDir, ".dm"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".dm", "settings.json"), "{bad-json", "utf-8");
	...
	it("surfaces malformed project settings files instead of silently ignoring them", () => {
		...
		assert.throws(
			...
			/Failed to read skills settings file .+\.dm[\\/]settings\.json/,
		);
	});

	it("surfaces malformed explicit settings package manifests instead of silently ignoring them", () => {
		const packageRoot = path.join(tempDir, ".dm", "packages", "broken-package");
