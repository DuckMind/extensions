...
function assertNestedPiSpawnHidesWindows(sourcePath: string): void {
	...
	assert.match(
		...
		`${sourcePath} nested DM spawn should set windowsHide: true`,
	);
}

describe("nested child DM process visibility", () => {
	it("hides foreground child DM process windows on Windows", () => {
		assertNestedPiSpawnHidesWindows("src/runs/foreground/execution.ts");
	});

	it("hides background child DM process windows on Windows", () => {
