...
test("process-terminal proof requires the matching runner instance and writer close records", () => {
	const asyncDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-process-terminal-"));
	try {
...
test("process-terminal preserves stopped non-resumability and requires lease release acknowledgement", () => {
	...
	try {
		writeProcessTerminalCandidate(asyncDir, {
			...
			expectedWriters: { "0": 1 },
			writers: {
				"0": [{ processInstanceId: "stopped-writer", kind: "dm-writer", attempt: 0, closeObservedAt: 10, exitCode: 0, signal: null, processTree: { state: "observed", mechanism: "posix-process-group", processGroupId: 124, verifiedAt: 11 } }],
			},
...
test("process-terminal rejects cross-run observed sidecars and inconsistent expected writer maps", () => {
	...
	try {
		...
		writeProcessTerminalCandidate(asyncDir, {
			...
			expectedWriters: { "0": 0 },
			writers: {
				"0": [{ processInstanceId: "unexpected-writer", kind: "dm-writer", attempt: 0, closeObservedAt: 10, exitCode: 0, signal: null, processTree: { state: "observed", mechanism: "posix-process-group", processGroupId: 125, verifiedAt: 11 } }],
			},
		});
