...
describe("Fleet inspector structured transcript", () => {
	it("projects assistant Markdown, compact tools, errors, and later supervisor messages", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-transcript-"));
		try {
	...
	it("renders compact bash output and expanded read/bash-style tool results", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-tool-render-"));
		try {
			const transcriptPath = writeTranscript(root, [
	...
	it("escapes unsafe transcript code points across messages, notices, and tool fields", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-unsafe-transcript-"));
		try {
	...
	it("preserves safe tool argument JSON formatting while sanitizing decoded values", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-args-payload-"));
		try {
	...
	it("replaces obvious binary transcript content with a warning placeholder", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-binary-transcript-"));
		try {
	...
	it("preserves normal Unicode, newlines, tabs, Markdown, code highlighting, and width", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-unicode-transcript-"));
		try {
	...
	it("associates parallel same-name tool results by tool call id", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-parallel-tools-"));
		try {
			const transcriptPath = writeTranscript(root, [
	...
	it("does not attach an id-bearing result to another tool when its start was omitted", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-omitted-tool-start-"));
		try {
			const transcriptPath = writeTranscript(root, [
	...
	it("accepts a complete final record without a newline and drops an incomplete append", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-final-record-"));
		try {
			const transcriptPath = path.join(root, "child-transcript.jsonl");
	...
	it("keeps the first tool result authoritative when records arrive out of order", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-tool-order-"));
		try {
			const transcriptPath = writeTranscript(root, [
	...
	it("keeps a later supervisor message when bounded tailing omits the preceding assistant record", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-transcript-tail-"));
		try {
			const transcriptPath = writeTranscript(root, [
	...
	it("refuses symlinked transcripts and parent-directory escapes", (context) => {
		const trustedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-symlink-trusted-"));
		const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-symlink-outside-"));
		try {
	...
	it("refuses transcript paths outside trusted roots", () => {
		const trustedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-trusted-"));
		const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-outside-"));
		try {
	...
	it("trusts only the recorded session file inside the DM sessions base", () => {
		const sessionsBase = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-recorded-session-"));
		const outsideBase = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-recorded-outside-"));
		try {
	...
	it("sanitizes read-boundary warnings before other Fleet views consume them", () => {
		const trustedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-warning-trusted-"));
		const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-warning-outside-"));
		try {
