...
function createExecutor() {
	return createSubagentExecutor({
		dm: { events: { emit() {}, on() { return () => {}; } }, getSessionName() { return "parent"; } } as any,
		state: createState(),
...
describe("workflow chat progress policy", () => {
	it("treats managed worktrees as same repo and sibling repos as other repo", () => {
		const repo = createRepo("dm-workflow-progress-repo-");
		const other = createRepo("dm-workflow-progress-other-");
		const worktree = path.join(os.tmpdir(), `dm-workflow-progress-wt-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		try {
...
describe("workflow chat progress rendering", () => {
	it("emits live-card updates for same-repo watched workflowScript runs", async () => {
		const repo = createRepo("dm-workflow-progress-executor-");
		try {
	...
	it("writes mission binding before async workflow child launch", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-workflow-child-binding-"));
		const asyncId = `workflow-child-${process.pid}-${Date.now()}`;
		const asyncDir = path.join(DIRS.async, asyncId);
		try {
			const location = {
				projectRoot: root,
				missionDir: path.join(root, ".dm/subagents", "missions"),
				globalIndexDir: path.join(root, ".dm/subagents", "mission-index"),
	...
	it("marks workflow launch preparation failures as failed mission children", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-workflow-prep-failure-"));
		try {
			const location = {
				projectRoot: root,
				missionDir: path.join(root, ".dm/subagents", "missions"),
	...
	it("warns once when a workflow child outlives its mission record", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-workflow-missing-mission-"));
	...
	it("renders stable rows keyed by workflow trace entries", () => {
		const text = componentText(renderSubagentResult({
			...
			details: {
				...
				runId: "wf_8f3a123456",
				results: [],
				chatProgress: { mode: "live-card", repoRelation: "same", repoLabel: "dm-subagents" },
				workflow: {
					trace: [
		...
		assert.match(text, /Repo   dm-subagents/);
	...
	it("keeps each expanded preflight warning visible as a bounded row", () => {
		const result = {
			...
			details: {
				...
				chatProgress: { mode: "live-card" as const, repoRelation: "same" as const, repoLabel: "dm-subagents" },
				workflow: {
	...
	it("applies main-window density settings to collapsed workflow live cards", () => {
		...
		assert.match(compact[1]!, /^Repo   dm-subagents\s*$/);
		...
		assert.match(expanded[1]!, /^  Repo   dm-subagents\s*$/);
