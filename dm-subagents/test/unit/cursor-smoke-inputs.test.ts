...
function roots(): { root: string; workspace: string; stateRoot: string } {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-cursor-smoke-inputs-"));
...
function env(workspace: string, stateRoot: string): NodeJS.ProcessEnv {
	return {
		DM_SUBAGENTS_CURSOR_SMOKE_DISPOSABLE: "1",
		DM_SUBAGENTS_CURSOR_SMOKE_WORKSPACE: workspace,
		DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT: stateRoot,
	};
}
...
test("resolves existing disposable Cursor smoke roots", () => {
	...
	try {
		...
		assert.deepEqual(cursorSmokeInputs(env(input.workspace, input.stateRoot)), {
			workspace,
			stateRoot,
			canaryPath: path.join(workspace, "dm-subagents-cursor-write-canary.txt"),
...
test("requires disposable attestation and separate existing roots", () => {
	const input = roots();
	try {
		assert.throws(() => cursorSmokeInputs({ ...env(input.workspace, input.stateRoot), DM_SUBAGENTS_CURSOR_SMOKE_DISPOSABLE: undefined }), /DISPOSABLE=1 is required/);
...
test("refuses pre-existing Cursor smoke handoff and canary paths", () => {
	...
	try {
		...
		fs.writeFileSync(path.join(input.workspace, "dm-subagents-cursor-write-canary.txt"), "existing");
		assert.throws(() => cursorSmokeInputs(values), /canary path must not exist/);
		fs.rmSync(path.join(input.workspace, "dm-subagents-cursor-write-canary.txt"));
