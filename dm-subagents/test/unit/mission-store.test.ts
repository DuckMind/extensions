...
function fixture() {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-missions-"));
...
describe("mission store", () => {
	it("stores default missions outside the project and preserves configured project storage", () => {
		...
		try {
			...
			assert.equal(
				resolveMissionStoreLocation({ projectRoot: test.projectRoot, agentDir: test.agentDir, config: { directory: ".dm/subagents/missions" } }).missionDir,
				path.join(test.projectRoot, ".dm/subagents", "missions"),
			);
		} finally {
