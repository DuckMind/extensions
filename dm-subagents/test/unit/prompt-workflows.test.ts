...
const originalAgentDir = process.env.DM_CODING_AGENT_DIR;
...
describe("prompt workflows", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-prompt-workflows-"));
	...
	afterEach(() => {
		if (originalAgentDir === undefined) delete process.env.DM_CODING_AGENT_DIR;
		else process.env.DM_CODING_AGENT_DIR = originalAgentDir;
...
User body
`);
		writePrompt(path.join(cwd, ".dm", "prompts"), "native-test", `---
...
`);
	...
	it("runs a named workflow through native subagent execution", async () => {
		writePrompt(path.join(cwd, ".dm", "prompts"), "native-run", `---
...
`);
	...
	it("runs declared prompt sequences through workflowScript", async () => {
		writePrompt(path.join(cwd, ".dm", "prompts"), "native-analyze", `---
description: Analyze
subagent: scout
...
Analyze $@
`);
		writePrompt(path.join(cwd, ".dm", "prompts"), "native-fix", `---
description: Fix
subagent: worker
---
Fix from {previous}: $@
