...
import { visibleWidth, type MarkdownTheme } from "@duckmind/dm-tui";
...
import { fauxAssistantMessage } from "@duckmind/dm-ai/providers/faux";
import { createAssistantMessageEventStream } from "@duckmind/dm-ai";
...
describe("native subagent fleet", () => {
	...
	it("collects current-session foreground and flattened async children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-collect-"));
		try {
	...
	it("keeps tracked active workflows ahead of older failed terminal history", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-history-"));
		try {
	...
	it("bounds Fleet history status-file reads", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-bounded-history-"));
		try {
	...
	it("shows async model and thinking metadata in the structured header", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-model-thinking-"));
		try {
			writeAsyncRun(root, {
				id: "model-thinking",
				state: "running",
	...
	it("shows a scripted workflow as one fleet parent instead of unrelated children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-workflow-parent-"));
		try {
			const state = stateForTest();
	...
	it("accepts default Fleet navigation from Kitty CSI-u input", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-kitty-default-"));
		try {
	...
	it("uses custom Fleet keybindings without applying them inside modal text input", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-custom-keys-"));
		try {
			writeAsyncRun(root, { id: "newer", lastUpdate: 300 });
			writeAsyncRun(root, { id: "older", lastUpdate: 200 });
			const state = stateForTest();
	...
	it("restores current-session completed foreground history from the compact index", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-foreground-history-"));
		try {
	...
	it("does not restore nonterminal foreground history rows", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-foreground-terminal-"));
		try {
			const resultsDir = path.join(root, "results");
	...
	it("does not restore or persist oversized foreground resume contracts", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-foreground-contract-bound-"));
		try {
			const resultsDir = path.join(root, "results");
			fs.mkdirSync(resultsDir, { recursive: true });
	...
	it("keeps restored foreground rows when output artifacts are unavailable and bounds history", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-foreground-missing-"));
		try {
			const resultsDir = path.join(root, "results");
			const cwd = path.join(root, "project");
			const artifactsRoot = getProjectArtifactsDir(cwd);
	...
	it("renders selectable transcript detail and completed artifact paths within terminal width", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-render-"));
		try {
	...
	it("passes current-session trusted roots to async session transcript fallback", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-session-fallback-"));
		try {
	...
	it("previews an exact runtime-recorded workflow session under the DM sessions base", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-recorded-session-"));
		try {
	...
	it("renders structured tool activity and assistant Markdown when a child transcript is available", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-structured-"));
		try {
	...
	it("uses parent-tracked async state and cwd without rescanning status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-async-cwd-"));
		try {
	...
	it("toggles bounded tool output expansion with x and Ctrl+O", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-tool-toggle-"));
		try {
			writeAsyncRun(root, {
				id: "async-tools",
				state: "complete",
	...
	it("renders the selected foreground parallel child's structured transcript", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-foreground-"));
		try {
			const state = stateForTest();
			state.baseCwd = path.join(root, "parent-cwd");
			state.artifactDirPreference = "project";
	...
	it("renders configured session and temp transcripts for active foreground, completed foreground, and async children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-artifact-roots-"));
	...
	it("shows disk-only current-session async runs when the overlay opens", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-overlay-reconcile-"));
		try {
	...
	it("focuses the Herdr pane the operator opens with the inspect key", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-inspect-focus-"));
		try {
	...
	it("steers the selected async child with an inline message", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-steer-"));
		try {
	...
	it("opens the selected async child in a Herdr inspector", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-herdr-"));
		try {
	...
	it("confirms stop for the selected async child before calling the action", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-stop-"));
		try {
	...
	it("explains unavailable controls for completed async children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-unavailable-"));
		try {
	...
	it("refreshes the roster while the overlay remains open", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-fleet-refresh-"));
		try {
			const state = stateForTest();
			let renderRequests = 0;
			const tui = { terminal: { rows: 28, columns: 90 }, requestRender: () => { renderRequests++; } };
