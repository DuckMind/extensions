...
test("resolveCompileFromPackageRoot loads typebox/compile from a fake DM host package root", async () => {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-host-root-"));
	try {
		fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "fake-dm-coding-agent", version: "0.0.0" }));
	...
	const emptyRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-empty-root-"));
	try {
...
test("resolveCompileFromPackageRoot resolves typebox hoisted to an ancestor node_modules", async () => {
	const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-hoisted-root-"));
	try {
		writeFakeTypeboxPackage(path.join(tmp, "node_modules", "typebox"));
		const packageRoot = path.join(tmp, "apps", "dm", "node_modules", "@earendil-works", "dm-coding-agent");
		fs.mkdirSync(packageRoot, { recursive: true });
		fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "@duckmind/dm-coding-agent", version: "0.0.0" }));
