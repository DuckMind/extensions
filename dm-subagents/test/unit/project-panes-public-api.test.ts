...
describe("public project-panes package export", () => {
	...
	it("runs the structured lifecycle and verifies pane ownership before idle close", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-"));
		try {
			...
			const client: ProjectPaneCommandClient = {
				run: async <T>(args: string[]) => {
					...
					if (args[0] === "pane" && args[1] === "get" && opened) return { ok: true, data: { pane: {
						pane_id: "w1:p20", agent: "dm", agent_status: "IDLE", cwd: projectRoot,
	...
	it("distinguishes a stale binding on the default close path", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-stale-"));
		try {
			const projectRoot = fs.realpathSync(root);
			...
			fs.writeFileSync(projectPaneBindingPath(projectRoot), JSON.stringify({
				...
				openedAt: "2026-01-01T00:00:00.000Z", command: "dm",
			}));
			const client: ProjectPaneCommandClient = {
	...
	it("returns a structured error when an existing binding cannot be read", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-read-failure-"));
		try {
			const projectRoot = fs.realpathSync(root);
	...
	it("returns a structured error and closes the pane when binding persistence fails", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-write-failure-"));
		try {
			fs.mkdirSync(path.join(root, ".dm"));
			fs.writeFileSync(path.join(root, ".dm/subagents"), "directory collision");
			const calls: string[][] = [];
			const client: ProjectPaneCommandClient = {
				run: async <T>(args: string[]) => {
					calls.push(args);
	...
	it("returns a structured error when binding removal fails after closing the pane", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-remove-failure-"));
		try {
			const projectRoot = fs.realpathSync(root);
			const bindingPath = projectPaneBindingPath(projectRoot);
	...
	it("returns a structured error when focus cannot update the binding", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-focus-write-failure-"));
		try {
			const projectRoot = fs.realpathSync(root);
			const bindingPath = projectPaneBindingPath(projectRoot);
			const paneId = "w1:p26";
			fs.mkdirSync(path.dirname(bindingPath), { recursive: true });
			fs.writeFileSync(bindingPath, JSON.stringify({ schemaVersion: 1, kind: "herdr-project-pane", projectRoot, paneId, openedAt: "2026-01-01T00:00:00.000Z", command: "dm" }));
			const client: ProjectPaneCommandClient = {
				run: async <T>(args: string[]) => {
	...
	it("rejects idle close when Herdr pane ownership no longer matches the project", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-owner-"));
		const other = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-other-"));
		try {
			const projectRoot = fs.realpathSync(root);
			fs.mkdirSync(path.dirname(projectPaneBindingPath(projectRoot)), { recursive: true });
			fs.writeFileSync(projectPaneBindingPath(projectRoot), JSON.stringify({
	...
	it("does not expose or lifecycle-manage malformed public bindings as if they were absent", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-public-project-pane-binding-"));
		try {
			const projectRoot = fs.realpathSync(root);
			fs.mkdirSync(path.dirname(projectPaneBindingPath(projectRoot)), { recursive: true });
			fs.writeFileSync(projectPaneBindingPath(projectRoot), JSON.stringify({
				schemaVersion: 1, kind: "herdr-project-pane", projectRoot, paneId: "w1:p23",
				openedAt: "2026-01-01T00:00:00.000Z", command: "dm", startupMessage: 42,
			}));
			const binding = readProjectPaneBinding(root);
			assert.equal(binding.ok, false);
