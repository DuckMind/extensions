...
import type { Message, Usage } from "@duckmind/dm-ai";
import {
...
describe("resolveSingleOutputPath", () => {
	...
	it("keeps absolute paths unchanged", () => {
		const absolutePath = path.join(os.tmpdir(), "dm-subagents-abs", "report.md");
	...
	it("resolves relative output paths against an explicit artifact base", () => {
		const resolved = resolveSingleOutputPath("reviews/report.md", "/runtime", "/requested", "/repo/.dm/subagents/artifacts/outputs/run-1");
		assert.equal(resolved, path.resolve("/repo/.dm/subagents/artifacts/outputs/run-1", "reviews/report.md"));
	});
});
...
describe("resolveSingleOutput", () => {
	it("keeps agent-written file content when the file changed during the run", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-output-test-"));
		tempDirs.push(dir);
