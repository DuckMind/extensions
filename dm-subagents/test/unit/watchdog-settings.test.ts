...
function userSettingsPath(): string {
	return path.join(tempHome, ".dm", "agent", "settings.json");
}

function projectSettingsPath(): string {
	return path.join(tempProject, ".dm", "settings.json");
}

describe("watchdog settings", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-watchdog-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-watchdog-project-"));
