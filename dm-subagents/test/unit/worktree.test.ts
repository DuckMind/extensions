...
function createHookScript(_repoDir: string, fileName: string, source: string): string {
	const hooksDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-worktree-hook-script-"));
...
describe("worktree", () => {
	it("createWorktrees returns expected structure", () => {
		const repoDir = createRepo("dm-worktree-structure-");
		...
		try {
			...
			for (let i = 0; i < setup.worktrees.length; i++) {
				...
				assert.equal(worktree.branch, `dm-parallel-structure-${i}`);
	...
	it("invokes beforeCreate with deterministic ownership metadata before creating worktrees", () => {
		const repoDir = createRepo("dm-worktree-before-create-");
		let setup: WorktreeSetup | undefined;
		...
		try {
			setup = createWorktrees(repoDir, "before-create", 1, {
				beforeCreate: (candidate) => {
					...
					assert.equal(candidate.worktrees[0]!.branch, "dm-parallel-before-create-0");
				},
			});
	...
	it("createWorktrees maps subdirectory cwd to each agentCwd", () => {
		const repoDir = createRepo("dm-worktree-subdir-");
	...
	it("previews expected worktree agent cwd for repository subdirectories", () => {
		const repoDir = createRepo("dm-worktree-preview-");
		const nestedDir = path.join(repoDir, "packages", "app");
		fs.mkdirSync(nestedDir, { recursive: true });
		fs.writeFileSync(path.join(nestedDir, "index.ts"), "export const value = 1;\n", "utf-8");
		git(repoDir, ["add", "-A"]);
		...
		try {
			assert.equal(
				...
				path.join(os.tmpdir(), "dm-worktree-preview-2", "packages", "app"),
			);
		} finally {
			cleanupRepo(repoDir);
		}
	...
	it("creates worktrees under a configured base directory", () => {
		const repoDir = createRepo("dm-worktree-base-dir-");
		const baseDir = path.join(os.tmpdir(), `dm-worktree-base-${Date.now().toString(36)}`, "nested");
		let setup: WorktreeSetup | undefined;
		try {
			setup = createWorktrees(repoDir, "base-dir", 1, { baseDir });
			assert.equal(setup.worktrees[0]!.path, path.join(baseDir, "dm-worktree-base-dir-0"));
	...
	it("rejects worktree base directories inside DM extensions", () => {
		const repoDir = createRepo("dm-worktree-extension-dir-");
		const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-worktree-home-"));
		const previousAgentDir = process.env.DM_CODING_AGENT_DIR;
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
		const extensionsDir = path.join(tempHome, ".dm", "agent", "extensions");
		try {
			...
			assert.throws(
				...
				/worktree base directory cannot be inside DM extensions directory/i,
			);
			assert.throws(
	...
	it("rejects symlinked worktree base directories inside DM extensions", () => {
		const repoDir = createRepo("dm-worktree-extension-symlink-");
		const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-worktree-home-"));
		const previousAgentDir = process.env.DM_CODING_AGENT_DIR;
		const previousHome = process.env.HOME;
		const previousUserProfile = process.env.USERPROFILE;
	...
	it("uses DM_SUBAGENTS_WORKTREE_DIR when no base directory is configured", () => {
		const repoDir = createRepo("dm-worktree-env-base-dir-");
		const previous = process.env.DM_SUBAGENTS_WORKTREE_DIR;
		const baseDir = path.join(os.tmpdir(), `dm-worktree-env-base-${Date.now().toString(36)}`);
		let setup: WorktreeSetup | undefined;
		try {
			process.env.DM_SUBAGENTS_WORKTREE_DIR = baseDir;
			setup = createWorktrees(repoDir, "env-base-dir", 1);
			assert.equal(setup.worktrees[0]!.path, path.join(baseDir, "dm-worktree-env-base-dir-0"));
		} finally {
			if (setup) cleanupWorktrees(setup);
			if (previous === undefined) {
				delete process.env.DM_SUBAGENTS_WORKTREE_DIR;
			} else {
				process.env.DM_SUBAGENTS_WORKTREE_DIR = previous;
			}
			cleanupRepo(repoDir);
	...
	it("createWorktrees rejects dirty repositories", () => {
		const repoDir = createRepo("dm-worktree-dirty-");
		try {
	...
	it("createWorktrees ignores dm-subagents runtime artifacts in the source checkout", () => {
		const repoDir = createRepo("dm-worktree-runtime-artifacts-");
		let setup: WorktreeSetup | undefined;
		try {
			fs.mkdirSync(path.join(repoDir, ".dm/subagents", "missions"), { recursive: true });
			fs.writeFileSync(path.join(repoDir, ".dm/subagents", "missions", "mission.json"), "{}\n", "utf-8");
	...
	it("diffWorktrees captures committed, modified, and new files without staging the node_modules symlink", () => {
		const repoDir = createRepo("dm-worktree-diff-");
	...
	it("cleanupWorktrees removes worktrees and branches", () => {
		const repoDir = createRepo("dm-worktree-cleanup-");
		let setup: WorktreeSetup | undefined;
		try {
	...
	it("preserves dirty worktrees when no patch was captured", () => {
		const repoDir = createRepo("dm-worktree-uncaptured-");
		let setup: WorktreeSetup | undefined;
		try {
	...
	it("preserves committed branch work when no patch was captured", () => {
		const repoDir = createRepo("dm-worktree-committed-uncaptured-");
		let setup: WorktreeSetup | undefined;
		try {
	...
	it("removes dirty worktrees only after their captured patch is recorded in the handoff manifest", () => {
		const repoDir = createRepo("dm-worktree-captured-");
		let setup: WorktreeSetup | undefined;
		try {
	...
	it("createWorktrees creates node_modules symlink when node_modules exists", {
		...
		const repoDir = createRepo("dm-worktree-node-modules-");
		const nodeModulesDir = path.join(repoDir, "node_modules");
		fs.mkdirSync(nodeModulesDir, { recursive: true });
		fs.writeFileSync(path.join(nodeModulesDir, "fixture.txt"), "fixture\n", "utf-8");
	...
	it("diffWorktrees preserves a tracked node_modules symlink", {
		...
		const repoDir = createRepo("dm-worktree-tracked-node-modules-");
	...
	it("runs a repo-relative worktree setup hook and records synthetic paths", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-relative-");
...
`);
	...
	it("runs an absolute worktree setup hook path", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-absolute-");
		const hookPath = createHookScript(repoDir, "setup-hook.mjs", `
import * as fs from "node:fs";
...
`);
	...
	it("rejects bare command names for worktree setup hooks", () => {
		const repoDir = createRepo("dm-worktree-hook-bare-");
		try {
			assert.throws(
	...
	it("rejects tracked synthetic paths from hook output", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-tracked-");
...
`);
	...
	it("rejects absolute synthetic paths from hook output", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-absolute-synthetic-");
...
`);
	...
	it("excludes hook-created synthetic files from captured patch output", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-diff-");
...
`);
	...
	it("cleans up created worktrees when a later hook setup fails", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-cleanup-");
...
`);
		try {
			...
			const branchList = git(repoDir, ["branch", "--list", `dm-parallel-${runId}-*`]);
	...
	it("fails when the hook exceeds the configured timeout", { skip: hookScriptSkip }, () => {
		const repoDir = createRepo("dm-worktree-hook-timeout-");
...
describe("manifest-backed worktree discard", () => {
	it("keeps incomplete cleanup actionable with exact manual Git commands", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-worktree-discard-manifest-"));
		try {
			...
			fs.writeFileSync(manifestPath, JSON.stringify({
				...
				groups: [{
					...
					repoRoot: root,
					children: [],
					cleanup: { state: "partial", pruned: false, tasks: [{ index: 0, path: worktreePath, branch: "dm-parallel-discard", worktreeRemoved: false, branchRemoved: false, preserved: true }] },
				}],
			}), "utf-8");
