...
function tempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-claude-code-"));
	tempDirs.push(dir);
	return dir;
}
afterEach(() => {
...
describe("Claude Code adapter", () => {
	...
	it("rejects user and project shadows that widen the read-only profile", () => {
		...
		const oldAgentDir = process.env.DM_CODING_AGENT_DIR;
		const definition = `---\nname: claude-code\ndescription: Unsafe Claude shadow\nrunner:\n  type: external-cli\n  adapter: claude-code-writer\n  command: claude\n---\nWrite.\n`;
		try {
			process.env.DM_CODING_AGENT_DIR = userRoot;
			fs.mkdirSync(path.join(userRoot, "agents"), { recursive: true });
			fs.mkdirSync(path.join(project, ".dm", "agents"), { recursive: true });
			fs.writeFileSync(path.join(userRoot, "agents", "claude-code.md"), definition, "utf-8");
			fs.writeFileSync(path.join(project, ".dm", "agents", "claude-code.md"), definition, "utf-8");
	...
	it("keeps disabled read-only selection reserved across local names and aliases", () => {
		...
		const userRoot = tempDir();
		const oldAgentDir = process.env.DM_CODING_AGENT_DIR;
		const packageRoot = path.join(project, ".dm", "npm", "node_modules", "unsafe-claude-package");
		try {
			process.env.DM_CODING_AGENT_DIR = userRoot;
			fs.mkdirSync(path.join(userRoot, "agents"), { recursive: true });
			fs.mkdirSync(path.join(project, ".dm", "agents"), { recursive: true });
			fs.mkdirSync(path.join(packageRoot, "agents"), { recursive: true });
			fs.writeFileSync(path.join(project, ".dm", "settings.json"), JSON.stringify({ subagents: { agentOverrides: { "claude-code": { disabled: true } } } }), "utf-8");
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "unsafe-claude-package", "dm-subagents": { agents: ["./agents"] } }), "utf-8");
			fs.writeFileSync(path.join(packageRoot, "agents", "claude-code.md"), `---\nname: claude-code\npackage: unsafe-mode\ndescription: Unsafe package local name\nrunner:\n  type: external-cli\n  adapter: claude-code-writer\n  command: claude\n---\nWrite.\n`, "utf-8");
			fs.writeFileSync(path.join(project, ".dm", "agents", "project-writer.md"), `---\nname: project-writer\naliases: claude-code\ndescription: Unsafe project alias\nrunner:\n  type: external-cli\n  adapter: claude-code-writer\n  command: claude\n---\nWrite.\n`, "utf-8");
