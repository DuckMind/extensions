...
afterEach(() => {
	...
	removeProgressFiles("standalone-dm-");
});

function tempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-orca-tabs-test-"));
	tempDirs.push(dir);
	return dir;
}
...
test("resolveOrcaCommand only returns executable commands", () => {
	...
	assert.equal(resolveOrcaCommand({ PATH: "", DM_SUBAGENT_ORCA_BINARY: path.join(dir, "missing") }), undefined);
});
...
test("an unavailable Orca command leaves native execution untouched", () => {
	...
	assert.equal(createOrcaProgressTab({
		...
		index: 0,
		config: { enabled: true },
		env: { PATH: "", DM_SUBAGENT_ORCA_BINARY: path.join(dir, "missing") },
	}), undefined);
});

test("standalone DM executables use PATH Node for the watchdog and viewer", { skip: process.platform === "win32" ? "Orca progress tabs are not supported on Windows" : undefined }, async () => {
	const dir = tempDir();
	const capture = path.join(dir, "capture.json");
	const fakeOrca = writeCaptureOrca(dir);
	const fakePi = path.join(dir, "dm");
...
test("enabled tabs use a worktree sequence and successful DM sessions get cleanup guidance", { skip: process.platform === "win32" ? "Orca progress tabs are not supported on Windows" : undefined }, async () => {
	const dir = tempDir();
	const capture = path.join(dir, "capture.json");
	...
	const manifestDir = path.join(dir, ".dm", "subagents", "views", "orca");
	...
	assert.ok(text.includes(`completed. To remove the DM session for this run, run rm -- ${quotedSessionFile}`));
	assert.doesNotMatch(text, /find ~\/\.dm\/agent\/sessions/);
	...
	assert.doesNotMatch(secondText, /To remove the DM session/);
});
