...
describe("subagent startup retry", () => {
	...
	it("adds actionable guidance for ambient extension registration conflicts", () => {
		const conflict = 'Error: Failed to load extension "/tmp/dm-mcp-adapter-clone/index.ts": Tool "mcpScript" conflicts with /tmp/dm-mcp-adapter/index.ts\nError: Failed to load extension "/tmp/dm-mcp-adapter-clone/index.ts": Flag "--mcp-config" conflicts with /tmp/dm-mcp-adapter/index.ts';
		const formatted = formatSubagentExtensionConflictError(conflict, {
			agent: "reviewer",
			ambientExtensionsEnabled: true,
		});

		assert.match(formatted ?? "", /loaded conflicting ambient DM extensions/);
		assert.match(formatted ?? "", /\.dm\/settings\.json/);
	...
	it("formats retry and exhaustion diagnostics without task content", () => {
		...
		assert.match(
			...
			/failed to start after 4 attempts.*concurrent DM startup race.*lower subagent concurrency/i,
		);
	});
