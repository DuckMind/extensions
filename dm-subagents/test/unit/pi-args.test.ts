...
const originalEnv = {
	...
	DM_CODING_AGENT_DIR: process.env.DM_CODING_AGENT_DIR,
	DM_SUBAGENT_FANOUT_CHILD: process.env.DM_SUBAGENT_FANOUT_CHILD,
	DM_SUBAGENT_PARENT_EVENT_SINK: process.env.DM_SUBAGENT_PARENT_EVENT_SINK,
	DM_SUBAGENT_PARENT_CONTROL_INBOX:
		process.env.DM_SUBAGENT_PARENT_CONTROL_INBOX,
	DM_SUBAGENT_PARENT_ROOT_RUN_ID: process.env.DM_SUBAGENT_PARENT_ROOT_RUN_ID,
	DM_SUBAGENT_PARENT_RUN_ID: process.env.DM_SUBAGENT_PARENT_RUN_ID,
	DM_SUBAGENT_PARENT_CHILD_INDEX: process.env.DM_SUBAGENT_PARENT_CHILD_INDEX,
	DM_SUBAGENT_PARENT_DEPTH: process.env.DM_SUBAGENT_PARENT_DEPTH,
	DM_SUBAGENT_PARENT_PATH: process.env.DM_SUBAGENT_PARENT_PATH,
	DM_SUBAGENT_PARENT_CAPABILITY_TOKEN:
		process.env.DM_SUBAGENT_PARENT_CAPABILITY_TOKEN,
	DM_SUBAGENT_PARENT_SESSION: process.env.DM_SUBAGENT_PARENT_SESSION,
	...
	[DM_CODING_AGENT_PACKAGE_ROOT_ENV]:
		process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV],
	...
	DM_SUBAGENT_TASK_DELIVERY: process.env.DM_SUBAGENT_TASK_DELIVERY,
};
...
function createMcpFixture(): McpFixture {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-args-mcp-"));
	tempRoots.push(root);
...
describe("buildPiArgs session wiring", () => {
	...
	it("uses --session when sessionFile is provided", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-args-session-"));
		try {
	...
	it("emits explicit parent session env for permission forwarding", () => {
		process.env.DM_SUBAGENT_PARENT_SESSION = "inherited-parent";
	...
	it("passes the child display session name through as DM_SUBAGENT_SESSION_NAME", () => {
		const { env } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
		...
		assert.equal(env.DM_SUBAGENT_SESSION_NAME, "worker: Fix the flaky test");
	});

	it("omits DM_SUBAGENT_SESSION_NAME when no session name is provided", () => {
		const { env } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
		...
		assert.equal(env.DM_SUBAGENT_SESSION_NAME, undefined);
	});
...
describe("buildPiArgs task delivery", () => {
	...
	it("delivers short tasks via file when DM_SUBAGENT_TASK_DELIVERY=file", () => {
		process.env.DM_SUBAGENT_TASK_DELIVERY = "file";
		const { args } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
	...
	it("falls back to auto when DM_SUBAGENT_TASK_DELIVERY is invalid", () => {
		process.env.DM_SUBAGENT_TASK_DELIVERY = "carrier-pigeon";
		const { args } = buildPiArgs({
			baseArgs: ["-p"],
			task: longTask,
			sessionEnabled: false,
	...
	it("lets the per-launch taskDelivery override beat the env setting", () => {
		delete process.env.DM_SUBAGENT_TASK_DELIVERY;
		const { args } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			taskDelivery: "file",
...
describe("buildPiArgs system prompt mode wiring", () => {
	...
	it("injects the subagent prompt runtime extension and env flags", () => {
		...
		assert.equal(env.DM_SUBAGENT_CHILD, "1");
		assert.equal(env.DM_SUBAGENT_INHERIT_PROJECT_CONTEXT, "0");
		assert.equal(env.DM_SUBAGENT_INHERIT_GLOBAL_CONTEXT, "0");
		assert.equal(env.DM_SUBAGENT_INHERIT_SKILLS, "1");
	});
	...
	it("propagates the global context inheritance flag through env", () => {
		...
		assert.equal(env.DM_SUBAGENT_INHERIT_PROJECT_CONTEXT, "1");
		assert.equal(env.DM_SUBAGENT_INHERIT_GLOBAL_CONTEXT, "1");
	});
	...
	it("passes child intercom and orchestrator metadata through env", () => {
		...
		assert.equal(
			env.DM_SUBAGENT_INTERCOM_SESSION_NAME,
		...
		assert.equal(env.DM_SUBAGENT_ORCHESTRATOR_TARGET, "subagent-chat-parent");
		assert.equal(
			env[SUBAGENT_ORCHESTRATOR_SESSION_ID_ENV],
			"session-parent-123",
		);
		assert.equal(env.DM_SUBAGENT_RUN_ID, "78f659a3");
		assert.equal(env.DM_SUBAGENT_CHILD_AGENT, "worker");
		assert.equal(env.DM_SUBAGENT_CHILD_INDEX, "2");
	...
	it("clears inherited dm-intercom identity when no child intercom session name is set", () => {
		process.env[PI_INTERCOM_STABLE_ID_ENV] = "subagent-chat-parent";
		process.env[PI_INTERCOM_SESSION_ID_ENV] = "session-parent-runtime";
		const { env } = buildPiArgs({
			baseArgs: ["-p"],
	...
	it("creates a private permission audit path without enabling the supervisor channel", () => {
		...
		assert.equal(env.DM_SUBAGENT_ORCHESTRATOR_TARGET, undefined);
	...
	it("forwards the DM package root to child processes for host peer resolution", () => {
		process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] = "/opt/dm-coding-agent";
		const { env } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
		...
		assert.equal(env[DM_CODING_AGENT_PACKAGE_ROOT_ENV], "/opt/dm-coding-agent");
	});
	...
	it("matches dm-mcp-adapter 2.26.0 metadata cache hashes", () => {
	...
	it("resolves project MCP config from the child cwd and expands DM_CODING_AGENT_DIR", () => {
		const fixture = createMcpFixture();
		process.env.DM_CODING_AGENT_DIR = "~/.dm/agent";
		process.chdir(fixture.root);
		fs.mkdirSync(path.join(fixture.projectDir, ".dm"));
		writeMcpFixture(fixture, {
	...
	it("resolves direct MCP tools from DM package manifests", () => {
		const fixture = createMcpFixture();
		...
		writeJson(path.join(packageRoot, "package.json"), { name: "@acme/tools", dm: { mcp: "./mcp.json" } });
	...
	it("fails closed on malformed MCP server fields from DM package manifests", () => {
		const fixture = createMcpFixture();
		const packageRoot = path.join(fixture.agentDir, "npm", "node_modules", "@acme", "tools");
	...
	it("resolves direct MCP tools from the git-root package when child cwd has an incidental .dm directory", () => {
		const fixture = createMcpFixture();
		const nestedCwd = path.join(fixture.projectDir, "packages", "app");
		const packageRoot = path.join(fixture.projectDir, ".dm", "npm", "node_modules", "@acme", "tools");
		const definition = { command: "node", args: ["package-mcp"] };
		fs.mkdirSync(nestedCwd, { recursive: true });
		fs.mkdirSync(path.join(nestedCwd, ".dm"));
		fs.mkdirSync(path.join(fixture.projectDir, ".git"));
		writeJson(path.join(fixture.projectDir, ".dm", "settings.json"), {
