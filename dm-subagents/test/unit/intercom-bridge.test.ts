...
describe("resolveIntercomSessionTarget", () => {
	...
	it("uses the current dm-intercom runtime id for unnamed fallback when present", () => {
...
describe("resolveIntercomBridge", () => {
	...
	it("loads custom instructions from instructionFile", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-native-intercom-bridge-test-"));
