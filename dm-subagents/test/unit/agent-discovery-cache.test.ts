...
const managedEnv = ["HOME", "USERPROFILE", "DM_CODING_AGENT_DIR", "DM_SUBAGENT_EXTRA_AGENT_DIRS", "PI_OFFLINE"];
...
describe("agent discovery snapshots", () => {
	beforeEach(() => {
		tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-agent-discovery-cache-"));
	...
	it("keeps effective and all-source projections distinct in one snapshot", () => {
		const userAgentPath = path.join(home, ".dm", "agent", "agents", "shared.md");
		const projectAgentPath = path.join(project, ".dm", "agents", "shared.md");
		writeAgent(userAgentPath, "shared", "User shared");
		writeAgent(projectAgentPath, "shared", "Project shared");
		writeAgent(path.join(home, ".dm", "agent", "agents", "scope-hidden.md"), "scope-hidden", "User hidden");
		writeJson(path.join(project, ".dm", "settings.json"), {
	...
	it("retains hidden definitions for runtime collision checks", () => {
		const hiddenPath = path.join(home, ".dm", "agent", "agents", "scope-hidden.md");
		...
		try {
			...
			assert.equal(snapshot.effective.agents.some((agent) => agent.name === "scope-hidden"), false);
			assert.throws(
				() => mergeRuntimeAgents(dm, snapshot.effective, configured),
	...
	it("invalidates agent and chain projections when files change or appear", () => {
		const agentPath = path.join(project, ".dm", "agents", "fresh.md");
		const chainPath = path.join(project, ".dm", "chains", "fresh.chain.md");
		...
		fs.rmSync(agentPath);
		const addedPath = path.join(project, ".dm", "agents", "added.md");
	...
	it("matches scoped discovery settings and precedence for user and project views", () => {
		writeAgent(path.join(home, ".dm", "agent", "agents", "shared.md"), "shared", "User shared");
		writeAgent(path.join(project, ".dm", "agents", "shared.md"), "shared", "Project shared");
		writeAgent(path.join(home, ".dm", "agent", "agents", "user-only.md"), "user-only", "User only");
		writeAgent(path.join(project, ".dm", "agents", "project-only.md"), "project-only", "Project only");
		writeJson(path.join(home, ".dm", "agent", "settings.json"), { subagents: { agentOverrides: { shared: { description: "User override" } } } });
		writeJson(path.join(project, ".dm", "settings.json"), { subagents: { agentOverrides: { shared: { description: "Project override" } } } });
	...
	it("invalidates when a new agent appears in a previously inspected nested directory", () => {
		const nestedDir = path.join(project, ".dm", "agents", "nested");
	...
	it("does not read out-of-scope user settings for a project snapshot", () => {
		writeAgent(path.join(project, ".dm", "agents", "project-agent.md"), "project-agent", "Project agent");
		const userSettingsPath = path.join(home, ".dm", "agent", "settings.json");
	...
	it("keeps shared package agent metadata scoped to the selected settings source", () => {
		...
		writeJson(path.join(home, ".dm", "agent", "settings.json"), { packages: [userPackageRoot] });
		writeJson(path.join(project, ".dm", "settings.json"), { packages: [projectPackageRoot] });
		writeJson(path.join(userPackageRoot, "package.json"), {
			name: "user-package",
			"dm-subagents": { agents: [path.relative(userPackageRoot, sharedAgentDir)] },
		});
		writeJson(path.join(projectPackageRoot, "package.json"), {
			name: "project-package",
			"dm-subagents": { agents: [path.relative(projectPackageRoot, sharedAgentDir)] },
		});
	...
	it("surfaces malformed in-scope settings while collecting package roots", () => {
		const settingsPath = path.join(home, ".dm", "agent", "settings.json");
	...
	it("invalidates when project-root policy changes", () => {
		...
		writeAgent(path.join(outer, ".dm", "agents", "outer.md"), "outer", "Outer agent");
		writeAgent(path.join(nested, ".dm", "agents", "nested.md"), "nested", "Nested agent");
		writeJson(path.join(nested, ".dm", "settings.json"), { subagents: { projectRootResolution: "git-root" } });

		const gitRoot = discoverAgents(nested, "both");
		assert.equal(gitRoot.projectAgentsDir, path.join(outer, ".dm", "agents"));
		...
		writeJson(path.join(nested, ".dm", "settings.json"), { subagents: { projectRootResolution: "nearest" } });
		const nearest = discoverAgents(nested, "both");
		assert.equal(nearest.projectAgentsDir, path.join(nested, ".dm", "agents"));
	...
	it("does not parse chains on the ordinary effective discovery fast path", () => {
		const agentPath = path.join(project, ".dm", "agents", "worker.md");
		const chainPath = path.join(project, ".dm", "chains", "broken.chain.md");
