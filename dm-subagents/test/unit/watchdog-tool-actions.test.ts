...
describe("watchdog tool actions", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-watchdog-tool-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dm-watchdog-tool-project-"));
		process.env.HOME = tempHome;
		process.env.USERPROFILE = tempHome;
		process.env.DM_CODING_AGENT_DIR = path.join(tempHome, ".dm", "agent");
	});
	...
	it("configures the recommended model for the current session without writing settings", () => {
		...
		assert.equal(fs.existsSync(path.join(tempHome, ".dm", "agent", "settings.json")), false);
	...
	it("persists the recommended model when user scope is explicit", () => {
		...
		assert.equal(result.isError, undefined);
		const settings = JSON.parse(fs.readFileSync(path.join(tempHome, ".dm", "agent", "settings.json"), "utf-8"));
