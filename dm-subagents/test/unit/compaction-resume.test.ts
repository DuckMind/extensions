...
describe("async compaction resume", () => {
	it("continues an interactive parent after compaction while async work is active", () => {
		const script = String.raw`
			...
			sent.length = 0;
			events.emit("subagent:async-started", { id: "running", pid: 1, sessionId: "compact-session", mode: "single", agent: "worker", asyncDir: path.join(os.tmpdir(), "dm-compaction-test-running") });
			widgets.length = 0;
			renders = 0;
			...
			events.emit("subagent:async-started", { id: "running-2", pid: 2, sessionId: "compact-session", mode: "single", agent: "worker", asyncDir: path.join(os.tmpdir(), "dm-compaction-test-running-2") });
			...
			events.emit("subagent:async-started", { id: "running-3", pid: 3, sessionId: "compact-session", mode: "single", agent: "worker", asyncDir: path.join(os.tmpdir(), "dm-compaction-test-running-3") });
			widgets.length = 0;
	...
	it("ignores stale cached UI context during compaction lifecycle callbacks", () => {
		const script = String.raw`
			...
			registerSubagentExtension(dm);
			handlers.get("session_start")({}, ctx);
			events.emit("subagent:async-started", { id: "running", sessionId: "stale-context-session", mode: "single", agent: "worker", asyncDir: "/tmp/dm-stale-context-running" });
			stale = true;
			handlers.get("session_compact")({ reason: "threshold" });
			handlers.get("session_before_compact")({ reason: "threshold", signal: new AbortController().signal });
			await handlers.get("session_shutdown")();
