...
describe("buildDoctorReport", () => {
	it("formats a bounded successful environment summary", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-doctor-success-"));
		try {
			...
			const report = buildDoctorReport({
				...
				deps: {
					...
					discoverAgentsAll: () => ({
						...
						agentDiagnostics: [{ source: "project", filePath: path.join(root, ".dm", "agents", "broken.md"), name: "broken", error: "invalid runner" }],
						chains: [makeChain("user-flow", "user"), makeChain("project-flow", "project")],
						userDir: path.join(root, "home", ".agents"),
						projectDir: path.join(root, ".dm", "agents"),
						userChainDir: path.join(root, "home", ".dm", "agent", "chains"),
						projectChainDir: path.join(root, ".dm", "chains"),
						userSettingsPath: path.join(root, "home", ".dm", "agent", "settings.json"),
						projectSettingsPath: path.join(root, ".dm", "settings.json"),
					}),
					...
					diagnoseIntercomBridge: () => ({
						...
						extensionDir: "native:dm-subagents-supervisor-channel",
						orchestratorTarget: "subagent-chat-abc123",
					}),
				},
			});
			...
			assert.match(report, /if runs\.all is missing, reload or update dm-subagents; await Promise\.all\(\[runs\.run\(\.\.\.\)\]\) is also supported/);
			assert.match(report, /- skills: total 2 \(project 1, user-package 1\)/);
			assert.match(report, /- bridge: active/);
			assert.match(report, /- supervisor channel: available \(native:dm-subagents-supervisor-channel\)/);
	...
	it("reports the effective source when the run fan-out environment value is invalid", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-doctor-fanout-source-"));
		const previous = process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN;
		try {
			process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN = "invalid";
			const report = buildDoctorReport({
				cwd: root,
			...
			if (previous === undefined) delete process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN;
			else process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN = previous;
			fs.rmSync(root, { recursive: true, force: true });
		}
	});

	it("keeps reporting when a directory or discovery check fails", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-doctor-failure-"));
		try {
