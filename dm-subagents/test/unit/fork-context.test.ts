...
import { SessionManager } from "@duckmind/dm-coding-agent";
...
describe("canPreferFork", () => {
	it("requires a persisted session file and a current leaf", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-prefer-fork-"));
		try {
...
describe("createForkContextResolver", () => {
	...
	it("opens a throwaway manager from the persisted parent session file", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-open-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("creates forked sessions through the default package opener", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-default-"));
		try {
	...
	it("nests forked sessions under a per-parent directory so dm -c never picks them", () => {
		// Regression test: fork files used to be created as top-level siblings of
		// the parent session. DM's findMostRecentSession (`dm -c`) scans that
		...
		// `dm -c` resumed the subagent instead of the conversation the user left.
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-nested-"));
		try {
			const sessionDir = path.join(tempDir, "sessions");
			const parent = SessionManager.create(tempDir, sessionDir);
			parent.appendMessage({ role: "user", content: "parent prompt" });
			...
			// Top-level listing — what dm -c sees — still contains only the parent.
	...
	it("fails clearly for an unflushed user-only parent", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-user-only-"));
		try {
			const sessionDir = path.join(tempDir, "sessions");
			const parent = SessionManager.create(tempDir, sessionDir);
			...
			assert.throws(
				() => resolver.sessionFileForIndex(0),
				/Failed to create forked subagent session: Parent session file does not exist: .*DM has not persisted enough history to fork yet\./,
			);
	...
	it("creates isolated branched sessions per index (parallel and chain compatible)", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-index-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			let count = 0;
	...
	it("memoizes per index to keep behavior deterministic", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-memo-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			let calls = 0;
	...
	it("persists a forked session when DM defers writing the branched file", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-deferred-child-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("removes signed Anthropic thinking blocks before forwarding a forked session", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("keeps requested thinking for forked children when the force-off predicate declines", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-keep-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("still forces thinking off when the predicate confirms an Anthropic child", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-confirm-off-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("allows unsigned thinking blocks in forked sessions", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-unsigned-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("allows non-Anthropic signed thinking blocks in forked sessions", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-openai-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("removes redacted thinking blocks before fallback-persisting a forked session", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-redacted-thinking-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const childSessionFile = path.join(tempDir, "child.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
	...
	it("fails clearly when branch extraction returns a missing child file without fallback state", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-missing-child-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
	...
	it("does not silently fallback to fresh when branch extraction fails", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-fork-no-path-"));
		try {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			writeMinimalSessionFile(parentSessionFile, "parent");
			const resolver = createForkContextResolver({
