...
describe("subagent extension RPC bridge", () => {
	it("emits ready and answers ping with versioned capability metadata", async () => {
		...
		assert.deepEqual(
			...
			{ kind: "dm-subagents.async-status-snapshot", version: 1 },
		);
		assert.deepEqual(
	...
	it("uses the async stop control path for stop", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-stop-"));
		try {
			const events = new FakeEvents();
	...
	it("acknowledges RPC child stop for exactly one async child", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-child-stop-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("uses the live workflow child stopper for RPC child stop", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-workflow-child-stop-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("uses the live workflow controller for RPC run-level workflow stop", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-workflow-run-stop-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("rejects RPC child stop when the child is absent or terminal", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-child-stop-reject-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("stops reload-recovered workflows through the durable control channel", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-stop-workflow-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("rejects stop requests for async runs from a different session", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-rpc-stop-session-"));
		try {
			const events = new FakeEvents();
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
