...
import { SUBAGENT_CHILD_ENV, SUBAGENT_FANOUT_CHILD_ENV } from "../../src/runs/shared/dm-args.ts";
...
function parentToolEnv(agentDir?: string): NodeJS.ProcessEnv {
	...
	if (agentDir) env.DM_CODING_AGENT_DIR = agentDir;
	return env;
}

describe("subagent extension child mode", () => {
	it("defers persistence when startup config expires a cached exclusion", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-model-exclusion-startup-"));
	...
	it("applies model exclusion TTL from extension config at registration", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-model-exclusion-config-"));
		const exclusionPath = path.join(agentDir, "model-exclusions.json");
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("shows omitted workflow async as background even when asyncByDefault is false", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-workflow-manifest-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
			...
			env.DM_CODING_AGENT_DIR = agentDir;
			execFileSync(process.execPath, ["--experimental-strip-types", "--import", "./test/support/register-loader.mjs", "--input-type=module", "--eval", script], { cwd: projectRoot, env, stdio: "pipe" });
		} finally {
			fs.rmSync(agentDir, { recursive: true, force: true });
		}
	...
	it("keeps summary inline tool display to one stable row for every supported state", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-inline-display-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("uses configured main-window renderer spacing for call rows", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-renderer-density-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("uses configured main-window renderer density for slash results", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-slash-renderer-density-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("registers bg_wait and its compatibility alias and honors waitTool disabled config", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-wait-tool-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("does not restore the async widget from tool results when asyncWidget is disabled", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-async-widget-config-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("shows active async work in the under-editor widget when FleetView is enabled", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-async-widget-fleet-"));
		try {
			const configDir = path.join(agentDir, "extensions", "subagent");
			fs.mkdirSync(configDir, { recursive: true });
	...
	it("keeps independent extension runtimes active in one process", () => {
		const script = String.raw`
			...
			function createRuntime(sessionId) {
				...
				return { dm, events, eventDeliveries, handlers, ctx };
			}

			const first = createRuntime("independent-first");
			registerSubagentExtension(first.dm);
			...
			registerSubagentExtension(second.dm);
			...
			if (process.env.DM_SUBAGENT_PARENT_SESSION !== "independent-second") {
			...
			if (process.env.DM_SUBAGENT_PARENT_SESSION !== undefined) {
	...
	it("disposes pending completion notifications on session shutdown", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-notify-shutdown-"));
		const configDir = path.join(agentDir, "extensions", "subagent");
		fs.mkdirSync(configDir, { recursive: true });
	...
	it("disposes pending completion notifications during runtime reload cleanup", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-notify-reload-"));
		const configDir = path.join(agentDir, "extensions", "subagent");
		fs.mkdirSync(configDir, { recursive: true });
		fs.writeFileSync(path.join(configDir, "config.json"), JSON.stringify({ completionBatch: { enabled: true, debounceMs: 150 } }), "utf-8");
		const script = String.raw`
			...
			function createRuntime() {
				...
				return { dm, events, handlers, sent, ctx };
			}

			const oldRuntime = createRuntime();
			registerSubagentExtension(oldRuntime.dm);
			...
			registerSubagentExtension(newRuntime.dm);
			...
			if (eventListeners.get("subagent:async-complete")?.size !== oldListenerCount
				|| process.env.DM_SUBAGENT_PARENT_SESSION !== "notify-reload-session") {
	...
	it("ignores the current stale UI context during runtime reload cleanup", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-stale-ui-reload-"));
		const configDir = path.join(agentDir, "extensions", "subagent");
		fs.mkdirSync(configDir, { recursive: true });
	...
	it("claims the explicit predecessor session during session replacement", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-session-transition-"));
		const configDir = path.join(agentDir, "extensions", "subagent");
		fs.mkdirSync(configDir, { recursive: true });
		...
		const script = String.raw`
			...
			const sessions = path.join(process.env.DM_CODING_AGENT_DIR, "sessions");
