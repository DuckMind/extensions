...
describe("native supervisor channel", () => {
	it("delivers requests only to the exact current session id and wakes the parent", () => {
		...
		const channel = createNativeSupervisorChannel(dm as never, makeState(currentSessionId, ctx), { platform: "darwin" });
	...
	it("uses polling instead of native watchers on Windows", () => {
		...
		let watchCalls = 0;
		const channel = createNativeSupervisorChannel(dm as never, makeState(currentSessionId, ctx), {
			platform: "win32",
			watch: (() => {
				watchCalls += 1;
	...
	it("activates Darwin supervisor polling only while child work is live", () => {
		...
		const intervals: number[] = [];
		const channel = createNativeSupervisorChannel(dm as never, state, {
			platform: "darwin",
	...
	it("matches supervisor requests against the runtime session id instead of persisted session file path", () => {
		...
		const channel = createNativeSupervisorChannel(dm as never, makeState(persistedSessionFile, ctx));
	...
	it("removes the request file when a child supervisor ask is cancelled", async () => {
		...
		registerNativeSupervisorClient(dm as never);
