...
describe("parallel handoff", () => {
	it("journals worktree ownership before child results exist", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-pending-"));
		try {
	...
	it("writes and aggregates versioned worktree handoff groups", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
	...
	it("records missing diff artifacts instead of throwing", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-missing-diff-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
	...
	it("rejects reusing a manifest path for another run", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-owner-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			const common = {
				manifestPath,
	...
	it("round-trips lane identity and resolves a workflow child to its manifest task", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-lane-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			const lane = { version: 1 as const, key: "writer", mode: "mutation" as const, sourceRef: "nicobailon/dm-subagents#1621", claims: ["src/shared/types.ts"], outputPaths: ["reports/lane.md"] };
			writeParallelHandoffGroup({
				manifestPath,
				runId: "workflow-run",
				mode: "single",
	...
	it("fails closed when persisted lane identity does not match the workflow key", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-lane-invalid-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writeParallelHandoffGroup({
				manifestPath,
	...
	it("records merge evidence and renders stored terminal eligibility without deleting", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-merge-evidence-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writeParallelHandoffGroup({
				manifestPath,
	...
	it("rejects mismatched lanes and stale reviewed heads", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-merge-stale-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writeParallelHandoffGroup({
				manifestPath,
	...
	it("keeps pending, unknown, and ineligible stored states fail closed", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-merge-unknown-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writePendingParallelHandoff({
				manifestPath,
	...
	it("refuses to record evidence while a stored child owner is active", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-active-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
	...
	it("records supersession evidence as an eligible stored state", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-parallel-handoff-supersession-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writeParallelHandoffGroup({
				manifestPath,
