...
describe("subagent guide", () => {
	it("reads the packaged overview by default", () => {
		...
		assert.match(guide, /# dm-subagents/);
	});
	...
	it("documents external CLI runner limits in packaged guide topics", () => {
		assert.match(readSubagentGuide("tool-reference"), /External CLI agent profiles[\s\S]*native DM child options[\s\S]*model override[\s\S]*native DM tools/);
		assert.match(readSubagentGuide("agents"), /External CLI agents use their own runner contract[\s\S]*native DM child options/);
	});
});
