...
describe("createTaskMutationArbiter", () => {
	it("is disabled by the env opt-out", () => {
		const previous = process.env.DM_SUBAGENTS_LLM_INTENT_ARBITER;
		process.env.DM_SUBAGENTS_LLM_INTENT_ARBITER = "0";
		try {
			assert.equal(createTaskMutationArbiter(fakeCtx()), undefined);
		} finally {
			if (previous === undefined) delete process.env.DM_SUBAGENTS_LLM_INTENT_ARBITER;
			else process.env.DM_SUBAGENTS_LLM_INTENT_ARBITER = previous;
		}
	});
