...
describe("builtin agent disabling", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-disabled-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-disabled-project-"));
	...
	it("hides disabled builtins from agent-facing management list output", () => {
		...
		const agentsDir = path.join(tempProject, ".dm", "agents");
