...
describe("async run status inspection", () => {
	...
	it("repairs stale running status and reports diagnosis plus result path", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-stale-"));
	...
	it("renders bounded recovery guidance from a failed status step", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-recovery-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows a follow-up hint for completed external-job runs when the provider supports it", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-external-follow-up-"));
		try {
	...
	it("normalizes old external-cli runner status before rendering adapter details", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-old-external-cli-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows parallel mode and aggregate progress for top-level async parallel runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-parallel-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("tails a readable transcript from async output artifacts", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("does not fall back to another child output when an explicit transcript index output is missing", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-index-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("does not advertise an output artifact that was never written", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-phantom-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("refuses to tail status outputFile paths outside the async directory", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-escape-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("escapes terminal control sequences in async output transcripts", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-unsafe-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("keeps run metadata when child output contains binary content", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-binary-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("refuses symlink session transcript paths even under trusted roots", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-session-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows an active read-only fleet view with transcript commands", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-fleet-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("scopes fleet active-run discovery to the current session", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-fleet-session-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("escapes terminal control sequences in remembered foreground transcripts", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-foreground-transcript-unsafe-"));
		try {
			const state = {
				currentSessionId: "session-current",
				asyncJobs: new Map(),
	...
	it("refuses transcript reads for async runs owned by another session", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-transcript-session-scope-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("does not fall back to aggregate result output for an explicit completed child index", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-result-index-fallback-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("surfaces steering counts and timestamps in exact and list status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-steering-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows nested runs under owning steps with exact status hints", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-nested-root-"));
	...
	it("repairs stale nested async descendants before rendering root status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-stale-nested-"));
	...
	it("shows a warning when nested projection fails for detailed status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-nested-warning-"));
	...
	it("shows a warning when nested projection fails for active status lists", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-nested-list-warning-"));
	...
	it("keeps safe nested session content parts around binary content", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-nested-session-binary-"));
	...
	it("resolves exact nested run ids from the nested registry", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-nested-exact-"));
	...
	it("shows indexed revive guidance for completed multi-child async runs with child sessions", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-multi-resume-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows direct run-id recovery for workflow children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-workflow-resume-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("keeps supervisor-detached workflow children out of generic revive guidance", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-workflow-detached-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("uses original child indexes when result metadata contains invalid children", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-original-index-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("labels chain parallel group children with logical step and agent numbers", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-chain-parallel-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("shows expected intercom target for still-running async steps", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-intercom-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("does not advertise a workflow steering command without a live foreground route", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-workflow-route-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects ambiguous async run id prefixes", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-ambiguous-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects path-like async run ids", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-paths-"));
		try {
	...
	it("does not advertise revive for result fallback with only a top-level session file", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-result-no-child-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("treats a top-level completed result as one transcript child", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-result-transcript-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("validates completed result transcript indexes as integers", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-result-transcript-index-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("shows stopped result-only runs without revive guidance", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-stopped-result-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("shows signal-terminated result-only runs as stopped", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-signal-result-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("falls back to an existing result when async dir has no status file", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-run-status-result-fallback-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
