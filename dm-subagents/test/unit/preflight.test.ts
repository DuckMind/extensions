...
function writeSkill(cwd: string, name: string): void {
	const skillDir = path.join(cwd, ".dm", "skills", name);
...
function writeMcpFixture(): void {
	const agentDir = process.env.DM_CODING_AGENT_DIR;
...
describe("public launch contract preflight", () => {
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-preflight-"));
	...
	it("resolves an ordinary single-agent contract without creating launch directories", async () => {
		...
		writeAgent(path.join(cwd, ".dm", "agents", "worker.md"), `---
name: worker
...
`);
		...
		try {
			...
			assert.equal(fs.existsSync(path.join(cwd, ".dm/subagents")), false);
		} finally {
			handle.dispose();
		}
	});
	...
	it("binds canonical extension metadata into public preflight provenance", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeAgent(path.join(cwd, ".dm", "agents", "binding-worker.md"), `---\nname: binding-worker\ndescription: Binding worker\n---\nWorker.\n`);
	...
	it("uses the parent provider for provider-scoped agent overrides", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeJson(path.join(cwd, ".dm", "settings.json"), {
			subagents: {
	...
	it("binds fast mode runtime extension into public preflight provenance", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeAgent(path.join(cwd, ".dm", "agents", "fast-worker.md"), `---
name: fast-worker
...
`);
	...
	it("enforces maxThinking before a child launch and accepts levels at the ceiling", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeJson(path.join(cwd, ".dm", "settings.json"), { subagents: { maxThinking: "xhigh" } });
		writeAgent(path.join(cwd, ".dm", "agents", "worker.md"), `---\nname: worker\ndescription: Project worker\nmodel: test/worker\nthinking: xhigh\n---\nWorker.\n`);
...
`);
	...
	it("uses subagents.defaultProvider when resolving launch model ids", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeJson(path.join(process.env.HOME!, ".dm", "agent", "settings.json"), {
...
`);
	...
	it("bypasses native model validation for external CLI runners", async () => {
		...
		writeAgent(path.join(cwd, ".dm", "agents", "external.md"), `---
name: external
...
Project prompt.
`);
		writeAgent(path.join(cwd, ".dm", "agents", "reviewer.md"), `---
name: reviewer
...
`);
	...
	it("changes definition and launch digests when selected agent content changes", async () => {
		const cwd = path.join(tempDir, "repo");
		fs.mkdirSync(cwd, { recursive: true });
		const agentPath = path.join(cwd, ".dm", "agents", "worker.md");
...
`);
		...
		assert.equal(before.ok, true);
		fs.writeFileSync(path.join(cwd, ".dm", "skills", "digest-skill", "SKILL.md"), "---\ndescription: updated digest-skill\n---\n\nUse digest-skill.\n", "utf-8");
		clearSkillCache();
...
`);
		...
		assert.match(missingAgent.message, /project: .*\.dm[\\/]agents \(1 candidate\)/);
		assert.match(missingAgent.message, /Discovered agents:\n[\s\S]*worker \(project\)/);
		writeAgent(path.join(cwd, ".dm", "agents", "broken.md"), `---
name: broken
...
`);
		...
		writeAgent(path.join(cwd, ".dm", "agents", "code-analysis.zeta-worker.md"), `---
name: zeta-worker
...
`);
		...
		writeAgent(path.join(process.env.DM_CODING_AGENT_DIR!, "agents", "foo.md"), `---
name: foo
description: User foo
---
User prompt.
`);
		writeAgent(path.join(cwd, ".dm", "agents", "acme.foo.md"), `---
name: foo
package: acme
...
`);
		...
		writeAgent(path.join(process.env.DM_CODING_AGENT_DIR!, "agents", "package-shadow.md"), `---
name: package-shadow
description: User package shadow
---
User prompt.
`);
		writeAgent(path.join(cwd, ".dm", "agents", "package-shadow.md"), `---
name: package-shadow
package: !!!
...
Legacy prompt.
`);
		writeAgent(path.join(cwd, ".dm", "agents", "shared.md"), `---
name: shared
...
`);
		...
		fs.writeFileSync(path.join(cwd, ".dm", "settings.json"), JSON.stringify({ packages: [packageRoot] }));
		writeAgent(path.join(process.env.DM_CODING_AGENT_DIR!, "agents", "ambiguous.md"), `---
name: ambiguous
description: User ambiguous
---
User prompt.
`);
		writeAgent(path.join(cwd, ".dm", "agents", "ambiguous.md"), `---
name: ambiguous
...
`);
	...
	it("projects MCP, extension, fanout, structured-output, and fork diagnostics", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeMcpFixture();
		writeAgent(path.join(cwd, ".dm", "agents", "fanout.md"), `---
name: fanout
...
Project prompt.
`);
		writeJson(path.join(process.env.DM_CODING_AGENT_DIR!, "extensions", "subagent", "config.json"), {
