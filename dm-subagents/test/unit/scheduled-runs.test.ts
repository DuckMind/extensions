...
function harness(options: { cwd?: string; sessionId?: string; now?: number; config?: ExtensionConfig; randomId?: () => string } = {}): Harness {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-test-"));
	roots.push(root);
...
describe("schedule helpers", () => {
	...
	it("uses a stable project store independent of session id", () => {
		...
		assert.equal(scheduledRunStorePath("/project"), path.join(path.resolve("/project"), ".dm/subagents", "schedules"));
	});
...
describe("project schedule management", () => {
	...
	it("treats a deleted project cwd as having no schedules during restore and listing", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-deleted-project-"));
		roots.push(root);
	...
	it("rejects a default project schedule root that escapes through .dm/subagents", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-root-link-"));
		roots.push(root);
		const project = path.join(root, "project");
		...
		manager.bindSession(ctx);
		fs.mkdirSync(path.join(project, ".dm"));
		fs.symlinkSync(outside, path.join(project, ".dm/subagents"), process.platform === "win32" ? "junction" : "dir");
	...
	it("allows a default project schedule root through a shared Git worktree .dm symlink", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-worktree-link-"));
		roots.push(root);
		...
		fs.mkdirSync(path.join(repository, ".dm"), { recursive: true });
		fs.symlinkSync(path.join(repository, ".dm"), path.join(worktree, ".dm"), process.platform === "win32" ? "junction" : "dir");
		...
		assert.equal(result.isError, undefined);
		assert.equal(fs.existsSync(path.join(repository, ".dm", "subagents", "schedules", "shared", "schedule.json")), true);
	});

	it("rejects a Git worktree .dm symlink outside the shared config root", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-worktree-escape-"));
		roots.push(root);
		const repository = path.join(root, "repository");
		const worktree = path.join(repository, ".worktrees", "feature");
		const outside = path.join(root, "outside");
		...
		fs.mkdirSync(path.join(repository, ".dm"), { recursive: true });
		fs.mkdirSync(outside);
		fs.symlinkSync(outside, path.join(worktree, ".dm"), process.platform === "win32" ? "junction" : "dir");
	...
	it("rejects separate-git-dir primary checkout sharing without a registered root", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-separate-git-dir-"));
		roots.push(root);
		const repository = path.join(root, "repository");
		...
		git(repository, ["commit", "-m", "base"]);
		git(repository, ["worktree", "add", "-b", "feature", worktree]);
		const primaryConfig = path.join(repository, ".dm");
		fs.mkdirSync(primaryConfig, { recursive: true });
		fs.symlinkSync(primaryConfig, path.join(worktree, ".dm"), process.platform === "win32" ? "junction" : "dir");
	...
	it("rejects an unregistered checkout ancestor with the same Git directory", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-schedule-unregistered-git-dir-"));
		roots.push(root);
		const repository = path.join(root, "repository");
		const worktree = path.join(repository, ".worktrees", "feature");
		const outside = path.join(root, "outside");
		...
		fs.mkdirSync(path.dirname(worktree), { recursive: true });
		git(repository, ["worktree", "add", "-b", "feature", worktree]);
		const outsideConfig = path.join(outside, ".dm");
		fs.mkdirSync(outsideConfig, { recursive: true });
		fs.writeFileSync(path.join(outside, ".git"), `gitdir: ${path.join(repository, ".git")}\n`, "utf-8");
		fs.symlinkSync(outsideConfig, path.join(worktree, ".dm"), process.platform === "win32" ? "junction" : "dir");
