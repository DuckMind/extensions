...
describe("profiles helpers", () => {
	beforeEach(() => {
		homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subprofiles-home-"));
	...
	it("applies a saved profile while preserving unrelated subagent settings", () => {
		...
		const settingsPath = path.join(homeDir, ".dm", "agent", "settings.json");
	...
	it("applies profile models and thinking to user agents without frontmatter pins", () => {
		...
		const agentDir = path.join(homeDir, ".dm", "agent", "agents");
	...
	it("refreshes a provider model catalog using the resolved DM executable and writes a cache file", async () => {
		const piBinary = path.join(homeDir, "dm-wrapper.exe");
		process.env[DM_SUBAGENT_PI_BINARY_ENV] = piBinary;
		...
		const result = await refreshProviderModelCatalog(dm, ctx as never, "openai-codex");
	...
	it("does not count heuristic fallback when official metadata is present", async () => {
		...
		const result = await refreshProviderModelCatalog(dm, ctx as never, "openai");
	...
	it("reuses a fresh provider model catalog", async () => {
		...
		const result = await refreshProviderModelCatalog(dm, makeCtx(process.cwd(), []) as never, "openai-codex");
	...
	it("generates quota and quality profiles from sorted provider models", async () => {
		...
		const result = await generateProfilesForProvider(dm, ctx as never, "openai-codex");
		assert.equal(result.heuristicFallbackCount, 4);
	...
	it("does not treat provider names like MiniMax as mini-tier tokens and prefers M3 over M2.7-highspeed for quality", async () => {
		...
		const refreshed = await refreshProviderModelCatalog(dm, ctx as never, "minimax");
		...
		const result = await generateProfilesForProvider(dm, ctx as never, "minimax", { forceRefresh: true });
	...
	it("checks a profile against the registry and live probe", async () => {
		...
		const result = await checkSubagentProfile(dm, ctx as never, "demo");
