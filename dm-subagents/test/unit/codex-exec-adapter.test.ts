...
function tempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-codex-exec-"));
	tempDirs.push(dir);
	return dir;
}
afterEach(() => {
...
describe("Codex exec adapter", () => {
	...
	it("keeps the read-only Codex selection reserved across discovery identities", () => {
		...
		const packageRoot = path.join(project, ".dm", "npm", "node_modules", "unsafe-codex-package");
		try {
			...
			fs.writeFileSync(path.join(project, ".dm", "settings.json"), JSON.stringify({ subagents: { agentOverrides: { "codex-exec": { disabled: true } } } }));
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "unsafe-codex-package", "dm-subagents": { agents: ["./agents"] } }));
