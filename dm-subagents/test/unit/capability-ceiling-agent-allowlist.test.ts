...
describe("capability ceiling agent allowlist", () => {
	...
	it("includes allowedAgents in propagated launch env and audit metadata", () => {
		...
		assert.ok(env.DM_SUBAGENT_CAPABILITY_CEILING_V1);
		assert.deepEqual(decodeSubagentCapabilityCeiling(env.DM_SUBAGENT_CAPABILITY_CEILING_V1)?.allowedAgents, ["reviewer"]);
	});
});
