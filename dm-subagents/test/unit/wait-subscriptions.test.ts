...
describe("non-blocking wait subscriptions", () => {
	it("returns immediately and binds an id prefix to one exact run", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-arm-"));
		try {
	...
	it("rejects non-blocking subscriptions from headless tool calls", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-headless-"));
		try {
	...
	it("restores durable registrations and wakes on exact completion", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-restore-"));
		const asyncRoot = path.join(root, "runs");
		...
		try {
			...
			const first = createWaitSubscriptionManager(dm as never, firstState, { asyncDirRoot: asyncRoot, resultsDir, subscriptionsDir, pollIntervalMs: 60_000, kill: () => true });
			...
			const restored = createWaitSubscriptionManager(dm as never, restoredState, { asyncDirRoot: asyncRoot, resultsDir, subscriptionsDir, pollIntervalMs: 60_000, kill: () => true });
			restored.restore();
	...
	it("keeps subscriptions active across a session restart", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-restart-"));
		const asyncRoot = path.join(root, "runs");
		const subscriptionsDir = path.join(root, "subscriptions");
		const bus = new TestBus();
		const sent: string[] = [];
	...
	it("tells the parent to revive the failed child before replacing resumable async runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-revive-first-"));
		const asyncRoot = path.join(root, "runs");
		const subscriptionsDir = path.join(root, "subscriptions");
	...
	it("tells the parent to reply and wait for an intercom-detached failed async run", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-intercom-detach-"));
		const asyncRoot = path.join(root, "runs");
		const subscriptionsDir = path.join(root, "subscriptions");
	...
	it("waits for foreground run restoration before reconciling a restored subscription", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-foreground-restore-"));
		const subscriptionsDir = path.join(root, "subscriptions");
		const bus = new TestBus();
		const sent: string[] = [];
		const firstState = makeState();
		...
		const first = createWaitSubscriptionManager(dm as never, firstState, { subscriptionsDir, pollIntervalMs: 60_000 });
		try {
			...
			const restored = createWaitSubscriptionManager(dm as never, restoredState, { subscriptionsDir, pollIntervalMs: 60_000 });
			try {
				restored.restore();
	...
	it("settles a missing live foreground run as unreconcilable", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-foreground-missing-"));
		const state = makeState();
	...
	it("keeps a subscription armed when cleanup fails before delivery", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-cleanup-failure-"));
		const asyncRoot = path.join(root, "runs");
		const subscriptionsDir = path.join(root, "subscriptions");
		const sent: string[] = [];
		const state = makeState();
	...
	it("wakes when an exact async run cannot be reconciled", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-reconcile-error-"));
	...
	it("wakes for attention and timeout without treating subscriptions as child work", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-outcomes-"));
		const asyncRoot = path.join(root, "runs");
		const resultsDir = path.join(root, "results");
		const subscriptionsDir = path.join(root, "subscriptions");
		const bus = new TestBus();
...
describe("wait subscriptions armed by another session", () => {
	...
	it("sweeps an expired record whose owning session never came back", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-orphan-"));
		const subscriptionsDir = path.join(root, "subscriptions");
		const sent: string[] = [];
		const dm = {
			events: new TestBus(),
		...
		const armer = createWaitSubscriptionManager(dm as never, owner, {
			subscriptionsDir,
		...
		try {
			...
			const restarted = createWaitSubscriptionManager(dm as never, other, {
				subscriptionsDir,
				pollIntervalMs: 60_000,
				now: () => now,
			});
	...
	it("keeps another session's record until it expires", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-foreign-live-"));
		const subscriptionsDir = path.join(root, "subscriptions");
		const dm = {
			events: new TestBus(),
			sendMessage() {},
	...
	it("sweeps a foreign record that expires while another session stays open", () => {
		...
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-late-expiry-"));
		const subscriptionsDir = path.join(root, "subscriptions");
		const dm = {
			events: new TestBus(),
			sendMessage() {},
		...
		let now = 1_000;
		const armer = createWaitSubscriptionManager(dm as never, makeState("session-a"), {
			subscriptionsDir,
			pollIntervalMs: 60_000,
			now: () => now,
		});
		try {
			...
			const other = makeState("session-b");
			const open = createWaitSubscriptionManager(dm as never, other, {
				subscriptionsDir,
				pollIntervalMs: 60_000,
				now: () => now,
			});
	...
	it("keeps an expired foreign record through the grace window", () => {
		...
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-grace-"));
		const subscriptionsDir = path.join(root, "subscriptions");
		const dm = { events: new TestBus(), sendMessage() {} };
		let now = 1_000;
		const armer = createWaitSubscriptionManager(dm as never, makeState("session-a"), {
			subscriptionsDir,
			pollIntervalMs: 60_000,
		...
		try {
			...
			const other = createWaitSubscriptionManager(dm as never, makeState("session-b"), {
				subscriptionsDir,
				pollIntervalMs: 60_000,
				now: () => now,
			});
	...
	it("still gives the owning session its timeout notice after a restart", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-wait-subscribe-owner-timeout-"));
		const asyncRoot = path.join(root, "runs");
		const subscriptionsDir = path.join(root, "subscriptions");
		const sent: string[] = [];
		const dm = {
		...
		const first = createWaitSubscriptionManager(dm as never, makeState("session-a"), {
			asyncDirRoot: asyncRoot,
			subscriptionsDir,
			pollIntervalMs: 60_000,
			now: () => now,
		...
		try {
			...
			const resumed = createWaitSubscriptionManager(dm as never, makeState("session-a"), {
				asyncDirRoot: asyncRoot,
				subscriptionsDir,
				pollIntervalMs: 60_000,
				now: () => now,
