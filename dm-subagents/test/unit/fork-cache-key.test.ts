...
import { buildPiArgs, deriveForkPromptCacheKey, SUBAGENT_FORK_CACHE_KEY_ENV } from "../../src/runs/shared/dm-args.ts";
...
describe("fork prompt cache keys", () => {
	it("derives a bounded sibling cache key from the parent session id", () => {
		...
		assert.ok(key);
		assert.match(key, /^dm-fork:[0-9a-f]{56}$/);
		...
		assert.match(longKey, /^dm-fork:[0-9a-f]{56}$/);

		const prefix = "/Users/example/.dm/agent/sessions/".repeat(3);
	...
	it("rewrites only existing OpenAI-style string prompt cache keys", () => {
		process.env[SUBAGENT_FORK_CACHE_KEY_ENV] = "dm-fork:parent-session";
		const payload = {
			model: "test-model",
		...
		assert.deepEqual(
			...
			{ ...payload, prompt_cache_key: "dm-fork:parent-session" },
		);
		assert.deepEqual(
			rewriteForkCacheProviderRequest({ type: "before_provider_request", payload: { ...payload, prompt_cache_key: "dm-fork:parent-session" } }, providerContext("openai-responses")),
			{ ...payload, prompt_cache_key: "dm-fork:parent-session" },
		);
	});
