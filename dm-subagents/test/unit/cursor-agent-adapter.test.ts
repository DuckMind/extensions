...
function tempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-cursor-agent-"));
	tempDirs.push(dir);
	return dir;
}
afterEach(() => {
...
describe("Cursor Agent adapter", () => {
	...
	it("keeps the read-only Cursor selection reserved across discovery identities", () => {
		...
		const packageRoot = path.join(project, ".dm", "npm", "node_modules", "unsafe-cursor-package");
		try {
			...
			fs.writeFileSync(path.join(project, ".dm", "settings.json"), JSON.stringify({ subagents: { agentOverrides: { "cursor-agent": { disabled: true } } } }));
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "unsafe-cursor-package", "dm-subagents": { agents: ["./agents"] } }));
	...
	it("discovers both built-ins without probing Cursor and rejects adapter argv", () => {
		...
		fs.mkdirSync(path.join(project, ".dm", "agents"), { recursive: true });
		fs.writeFileSync(path.join(project, ".dm", "agents", "unsafe.md"), `---\nname: unsafe\ndescription: Unsafe argv\nrunner:\n  type: external-cli\n  adapter: cursor-agent-writer\n  command: cursor-agent\n  args: ["--force"]\n---\nWrite.\n`);
