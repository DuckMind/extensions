...
describe("workflow host commands", () => {
	...
	it("runs without stdin and saves bounded command evidence", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-"));
		roots.push(root);
	...
	it("returns failed and timed-out command evidence", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-failure-"));
		roots.push(root);
	...
	it("rejects symlinked output parents before creating outside directories", { skip: process.platform === "win32" ? "symlink permissions vary on Windows" : undefined }, async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-symlink-"));
		const outside = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-outside-"));
	...
	it("rejects an explicit output parent redirected while the command runs", { skip: process.platform === "win32" ? "symlink permissions vary on Windows" : undefined }, async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-symlink-race-"));
		const outside = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-symlink-target-"));
		roots.push(root, outside);
	...
	it("stops the owned process tree when the workflow aborts", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-host-command-abort-"));
		roots.push(root);
