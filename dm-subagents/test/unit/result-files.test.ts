...
describe("result file indexes", () => {
	it("resolves run ids without enumerating session indexes", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-run-index-"));
	...
	it("removes orphan index entries without deleting flat result files", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-index-"));
		try {
	...
	it("promotes an indexed pending result payload", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-pending-payload-"));
	...
	it("writes an indexed pending result without publishing it", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-pending-only-"));
		const originalError = console.error;
		try {
			console.error = () => {};
	...
	it("keeps pending results for long session ids below filesystem component limits", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-long-session-"));
		const originalError = console.error;
		try {
			console.error = () => {};
			const sessionId = `/Users/zhouatie/.config/dm/sessions/${"界".repeat(100)}.jsonl`;
	...
	it("keeps pending results for near-limit run ids below filesystem component limits", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-long-run-"));
		const originalError = console.error;
		try {
			console.error = () => {};
	...
	it("keeps a legacy valid index while the result payload is not visible yet", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-legacy-late-payload-"));
		try {
			const resultPath = path.join(resultsDir, "late.json");
	...
	it("keeps an unindexed pending result recoverable when its session index cannot be written", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-index-failure-"));
		try {
	...
	it("treats a missing mission observer index directory as absent without logging", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-observer-enotdir-"));
		const originalError = console.error;
	...
	it("does not commit a result payload without a session id", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-no-session-"));
		try {
			const resultPath = path.join(resultsDir, "blocked.json");
	...
	it("prefers pending payload over an older public result", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-pending-wins-"));
		const originalError = console.error;
		try {
			console.error = () => {};
			const resultPath = path.join(resultsDir, "blocked.json");
	...
	it("keeps a newer pending payload readable when Windows cannot replace the public result", (t) => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-windows-pending-wins-"));
		try {
			const resultPath = path.join(resultsDir, "blocked.json");
			writeAsyncResultFile(resultPath, { id: "blocked", runId: "blocked", sessionId: "session-a", success: false });
	...
	it("removes pending payloads with result indexes", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-pending-cleanup-"));
		const originalError = console.error;
		try {
			console.error = () => {};
	...
	it("indexes results with oversized provider tool-call ids", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-long-tool-call-"));
		try {
	...
	it("commits a result payload when only an optional index fails", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-optional-index-"));
		const originalError = console.error;
		try {
			console.error = () => {};
	...
	it("round-trips results for Windows session file paths", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-win-session-"));
		try {
			const sessionId = String.raw`C:\Users\theap\.dm\agent\sessions\leaf.jsonl`;
	...
	it("treats an unaddressable legacy session alias as absent and keeps canonical fallback candidates", (t) => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-legacy-enametoolong-"));
		const sessionId = String.raw`C:\Users\theap\.dm\agent\sessions\leaf.jsonl`;
	...
	it("leaves an unaddressable legacy index untouched during cleanup", (t) => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-cleanup-enametoolong-"));
		const sessionId = String.raw`C:\Users\theap\.dm\agent\sessions\leaf.jsonl`;
	...
	it("reads a pre-hash URI-encoded session index after the encoding change", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-legacy-session-"));
		try {
			const sessionId = String.raw`C:\Users\theap\.dm\agent\sessions\leaf.jsonl`;
	...
	it("reads a pre-hash extension-like run index filename", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-legacy-run-"));
		try {
			const sessionId = "session-a";
	...
	it("reads a pre-hash extension-like pending filename", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-legacy-pending-"));
		const originalError = console.error;
		try {
			const sessionId = "session-a";
			const runId = "legacy.jsonl";
	...
	it("throws access-denied direct session index reads", (t) => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-eacces-index-"));
	...
	it("returns no session candidates when the session index is unlistable", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-files-eperm-scan-"));
