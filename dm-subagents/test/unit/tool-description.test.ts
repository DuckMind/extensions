...
describe("registered subagent tool description", () => {
	it("uses split metadata by default", () => {
		...
		assert.match(description, /External CLI agents.*model override.*native DM tools/i);
		assert.match(metadata.promptGuidelines!.join("\n"), /External CLI agents.*model override.*native DM tools/i);
	...
	it("keeps the full description when configured", () => {
		...
		assert.match(description, /cannot access filesystem, shell, arbitrary DM tools, or host globals/i);
	...
	it("renders a custom project description with placeholders and mandatory safety guidance", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-project-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-agent-"));
		const projectConfigDir = path.join(cwd, ".dm");
	...
	it("appends full safety guidance when custom prose only includes the safety heading", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-heading-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-agent-"));
		fs.mkdirSync(path.join(cwd, ".dm"), { recursive: true });
		fs.writeFileSync(
			path.join(cwd, ".dm", "subagent-tool-description.md"),
	...
	it("keeps mandatory safety guidance last when custom prose embeds it before an override", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-injection-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-agent-"));
		fs.mkdirSync(path.join(cwd, ".dm"), { recursive: true });
		fs.writeFileSync(
			path.join(cwd, ".dm", "subagent-tool-description.md"),
	...
	it("preserves custom guidance while trimming built-in legacy chain guidance", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-legacy-note-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-agent-"));
		fs.mkdirSync(path.join(cwd, ".dm"), { recursive: true });
		fs.writeFileSync(
			path.join(cwd, ".dm", "subagent-tool-description.md"),
	...
	it("falls back to full mode when custom mode has no valid file", () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-missing-"));
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-agent-"));
		const warnings: string[] = [];
	...
	it("registers split, full, compact, custom, and fallback descriptions from extension config", () => {
		const defaultAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-default-"));
		...
		const fullAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-full-"));
		...
		const compactAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-compact-"));
		...
		const customAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-custom-"));
		...
		const missingCustomAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-missing-"));
		...
		const invalidAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-invalid-"));
	...
	it("registers the trimmed schema and description by default", () => {
		const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-tool-desc-trimmed-"));
