...
function createFixture() {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-args-perm-"));
	tempRoots.push(root);
...
describe("resolvePermissionSystemExtension", () => {
	...
	it("throws when an installed extension dir has no package.json", () => {
		const { agentDir } = createFixture();
		const extDir = path.join(agentDir, "extensions", "dm-permission-system");
	...
	it("throws when package.json has no valid dm.extensions entry", () => {
		const { agentDir } = createFixture();
		process.env.DM_CODING_AGENT_DIR = agentDir;
		const extDir = path.join(agentDir, "extensions", "dm-permission-system");
		const pkgPath = path.join(extDir, "package.json");
		...
		for (const value of [
			{ name: "test" },
			{ name: "test", dm: { extensions: "./src/index.ts" } },
			{ name: "test", dm: { extensions: [123] } },
		]) {
	...
	it("uses a valid fallback installation when the first candidate is malformed or missing a manifest", () => {
		for (const primarySetup of [
			...
			const fallbackDir = path.join(agentDir, "extensions", "dm-permission-system");
			fs.mkdirSync(primaryDir, { recursive: true });
			primarySetup(primaryDir);
			fs.mkdirSync(path.join(fallbackDir, "src"), { recursive: true });
			fs.writeFileSync(
				path.join(fallbackDir, "package.json"),
				JSON.stringify({ name: "test", dm: { extensions: ["./src/index.ts"] } }),
			);
			fs.writeFileSync(
	...
	it("returns extension path when fully installed", () => {
		...
		assert.ok(
			result.endsWith(path.join("dm-permission-system", "src", "index.ts")),
		);
	});
...
describe("resolvePiLaunchToolPlan with permission system", () => {
	...
	it("does NOT include permission system when not installed", () => {
		...
		const permExt = plan.runtimeExtensions.find((e) =>
			e.includes("dm-permission-system"),
		);
