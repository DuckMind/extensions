...
describe("DM_CODING_AGENT_DIR runtime paths", () => {
	beforeEach(() => {
		oldAgentDir = process.env.DM_CODING_AGENT_DIR;
		oldHome = process.env.HOME;
		oldUserProfile = process.env.USERPROFILE;
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-coding-agent-dir-"));
	...
	it("resolves the agent dir dynamically and loads extension config from it", () => {
		...
		assert.equal(getProjectConfigDir(cwd), path.join(cwd, ".dm"));
		assert.equal(getAgentDir(), agentDir);

		process.env.DM_CODING_AGENT_DIR = "~";
		assert.equal(getAgentDir(), tempHome);

		process.env.DM_CODING_AGENT_DIR = "~/custom-agent-dir";
		assert.equal(getAgentDir(), path.join(tempHome, "custom-agent-dir"));

		delete process.env.DM_CODING_AGENT_DIR;
		assert.equal(getAgentDir(), path.join(tempHome, ".dm", "agent"));
...
`);
	...
	it("ignores nested .dm and sync-backups agent definitions", () => {
...
`);
		const nestedBackupAgentPath = path.join(userAgentsDir, ".dm", "agent", "sync-backups", "20260712-163714", "agents", "stale.md");
...
`);
		...
		writeFile(path.join(packageRoot, "package.json"), JSON.stringify({ name: "env-package", dm: { skills: ["./skills/package-skill.md"] } }, null, 2));
...
`);
	...
	it("uses the configured agent dir for subagent bridge instruction files", () => {
		...
		assert.equal(diagnostic.extensionDir, "native:dm-subagents-supervisor-channel");
		...
		assert.equal(bridge.extensionDir, "native:dm-subagents-supervisor-channel");
