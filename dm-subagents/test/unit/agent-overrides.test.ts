...
const originalExtraAgentDirs = process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
...
function writeProjectAgent(cwd: string, name: string, body: string): void {
	const filePath = path.join(cwd, ".dm", "agents", `${name}.md`);
	fs.mkdirSync(path.dirname(filePath), { recursive: true });
	fs.writeFileSync(filePath, body, "utf-8");
}

function writeUserAgent(home: string, name: string, body: string): void {
	const filePath = path.join(home, ".dm", "agent", "agents", `${name}.md`);
	fs.mkdirSync(path.dirname(filePath), { recursive: true });
	fs.writeFileSync(filePath, body, "utf-8");
}
...
describe("builtin agent overrides", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-project-"));
	...
	afterEach(() => {
		...
		if (originalExtraAgentDirs === undefined) delete process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
		else process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS = originalExtraAgentDirs;
	...
	it("applies subagents.defaultModel to builtin agents with explicit overrides winning", () => {
		writeJson(path.join(tempHome, ".dm", "agent", "settings.json"), {
			subagents: {
	...
	it("lets a builtin agent inherit DM's normal tools from an override", () => {
		writeJson(path.join(tempHome, ".dm", "agent", "settings.json"), {
			subagents: {
				agentOverrides: {
	...
	it("prefers project subagents.defaultModel over user defaultModel", () => {
		fs.mkdirSync(path.join(tempProject, ".dm"), { recursive: true });
		writeJson(path.join(tempHome, ".dm", "agent", "settings.json"), {
			subagents: { defaultModel: "deepseek-v4-flash" },
		});
		writeJson(path.join(tempProject, ".dm", "settings.json"), {
	...
	it("applies user settings overrides to builtin agents", () => {
		...
		assert.equal(reviewer.override?.path, path.join(tempHome, ".dm", "agent", "settings.json"));
	});
	...
	it("globally disables builtin thinking suffix defaults from user settings", () => {
		...
		assert.equal(
			...
			path.join(tempHome, ".dm", "agent", "settings.json"),
		);
	});
	...
	it("prefers project settings overrides over user settings overrides", () => {
		...
		assert.equal(reviewer.override?.path, path.join(tempProject, ".dm", "settings.json"));
	});
	...
	it("keeps provider overrides unambiguous for agents named like override fields", () => {
		...
		fs.mkdirSync(path.join(tempHome, ".dm", "agent", "agents"), { recursive: true });
		for (const name of ["model", "tools"]) {
			fs.writeFileSync(path.join(tempHome, ".dm", "agent", "agents", `${name}.md`), `---\nname: ${name}\ndescription: ${name}\n---\n`, "utf-8");
		}
	...
	it("does not read malformed out-of-scope settings files", () => {
		fs.mkdirSync(path.join(tempProject, ".dm"), { recursive: true });
		fs.mkdirSync(path.join(tempHome, ".dm", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHome, ".dm", "agent", "settings.json"), '{"subagents":', "utf-8");
		writeJson(path.join(tempProject, ".dm", "settings.json"), {
			subagents: { agentOverrides: { reviewer: { model: "openai-codex/gpt-5.4-mini" } } },
		});
	...
	it("fills in unset fields on a custom project agent from project agentOverrides", () => {
		...
		assert.equal(implementer.override?.path, path.join(tempProject, ".dm", "settings.json"));
	});
	...
	it("applies output and defaultReads overrides to bundled and package agents and supports false clears", () => {
		...
		writeJson(path.join(packageRoot, "package.json"), { "dm-subagents": { agents: ["agents"] } });
	...
	it("builds description changes and false sentinels when an override clears builtin fields", () => {
		...
		const savedOverride = JSON.parse(fs.readFileSync(path.join(tempProject, ".dm", "settings.json"), "utf-8"));
