...
function projectFixture() {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-mission-lifecycle-"));
	const projectRoot = path.join(root, "project");
	fs.mkdirSync(projectRoot, { recursive: true });
	return { root, projectRoot, missionConfig: { directory: ".dm/subagents/missions", globalIndexDir: path.join(root, "global-index") } };
}
...
describe("mission launch lifecycle", () => {
	...
	it("explains when an explicit mission belongs to another worktree", () => {
		...
		try {
			...
			assert.ok(binding);
			const missionDir = path.join(worktreeRoot, ".dm", "subagents", "missions");
			assert.throws(() => prepareMissionLaunch({
