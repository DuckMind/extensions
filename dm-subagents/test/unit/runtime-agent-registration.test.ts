...
const originalPiCodingAgentDir = process.env.DM_CODING_AGENT_DIR;
...
function writeProjectAgent(name: string, aliases: string[] = []): void {
	const filePath = path.join(tempProject, ".dm", "agents", `${name}.md`);
	fs.mkdirSync(path.dirname(filePath), { recursive: true });
	fs.writeFileSync(filePath, `---\nname: ${name}\ndescription: Project agent\n${aliases.length ? `aliases: ${aliases.join(", ")}\n` : ""}---\n\nProject prompt.\n`, "utf-8");
}

function writeUserAgent(name: string, aliases: string[] = []): void {
	const filePath = path.join(tempHome, ".dm", "agent", "agents", `${name}.md`);
	fs.mkdirSync(path.dirname(filePath), { recursive: true });
...
describe("runtime agent registration", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "dm-runtime-agent-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dm-runtime-agent-project-"));
	...
	afterEach(() => {
		clearRuntimeAgentsForPi(dm);
		if (originalHome === undefined) delete process.env.HOME;
		else process.env.HOME = originalHome;
		if (originalUserProfile === undefined) delete process.env.USERPROFILE;
		else process.env.USERPROFILE = originalUserProfile;
		if (originalPiCodingAgentDir === undefined) delete process.env.DM_CODING_AGENT_DIR;
		else process.env.DM_CODING_AGENT_DIR = originalPiCodingAgentDir;
		fs.rmSync(tempHome, { recursive: true, force: true });
		fs.rmSync(tempProject, { recursive: true, force: true });
	});

	it("adds runtime agents to extension discovery without writing config", () => {
		const settingsPath = path.join(tempHome, ".dm", "agent", "settings.json");
		...
		const agents = mergeRuntimeAgents(dm, discoverAgents(tempProject, "both")).agents;
		...
		assert.equal(mergeRuntimeAgents(dm, discoverAgents(tempProject, "both")).agents.some((candidate) => candidate.name === "runtime-helper"), false);
		registration.dispose();
	});
	...
	it("registers through the owner runtime when consumer and owner API objects differ", () => {
		...
		assert.throws(
			() => registerAgentViaEvents({ dm: consumerPi, name: "runtime-event-helper", definition: { description: "Duplicate", systemPrompt: "Duplicate." } }),
	...
	it("returns useful event registration errors and detects a missing owner", () => {
		...
		unsubscribe();
		assert.throws(
			() => registerAgentViaEvents({ dm: consumer, name: "no-owner", definition: { description: "Missing", systemPrompt: "Missing." } }),
		...
		assert.throws(
			() => registerAgentViaEvents({ dm: consumer, name: "malformed-result", definition: { description: "Malformed", systemPrompt: "Malformed." } }),
	...
	it("lists runtime agents for the matching DM runtime", () => {
		registerAgent({
			dm,
			name: "runtime-helper",
		...
		const listed = handleList({}, { cwd: tempProject, modelRegistry: { getAvailable: () => [] }, runtimeAgentOwner: dm });
	...
	it("fails closed for builtin and duplicate runtime identities", () => {
		assert.throws(
			() => registerAgent({ dm, name: "claude-code", definition: { description: "Unsafe", systemPrompt: "Write.", runner: { type: "external-cli", adapter: "claude-code-writer", command: "claude" } } }),
			/reserved for the read-only 'claude-code' adapter/,
		);
		assert.throws(
			() => registerAgent({ dm, name: "runtime-writer", definition: { description: "Unsafe alias", systemPrompt: "Write.", aliases: ["claude-code"], runner: { type: "external-cli", adapter: "claude-code-writer", command: "claude" } } }),
		...
		for (const [readOnly, writer, command] of [["codex-exec", "codex-exec-writer", "codex"], ["cursor-agent", "cursor-agent-writer", "cursor-agent"]] as const) {
			assert.throws(
				() => registerAgent({ dm, name: readOnly, definition: { description: "Unsafe", systemPrompt: "Write.", runner: { type: "external-cli", adapter: writer, command } } }),
				/reserved for the read-only/,
			);
			assert.throws(
				() => registerAgent({ dm, name: `runtime-${writer}`, definition: { description: "Unsafe alias", systemPrompt: "Write.", aliases: [readOnly], runner: { type: "external-cli", adapter: writer, command } } }),
				/Selection name .* is reserved/,
			);
		}
		assert.throws(
			() => registerAgent({ dm, name: "worker", definition: { description: "Bad", systemPrompt: "Bad." } }),
			/Worker|builtin agent 'worker'|collides with builtin agent 'worker'/i,
		);

		registerAgent({ dm, name: "runtime-a", definition: { description: "A", systemPrompt: "A.", aliases: ["shared"] } });
		assert.throws(
			() => registerAgent({ dm, name: "runtime-b", definition: { description: "B", systemPrompt: "B.", aliases: ["shared"] } }),
	...
	it("rejects malformed nested runtime definition fields at registration", () => {
		...
		for (const [name, extra, pattern] of cases) {
			assert.throws(
				() => registerAgent({ dm, name: `runtime-${name}`, definition: { description: "Bad", systemPrompt: "Bad.", ...extra } }),
				pattern,
			);
		}
	});
	...
	it("fails closed when cwd discovery introduces a configured collision", () => {
		registerAgent({ dm, name: "runtime-helper", definition: { description: "Runtime helper", systemPrompt: "Help.", aliases: ["helper"] } });
		writeProjectAgent("project-helper", ["helper"]);

		assert.throws(
			() => mergeRuntimeAgents(dm, discoverAgents(tempProject, "both")),
	...
	it("fails closed against configured definitions hidden by scope precedence", () => {
		registerAgent({ dm, name: "hidden-user", definition: { description: "Runtime helper", systemPrompt: "Help." } });
		...
		assert.throws(
			() => mergeRuntimeAgents(dm, discovered, all),
	...
	it("fails closed against configured definitions hidden by explicit scope", () => {
		...
		assert.throws(
			() => mergeRuntimeAgents(dm, projectScoped, [...allProject.builtin, ...allProject.package, ...allProject.user, ...allProject.project]),
			/collides with configured agent 'hidden-user' on name or alias 'hidden-user'/,
		);
	});

	it("fails closed for management lists when scoped discovery hides a configured collision", () => {
		registerAgent({ dm, name: "hidden-user", definition: { description: "Runtime hidden", systemPrompt: "Help." } });
		writeUserAgent("hidden-user");

		assert.throws(
			() => handleList({ agentScope: "project" }, { cwd: tempProject, modelRegistry: { getAvailable: () => [] }, runtimeAgentOwner: dm }),
			/collides with configured agent 'hidden-user' on name or alias 'hidden-user'/,
		);
	});
});
