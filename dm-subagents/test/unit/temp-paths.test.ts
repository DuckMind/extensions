...
describe("shared temp paths", () => {
	it("uses the explicit temp root before shared paths resolve", () => {
		const fixture = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-temp-override-"));
		...
		try {
			...
			const result = spawnSync(process.execPath, ["--experimental-strip-types", "--input-type=module", "--eval", script], {
				encoding: "utf-8",
				env: { ...process.env, HOME: isolatedHome, USERPROFILE: isolatedHome, DM_SUBAGENTS_TEMP_ROOT: override },
			});
	...
	it("anchors shared temp directories under one scoped root", () => {
		...
		assert.match(path.basename(TEMP_ROOT_DIR), /^dm-subagents-/);
