...
import { DM_CODING_AGENT_PACKAGE_ROOT_ENV } from "../../src/shared/utils.ts";
...
describe("agent management config parsing", () => {
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-management-"));
		oldAgentDir = process.env.DM_CODING_AGENT_DIR;
		process.env.DM_CODING_AGENT_DIR = path.join(tempDir, "agent-home");
		clearSkillCache();
	});

	afterEach(() => {
		if (oldAgentDir === undefined) delete process.env.DM_CODING_AGENT_DIR;
		else process.env.DM_CODING_AGENT_DIR = oldAgentDir;
		clearSkillCache();
	...
	it("hides lower-priority agents shadowed by project agents in list output", () => {
		const agentsDir = path.join(tempDir, ".dm", "agents");
	...
	it("rejects management attempts to widen the reserved read-only Claude profile", () => {
		...
		assert.match(fs.readFileSync(path.join(tempDir, ".dm", "agents", "claude-code.md"), "utf-8"), /adapter: claude-code\n/);
	...
	it("gets only the effective agent detail and respects explicit scope", () => {
		const projectAgentsDir = path.join(tempDir, ".dm", "agents");
		const userAgentsDir = path.join(tempDir, "agent-home", "agents");
		const packageDir = path.join(tempDir, ".dm", "npm", "node_modules", "test-agents");
		...
		fs.writeFileSync(path.join(packageDir, "package.json"), JSON.stringify({ "dm-subagents": { agents: ["agents"] } }));
	...
	it("surfaces package source and external-job provider status in list and get", () => {
		const packageDir = path.join(tempDir, ".dm", "npm", "node_modules", "test-surf");
		fs.mkdirSync(path.join(packageDir, "agents"), { recursive: true });
...
`);
	...
	it("blocks a malformed higher-precedence user agent from an extra user agent", () => {
		...
		try {
			...
			fs.mkdirSync(path.join(process.env.DM_CODING_AGENT_DIR!, "agents"), { recursive: true });
			fs.writeFileSync(path.join(process.env.DM_CODING_AGENT_DIR!, "agents", "foo.md"), "---\nname: foo\ndescription: Broken configured user foo\nrunner:\n  type: unknown\n---\nBroken user foo.\n");
	...
	it("blocks a malformed higher-precedence package agent from a lower package agent", () => {
		...
		for (const packageRoot of [highPackage, lowPackage]) {
			...
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ "dm-subagents": { agents: ["agents"] } }));
		}
		...
		fs.mkdirSync(path.join(tempDir, ".dm"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".dm", "settings.json"), JSON.stringify({ packages: [highPackage, lowPackage] }));
	...
	it("reports a malformed project agent before lower-priority ambiguity", () => {
		...
		fs.mkdirSync(path.join(tempDir, ".dm", "agents"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".dm", "settings.json"), JSON.stringify({ packages: [packageRoot] }));
		fs.writeFileSync(path.join(tempDir, ".dm", "agents", "foo.md"), "---\nname: foo\ndescription: Broken project foo\nrunner:\n  type: unknown\n---\nBroken foo.\n");
		fs.mkdirSync(path.join(process.env.DM_CODING_AGENT_DIR!, "agents"), { recursive: true });
	...
	it("blocks a malformed .dm agent from falling back to a legacy project agent", () => {
		const canonicalAgentsDir = path.join(tempDir, ".dm", "agents");
	...
	it("blocks a malformed custom canonical agent directory from falling back to legacy", () => {
		const previousPackageRoot = process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV];
		const packageRoot = path.join(tempDir, "coding-agent-root");
		try {
			fs.mkdirSync(packageRoot, { recursive: true });
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "@duckmind/dm-coding-agent", piConfig: { configDir: ".custom-dm" } }));
			process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] = packageRoot;
			const canonicalAgentsDir = path.join(tempDir, ".custom-dm", "agents");
			const legacyAgentsDir = path.join(tempDir, ".agents");
			fs.mkdirSync(canonicalAgentsDir, { recursive: true });
			fs.mkdirSync(legacyAgentsDir, { recursive: true });
			fs.writeFileSync(path.join(legacyAgentsDir, "shared.md"), "---\nname: shared\ndescription: Legacy shared\n---\nLegacy shared.\n");
			...
			if (previousPackageRoot === undefined) delete process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV];
			else process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] = previousPackageRoot;
		}
	});
	...
	it("blocks a malformed canonical .agents/agents definition from legacy .agents", () => {
		...
		try {
			fs.mkdirSync(packageRoot, { recursive: true });
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "@duckmind/dm-coding-agent", piConfig: { configDir: ".agents" } }));
			process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] = packageRoot;
			const legacyAgentsDir = path.join(tempDir, ".agents");
	...
	it("creates, gets, updates, and deletes a packaged agent by runtime name", () => {
		...
		const filePath = path.join(tempDir, ".dm", "agents", "code-analysis.scout.md");
		...
		const updatedPath = path.join(tempDir, ".dm", "agents", "documentation.scout.md");
	...
	it("creates, reports, and clears agent-local skill paths", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const skillFile = path.join(tempDir, ".dm", "agents", "skills", "private", "SKILL.md");
		...
		const filePath = path.join(tempDir, ".dm", "agents", "local.md");
		let content = fs.readFileSync(filePath, "utf-8");
	...
	it("ignores discovered legacy chains during agent management", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const chainsDir = path.join(tempDir, ".dm", "chains");
	...
	it("creates and updates agents with single-agent launch defaults", () => {
		...
		assert.equal(result.isError, false);
		const filePath = path.join(tempDir, ".dm", "agents", "background-reviewer.md");
		let content = fs.readFileSync(filePath, "utf-8");
	...
	it("creates and updates agents with tool budgets", () => {
		...
		assert.equal(result.isError, false);
		const filePath = path.join(tempDir, ".dm", "agents", "budgeted-reviewer.md");
		let content = fs.readFileSync(filePath, "utf-8");
	...
	it("creates, updates, reports, clears, and validates acceptance roles", () => {
		...
		const filePath = path.join(tempDir, ".dm", "agents", "explorer.md");
	...
	it("creates agents with completion guard disabled", () => {
		...
		assert.equal(result.isError, false);
		const filePath = path.join(tempDir, ".dm", "agents", "test-runner.md");
	...
	it("creates agents with subagent-only extensions", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const result = handleCreate(
			{ config: { name: "child-tool-user", description: "Uses child tools", scope: "project", subagentOnlyExtensions: "./tools/child-only.ts, /opt/dm/child.ts" } },
			ctx,
		);

		assert.equal(result.isError, false);
		const filePath = path.join(tempDir, ".dm", "agents", "child-tool-user.md");
		const content = fs.readFileSync(filePath, "utf-8");
		assert.match(content, /^subagentOnlyExtensions: \.\/tools\/child-only\.ts, \/opt\/dm\/child\.ts$/m);

		const got = handleManagementAction("get", { agent: "child-tool-user" }, ctx);
		assert.equal(got.isError, false);
		assert.ok(readText(got).includes("Subagent-only extensions: " + path.join(tempDir, ".dm", "agents", "tools", "child-only.ts") + ", /opt/dm/child.ts"));
	});

	it("preserves relative extension paths during unrelated updates", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const agentPath = path.join(tempDir, ".dm", "agents", "portable.md");
	...
	it("fails when extension frontmatter cannot be reread", () => {
		const filePath = path.join(tempDir, ".dm", "agents", "removed.md");
		assert.throws(
			() => editableAgentConfig({
				name: "removed",
				...
				source: "project",
				filePath,
				extensions: [path.join(tempDir, ".dm", "agents", "tools", "parent.ts")],
			}),
			/ENOENT/,
		);
	});
	...
	it("returns a structured error when a definition disappears after discovery", () => {
		const agentPath = path.join(tempDir, ".dm", "agents", "removed-during-update.md");
		fs.mkdirSync(path.dirname(agentPath), { recursive: true });
		fs.writeFileSync(
			agentPath,
	...
	it("does not serialize settings overrides into custom agent frontmatter during updates", () => {
		...
		const userSettingsPath = path.join(process.env.DM_CODING_AGENT_DIR!, "settings.json");
		const settingsPath = path.join(tempDir, ".dm", "settings.json");
		const agentPath = path.join(tempDir, ".dm", "agents", "implementer.md");
		fs.mkdirSync(path.dirname(agentPath), { recursive: true });
...
`, "utf-8");
	...
	it("reports override source and disabled builtin state in runtime model mappings", () => {
		const projectSettingsPath = path.join(tempDir, ".dm", "settings.json");
	...
	it("creates delegate with its builtin prompt defaults", () => {
		...
		assert.equal(result.isError, false);
		const filePath = path.join(tempDir, ".dm", "agents", "delegate.md");
		const content = fs.readFileSync(filePath, "utf-8");
	...
	it("lists proactive skill subagent suggestions from repeated configured skill use", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		fs.mkdirSync(path.join(tempDir, ".dm", "agents"), { recursive: true });
		fs.mkdirSync(path.join(tempDir, ".dm", "skills", "deslop"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".dm", "skills", "deslop", "SKILL.md"), `---
...
`, "utf-8");
		for (const name of ["cleanup-a", "cleanup-b"]) {
			fs.writeFileSync(path.join(tempDir, ".dm", "agents", `${name}.md`), `---
name: ${name}
...
`, "utf-8");
	...
	it("can disable proactive skill subagent suggestions in config", () => {
		...
		fs.mkdirSync(path.join(tempDir, ".dm", "agents"), { recursive: true });
		fs.mkdirSync(path.join(tempDir, ".dm", "skills", "deslop"), { recursive: true });
		fs.writeFileSync(path.join(tempDir, ".dm", "skills", "deslop", "SKILL.md"), "Review for cleanup.\n", "utf-8");
		for (const name of ["cleanup-a", "cleanup-b"]) {
			fs.writeFileSync(path.join(tempDir, ".dm", "agents", `${name}.md`), `---
name: ${name}
description: Cleanup ${name}
