...
describe("scripted workflow runtime", () => {
	...
	it("resolves the workflow parser from dm-subagents outside the project cwd", async () => {
		const originalCwd = process.cwd();
		const emptyCwd = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-no-acorn-"));
		try {
