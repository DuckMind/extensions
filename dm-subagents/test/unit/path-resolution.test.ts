...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-path-resolution-"));
...
function restoreEnv(name: "HOME" | "USERPROFILE" | "DM_CODING_AGENT_DIR", value: string | undefined): void {
...
before(() => {
	...
	process.env.DM_CODING_AGENT_DIR = path.join(homeDir, ".dm", "agent");
});
...
after(() => {
	try {
		...
		restoreEnv("DM_CODING_AGENT_DIR", originalAgentDir);
		clearSkillCache();
...
describe("Path resolution for .agents and ~/.agents", () => {
	...
	test("should resolve project agents from both .agents and .dm/agents", () => {
		const legacyDir = path.join(cwdDir, ".agents");
		const agentsDir = path.join(cwdDir, ".dm", "agents");
