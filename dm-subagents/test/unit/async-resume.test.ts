...
describe("async resume lookup", () => {
	it("resolves a completed single-child run from persisted status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-"));
		try {
	...
	it("resolves a workflow child session without a recovery descriptor", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-workflow-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("resolves a retained managed worktree cwd and fails closed when it is missing", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-missing-cwd-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("resolves a retained managed worktree cwd in the original repository subdirectory", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-nested-cwd-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("resumes a normal child when a sibling has a managed worktree handoff", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-mixed-cwd-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects a retained managed worktree cwd that resolves outside the preserved worktree", (context) => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-escaped-cwd-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("preserves and validates persisted capability ceilings for resume", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-ceiling-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects unknown, cross-run, and wrong-agent recovery descriptors", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-descriptor-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("ignores removed turn budgets in persisted recovery descriptors", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-turn-budget-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("accepts current acceptance metadata in recovery descriptors", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-acceptance-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects reviewed acceptance metadata in recovery descriptors", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-reviewed-acceptance-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects legacy acceptance metadata in recovery descriptors", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-inferred-acceptance-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects stopped runs instead of reviving them", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-stopped-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const sessionFile = path.join(root, "session.jsonl");
			fs.writeFileSync(sessionFile, "", "utf-8");
	...
	it("rejects ambiguous run id prefixes", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-ambiguous-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects path-like ids and directories outside the async root", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-paths-"));
		try {
			const asyncRoot = path.join(root, "runs");
			assert.throws(
	...
	it("rejects non-jsonl session files before reviving", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-session-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects malformed result metadata before using session fields", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-malformed-result-"));
		try {
			const resultsDir = path.join(root, "results");
	...
	it("rejects malformed result model metadata", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-malformed-result-model-"));
		try {
			const resultsDir = path.join(root, "results");
	...
	it("rejects malformed status session ids", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-malformed-session-id-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects malformed status model metadata", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-malformed-status-model-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("returns live child metadata for a running child", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-live-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("enforces current-session ownership for status and result-only runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-ownership-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
	...
	it("revives a completed child by index while a sibling async child is still running", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-partial-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("rejects pending indexed children in still-running async runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-pending-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("resolves a completed multi-child run when an index and per-child session file are available", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-multi-"));
		try {
			const asyncRoot = path.join(root, "runs");
	...
	it("resolves a completed result-only child with per-child model metadata", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-resume-result-only-"));
		try {
			const asyncRoot = path.join(root, "runs");
			const resultsDir = path.join(root, "results");
			const firstSession = path.join(root, "a.jsonl");
