...
beforeEach(() => {
	savedDepth = process.env.DM_SUBAGENT_DEPTH;
	savedMaxDepth = process.env.DM_SUBAGENT_MAX_DEPTH;
	savedMaxSpawns = process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION;
});

afterEach(() => {
	if (savedDepth === undefined) delete process.env.DM_SUBAGENT_DEPTH;
	else process.env.DM_SUBAGENT_DEPTH = savedDepth;
	if (savedMaxDepth === undefined) delete process.env.DM_SUBAGENT_MAX_DEPTH;
	else process.env.DM_SUBAGENT_MAX_DEPTH = savedMaxDepth;
	if (savedMaxSpawns === undefined) delete process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION;
	else process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION = savedMaxSpawns;
});
...
describe("resolveMaxSubagentSpawnsPerSession", () => {
	...
	it("ignores invalid env values and falls back to config", () => {
		for (const value of ["garbage", "-1", "1.5"]) {
			process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION = value;
			assert.equal(resolveMaxSubagentSpawnsPerSession(7), 7);
	...
	it("treats zero as an explicit unlimited override", () => {
		process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION = "0";
...
describe("resolveCurrentMaxSubagentDepth", () => {
	it("uses env when present", () => {
		process.env.DM_SUBAGENT_MAX_DEPTH = "5";
...
describe("checkSubagentDepth", () => {
	it("not blocked at depth=0, max=2", () => {
		...
		process.env.DM_SUBAGENT_MAX_DEPTH = "2";
	...
	it("blocked at depth=2, max=2", () => {
		process.env.DM_SUBAGENT_DEPTH = "2";
		process.env.DM_SUBAGENT_MAX_DEPTH = "2";
		const result = checkSubagentDepth();
		assert.equal(result.blocked, true);
	...
	it("blocked at depth=3, max=2", () => {
		process.env.DM_SUBAGENT_DEPTH = "3";
		process.env.DM_SUBAGENT_MAX_DEPTH = "2";
		assert.equal(checkSubagentDepth().blocked, true);
	});
	...
	it("blocked at depth=0, max=0 (disables subagent entirely)", () => {
		process.env.DM_SUBAGENT_DEPTH = "0";
		process.env.DM_SUBAGENT_MAX_DEPTH = "0";
		assert.equal(checkSubagentDepth().blocked, true);
	});
	...
	it("not blocked when depth is invalid (NaN)", () => {
		process.env.DM_SUBAGENT_DEPTH = "garbage";
		process.env.DM_SUBAGENT_MAX_DEPTH = "2";
		assert.equal(checkSubagentDepth().blocked, false);
	});
});
...
describe("getSubagentDepthEnv", () => {
	it("increments from depth=0", () => {
		...
		assert.equal(env.DM_SUBAGENT_DEPTH, "1");
		assert.equal(env.DM_SUBAGENT_MAX_DEPTH, "2");
	});
	...
	it("increments from depth=1", () => {
		...
		delete process.env.DM_SUBAGENT_MAX_DEPTH;
		const env = getSubagentDepthEnv();
		assert.equal(env.DM_SUBAGENT_DEPTH, "2");
		assert.equal(env.DM_SUBAGENT_MAX_DEPTH, "2");
	});
	...
	it("uses provided max depth override", () => {
		...
		assert.equal(env.DM_SUBAGENT_DEPTH, "1");
		assert.equal(env.DM_SUBAGENT_MAX_DEPTH, "1");
	});

	it("respects inherited DM_SUBAGENT_MAX_DEPTH when override is absent", () => {
		process.env.DM_SUBAGENT_DEPTH = "0";
		process.env.DM_SUBAGENT_MAX_DEPTH = "5";
		const env = getSubagentDepthEnv();
		assert.equal(env.DM_SUBAGENT_DEPTH, "1");
		assert.equal(env.DM_SUBAGENT_MAX_DEPTH, "5");
	});
	...
	it("falls back to depth=1 when env var is invalid (NaN)", () => {
		process.env.DM_SUBAGENT_DEPTH = "not-a-number";
		delete process.env.DM_SUBAGENT_MAX_DEPTH;
		const env = getSubagentDepthEnv();
		assert.equal(env.DM_SUBAGENT_DEPTH, "1");
	});
