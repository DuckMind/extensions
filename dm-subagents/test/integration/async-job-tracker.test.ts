...
interface AsyncJobTrackerModule {
	createAsyncJobTracker(
		dm: { events: { emit(channel: string, data: unknown): void } },
...
describe("async job tracker", { skip: !available ? "dm packages not available" : undefined }, () => {
	it("drops stale captured extension contexts instead of crashing background timers", () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-stale-context-");
		try {
			const state = createState();
			const recorder = createEventRecorder();
			const tracker = createTracker(recorder.dm, state as never, asyncRoot);
	...
	it("clears stale cached contexts before a status refresh can leak the error", async () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-stale-refresh-");
		try {
	...
	it("stores only parent-registered session roots from async start events", () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-session-root-");
		try {
			const state = createState();
			const recorder = createEventRecorder();
			const tracker = createTracker(recorder.dm, state as never, asyncRoot);
	...
	it("ignores unregistered session roots from async start events", () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-forged-session-root-");
		try {
			const state = createState();
			const recorder = createEventRecorder();
			const tracker = createTracker(recorder.dm, state as never, asyncRoot);
	...
	it("clears pending live session roots on reset", () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-reset-session-root-");
		try {
			const state = createState();
			const recorder = createEventRecorder();
			const tracker = createTracker(recorder.dm, state as never, asyncRoot);
	...
	it("removes completed jobs after retention and requests a rerender", async () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-");
		try {
			const state = createState();
	...
	it("keeps the async widget cleared when it is disabled", () => {
		const asyncRoot = createTempDir("dm-async-job-widget-disabled-");
		try {
			const state = createState();
			const ui = createUiContext();
			const recorder = createEventRecorder();
	...
	it("rebuilds the async widget on quiet animation ticks without a UI requestRender bridge", async () => {
		const asyncRoot = createTempDir("dm-async-job-widget-animation-");
		try {
			const state = createState();
			const ui = createUiContext();
			const recorder = createEventRecorder();
			const tracker = createTracker(recorder.dm, state as never, asyncRoot, { pollIntervalMs: 10 });
			const ctx = {
				...ui.ctx,
				ui: {
					...ui.ctx.ui,
	...
	it("restores active async runs into the widget after reset", async () => {
		const asyncRoot = createTempDir("dm-async-job-restore-");
		try {
	...
	it("refreshes restored nested async children from event files before the liveness sweep", async () => {
		const asyncRoot = createTempDir("dm-async-job-restore-nested-events-");
		...
		try {
			...
			const tracker = createTracker(createEventRecorder().dm, state as never, asyncRoot, { pollIntervalMs: 60_000 });
	...
	it("refreshes restored workflow status from filesystem events before the liveness sweep", async () => {
		const asyncRoot = createTempDir("dm-async-job-restore-workflow-");
		try {
	...
	it("refreshes newly started runs when status appears after the run directory", async () => {
		const asyncRoot = createTempDir("dm-async-job-late-status-");
		try {
	...
	it("does not sweep bridge requests for native jobs after an absent request directory", async () => {
		const asyncRoot = createTempDir("dm-async-job-native-bridge-");
		try {
			...
			const state = createState();
			const tracker = createTracker(createEventRecorder().dm, state as never, asyncRoot, {
				pollIntervalMs: 10,
	...
	it("services an existing request once an external-job runner is identified", async () => {
		const asyncRoot = createTempDir("dm-async-job-external-bridge-");
	...
	it("services requests created after the initial missing-directory sweep", async () => {
		const asyncRoot = createTempDir("dm-async-job-late-bridge-");
		let unregister: (() => void) | undefined;
		try {
	...
	it("keeps bridge servicing when status is missing", async () => {
		const asyncRoot = createTempDir("dm-async-job-malformed-bridge-");
		try {
	...
	it("does not request quiet running widget renders on every liveness tick", async () => {
		const asyncRoot = createTempDir("dm-async-job-quiet-widget-");
		try {
			...
			const state = createState();
			const ui = createUiContext();
			const tracker = createTracker(createEventRecorder().dm, state as never, asyncRoot, { pollIntervalMs: 10 });
			tracker.resetJobs(ui.ctx as never);
	...
	it("uses polling without native watchers on Darwin and fires terminal delivery hooks once", async () => {
		const asyncRoot = createTempDir("dm-async-job-darwin-poll-");
		try {
	...
	it("refreshes nested async children from event files before the liveness sweep", async () => {
		const asyncRoot = createTempDir("dm-async-job-nested-events-");
	...
	it("restores async workflow child ownership after reload", () => {
		const asyncRoot = createTempDir("dm-async-job-restore-workflow-child-");
		try {
	...
	it("restores only active async runs for the current session", () => {
		const asyncRoot = createTempDir("dm-async-job-restore-scope-");
		try {
	...
	it("ignores started and complete events without the current session id", async () => {
		const asyncRoot = createTempDir("dm-async-job-event-scope-");
		try {
			const state = createState();
			state.currentSessionId = "session-owner";
			const tracker = createTracker(createEventRecorder().dm, state as never, asyncRoot, {
	...
	it("does not throw during restore when a persisted async status is malformed", () => {
		const asyncRoot = createTempDir("dm-async-job-restore-bad-status-");
	...
	it("requests quiet animation repaints without rebuilding widgets and stops at terminal status", async () => {
		...
		try {
			...
			const ui = createUiContext();
			const recorder = createEventRecorder();
			tracker = createTracker(recorder.dm, state as never, asyncRoot, {
				pollIntervalMs: 10,
			});
			tracker.resetJobs(ui.ctx as never);
	...
	it("coalesces close per-job status refreshes into one widget replacement", async () => {
		const asyncRoot = createTempDir("dm-async-job-tracker-coalesce-");
		let tracker: ReturnType<AsyncJobTrackerModule["createAsyncJobTracker"]> | undefined;
		try {
			...
			const ui = createUiContext();
			const recorder = createEventRecorder();
			tracker = createTracker(recorder.dm, state as never, asyncRoot, { pollIntervalMs: 10000, watch });
			tracker.resetJobs(ui.ctx as never);
	...
	it("watches run directories created after the start event", async () => {
		const asyncRoot = createTempDir("dm-async-job-late-dir-");
		try {
	...
	it("repairs stale running jobs during polling", async () => {
		const asyncRoot = createTempDir("dm-async-job-stale-");
		try {
	...
	it("repairs started jobs whose runner dies before writing status", async () => {
		const asyncRoot = createTempDir("dm-async-job-no-status-");
		try {
			const resultsDir = path.join(asyncRoot, "results");
	...
	it("cleans up jobs when status polling hits a terminal read error", async () => {
		const asyncRoot = createTempDir("dm-async-job-bad-status-");
		try {
			const runDir = path.join(asyncRoot, "run-bad-status");
			fs.mkdirSync(runDir, { recursive: true });
	...
	it("does not clean up a status-read failure while nested descendants are live", async () => {
		const asyncRoot = createTempDir("dm-async-job-bad-status-nested-");
		let tracker: ReturnType<AsyncJobTrackerModule["createAsyncJobTracker"]> | undefined;
		const originalError = console.error;
		console.error = () => {};
		try {
	...
	it("keeps root jobs running when nested refresh fails during polling", async () => {
		const asyncRoot = createTempDir("dm-async-job-nested-refresh-um");
		let tracker: ReturnType<AsyncJobTrackerModule["createAsyncJobTracker"]> | undefined;
		const originalError = console.error;
		console.error = () => {};
		try {
	...
	it("cancels cleanup timers when polling observes a non-terminal status", async () => {
		const asyncRoot = createTempDir("dm-async-job-cleanup-cancel-");
		let tracker: ReturnType<AsyncJobTrackerModule["createAsyncJobTracker"]> | undefined;
		try {
