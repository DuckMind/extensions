...
const importHome = createTempDir("dm-doctor-executor-import-home-");
...
describe("doctor action executor routing", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, () => {
	...
	beforeEach(() => {
		tempDir = createTempDir("dm-doctor-executor-project-");
		tempHome = createTempDir("dm-doctor-executor-home-");
	...
	it("returns a doctor report for the tool action", async () => {
		...
		assert.match(text, /- supervisor channel: available \(native:dm-subagents-supervisor-channel\)/);
	});
