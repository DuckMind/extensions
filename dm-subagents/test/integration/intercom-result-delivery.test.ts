...
describe("intercom result delivery cutover", { skip: !available ? "executor not importable" : undefined }, () => {
	...
	before(() => {
		...
		homeDir = createTempDir("dm-subagent-intercom-home-");
		process.env.HOME = homeDir;
		process.env.USERPROFILE = homeDir;
		mockPi = createMockPi();
		mockPi.install();
		fs.mkdirSync(path.join(os.homedir(), ".dm", "agent", "extensions", "dm-intercom"), { recursive: true });
		fs.mkdirSync(path.join(os.homedir(), ".dm", "agent", "intercom"), { recursive: true });
		fs.writeFileSync(path.join(os.homedir(), ".dm", "agent", "intercom", "config.json"), JSON.stringify({ enabled: true }), "utf-8");
	});
	...
	beforeEach(() => {
		tempDir = createTempDir("dm-subagent-intercom-result-");
		mockPi.reset();
	});
	...
	async function readMockCallArgs(index: number): Promise<string[]> {
		...
		if (callFile && parseError) {
			throw new Error(`Mock DM call record '${callFile}' remained incomplete after 10s.`, { cause: parseError });
		}
		assert.fail(`expected mock dm call at index ${index}`);
	}
	...
	async function waitForCallCount(minimum: number, timeoutMs = 10_000): Promise<void> {
		...
		while (mockPi.callCount() < minimum) {
			if (Date.now() > deadline) assert.fail(`Timed out waiting for ${minimum} mock dm calls; saw ${mockPi.callCount()}`);
			await new Promise((resolve) => setTimeout(resolve, 50));
		}
	}
	...
	it("rejects concurrent direct revival of the same completed async session and releases ownership", async () => {
		...
		try {
			...
			assert.equal(fs.readdirSync(mockPi.dir).filter((name) => name.startsWith("call-")).length, 1, "contending revival must not spawn DM");
	...
	it("applies the same exclusive lease to remembered foreground revival", async () => {
		...
		try {
			...
			assert.equal(blocked.isError, true);
			assert.match(blocked.content[0]?.text ?? "", new RegExp(`already owned by run '${firstRevivedId}'`));
			// The original and the first revival each spawn DM asynchronously, so wait for both markers
			// before asserting the contending revival added no third spawn.
			await waitForCallCount(2);
			assert.equal(mockPi.callCount(), 2, "contending foreground revival must not spawn DM");
