...
const enabled = process.env.DM_SUBAGENTS_CURSOR_AGENT_SMOKE === "1";

test("maintainer Cursor Agent prompt-file read-only smoke", { skip: enabled ? undefined : "set DM_SUBAGENTS_CURSOR_AGENT_SMOKE=1" }, async () => {
	const reportPath = process.env.DM_SUBAGENTS_CURSOR_AGENT_SMOKE_REPORT;
	assert.ok(reportPath, "DM_SUBAGENTS_CURSOR_AGENT_SMOKE_REPORT is required");
