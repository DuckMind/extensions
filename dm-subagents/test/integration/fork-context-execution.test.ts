...
describe("fork context execution wiring", { skip: !available ? "subagent executor not importable" : undefined }, () => {
	...
	beforeEach(() => {
		tempDir = createTempDir("dm-subagent-fork-test-");
		mockPi.reset();
	...
	function readCallArgsForTask(taskText: string): string[] {
		...
		assert.ok(args, `expected a recorded mock dm call for task '${taskText}'`);
		return args;
	}
	...
	function writeAgent(projectRoot: string, name: string, model: string): void {
		const filePath = path.join(projectRoot, ".dm", "agents", `${name}.md`);
	...
	function writeProjectOverride(projectRoot: string, agentName: string, model: string): void {
		const settingsPath = path.join(projectRoot, ".dm", "settings.json");
	...
	it("uses request cwd for management actions", async () => {
		...
		fs.mkdirSync(path.join(worktreeDir, ".dm"), { recursive: true });
		...
		assert.equal(fs.existsSync(path.join(worktreeDir, ".dm", "agents", "local-helper.md")), true);
		assert.equal(fs.existsSync(path.join(tempDir, ".dm", "agents", "local-helper.md")), false);
	});
	...
	it("uses request cwd for execution-time agent discovery", async () => {
		...
		assert.ok(args, "expected a recorded mock dm call for this test task");
	...
	it("uses request cwd for project builtin overrides during management", async () => {
		const tempHome = createTempDir("dm-subagent-home-");
