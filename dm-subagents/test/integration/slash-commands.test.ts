...
import { SUBAGENT_FANOUT_CHILD_ENV } from "../../src/runs/shared/dm-args.ts";
...
async function withIsolatedHome<T>(fn: () => Promise<T>): Promise<T> {
	const home = fs.mkdtempSync(path.join(os.tmpdir(), "dm-slash-home-"));
	const previousAgentDir = process.env.DM_CODING_AGENT_DIR;
	const previousHome = process.env.HOME;
	const previousUserProfile = process.env.USERPROFILE;
	process.env.DM_CODING_AGENT_DIR = path.join(home, ".dm", "agent");
...
async function withTempProject<T>(prefix: string, fn: (root: string) => Promise<T>): Promise<T> {
	...
	fs.mkdirSync(path.join(root, ".dm", "agents"), { recursive: true });
	fs.mkdirSync(path.join(root, ".dm", "chains"), { recursive: true });
	try {
...
function writeProjectChain(root: string, fileName: string, content: string): void {
	fs.writeFileSync(path.join(root, ".dm", "chains", fileName), content, "utf-8");
}
...
function createWatchdogHarness(review?: WatchdogReviewFunction) {
	...
	const runtime = registerMainWatchdog!(dm as never, review ? { review } : undefined);
...
async function captureSlashCommandParams(
	...
	return withIsolatedHome(async () => {
		...
		try {
			setup?.(dm);
			registerSlashCommands!(dm, createState(cwd));
			...
			clearRuntimeAgentsForPi(dm as never);
		}
	});
}
...
describe("subagents watchdog slash command", { skip: !available ? "watchdog command not importable" : undefined }, () => {
	it("shows default-off status with runtime state, sources, and review seam", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-status-", async (root) => {
	...
	it("recommends and saves a strong complementary watchdog model", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-model-", async (root) => {
				...
				const settingsPath = path.join(process.env.HOME!, ".dm", "agent", "settings.json");
	...
	it("supports session-scoped recommended watchdog models without writing settings", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-session-model-", async (root) => {
				const opus = { provider: "anthropic", id: "claude-opus-4-8", reasoning: true };
				const gpt = { provider: "openai-codex", id: "gpt-5.5", reasoning: true };
				...
				assert.equal(fs.existsSync(path.join(process.env.HOME!, ".dm", "agent", "settings.json")), false);
				const content = String((sent[0] as { content?: unknown }).content ?? "");
	...
	it("shows explicit watchdog model thinking accurately when no thinking is configured", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-status-model-", async (root) => {
				const settingsPath = path.join(process.env.HOME!, ".dm", "agent", "settings.json");
	...
	it("writes only user watchdog enabled settings and preserves existing settings", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-toggle-", async (root) => {
				const settingsPath = path.join(process.env.HOME!, ".dm", "agent", "settings.json");
				const projectSettingsPath = path.join(root, ".dm", "settings.json");
				fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
	...
	it("uses session on/off overrides without writing settings files", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-session-", async (root) => {
				const settingsPath = path.join(process.env.HOME!, ".dm", "agent", "settings.json");
				const projectSettingsPath = path.join(root, ".dm", "settings.json");
				const { commands, sent } = createWatchdogHarness();
	...
	it("sends accepted review warnings as visible custom watchdog messages", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("dm-watchdog-review-warning-", async (root) => {
...
describe("slash command custom message delivery", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("/subagent-cost recovers async workflow usage from receipts and metadata", async () => {
		await withTempProject("dm-subagent-cost-async-", async (root) => {
			...
			try {
				registerSlashCommands!(dm as never, createState(root));
	...
	it("/subagents-stop keeps the selector within its allocated width", async () => {
		await withTempProject("dm-stop-selector-width-", async (root) => {
	...
	it("/subagents-stop forwards a child id for child-scoped stops", async () => {
		...
		registerSlashCommands!(dm, createState(process.cwd()));
	...
	it("/run abandons captured context when commands are disposed during reload", async () => {
		...
		const disposer = registerSlashCommands!(dm, createState(process.cwd()));
		let stale = false;
	...
	it("/run reports discovery evidence for a missing agent", async () => {
		await withTempProject("dm-slash-missing-agent-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "worker.md"), "---\nname: worker\ndescription: Worker\n---\n", "utf-8");
	...
	it("/run reports malformed agent configuration", async () => {
		await withTempProject("dm-slash-invalid-agent-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "broken.md"), "---\nname: broken\ndescription: Broken\nrunner:\n  type: unknown\n---\nBroken agent.\n");
	...
	it("/run blocks a malformed project agent from falling back to builtin", async () => {
		await withTempProject("dm-slash-invalid-agent-shadow-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "reviewer.md"), "---\nname: reviewer\ndescription: Broken reviewer\nrunner:\n  type: unknown\n---\nBroken agent.\n");
	...
	it("/run reports malformed packaged agent configuration by runtime name", async () => {
		await withTempProject("dm-slash-invalid-packaged-agent-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "code-analysis.zeta-worker.md"), "---\nname: zeta-worker\npackage: code-analysis\ndescription: Broken packaged worker\nrunner:\n  type: unknown\n---\nBroken agent.\n");
	...
	it("/run accepts runtime-registered agents", async () => {
		await withTempProject("dm-slash-runtime-agent-", async (root) => {
			const run = await captureSlashCommandParams("run", "runtime-helper Inspect", root, (dm) => {
				registerAgent({
					dm: dm as never,
					name: "runtime-helper",
	...
	it("/run preserves existing relative reads and omits missing reads", async () => {
		await withTempProject("dm-slash-reads-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "scout.md"), `---
name: scout
description: Scout
---
...
`, "utf-8");
	...
	it("/run accepts dotted packaged runtime agent names", async () => {
		await withTempProject("dm-packaged-agent-slash-", async (root) => {
			fs.writeFileSync(path.join(root, ".dm", "agents", "code-analysis.scout.md"), `---
name: scout
package: code-analysis
...
`, "utf-8");
	...
	it("/run reports malformed packaged local-name fallback configuration", async () => {
		await withTempProject("dm-packaged-agent-local-slash-", async (root) => {
...
Valid foo.
`, "utf-8");
			fs.writeFileSync(path.join(root, ".dm", "settings.json"), JSON.stringify({ packages: [highPackage, lowPackage] }));
...
describe("subagents-inspect-rpc command", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	function writeInspectableRun(runId: string, sessionId: string) {
		...
		const sessionRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dm-inspect-itest-session-"));
	...
	function registerWithState(sessionId: string | null, trustedSessionRoots: string[] = []) {
		...
		registerSlashCommands!(dm as never, state as never);
		return commands;
	}
	...
	it("emits a correlated payload widget then retracts it on RPC surfaces", async () => {
		...
		assert.ok(lines[0]!.startsWith("DM_SUBAGENT_INSPECT_JSON:"));
		const reply = JSON.parse(lines[0]!.slice("DM_SUBAGENT_INSPECT_JSON:".length));
		assert.equal(reply.kind, "dm-subagents.inspect-reply");
