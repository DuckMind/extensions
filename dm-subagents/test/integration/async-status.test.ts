...
describe("async status helpers", () => {
	...
	it("lists only requested states and includes flattened step summaries", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-"));
	...
	it("forwards bounded timeout recovery evidence and formats the recovery route", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-recovery-"));
		try {
	...
	it("reports the planned runs.lanes stage count while the first child is running", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-staged-lane-"));
		try {
	...
	it("loads typed host monitor nodes from workflow status without child identity", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-host-step-"));
		try {
	...
	it("fails closed when a persisted host monitor node is malformed", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-host-step-invalid-"));
		try {
	...
	it("projects bounded lane and display-only worktree metadata from status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-lane-"));
		try {
			...
			assert.equal(run.steps[0]?.branch, "dm-subagent/run-lane-0");
			assert.match(formatAsyncRunList([run]), /lane writer/);
			assert.match(formatAsyncRunList([run]), /worktree .*run-lane-0 .*branch dm-subagent\/run-lane-0/);
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
	...
	it("rejects malformed or mismatched lane identity in status", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-lane-invalid-"));
		try {
	...
	it("renders persisted preflight mismatch warnings in async status lists", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-preflight-"));
		try {
	...
	it("preserves agent contract projections on step summaries", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-contract-"));
		try {
	...
	it("preserves capability ceiling and audit projections on summaries", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-capability-"));
		try {
	...
	it("formats async run and step context labels", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-context-"));
		try {
	...
	it("formats model thinking in step summaries", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-model-thinking-"));
		try {
	...
	it("uses persisted running attention state from detached runners", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-running-state-"));
		try {
	...
	it("renders deferred turn-budget termination distinctly from a soft wrap-up request", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-turn-budget-deferred-"));
		try {
	...
	it("does not infer attention state when the runner has not persisted one", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-no-derived-attention-"));
		try {
			const now = Date.now();
			createAsyncDir(root, "run-running", {
				runId: "run-running",
	...
	it("does not infer workflow activity from the workflow launch time", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-workflow-no-false-activity-"));
		try {
			const now = Date.now();
	...
	it("does not smear run-level attention state across running siblings when step metadata exists", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-step-attention-"));
		try {
			const now = Date.now();
	...
	it("formats paused runs as lifecycle state without activity state", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-paused-status-"));
		try {
	...
	it("surfaces malformed status files instead of silently skipping them", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-bad-status-"));
	...
	it("rejects malformed persisted session ids", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-bad-session-id-"));
		try {
	...
	it("repairs stale running runs before listing active async runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-stale-list-"));
	...
	it("uses foreground-style wording for top-level async parallel runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-top-parallel-wording-"));
		try {
	...
	it("includes terminal outcome counts for failed top-level async parallel runs", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-terminal-parallel-counts-"));
		try {
	...
	it("uses explicit parallel group wording for async chains", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-parallel-wording-"));
		try {
			createAsyncDir(root, "run-parallel", {
				runId: "run-parallel",
				mode: "chain",
	...
	it("uses parallel group wording even when concurrency leaves one agent running", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-parallel-one-running-"));
		try {
	...
	it("ignores invalid persisted parallel group metadata", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-invalid-parallel-group-"));
		try {
	...
	it("keeps top-level parallel wording without valid group metadata", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-parallel-invalid-group-"));
		try {
	...
	it("keeps serial step wording for sequential running chains", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-sequential-wording-"));
		try {
	...
	it("lists indexed active runs without reading historical status files", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-indexed-"));
		try {
			for (let i = 0; i < 200; i++) {
	...
	it("lists recent terminal runs from time-sortable session markers", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-terminal-index-"));
		try {
	...
	it("removes invalid terminal markers while reading the advisory index", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-terminal-heal-"));
		try {
	...
	it("does not enumerate the async root for full-id or unsafe-prefix misses", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-exact-miss-"));
	...
	it("keeps terminal active markers until observed process-terminal proof releases them", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-index-prune-"));
		try {
	...
	it("prunes old terminal active markers without process-terminal proof", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-async-status-index-aged-prune-"));
		try {
			const asyncDir = createAsyncDir(root, "finished", {
				runId: "finished",
				mode: "single",
