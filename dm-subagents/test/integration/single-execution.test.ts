 ...
 *
 * Uses the local createMockPi() helper to simulate the dm CLI.
 * Tests the full spawn→parse→result pipeline in runSync without a real LLM.
 *
 * These tests require dm packages to be importable (they run inside a dm
 * environment or with dm packages installed). If unavailable, tests skip
 * gracefully.
 */
...
describe("single sync execution", { skip: !available ? "dm packages not available" : undefined }, () => {
	let tempDir: string;
	let agentDir: string;
	let mockPi: MockPi;
	...
	beforeEach(() => {
		...
		agentDir = createTempDir("dm-subagent-agent-");
		previousAgentDir = process.env.DM_CODING_AGENT_DIR;
	...
	function makeExecutor(
		...
		return createSubagentExecutor!({
			dm: { events: piEvents, getSessionName: () => undefined },
			state: {
				baseCwd: tempDir,
			...
			getSubagentSessionRoot: () => path.join(tempDir, ".dm/subagents", "sessions"),
	...
	it("derives a child session name and passes it to the child env", async () => {
		mockPi.onCall({ echoEnv: ["DM_SUBAGENT_SESSION_NAME"] });
		const agents = makeAgentConfigs(["echo"]);
		...
		assert.equal(echoed.DM_SUBAGENT_SESSION_NAME, "echo: Say hello to the world");
	});

	it("rejects invalid foreground cwd before spawning DM", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
	...
	it("consumes one exact host-only workflow child permit before spawn", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		assert.match(denialText, /child key mismatch.*agent mismatch.*final launch projection.*runs\.all.*retained resume.*native DM children.*foreground workflow roots/s);
	...
	it("starts workflow scripts asynchronously with a portable internal run id", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		assert.equal(asyncJobs.get(workflowRunId)?.sessionRoot, path.join(tempDir, ".dm/subagents", "sessions"));
		...
		assert.equal(status.sessionRoot, path.join(tempDir, ".dm/subagents", "sessions"));
	...
	it("notifies the parent when an async workflow child needs attention", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		handleSubagentControlNotice({
			dm: { sendMessage(_message, options) { sent.push({ options: options as { triggerTurn?: boolean } }); } },
	...
	it("keeps workflow children mission-detached when automatic mission persistence fails", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		const executor = makeExecutor([makeAgent("echo")], { missions: { directory: ".dm/subagents/missions", globalIndexDir: blockedIndex } });
		...
		assert.equal(result.details.results.length, 2);
		const missionDir = path.join(tempDir, ".dm/subagents", "missions");
	...
	it("creates a plan-only worktree cleanup action without removing managed state", { skip: !createSubagentExecutor ? "executor not importable" : process.platform === "win32" ? "worktree paths differ on Windows" : undefined }, async () => {
		...
		const manifestPath = path.join(repo, ".dm", "subagents", "artifacts", "handoff.json");
		...
		fs.writeFileSync(manifestPath, JSON.stringify({
			...
			groups: [{
				...
				children: [{
					...
					status: "completed",
					summary: "done",
					patch: { path: path.join(repo, ".dm", "subagents", "artifacts", "worktree.patch"), branch: worktree.branch, changed: false, diffStat: "", filesChanged: 0, insertions: 0, deletions: 0 },
				}],
		...
		fs.writeFileSync(path.join(repo, ".dm", "subagents", "artifacts", "status.json"), JSON.stringify({ runId: "cleanup-action-run", state: "complete" }), "utf-8");
		try {
			...
			const planFiles = fs.readdirSync(path.join(repo, ".dm", "subagents", "cleanup-plans"));
	...
	it("ignores stale foreground control notification contexts after reload", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		const executor = createSubagentExecutor!({
			dm: { events: staleEvents, getSessionName: () => undefined },
			state,
	...
	it("does not create a temporary structured output directory before file-only validation", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		try {
			...
			assert.equal(mockPi.callCount(), 0);
			assert.equal(fs.readdirSync(tempDir).some((name) => name.startsWith("dm-subagent-structured-")), false);
		} finally {
	...
	it("reports an actionable error after startup retries are exhausted", async () => {
		...
		assert.deepEqual(result.attemptedModels, ["openai/gpt-5-mini"]);
		assert.match(result.error ?? "", /failed to start after 4 attempts.*concurrent DM startup race/i);
	...
	it("rejects an unresolved agent model before spawning DM", async () => {
	...
	it("resolves skills from the effective task cwd", async () => {
		const taskCwd = createTempDir("dm-subagent-task-cwd-");
		try {
	...
	it("fails foreground runs on explicit unavailable dm-subagents skill requests without spawning", async () => {
		const agents = [makeAgent("worker")];

		const result = await runSync(tempDir, agents, "worker", "Task", { skills: ["dm-subagents"] });

		assert.equal(result.exitCode, 1);
		assert.equal(result.error, "Skills not found: dm-subagents");
		assert.equal(mockPi.callCount(), 0);
	});

	it("fails foreground runs when an agent default requests dm-subagents skill", async () => {
		const agents = [makeAgent("worker", { skills: ["dm-subagents"] })];
	...
	it("passes maxSubagentDepth through to child execution env", async () => {
		mockPi.onCall({ echoEnv: ["DM_SUBAGENT_DEPTH", "DM_SUBAGENT_MAX_DEPTH"] });
		const agents = makeAgentConfigs(["echo"]);
		const prevDepth = process.env.DM_SUBAGENT_DEPTH;
		const prevMaxDepth = process.env.DM_SUBAGENT_MAX_DEPTH;
		delete process.env.DM_SUBAGENT_DEPTH;
		delete process.env.DM_SUBAGENT_MAX_DEPTH;
		...
		try {
			...
			assert.deepEqual(JSON.parse(result.finalOutput ?? "{}"), {
				DM_SUBAGENT_DEPTH: "1",
				DM_SUBAGENT_MAX_DEPTH: "1",
			});
		} finally {
			if (prevDepth === undefined) delete process.env.DM_SUBAGENT_DEPTH;
			else process.env.DM_SUBAGENT_DEPTH = prevDepth;
			if (prevMaxDepth === undefined) delete process.env.DM_SUBAGENT_MAX_DEPTH;
			else process.env.DM_SUBAGENT_MAX_DEPTH = prevMaxDepth;
		}
	});
	...
	it("passes prompt inheritance env flags through to child execution", async () => {
		mockPi.onCall({ echoEnv: ["DM_SUBAGENT_INHERIT_PROJECT_CONTEXT", "DM_SUBAGENT_INHERIT_SKILLS"] });
		const agents = [makeAgent("echo", {
		...
		assert.equal(result.exitCode, 0);
		assert.deepEqual(JSON.parse(result.finalOutput ?? "{}"), {
			DM_SUBAGENT_INHERIT_PROJECT_CONTEXT: "0",
			DM_SUBAGENT_INHERIT_SKILLS: "0",
		});
	});
	...
	it("passes fanout routing env only when nested fanout is explicitly authorized", async () => {
		...
		try {
			...
			assert.deepEqual(JSON.parse(fanout.finalOutput ?? "{}"), {
				DM_SUBAGENT_FANOUT_CHILD: "1",
				DM_SUBAGENT_PARENT_EVENT_SINK: "/tmp/inherited/events.jsonl",
				DM_SUBAGENT_PARENT_CONTROL_INBOX: "/tmp/inherited/control",
				DM_SUBAGENT_PARENT_RUN_ID: "fanout-run",
				DM_SUBAGENT_PARENT_CHILD_INDEX: "2",
			});
			...
			assert.deepEqual(JSON.parse(inheritedToolFanout.finalOutput ?? "{}"), {
				...
				DM_SUBAGENT_PARENT_EVENT_SINK: "/tmp/inherited/events.jsonl",
				DM_SUBAGENT_PARENT_CONTROL_INBOX: "/tmp/inherited/control",
				DM_SUBAGENT_PARENT_RUN_ID: "inherited-tool-fanout",
				DM_SUBAGENT_PARENT_CHILD_INDEX: "3",
			});
			...
			assert.deepEqual(JSON.parse(nonFanout.finalOutput ?? "{}"), {
				DM_SUBAGENT_FANOUT_CHILD: "0",
				DM_SUBAGENT_PARENT_EVENT_SINK: "",
				DM_SUBAGENT_PARENT_CONTROL_INBOX: "",
				DM_SUBAGENT_PARENT_RUN_ID: "",
				DM_SUBAGENT_PARENT_CHILD_INDEX: "",
			});
		} finally {
	...
	it("passes supervisor metadata through to child execution", async () => {
		mockPi.onCall({ echoEnv: [
			"DM_SUBAGENT_INTERCOM_SESSION_NAME",
			"DM_SUBAGENT_ORCHESTRATOR_TARGET",
			"DM_SUBAGENT_RUN_ID",
			"DM_SUBAGENT_CHILD_AGENT",
			"DM_SUBAGENT_CHILD_INDEX",
		] });
		const agents = makeAgentConfigs(["echo"]);
		...
		assert.equal(result.exitCode, 0);
		assert.deepEqual(JSON.parse(result.finalOutput ?? "{}"), {
			DM_SUBAGENT_INTERCOM_SESSION_NAME: "subagent-echo-78f659a3-3",
			DM_SUBAGENT_ORCHESTRATOR_TARGET: "subagent-chat-parent",
			DM_SUBAGENT_RUN_ID: "78f659a3",
			DM_SUBAGENT_CHILD_AGENT: "echo",
			DM_SUBAGENT_CHILD_INDEX: "2",
		});
	});
	...
	it("returns actionable guidance for ambient extension registration conflicts", async () => {
		mockPi.onCall({
			exitCode: 1,
			stderr: 'Error: Failed to load extension "/tmp/dm-mcp-adapter-clone/index.ts": Tool "mcpScript" conflicts with /tmp/dm-mcp-adapter/index.ts',
		});
		const agents = makeAgentConfigs(["echo"]);
		...
		assert.equal(result.exitCode, 1);
		assert.match(result.error ?? "", /loaded conflicting ambient DM extensions/);
