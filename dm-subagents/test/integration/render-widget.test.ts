...
describe("subagent async widget rendering", () => {
	...
	it("renders a compact component widget for three active parallel agents without core truncation", () => {
		...
		assert.ok(lines.length <= 10, "collapsed component should stay under DM's string-widget cap even though it bypasses it");
	});
	...
	it("uses a bounded string-array snapshot widget in RPC mode", () => {
		...
		assert.ok(line?.startsWith("DM_SUBAGENT_ASYNC_JSON:"));
		const snapshot = JSON.parse(line.slice("DM_SUBAGENT_ASYNC_JSON:".length));
		assert.equal(snapshot.kind, "dm-subagents.async-status-snapshot");
	...
	it("honors the component render width instead of the terminal width", () => {
		...
		withStdoutSize(50, 120, () => {
			...
			renderWidget(ui.ctx as never, [{
				asyncId: "run-narrow",
				asyncDir: "/tmp/dm-subagents-uid-1000/async-subagent-runs/call_with_a_long_identifier",
				status: "running",
				mode: "parallel",
	...
	it("compacts repeated subagent status snapshots inside mixed live output", () => {
			...
			steps: [
				{
					...
					recentOutput: [
						...
						"repo=nicobailon/dm-subagents",
						"sha=d61aca... | 2m32s",
						"output: /var/folders/x/T/dm-subagents-uid-501/async-subagent-runs/run-status-snapshots/output.log",
					],
				},
			],
		};
		...
		assert.match(expandedText, /latest: output: \/var\/folders\/x\/T\/dm-subagents-uid-501\/async-subagent-runs\/run-status-snapshots\/output.log/);
		assert.match(expandedText, /repo=nicobailon\/dm-subagents/);
