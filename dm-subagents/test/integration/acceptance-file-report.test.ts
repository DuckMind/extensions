 ...
 * full spawn → stream-parse → acceptance pipeline against the mock dm CLI.
 */
...
describe("acceptance file reports", { skip: !runSync ? "dm packages not available" : undefined }, () => {
	let tempDir: string;
	let mockPi: MockPi;
	...
	describe("background runner", { skip: isAsyncAvailable && !isAsyncAvailable() ? "jiti not available" : undefined }, () => {
		function runAsyncSingle(id: string, outputPath: string, outputMode: "inline" | "file-only", artifactConfig = DISABLED_ARTIFACTS) {
			executeAsyncSingle!(id, {
				...
				ctx: { dm: { events: { emit() {} } }, cwd: tempDir, currentSessionId: "session-file-report" },
				artifactConfig,
