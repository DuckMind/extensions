...
const enabled = process.env.DM_SUBAGENTS_CLAUDE_CODE_SMOKE === "1";

test("maintainer Claude Code no-tools smoke", { skip: enabled ? undefined : "set DM_SUBAGENTS_CLAUDE_CODE_SMOKE=1" }, async () => {
	const reportPath = process.env.DM_SUBAGENTS_CLAUDE_CODE_SMOKE_REPORT;
	assert.ok(reportPath, "DM_SUBAGENTS_CLAUDE_CODE_SMOKE_REPORT is required");
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-claude-smoke-"));
