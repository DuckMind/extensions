...
const enabled = process.env.DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE === "1";

test("maintainer Codex exec workspace writer smoke", { skip: enabled ? undefined : "set DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE=1" }, async () => {
	const reportPath = process.env.DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE_REPORT;
	assert.ok(reportPath, "DM_SUBAGENTS_CODEX_EXEC_WRITER_SMOKE_REPORT is required");
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-codex-writer-smoke-"));
