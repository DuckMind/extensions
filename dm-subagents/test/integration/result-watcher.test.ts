...
function createResultWatcher(
	dm: Parameters<typeof createRawResultWatcher>[0],
	...
	return createRawResultWatcher(dm, state, resultsDir, completionTtlMs, { platform: "linux", coalesceDelayMs: 0, ...deps });
}
...
describe("result watcher", () => {
	it("does not create Darwin native watchers or idle timers", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-darwin-idle-"));
		try {
			let watchCalls = 0;
	...
	it("uses demand-gated Darwin polling for result delivery", () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-darwin-demand-"));
		try {
			let demand = true;
	...
	it("processes deferred session-scoped results after session identity is restored", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-session-"));
		try {
			...
			const watcher = createResultWatcher(dm, state, resultsDir, 60_000);
			try {
	...
	it("hands predecessor results to the replacement session without widening ownership", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-transition-"));
		try {
			const state = createState();
	...
	it("includes scheduled observer ids while priming current-session results", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-current-prime-"));
		try {
			const state = createState();
			state.currentSessionId = "session-current";
	...
	it("skips full parsing for unrelated sessions during priming, safety scans, and native watch events", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-filter-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
	...
	it("syncs mission workflow child completion before result cleanup", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-mission-"));
	...
	it("observes a hash-only mission result once when its public alias is unaddressable", async (t) => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-mission-enametoolong-"));
		const resultsDir = path.join(root, "results");
		const project = path.join(root, "project");
		const asyncDir = path.join(root, "async-child");
		fs.mkdirSync(resultsDir, { recursive: true });
	...
	it("keeps result files until failed completion observers retry successfully", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-observer-retry-"));
		const originalError = console.error;
		try {
			console.error = () => {};
	...
	it("does not redeliver user notification after reload while observer retry is pending", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-observer-reload-"));
		const originalError = console.error;
		try {
			console.error = () => {};
			...
			let deliveries = 0;
			let emitted = 0;
			const dm = { events: { on: () => () => {}, emit: () => { emitted += 1; } } };
			const firstState = createState();
			firstState.currentSessionId = "session-current";
			const firstWatcher = createResultWatcher(dm, firstState, resultsDir, 60_000, {
				observeCompletion: () => {
					observerCalls += 1;
			...
			const secondWatcher = createResultWatcher(dm, secondState, resultsDir, 60_000, {
	...
	it("observes retained-project completions without changing active-session delivery", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-scheduled-"));
		const resultsDir = path.join(root, "results");
	...
	it("delivers indexed pending results during reload when public promotion is blocked", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-pending-index-"));
		const originalError = console.error;
		try {
			console.error = () => {};
			const emitted: Array<{ event: string; data: unknown }> = [];
	...
	it("uses indexed result files and ignores unindexed stale files during reload", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-index-"));
		try {
	...
	it("includes observer-known run ids when current-session results are also indexed", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-mixed-observer-"));
		try {
			const resultsDir = path.join(root, "results");
	...
	it("delivers observed indexed pending results when public promotion is blocked", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-observed-pending-"));
		const originalError = console.error;
		try {
			console.error = () => {};
			for (let i = 0; i < 300; i += 1) {
	...
	it("uses observed run ids directly without scanning a stale result pile", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-observed-index-"));
		try {
			for (let i = 0; i < 300; i += 1) {
				fs.writeFileSync(path.join(resultsDir, `stale-${i}.json`), JSON.stringify({ id: `stale-${i}`, sessionId: "session-stale", success: true, summary: "done" }), "utf-8");
			}
	...
	it("uses native completion delivery without attempting external grouped intercom when disabled", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-native-delivery-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const delivered: unknown[] = [];
			const dm = {
	...
	it("delivers result files only to the exact owning session when another watcher shares the same repo", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-scope-"));
		const createPi = () => {
			const emitted: Array<{ event: string; data: unknown }> = [];
					...
					emit(event: string, data: unknown) {
						...
						if (event === "subagent:result-intercom") {
							...
							if (typeof requestId === "string") {
								setImmediate(() => dm.events.emit("subagent:result-intercom-delivery", { requestId, delivered: true }));
							}
						}
					},
				},
		...
		try {
			...
			const ownerWatcher = createResultWatcher(owner.dm, ownerState, resultsDir, 60_000);
			const otherWatcher = createResultWatcher(other.dm, otherState, resultsDir, 60_000);
	...
	it("leaves same-session completions for the exact parent owner", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-owner-scope-"));
		try {
			const ownerState = createState();
	...
	it("logs malformed result files instead of swallowing them silently", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-"));
		try {
	...
	it("diagnoses non-filesystem ENAMETOOLONG from parser and notifier callbacks", async () => {
		for (const source of ["parser", "notifier"] as const) {
			const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), `dm-result-watcher-${source}-enametoolong-`));
			const originalError = console.error;
			const logged: unknown[][] = [];
			try {
	...
	it("does not mark completed siblings as stopped when the overall async result is stopped", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-stopped-siblings-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const listeners = new Map<string, Set<(payload: unknown) => void>>();
			const dm = {
				events: {
					...
					emit(event: string, data: unknown) {
						...
						if (event === "subagent:result-intercom") {
							const requestId = data && typeof data === "object" ? (data as { requestId?: unknown }).requestId : undefined;
							if (typeof requestId === "string") setImmediate(() => dm.events.emit("subagent:result-intercom-delivery", { requestId, delivered: true }));
						}
					},
				},
			};
	...
	it("enriches async completion and intercom payloads with nested registry children before deletion", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-nested-"));
	...
	it("filters malformed explicit nested children in result files before compacting", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-nested-malformed-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const listeners = new Map<string, Set<(payload: unknown) => void>>();
			const dm = {
	...
	it("retries and delivers result files after nested registry enrichment recovers", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-nested-retry-"));
	...
	it("delivers a later terminal result after a paused payload for the same run", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-paused-then-complete-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const dm = {
				events: {
	...
	it("keeps a same-state paused replacement written during paused delivery", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-paused-same-state-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const dm = {
				events: {
	...
	it("delivers a terminal result after the paused file was already consumed", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-paused-consumed-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const dm = {
				events: {
			...
			const watcher = createResultWatcher(dm, state, resultsDir, 60_000, { deliverIntercomResults: false });
			try {
				writeIndexedResult(resultFile, {
	...
	it("warns when an available grouped-result listener does not acknowledge delivery", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-unacknowledged-listener-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const listeners = new Map<string, Set<(payload: unknown) => void>>();
			const dm = {
	...
	it("marks acknowledged grouped intercom results so local notification is suppressed", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-intercom-ack-"));
		try {
			const emitted: Array<{ event: string; data: unknown }> = [];
			const listeners = new Map<string, Set<(payload: unknown) => void>>();
			const dm = {
	...
	it("delivers a result when a previous duplicate key has expired", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-expired-"));
		try {
			const state = createState();
			state.currentSessionId = "session-1";
	...
	it("keeps an unaccepted result for retry and deletes it only after notifier acceptance", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-notifier-retry-"));
		try {
			const state = createState();
			state.currentSessionId = "session-1";
			const emitted: string[] = [];
	...
	it("retries delivery after a transient session index access denial", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-index-retry-"));
		const originalError = console.error;
	...
	it("drops stale watcher authority without emitting or deleting", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-stale-"));
		try {
			const state = createState();
			state.currentSessionId = "session-1";
			const emitted: string[] = [];
	...
	it("marks reload backlog display-only", async () => {
		const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-quiet-"));
		try {
			const state = createState();
			state.currentSessionId = "session-1";
	...
	describe("resultScanLogging", () => {
		...
		const runScan = (deps: Record<string, unknown>) => {
			const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-scanlog-"));
			const state = createState();
			const watcher = createResultWatcher({ events: { on: () => () => {}, emit() {} } }, state, resultsDir, 60_000, {
		...
		it('keeps slow scans that found work under "activity"', () => {
			const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-scanlog-work-"));
			const state = createState();
			state.currentSessionId = "session-current";
		...
		it('silences all slow scans under "off"', () => {
			const resultsDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-result-watcher-scanlog-off-"));
			const state = createState();
			state.currentSessionId = "session-current";
