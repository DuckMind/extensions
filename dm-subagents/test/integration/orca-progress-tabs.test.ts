...
describe("Orca progress-tab observer", () => {
	it("mirrors a native DM child without replacing its execution path", { skip: process.platform === "win32" ? "Orca progress tabs are not supported on Windows" : undefined }, async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-orca-native-"));
		tempDirs.push(dir);
		...
		const childEvents = [
			...
			{ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: "native DM result" }], stopReason: "stop", usage: { input: 1, output: 2, cacheRead: 0, cacheWrite: 0, cost: { total: 0 } } } },
			{ type: "agent_settled" },
		];
		const fakePi = writeNodeCommand(dir, "dm", `for (const event of ${JSON.stringify(childEvents)}) process.stdout.write(JSON.stringify(event)+'\\n')`);
		...
		const exitCode = await runProcess(
			...
			{
				...process.env,
				DM_CODING_AGENT_DIR: agentDir,
				DM_SUBAGENT_ORCA_BINARY: fakeOrca,
				DM_SUBAGENT_PI_BINARY: fakePi,
		...
		assert.match(result.results[0].output, /native DM result/);
		assert.match(fs.readFileSync(path.join(asyncDir, "output-0.log"), "utf-8"), /read: README\.md[\s\S]*native DM result/);
	...
	it("uses one observer tab for a background parallel run", { skip: process.platform === "win32" ? "Orca progress tabs are not supported on Windows" : undefined }, async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-orca-parallel-"));
		tempDirs.push(dir);
		const asyncDir = path.join(dir, "async");
		const agentDir = path.join(dir, "agent-dir");
		...
		const fakePi = writeNodeCommand(dir, "dm", "process.stdout.write(JSON.stringify({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'parallel result'}]}})+'\\n')");
		...
		fs.writeFileSync(configPath, JSON.stringify({
			...
			steps: [{
				parallel: [
					{ agent: "worker", task: "One", systemPrompt: "Use native DM", systemPromptMode: "replace", inheritProjectContext: false, inheritSkills: false },
					{ agent: "reviewer", task: "Two", systemPrompt: "Use native DM", systemPromptMode: "replace", inheritProjectContext: false, inheritSkills: false },
				],
				concurrency: 2,
			}],
			resultPath,
	...
	it("allocates unique worktree-wide numbers across concurrent processes and nested cwd values", { skip: process.platform === "win32" ? "Orca progress tabs are not supported on Windows" : undefined }, async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagents-orca-counter-"));
		tempDirs.push(dir);
		...
		const processes = Array.from({ length: 8 }, (_, index) => runProcess(
			...
			repo,
			{ ...process.env, DM_SUBAGENT_ORCA_BINARY: fakeOrca, ORCA_TEST_CAPTURE_DIR: captures, ORCA_TEST_ORDER: orderFile, CHILD_CWD: index % 2 === 0 ? dir : nested },
		));
