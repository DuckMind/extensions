/**
 * Real DM-session end-to-end test for the subagent extension.
 *
 * Spawns an actual child `dm` subprocess (a repo-local child CLI that runs a
 ...
 *
 * Skips gracefully when the dm runtime packages are not importable.
 */
...
const piCodingAgent = await tryImport<{ __piSubagentsTestShim?: boolean }>("@duckmind/dm-coding-agent");
const piAi = await tryImport<unknown>("@duckmind/dm-ai");
...
const BOGUS_EXTRA_DIRS = path.join(os.tmpdir(), "nonexistent-dm-subagents-e2e-extra-dirs");
const BOGUS_PI_BINARY = path.join(os.tmpdir(), "nonexistent-dm-binary-e2e");
const BOGUS_PI_PACKAGE_ROOT = path.join(os.tmpdir(), "nonexistent-dm-coding-agent-package-root-e2e");
const ISOLATED_ENV_KEYS = [
	"DM_SUBAGENT_CHILD",
	"DM_SUBAGENT_FANOUT_CHILD",
	"DM_SUBAGENT_DEPTH",
	"DM_SUBAGENT_MAX_DEPTH",
	"DM_SUBAGENT_EXTRA_AGENT_DIRS",
	"DM_SUBAGENT_PARENT_SESSION",
	"DM_SUBAGENT_PI_BINARY",
	"DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT",
] as const;

describe("real DM-session subagent E2E", { skip: !available ? "dm runtime packages not available" : undefined }, () => {
...
---
Use the available tools.`;
		const fixtureExtension = `export default function (dm) {
	dm.registerTool({
		name: "fixture_search",
		...
		run = await runRealSubagentSession({
			...
			projectFiles: {
				".dm/agents/extension-worker.md": extensionAgent,
				".dm/agents/missing-extension-worker.md": missingAgent,
...
---
Report active tools.`;
		const fixtureAsyncExtension = `export default function (dm) {
	dm.on("before_agent_start", async () => {
		...
		run = await runRealSubagentSession({
			...
			reportChildTools: true,
			projectFiles: {
				".dm/agents/async-extension-worker.md": asyncExtensionAgent,
	...
	it("boots the extension in a real parent session and delivers a faux child result", async () => {
		...
		process.env.DM_SUBAGENT_CHILD = "1";
		process.env.DM_SUBAGENT_FANOUT_CHILD = "1";
		process.env.DM_SUBAGENT_DEPTH = "1";
		process.env.DM_SUBAGENT_MAX_DEPTH = "1";
		process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS = BOGUS_EXTRA_DIRS;
		process.env.DM_SUBAGENT_PARENT_SESSION = "polluted-parent";
		process.env.DM_SUBAGENT_PI_BINARY = BOGUS_PI_BINARY;
		process.env.DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT = BOGUS_PI_PACKAGE_ROOT;
