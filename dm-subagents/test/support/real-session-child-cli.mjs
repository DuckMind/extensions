...
import { fauxAssistantMessage, fauxProvider, fauxText, fauxToolCall } from "@duckmind/dm-ai/providers/faux";
import {
	createAgentSession,
...
async function main() {
	...
	const responseText = process.env.DM_SUBAGENTS_E2E_CHILD_TEXT ?? "CHILD_REAL_SESSION_OK";
	const reportChildTools = process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS === "1";
	const cwd = process.cwd();
	const ownedAgentDir = process.env.DM_CODING_AGENT_DIR
		? undefined
		: mkdtempSync(path.join(os.tmpdir(), "dm-e2e-agent-dir-"));
	const agentDir = process.env.DM_CODING_AGENT_DIR ?? ownedAgentDir;
	...
	let session;
	faux.setResponses([
		() => process.env.DM_SUBAGENT_STRUCTURED_OUTPUT_CAPTURE
