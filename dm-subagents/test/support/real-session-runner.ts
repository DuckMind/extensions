...
import { type AssistantMessage, type Context, type ToolCall } from "@duckmind/dm-ai";
import {
	...
	fauxToolCall,
	fauxProvider,
} from "@duckmind/dm-ai/providers/faux";
import {
	type AgentSession,
...
function installChildPiShim(childText: string, reportChildTools = false): () => void {
	const rootDir = mkdtempSync(path.join(os.tmpdir(), "dm-real-session-cli-"));
	const binDir = path.join(rootDir, "bin");
	const piPackageDir = path.join(rootDir, "dm-package");
	const childCliPath = path.join(piPackageDir, "dist", "cli.mjs");
	const previousPath = process.env.PATH;
	const previousPiBinary = process.env.DM_SUBAGENT_PI_BINARY;
	const previousChildText = process.env.DM_SUBAGENTS_E2E_CHILD_TEXT;
	const previousReportTools = process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS;
	...
	writeFileSync(
		path.join(binDir, "dm.cmd"),
		`@echo off\r\n"${process.execPath}" "${childCliPath}" %*\r\n`,
	);

	process.env.PATH = `${binDir}${path.delimiter}${previousPath ?? ""}`;
	if (process.platform === "win32") {
		delete process.env.DM_SUBAGENT_PI_BINARY;
		process.argv[1] = childCliPath;
	} else {
		process.env.DM_SUBAGENT_PI_BINARY = path.join(binDir, "dm");
	}
	process.env.DM_SUBAGENTS_E2E_CHILD_TEXT = childText;
	if (reportChildTools) process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS = "1";
	else delete process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS;

	return () => {
		if (previousPath === undefined) delete process.env.PATH;
		else process.env.PATH = previousPath;
		if (previousPiBinary === undefined) delete process.env.DM_SUBAGENT_PI_BINARY;
		else process.env.DM_SUBAGENT_PI_BINARY = previousPiBinary;
		if (process.platform === "win32") {
			if (previousArgv1 === undefined) delete process.argv[1];
			else process.argv[1] = previousArgv1;
		}
		if (previousChildText === undefined) delete process.env.DM_SUBAGENTS_E2E_CHILD_TEXT;
		else process.env.DM_SUBAGENTS_E2E_CHILD_TEXT = previousChildText;
		if (previousReportTools === undefined) delete process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS;
		else process.env.DM_SUBAGENTS_E2E_REPORT_CHILD_TOOLS = previousReportTools;
...
export async function runRealSubagentSession(options: RealSessionRunOptions): Promise<RealSessionRun> {
	const cwd = mkdtempSync(path.join(os.tmpdir(), "dm-real-session-cwd-"));
	const home = mkdtempSync(path.join(os.tmpdir(), "dm-real-session-home-"));
	const previousCwd = process.cwd();
	const envSnapshot = new Map([
		["HOME", process.env.HOME],
		["USERPROFILE", process.env.USERPROFILE],
		["DM_SUBAGENTS_TEMP_ROOT", process.env.DM_SUBAGENTS_TEMP_ROOT],
		["DM_CODING_AGENT_DIR", process.env.DM_CODING_AGENT_DIR],
		["DM_SUBAGENT_EXTRA_AGENT_DIRS", process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS],
		["DM_SUBAGENT_CHILD", process.env.DM_SUBAGENT_CHILD],
		["DM_SUBAGENT_FANOUT_CHILD", process.env.DM_SUBAGENT_FANOUT_CHILD],
		["DM_SUBAGENT_DEPTH", process.env.DM_SUBAGENT_DEPTH],
		["DM_SUBAGENT_MAX_DEPTH", process.env.DM_SUBAGENT_MAX_DEPTH],
		["DM_SUBAGENT_PARENT_SESSION", process.env.DM_SUBAGENT_PARENT_SESSION],
		["DM_SUBAGENT_PI_BINARY", process.env.DM_SUBAGENT_PI_BINARY],
		["DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT", process.env.DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT],
	]);
	...
	try {
		...
		process.env.DM_SUBAGENTS_TEMP_ROOT ??= path.join(home, "dm-subagents-temp");
		process.env.DM_CODING_AGENT_DIR = home;
		delete process.env.DM_SUBAGENT_EXTRA_AGENT_DIRS;
		delete process.env.DM_SUBAGENT_CHILD;
		delete process.env.DM_SUBAGENT_FANOUT_CHILD;
		delete process.env.DM_SUBAGENT_DEPTH;
		delete process.env.DM_SUBAGENT_MAX_DEPTH;
		delete process.env.DM_SUBAGENT_PARENT_SESSION;
		delete process.env.DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT;
