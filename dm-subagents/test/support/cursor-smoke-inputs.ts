...
export function cursorSmokeInputs(env: NodeJS.ProcessEnv): CursorSmokeInputs {
	if (env.DM_SUBAGENTS_CURSOR_SMOKE_DISPOSABLE !== "1") {
		throw new Error("DM_SUBAGENTS_CURSOR_SMOKE_DISPOSABLE=1 is required to attest that the operator-managed workspace is disposable.");
	}
	const workspace = existingDirectory(env.DM_SUBAGENTS_CURSOR_SMOKE_WORKSPACE, "DM_SUBAGENTS_CURSOR_SMOKE_WORKSPACE");
	const stateRoot = existingDirectory(env.DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT, "DM_SUBAGENTS_CURSOR_SMOKE_STATE_ROOT");
	...
	const canaryPath = path.join(workspace, "dm-subagents-cursor-write-canary.txt");
