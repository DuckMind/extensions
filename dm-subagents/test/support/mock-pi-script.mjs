...
function writeStructuredOutputCapture(response) {
	...
	const outputPath = process.env.DM_SUBAGENT_STRUCTURED_OUTPUT_CAPTURE;
	...
	if (Object.prototype.hasOwnProperty.call(response, "structuredOutputAcceptanceReport")) {
		const acceptancePath = process.env.DM_SUBAGENT_STRUCTURED_OUTPUT_ACCEPTANCE_CAPTURE;
...
function writeRuntimeAcknowledgedExtensions(response) {
	...
	const outputPath = process.env.DM_SUBAGENT_RUNTIME_ACKNOWLEDGED_EXTENSIONS;
	if (!outputPath) return;
	fs.mkdirSync(path.dirname(outputPath), { recursive: true });
...
function writeToolDiagnostic(response) {
	...
	const diagnosticPath = process.env.DM_SUBAGENT_TOOL_DIAGNOSTIC_PATH;
	const required = JSON.parse(process.env.DM_SUBAGENT_REQUIRED_TOOLS ?? "[]");
	...
	fs.writeFileSync(diagnosticPath, JSON.stringify({
		agent: process.env.DM_SUBAGENT_CHILD_AGENT,
		required,
		available,
		missing,
	}), "utf-8");
...
function readRequiredToolsEnv() {
	try {
		const encoded = process.env.DM_SUBAGENT_REQUIRED_TOOLS;
