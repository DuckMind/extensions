...
- Match dm-mcp-adapter direct-tool names when configured MCP server names contain hyphens. Thanks to [@unrelentingfox](https://github.com/unrelentingfox) for #1685.
...
- Allow scheduled project roots shared through a registered Git worktree's `.dm` symlink while rejecting unrelated or unproven Git-layout escapes. Thanks to [@sususu98](https://github.com/sususu98) for #1656.
...
- Accept path-like DM session ids up to 4,096 characters when snapshotting background work. Thanks to [@dvishoot](https://github.com/dvishoot) for #1616.
...
- Ignore nested `.dm` and `sync-backups` directories during agent discovery so stale backup definitions cannot become executable agents. Thanks to [@arlishansenn](https://github.com/arlishansenn) for #1596.
...
### Highlights
- Launch MCP tools from more places, including runtime-registered servers, DM package manifests, and Agent Plugin configs.
...
- Add process-local event registration so independent DM extensions can register runtime agents through the installed owner. Thanks to [@fmoda3](https://github.com/fmoda3) for #1533.
...
- Resolve direct MCP tool selections from DM package manifests and Agent Plugin configs. Thanks to [@fmoda3](https://github.com/fmoda3) for #1541.
...
- Resolve package subagents from bare HTTP(S) Git URLs stored in DM settings. Thanks to [@trancikk](https://github.com/trancikk) for #1452.
...
- Fail a DM child launch when the child reports a different provider/model than the resolved requested model. Thanks to [@zzzubair](https://github.com/zzzubair) for #1377.
...
- Clarify that Council Mode can include installed external-runner advisors such as `gpt-pro` from [Surf](https://github.com/nicobailon/surf-cli/) when the `surf-cli` DM extension has registered `surf-oracle`, with text JSON reports instead of `outputSchema`.
...
- DM extensions can now register runtime agents without writing user or project config.
...
- Add runtime agent registration for DM extensions, with name and alias collision checks. Thanks to [@fmoda3](https://github.com/fmoda3) for #1310.
...
- Keep forked subagent sessions out of top-level `dm -c` discovery by storing them under the parent session root. Thanks to [@xz-dev](https://github.com/xz-dev) for #1297.
...
- DM extension worktrees now have clearer guidance to avoid duplicate auto-loaded tools and shortcuts.
...
### Changed
- Add a dm-subagents reference for coordinating multiple tasks, worktrees, and repositories, including guidance for keeping DM extension worktrees outside auto-discovered extension directories.
...
### Added
- Add `/subagents-inspect-rpc`, a host-facing bridge command that answers on-demand async child inspection requests with a correlated, bounded `DM_SUBAGENT_INSPECT_JSON:` widget payload (task, transcript window, final output), so RPC hosts can inspect children without a model turn while the live status feed stays small. Thanks to [@yanqianglu](https://github.com/yanqianglu) for #1254.
...
- Make Surf's `gpt-pro` agent an optional package integration instead of a dm-subagents builtin. If you disabled the old builtin workaround, remove `agentOverrides.gpt-pro.disabled` before using Surf's package agent. Thanks to [@binhex](https://github.com/binhex) for #1256.
...
- Async runs are harder to lose when storage is full, file access is temporarily denied, identifiers are too long, or multiple DM windows share one session.
...
- Add `DM_SUBAGENT_FS_RETRY_MAX_TOTAL_MS` so hosts can cap filesystem retry waits. Unset by default. Thanks to [@MarcusNeufeldt](https://github.com/MarcusNeufeldt) for #1143.
...
- Route async completion notifications and cleanup only to the parent DM process that launched the run, so concurrent windows sharing one session file cannot consume each other's results. Thanks to [@wangjianming](https://github.com/wangjianming) for #1225.
- Keep extension reload cleanup scoped to the replaced session runtime, so concurrent DM sessions in one process do not remove each other's subscriptions or parent-session identity. Thanks to [@ryanbbrown](https://github.com/ryanbbrown) for #1222.
...
- Resolve `/subagents-generate-profiles` provider probes through the shared DM executable resolver so configured and Windows-specific DM commands work. Thanks to [@Wumpf](https://github.com/Wumpf) for #1199.
- Resolve the workflowScript parser from dm-subagents instead of the caller's working directory, so workflows start in projects that do not install Acorn. Thanks to [@xz-dev](https://github.com/xz-dev) for #1214, following up #1190.
- Keep workflowScript child-launch tracking working on Bun-built DM without a hard dependency on V8 promise hooks. Thanks to [@rochecompaan](https://github.com/rochecompaan) for #1158 and [@rholak](https://github.com/rholak) for the version-window diagnosis.
...
- Keep `mcp:<server>` direct tools available when dm-mcp-adapter cache identity includes a request-header command. Thanks to [@xz-dev](https://github.com/xz-dev) for #1141.
...
### Added
- Add optional Orca progress tabs with bounded, sanitized mirrors for native DM and external CLI children. Thanks to @hyein-cbio for #1080.
...
- Report an actionable project-settings override when duplicate ambient DM extensions prevent a child from starting (#1114).
...
- Reject configured subagent models that are not in the active host model registry before spawning a child, instead of forwarding an invalid `--model` argument to DM. Thanks to @DresvyanskiyDenis for #1093.
...
- Keep deleted-schedule timers from exiting DM and re-arm recurring schedules after unexpected timer fire failures. Thanks to @albertgwo for #1084.
...
- Let builtin role overrides keep DM's normal tools and extensions with `tools: "inherit"`. Thanks to @estanexanavsem for #1047 and @davidarny for #1049.
...
- Keep watchdog reviews, permission checks, Prompt Audit rewriting, and completion intent checks on the authenticated provider stream across the DM 0.81 and 0.84 APIs. Thanks to @nuzayets for #1067.
...
- Launch Herdr inspector panes with Node when DM runs as a standalone executable. Thanks to @kevinpita for #1051.
...
- Add `DM_SUBAGENT_TASK_DELIVERY=auto|file` for hosts that block child processes when the task text appears in the command line. File mode writes the task to a temporary `task.md` and passes that path instead. Thanks to @yanqianglu for #1028.
...
- Stop async DM writer processes as full process groups, and only mark process cleanup as proven after the process tree has actually exited. Thanks to @asjer for #1030.
...
- Make unnamed intercom fallback targets match dm-intercom's registered name length, so subagents without a custom session name can still reach their parent. Thanks to @mystery4f for #1017.
...
- Let single external-CLI workflow children start without inheriting a DM model, so model-less external runners do not fail preflight. Thanks to @twosunnus for #1016.
...
- Move project-scoped dm-subagents storage from `.dm-subagents/` to `.dm/subagents/` for cleaner project roots. Thanks to @yceachan for #971.
...
- Keep `git-root` project agent and package discovery stable when incidental `.dm` state appears in a nested linked worktree. Thanks to @klajdo-f for #950.
...
### Changed
- Clarified mission-use policy in the packaged `dm-subagents` skill.
...
### Changed
- Documented the one-command `gate` shorthand, retained `children.list`/`resume` flow, and refinement overlays in the tool reference and agents docs, and refreshed the packaged `dm-subagents` skill for the current mission `objective` shape, workflow `state`, and child-protocol limits.
...
### Fixed
- Prevent DM from crashing when subagent status widgets and overlays are shown in narrow or resized terminal layouts. Thanks to @alanvardy for the report in #858 and @meatcar for the fix.
...
- Added native `@gotgenes/dm-permission-system` compatibility for child processes. Thanks to @jagaliano for #847.
...
- Match dm-mcp-adapter cache identities that include `protocolVersion`, so direct MCP tool selections resolve from current adapter caches. Thanks to @ProCleiton for #848.
...
- Removed the DM CLI devDependency from the default install and test against a local runtime shim, so repo audits no longer report the upstream dev-only Undici advisory while real DM E2E remains optional. Thanks to @dmg-egg for #782.
...
- Added an observational `dm-subagents/external-runs` provider API for visible terminal work, without taking process ownership. Thanks to @nicobailon for #795.
...
- Added opt-in native DM-child tool permissions with global and per-agent `allow`/`ask`/`deny` rules, watchdog-owned exact-call decisions, and bounded redacted audit records. Unconfigured tools pass through, while bash policy remains with `dm-guard`.
...
- Added Herdr project panes so an orchestrator can open a project-rooted DM session for substantial cross-codebase work.
...
- Clarified packaged dm-subagents guidance for cross-repository delegation, authority boundaries, and evidence-only external gates.
...
- Documented the headless dm-guard compatibility path for child-specific explicit allow/deny policy. Thanks to @chama-chomo for #742.
...
- Isolated each mock DM test queue so late child processes cannot consume or lose responses after the next test resets the harness. Thanks to @nicobailon for #810.
...
- Show current-session async runs in the Fleet inspector even when this DM process did not start them.
...
- Launch standalone DM child processes directly instead of prepending the resolved DM CLI script path. Thanks to @ZacharyQin for #764.
...
- Project oversized redundant DM `turn_end` and `agent_end` child events to bounded lifecycle records instead of failing image-heavy runs with `protocol_output_limit`, while preserving `agent_end.willRetry` drain behavior. Thanks to @barto-sh for #743.
...
- Routed foreground chain scratch files to user-scoped temp storage when `artifactDir` is `"session"` or `"temp"`, preventing `.dm-subagents/` clutter in the working directory. Thanks to @magoz for #729.
...
- Kept successful background subagent completions quiet so inactive DM tabs are not marked unread, while failed and paused completions still notify the originating session. Thanks to @killianMei for #728.
...
### Changed
- Refreshed the bundled `dm-subagents` skill for the 0.39 surface: Fleet inspector live controls (`s` steer, `D` stop), the recommended model-tiering recipe, `agentOverrides.description`, `projectRootResolution: "git-root"`, running-card live-detail/model badges, and the newer extension RPC capability projections (`fleetStatus`, `launchResolvedExtensions`, `runtimeAcknowledgedExtensions`, `(runId, index)` correlation). Corrected the stale README "inspection-only" fleet inspector wording.
...
### Changed
- Reduced repeated runtime filesystem work by caching stable DM config-directory resolution, incrementally sanitizing run history, and limiting nested control-result polling to files created for the active request.
...
- Kept dm-intercom stable IDs from leaking into child sessions and used the current intercom runtime ID for unnamed supervisor targets.
...
### Fixed
- Passed `--no-context-files` to child DM runs when an agent disables inherited project context, avoiding stale prompt-header parsing as DM's context block format changes. Thanks to @KorenKrita for #667.
...
- Split the bundled `dm-subagents` skill into a short router plus focused reference files to avoid truncation and unnecessary context loading. Thanks to @peedrr for #659.
- Added `fleetViewPlacement` so the persistent FleetView can be placed above or below the editor. Thanks to @rtbe for #660.
- Refreshed the bundled `dm-subagents` skill for 0.35–0.37 control and config surface: `/subagents`, `/subagents-stop`, `/subagents-watchdog`, `stop`/`append-step`, parallel `count`, watchdog overview, frontmatter `async`/`timeoutMs`/`turnBudget` defaults, `artifactDir`/`asyncWidget`, fresh/fork badges, and builtin worker/delegate ambient-tool boundaries.
...
- Matched dm-mcp-adapter metadata cache identity so valid direct MCP tools are not rejected as stale when tool filters, socket transport, URL interpolation, or command-backed secrets are configured. Thanks to @mattrobenolt for #649.
...
- Added a public `dm-subagents/preflight` API that resolves an ordinary single-agent launch contract without creating child sessions, temp prompt files, structured-output runtimes, or run artifacts. Thanks to @shaggitza for #634.
...
- Classified missing resolved MCP direct tools as a host/dm-mcp-adapter child-registration problem while preserving strict fail-closed diagnostics. Thanks to peedrr for #638.
...
- Added `artifactDir` config to store subagent artifacts in the project, DM session, or temp artifact directory while keeping project-local artifacts as the default. Thanks to WeZZard (@WeZZard) for #582.
...
- Replaced the flat `/subagents` admin model, thinking, and agent pickers with a searchable, bounded-scroll selector docked in place of the editor, matching DM's built-in `/model` picker so the current selection no longer scrolls off screen when the option list is long. Thanks to Chanyeong Lim (@asp345) for #568.
- Added `advisor` as an `oracle`-compatible bundled agent alias for users switching between Claude Code and DM naming. Thanks to Serhii Chernenko (@serhii-chernenko) for #552.
...
### Fixed
- Kept explicit empty and MCP-only child tool allowlists from falling back to DM's default builtin tools. Thanks to @jstokke for #628.
...
- Required `@duckmind/dm-ai` 0.80.0 or newer because watchdog reviews import its `./compat` entrypoint, preventing background runs from loading on older hosts. Thanks to @donwellsav for #599.
...
- Counted provider-native `dm-checkpoint` commit changes as mutation evidence so CompletionGuard does not falsely fail Cursor SDK writer runs that already edited files. Thanks to Matias Gigena (@MatiasGigena) for #615.
...
- Retried transient Windows filesystem locks while creating async result directories and stopped destructively recreating shared async directories during startup access checks, so concurrent DM instances are less likely to lose completed async results to `EPERM` directory handles. Thanks to AiraNadih (@AiraNadih) for #566.
...
- Use DM's native editor for `/subagents` system-prompt editing so terminal editors receive terminal ownership and cannot leave a stale waiting status. Thanks to Prodipta Guha (@proguha) for #576.
- Bundled TypeBox as a production dependency so detached runners can always load `typebox/compile`, including managed extension installs where DM's host package is not visible from the child process. Thanks to Matteo Collina (@mcollina) for #583.
- Updated the DM development SDK to 0.81.0 and passed the watchdog stream through the renamed `Agent.streamFunction` option, preventing watchdog reviews from terminating with `streamFunction is not a function`. Thanks to Wang Zixiong (@XWIlluDelu) for #574.
...
- Raised the child stdout line limit above DM’s resized-image payload range so image OCR subagents no longer fail with `protocol_output_limit` on valid `read` tool image events. Thanks to @zmarty for #538.
...
- Routed main watchdog reviews through matching provider-scoped `streamSimple` handlers before falling back to the compat dispatcher, restoring custom-provider watchdog models on newer DM runtimes. Thanks to @alexei-led for #527.
...
- Registered the native child `intercom` fallback before strict tool-allowlist diagnostics run and stopped treating DM core tools as missing extension tools, preventing read-only scouts and workers from failing before execution when strict child tool allowlists are active.
...
- Added the missing `context: "fork"` field to the fork-context example in the bundled `dm-subagents` skill. Thanks to Kier (@kierr) for #540.
...
- Stopped hashing the full content of very large changed/untracked files when computing the watchdog repo change signature, and made signature computation non-fatal, so `dm` no longer crashes at startup with `Failed to load extension … File size (N) is greater than 2 GiB` in repositories that contain files ≥ 2 GiB. Files larger than a threshold (64 MiB default, overridable via `DM_SUBAGENTS_MAX_HASH_FILE_BYTES`) are now fingerprinted by size and mtime instead of being read into memory. Thanks to Alexander Prilipko (@axelbaumlisto) for #553, @astarktc for #535, @restrolla for #536, and @pompanonb for #551.
...
### Fixed
- Updated DM development packages and real-session SDK coverage to 0.80.10, removing known dependency audit findings. Thanks to dmg (@dmg-egg) for #520.
- `subagent({ action: "get" })` now honors `agentScope` for agent and chain details. Thanks to Kyle (@kylegl) for #519.
- Removed timer-driven foreground spinner redraws that repeatedly rendered the full DM TUI and could survive session shutdown; running indicators now advance only with real progress updates.
...
- Skipped optional global npm package discovery while DM is offline, avoiding `npm root -g` subprocesses during agent and skill discovery. Thanks to Rafiq Rashid (@rrvsh) for #506.
...
- Moved DM-owned `@duckmind/dm-tui` and `typebox` imports to optional wildcard peer dependencies while retaining exact dev versions for local and CI tests. Thanks to Alexei Ledenev (@alexei-led) for #510.
...
- Added a versioned `dm-subagents/background-work` provider contract so `subagent_wait` can track exact current-session jobs from other extensions without count races. Child runtimes can expose the wait tool through their strict allowlist, effective wait config is propagated to every child launch path, and headless sessions drain active work before ending. Thanks to RoboBryce (@robobryce) for #472 and #473.
- Added a typed v1 foreground delegation contract for extension consumers through the existing `prompt-template:subagent:*` transport, with strict bounded controls, structured terminal states, cancellation, and a supported `dm-subagents/delegation` package export. Thanks to JT (@juicetin) for #465 and #467.
...
- Added agent-local `skillPath` discovery so custom agents can select private skills without publishing them to DM's parent/global catalog. Relative paths resolve from the defining agent file, local matches take precedence, and missing or unreadable candidates fall back to normal discovery. Thanks to Kylegl (@kylegl) for #428.
...
- Added canonical-session leases for direct child revival so independent parent processes cannot write the same persisted session concurrently. Lease ownership includes the revived/source run, parent session, runner and writer process identities, and host; a two-phase startup handshake rejects contention before DM starts, and stale recovery remains conservative. Thanks to Luke Parke (@LukasParke) for #446.
...
- Added an opt-in read-only subagent watchdog that reviews actual repo edits at safe agent-end boundaries, with visible warnings, main and child watchdog coordination, strong complementary model recommendations, changed-file TypeScript/JavaScript LSP diagnostics, `/subagents-watchdog` status/model commands, and agent-facing watchdog configuration actions. Thanks to can1357/oh-my-dm for the advisor/watchdog concept, and to apmantza/dm-lens, gjczone/dm-shazam, and can1357/oh-my-dm for LSP diagnostics patterns.
...
### Changed
- Updated the bundled `dm-subagents` skill so Fable mode is the default orchestration posture for complex work, and refreshed recent command/config guidance.
- Documented `contact_supervisor` structured interview requests in the default child bridge instructions.

### Fixed
- Moved the published extension entrypoint to the package root so DM displays the startup label as `dm-subagents` instead of an internal source path. Thanks to Ramin Hazegh (@rhazegh) for #475.
...
- Recognized DM's `max` thinking level in child model suffixes, Clarify selection, watchdog settings, and status formatting, while exposing it only when model metadata explicitly supports it. Thanks to mapleluv (@mapleluvr) for #423.
...
- Reused the verified current or installed DM CLI on POSIX instead of resolving a potentially missing or different `dm` from `PATH`. Thanks to Luke Parke (@LukasParke) for #443.
...
- Resolve the real DM CLI on Windows when dm-subagents runs inside an embedded SDK host instead of relaunching the host application's entry point. Thanks to Marc Kassubeck (@CompN3rd) for #413.
...
### Added
- Added `waitTool` config and `DM_SUBAGENT_WAIT_TOOL_ENABLED` so interactive users can keep the `subagent_wait` tool registered while making it return immediately instead of blocking on background subagents. Thanks to Rebecca Dessonville (@TwistedTabby) for #394.
...
### Fixed
- Avoid native supervisor-channel tool conflicts when `dm-intercom` is also installed by deferring native tool registration until runtime startup and keeping a namespaced native supervisor reply tool.
...
- Added a stable v1 in-process event-bus RPC for other DM extensions, with `ping`, `status`, async-only `spawn`, `interrupt`, and async `stop` over versioned request/reply envelopes.
...
- Added `subagent({ action: "steer", id, message, index? })` for non-terminal guidance to live async DM child sessions, with file-backed control requests, per-child steering inboxes, status/event visibility, and queued delivery for pending indexed async children when the runtime supports mid-run steering.
...
- Added fuzzy model resolution so callers can specify models with provider separator variations, optional date-stamp parts, and case differences instead of exact `provider/modelId` strings. When `subagents.modelScope: { enforce: true, allow: [...] }` is configured, explicit caller-supplied out-of-scope models error while frontmatter/parent-inherited/fallback models warn. Inspired by @tintinweb's dm-subagents.
...
- Scope async subagent completion notifications to the exact owning DM session so another session in the same repo no longer receives result notices.
...
- Derive live-detail and full-notification hints from DM's configured expand key instead of hard-coding `Ctrl+O`. Thanks to Kylegl (@kylegl) for #364.
...
- Added `DM_SUBAGENT_PI_BINARY` to let wrappers launch child agents through an explicit DM binary instead of resolving `dm` from `PATH`. Thanks to David Barroso (@dbarrosop) for #341.
- Added `worktreeBaseDir` and `DM_SUBAGENTS_WORKTREE_DIR` so worktree isolation can use a stable trusted base directory. Thanks to Matt Robenolt (@mattrobenolt) for #185.
...
- Added `maxSubagentSpawnsPerSession` and `DM_SUBAGENT_MAX_SPAWNS_PER_SESSION` to cap total subagent launches in one session. Thanks to @eightHundreds for #239.
...
### Fixed
- Keep generated subagent markdown outputs, progress files, and run artifacts under the project-local `.dm-subagents/` directory by default. Thanks to Carolina (@carolitascl) for #326.
...
- Avoid runtime peer imports from detached async runners while still forwarding the DM package root when available. Thanks to @aurbina83 for #352 and @huangkun3251 for #342.
...
- Persist forked child session files when DM returns a branch path before writing it to disk. Thanks to @trisforrestcam for #174.
...
- Restore queued and running detached async jobs into the widget after restarting DM. Thanks to Vicary (@vicary) for #362.
...
### Fixed
- Discover `dm-intercom` installations created by `--extension npm:dm-intercom` under DM's temporary npm extension cache. Thanks to loss-and-quick (@loss-and-quick) for #336.
...
- Discover nested grouped skills such as `.dm/skills/group/name/SKILL.md` so subagents match the host runtime's recursive skill lookup. Thanks to Weaxs (@Weaxs) for #262.
- Follow DM's configured project config directory for project-local agents, chains, skills, packages, settings, direct MCP config, and intercom package discovery instead of hardcoding `.dm`, while retaining `.dm` as the fallback for older DM versions.
...
### Fixed
- Resolve the async result watcher directory with `fs.realpathSync.native()` before `fs.watch()` so Windows profiles with 8.3 temp paths do not crash DM when async subagent results arrive. Thanks to kerushidao (@kerushidao) for #254.
...
- Launch detached async runners through Node when DM itself is not the Node executable. Thanks to Tetsuya.dev (@tetsuya-dev-jp) for #273.
...
- Honor `DM_CODING_AGENT_DIR` for runtime config, agent/chain/settings discovery, skills, run history, artifact cleanup, and intercom defaults.
- Hide nested child DM process windows on Windows for both foreground and background subagent runs.
...
- Declare `@duckmind/dm-tui` as a runtime dependency so packaged installs can load the extension without relying on dev dependencies or optional peers.
...
### Changed
- Migrated DM package imports and package metadata to the `@earendil-works/*` scope, switched async TypeScript execution discovery to upstream `jiti`, and hardened forked-session creation to use the public `SessionManager.open()` path.
...
### Fixed
- Detect `dm-intercom` when installed through the documented `dm install npm:dm-intercom` package flow, instead of only checking the legacy local extension path.

### Changed
- Store and discover saved chain workflows from dedicated chain directories: user chains in `~/.dm/agent/chains/**/*.chain.md` and project chains in `.dm/chains/**/*.chain.md`.
- Retry foreground subagent fallback models when DM reports a retryable provider error, such as 429/quota, even if the child process exits successfully.
...
- Render async subagent widgets through an adaptive component so active parallel agent rows fit without DM's fixed string-widget truncation marker.
...
- Pass supervisor intercom metadata into foreground, chain, parallel, and background child runs so the child-facing dm-intercom tool can resolve the delegating session automatically.
...
- Hide unsupported thinking levels in subagent clarify and agent-manager pickers when DM exposes per-model thinking metadata.
...
- Scoped `/subagents-status` to async runs launched from the current dm session instead of showing prior or unrelated sessions.
- Declared the DM TUI package as a direct dev dependency and added a manifest guard so CI installs do not rely on transitive optional peer dependencies for tests.
...
### Fixed
- Marked DM runtime peer dependencies as optional so npm package installs do not auto-install duplicate DM packages or emit unrelated transitive dependency warnings.
...
- Expanded the bundled `dm-subagents` skill with tool-level recipes for the packaged prompt workflows, including context-build and handoff-plan patterns that parent agents can apply without slash commands.
- Updated `README.md` to explain the bundled `dm-subagents` skill, what it covers, and how it helps the orchestrating agent.
...
- Resolve intercom bridge default paths at runtime so tests and isolated environments that change `HOME` use the correct `dm-intercom` location.
...
- Expanded builtin subagent guidance so agents with a safe dm-intercom target can hand results back with blocking `intercom ask`, documented the self-orchestrated clarify → plan → implement → review workflow, and added GPT-5.5-oriented subagent prompt guidance to the bundled skill and `context-builder`.
...
- Refreshed the packaged `dm-subagents` skill with doctor diagnostics, saved-chain launches, prompt shortcuts, builtin overrides, intercom bridge guidance, fresh-context review defaults, and parallel task behavior.
...
- Removed the incomplete e2e suite and stale `@marcfargas/dm-test-harness` dev dependency; `test:all` now runs the maintained unit and integration suites.
...
- Non-Windows subagent launches now use plain `dm` again instead of reusing the current CLI script path, avoiding runs that get confused by installed `dist/cli.js` entrypoints.

## [0.17.4] - 2026-04-22

### Added
- Bundled a `dm-subagents` skill that teaches agents how to use builtin subagents, slash-command vs tool workflows, management-mode agent creation/editing, fork/intercom coordination, clarify mode, worktrees, async status inspection, and chain templating.

### Changed
- Tightened the builtin `oracle` prompt so intercom-enabled forked reviews now prefer concise conversational handoffs during the review and send a short final recommendation via `dm-intercom` before returning the full structured result.
...
### Fixed
- Migrated extension tool schemas from `@sinclair/typebox` to `typebox` 1.x so packaged installs follow DM's current extension runtime contract.

### Changed
- Moved TypeBox from `peerDependencies` to a real `dependencies` entry so `dm install` production installs keep the schema package available at runtime.
...
### Changed
- Parallel subagent startup no longer applies any worker-start stagger in `mapConcurrent()`. `dm-subagents` now relies on DM core's settings/auth lock retry behavior instead of carrying its own startup-delay workaround.
...
### Changed
- Top-level parallel defaults and limits can now be configured through `~/.dm/agent/extensions/subagent/config.json` under `parallel.maxTasks` and `parallel.concurrency`, while keeping the existing defaults of 8 tasks and concurrency 4 when unset. This completes issue `#91`. Thanks @Gabrielgvl.
...
- Project agent and chain discovery now reads both `.agents/` and `.dm/agents/`, while preferring `.dm/agents/` when both locations define the same parsed name and keeping manager writes on the `.dm/agents/` path. This fixes issue `#88`. Thanks @desek.
...
### Added
- Added `systemPromptMode` so subagents can replace DM's base prompt with `--system-prompt` instead of always appending with `--append-system-prompt`, shipping the core of issue `#85` from @isvlasov.
- Added `inheritProjectContext` and `inheritSkills` so child runs can keep or strip inherited project instruction files (`AGENTS.md`, `CLAUDE.md`, etc.) and DM's discovered skills block.
...
### Fixed
- Cross-platform tests now avoid machine-specific DM install paths, align homedir-sensitive settings discovery on Windows CI, and use deterministic async config-write failure fixtures.
...
### Added
- Builtin agents can now be customized through settings-backed field overrides in `~/.dm/agent/settings.json` and `.dm/settings.json` under `subagents.agentOverrides`, with `/agents` exposing a create/edit override flow instead of forcing full-file copies for model/thinking/tool/prompt tweaks.
...
- Async/background runs now launch child `dm` processes in JSON mode, stream child events into `events.jsonl` with step metadata while the run is active, keep `output-<n>.log` live with human-readable child output, and document that `subagent-log-<id>.md` is a completion artifact.
...
### Fixed
- Intercom orchestration now uses a runtime-only `subagent-chat-<id>` fallback target for unnamed sessions instead of persisting a generic session title, so `dm --resume` keeps showing transcript snippets while delegated intercom routing still works.
...
- Async background runs now pass the parent process `argv[1]` through to the detached runner, so Windows child spawning keeps targeting the intended `dm` CLI entry point instead of accidentally treating the runner's `jiti` bootstrap script as `dm`.
...
- Skill discovery no longer depends on runtime imports from `@mariozechner/dm-coding-agent`; it now resolves skills directly from configured filesystem paths, preventing `ERR_MODULE_NOT_FOUND` crashes in local/integration test environments.
...
### Changed
- `intercomBridge` now defaults to `always` so intercom coordination instructions are injected for both `fresh` and `fork` delegated runs when `dm-intercom` is available.

## [0.13.1] - 2026-04-13

### Added
- Added optional intercom orchestration bridge for delegated runs. When enabled via `intercomBridge` (default `fork-only`) and `dm-intercom` is available, child subagents get runtime coordination instructions for contacting the orchestrator session via `intercom`, and `intercom` is auto-added to the child tool allowlist when needed.
...
- Documented `dm-intercom` installation and activation requirements in README.
...
- Declared dm runtime packages and `@sinclair/typebox` as peer dependencies so direct source-loading environments fail less often from missing package resolution.
...
- Async/background runs now reuse the current Node executable and prefer the resolved current dm CLI path on all platforms, avoiding PATH drift from wrapped or version-pinned parent launches.
...
### Changed
- Bumped dm package devDependencies to `^0.65.0` (`@mariozechner/dm-agent-core`, `@mariozechner/dm-ai`, `@mariozechner/dm-coding-agent`) to stay aligned with current dm SDK/runtime.

## [0.12.1] - 2026-04-03

### Changed
- Updated session lifecycle handling for dm 0.65.0 by removing legacy post-transition resets and relying on `session_start` reinitialization, matching dm's removal of `session_switch` and `session_fork` extension events.
...
- Integration tests now use a tiny local file-based mock `dm` harness instead of relying on the external subprocess harness for normal subagent execution.
- Removed legacy extra session lifecycle resets and now rely on immutable-session `session_start` reinitialization, matching dm's removal of post-transition `session_switch`/`session_fork` events.
...
### Changed
- Updated for dm 0.62.0 compatibility. `Skill.source` replaced with `Skill.sourceInfo` for skill provenance, `Widget` type replaced with `Component`. Bumped devDependencies to `^0.62.0`.
...
### Changed
- All slash and overlay subagent execution now routes through an event bus request/response protocol (`slash-bridge.ts`), matching the pattern used by dm-prompt-template-model. This replaces both the old `sendUserMessage` relay and the direct `executeChain` call in the overlay handler.
...
- Extracted shared dm CLI arg-builder (`dm-args.ts`) from duplicated logic in `execution.ts` and `subagent-runner.ts`.
...
### Fixed
- `--no-skills` was missing from the async runner (`subagent-runner.ts`). PR #41 added skill scoping to the sync path but the async runner spawns dm through its own code path, so background subagents with explicit skills still got the full `<available_skills>` catalog injected.
- `defaultSessionDir` and `sessionDir` with `~` paths (e.g. `"~/.dm/agent/sessions/subagent/"`) were not expanded — `path.resolve("~/...")` treats `~` as a literal directory name. Added tilde expansion matching the existing pattern in `skills.ts`.
...
### Added
- **Async parallel chain support**: Chains with `{ parallel: [...] }` steps now work in async mode. Previously they were rejected with "Async mode doesn't support chains with parallel steps." The async runner now spawns concurrent dm processes for parallel step groups with configurable `concurrency` and `failFast` options. Inspired by PR #31 from @marcfargas.
- **Comprehensive test suite**: 85 integration tests and 12 E2E tests covering all execution modes (single, parallel, chain, async), error handling, template resolution, and tool validation. Uses `@marcfargas/dm-test-harness` for subprocess mocking and in-process session testing. Thanks @marcfargas for PR #32.
...
- Skills from globally-installed npm packages (via `dm install npm:...`) are now discoverable by subagents. Previously only scanned local `.dm/npm/node_modules/` paths, missing the global npm root where dm actually installs packages.
- **Windows compatibility**: Fixed `ENAMETOOLONG` errors when tasks exceed command-line length limits by writing long tasks to temp files using dm's `@file` syntax. Thanks @marcfargas.
- **Windows compatibility**: Suppressed flashing console windows when spawning async runner processes (`windowsHide: true`).
- **Windows compatibility**: Fixed dm CLI resolution in async runner by passing `piPackageRoot` through to `getPiSpawnCommand`.
...
- `BUILTIN_AGENTS_DIR` resolved via `import.meta.url` instead of hardcoded `~/.dm/agent/extensions/subagent/agents` path. Works regardless of where the extension is installed.
...
### Fixed
- Async subagent session sharing no longer fails with `ERR_PACKAGE_PATH_NOT_EXPORTED`. The runner tried `require.resolve("@mariozechner/dm-coding-agent/package.json")` to find dm's HTML export module, but dm's `exports` map doesn't include that subpath. The fix resolves the package root in the main dm process by walking up from `process.argv[1]` and passes it to the spawned runner through the config, bypassing `require.resolve` entirely. The Windows CLI resolution fallback in `getPiSpawnCommand` benefits from the same walk-up function.

## [0.8.5] - 2026-02-16

### Fixed
- Async subagent execution no longer fails with "jiti not found" on machines without a global `jiti` install. The jiti resolution now tries three strategies: vanilla `jiti`, the `@mariozechner/jiti` fork, and finally resolves `@mariozechner/jiti` from dm's own installation via `process.argv[1]`. Since dm always ships the fork as a dependency, async mode now works out of the box.
- Improved the "jiti not found" error message to explain what's needed and how to fix it.

## [0.8.4] - 2026-02-13

### Fixed
- JSONL artifact files no longer written by default — they duplicated dm's own session files and were the sole cause of `subagent-artifacts` directories growing to 10+ GB. Changed `includeJsonl` default from `true` to `false`. `_output.md` and `_meta.json` still capture the useful data.
- Artifact cleanup now covers session-based directories, not just the temp dir. Previously `cleanupOldArtifacts` only ran on `os.tmpdir()/dm-subagent-artifacts` at startup, while sync runs (the common path) wrote to `<session-dir>/subagent-artifacts/` which was never cleaned. Now scans all `~/.dm/agent/sessions/*/subagent-artifacts/` dirs on startup and cleans the current session's artifacts dir on session lifecycle events.
...
- DM process spawning now uses a shared cross-platform helper in sync and async paths; on Windows it prefers direct Node + CLI invocation to avoid `ENOENT` and argument fragmentation.
...
### Changed
- Reworked `skills.ts` to resolve skills through DM core skill loading with explicit project-first precedence and support for project/user package and settings skill paths.
...
### Added
- Recursion depth guard (`DM_SUBAGENT_MAX_DEPTH`) to prevent runaway nested subagent spawning. Default max depth is 2 (main -> subagent -> sub-subagent). Deeper calls are blocked with guidance to the calling agent.
...
### Added
- **MCP direct tools for subagents** - Agents can request specific MCP tools as first-class tools via `mcp:` prefix in frontmatter: `tools: read, bash, mcp:chrome-devtools` or `tools: read, bash, mcp:github/search_repositories`. Requires dm-mcp-adapter.
...
### Fixed
- Adapt execute signatures to dm v0.51.0: reorder signal, onUpdate, ctx parameters for subagent tool; add missing parameters to subagent_status tool
...
- **README: Added agent file locations** - New "Agents" section near top of README clearly documents:
  ...
  - Project agents: `.dm/agents/{name}.md` (searches up directory tree)
...
- **TUI skill selector** - Press `[s]` to browse and select skills for any step
  ...
  - Project skills (`.dm/skills/`) override user skills (`~/.dm/agent/skills/`)
...
### Changed
- Added `dm-package` keyword for npm discoverability (dm v0.50.0 package system)
...
### Changed
- **Rebranded to `dm-subagents`** (was `dm-async-subagents`)
- Now installable via `npx dm-subagents`
...
- Async notifications no longer leak across dm sessions in different directories
