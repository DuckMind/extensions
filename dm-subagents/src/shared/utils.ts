...
import type { Message, Usage as PiUsage } from "@duckmind/dm-ai";
...
const DM_CODING_AGENT_PACKAGE_NAME = "@duckmind/dm-coding-agent";
export const DM_CODING_AGENT_PACKAGE_ROOT_ENV = "DM_SUBAGENTS_DM_CODING_AGENT_PACKAGE_ROOT";
...
function readConfigDirNameFromPackageRoot(packageRoot: string | undefined): string | undefined {
	...
	try {
		...
		if (pkg.name !== DM_CODING_AGENT_PACKAGE_NAME) return undefined;
...
function resolveConfigDirNameFromPackageJson(entryPoint = process.argv[1], packageRoot = process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV]): string | undefined {
...
export function getConfigDirName(): string {
	...
	const packageRoot = process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV];
 ...
 * every tick. DM serializes each streamed `tool_execution_update` as a single
