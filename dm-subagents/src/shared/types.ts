...
export interface WaitCompletionChild {
	...
	usage?: Usage;
	/** Persisted DM child session when available. */
	sessionFile?: string;
	success?: boolean;
	outputState?: SubagentOutputState;
	error?: string;
...
export interface AgentCapabilityRow {
	...
	aliases?: string[];
	runner: { type: "dm" } | { type: "external-cli"; adapter?: string; capabilities: ExternalCliCapabilities } | { type: "external-job"; provider: string; available?: boolean; capabilities: ExternalJobRunnerStatus["capabilities"] };
...
export interface AsyncStatus {
	lifecycleArtifactVersion?: SubagentLifecycleArtifactVersion;
	runId: string;
	/** Parent DM process/window that owns local completion delivery. */
	completionOwnerId?: string;
	/** Host tool-call id retained when it differs from the internal run id. */
	toolCallId?: string;
	sessionId?: string;
...
export interface SubagentState {
	...
	/** Reload-stable identity for this parent DM process/window. */
	completionOwnerId?: string;
	...
	/** DM sessions base that caps exact runtime-recorded session file trust. */
	...
	activeAsyncCapacity?: ActiveAsyncCapacitySnapshot;
	/** Herdr project panes opened by this DM session, keyed by project root. */
	...
	/** Temporarily suppress dynamic widgets while DM compacts the session. */
...
export const INTERCOM_DETACH_REQUEST_EVENT = "dm-intercom:detach-request";
export const INTERCOM_DETACH_RESPONSE_EVENT = "dm-intercom:detach-response";
...
export interface RunSyncOptions {
	...
	onUpdate?: (r: import("@duckmind/dm-agent-core").AgentToolResult<Details>) => void;
	...
	/** Parent DM event host used to snapshot runtime-registered MCP servers before child launch. */
...
export interface ExtensionConfig {
	 ...
	 * DM_SUBAGENT_TOOL_TIMEOUT_MS. Must be a positive integer; invalid values
	...
	/** Where to store subagent artifact files. Defaults to "session" (the dm session directory, or OS temp when unavailable). Set to "project" for cwd/.dm/subagents. */
...
const configuredTempRoot = process.env.DM_SUBAGENTS_TEMP_ROOT?.trim();
export const TEMP_ROOT_DIR = configuredTempRoot
	? path.resolve(configuredTempRoot)
	: path.join(os.tmpdir(), `dm-subagents-${resolveTempScopeId()}`);
...
export function resolveCurrentMaxSubagentDepth(configMaxDepth?: number): number {
	return normalizeMaxSubagentDepth(process.env.DM_SUBAGENT_MAX_DEPTH)
...
export function resolveMaxSubagentSpawnsPerSession(configMaxSpawns?: number): number | undefined {
	const envMaxSpawns = normalizeMaxSubagentSpawnsPerSession(process.env.DM_SUBAGENT_MAX_SPAWNS_PER_SESSION);
...
export function resolveMaxSubagentSpawnsPerRun(configMaxSpawns?: number): number {
	return normalizeMaxSubagentSpawnsPerRun(process.env.DM_SUBAGENT_MAX_SPAWNS_PER_RUN)
