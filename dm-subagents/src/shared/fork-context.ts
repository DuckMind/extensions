...
/** Keep DM from restoring a forked session into the parent's cwd instead of the child launch cwd. */
...
export function createForkContextResolver(
	...
	// DM's recent-session discovery (`dm -c` → findMostRecentSession) is
	...
	// went idle — would hijack the next `dm -c` away from the conversation the
	...
	const resolveFork = (index = 0): ForkContextResolution => {
		...
		try {
			if (!fs.existsSync(parentSessionFile)) {
				throw new Error(`Parent session file does not exist: ${parentSessionFile}. DM has not persisted enough history to fork yet.`);
			}
