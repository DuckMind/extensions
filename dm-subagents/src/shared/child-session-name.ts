 ...
 *
 *  1. Into the child process via `DM_SUBAGENT_SESSION_NAME`, where the prompt
 *     runtime calls `dm.setSessionName(...)` so the child's own session file
 *     is identifiable in `dm --resume` and host session browsers.
