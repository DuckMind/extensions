...
import type { ExtensionAPI, ToolDefinition } from "@duckmind/dm-coding-agent";
import { discoverAgents } from "../agents/agents.ts";
import { getArtifactsDir } from "../shared/artifacts.ts";
import { createSubagentExecutor, type SubagentParamsLike } from "../runs/foreground/subagent-executor.ts";
import { resolveWaitToolConfig } from "../runs/background/wait-config.ts";
import { SUBAGENT_CHILD_ENV, SUBAGENT_FANOUT_CHILD_ENV } from "../runs/shared/dm-args.ts";
...
function startNestedControlInboxListener(dm: ExtensionAPI, state: SubagentState, route: NestedRoute, inboxState: NestedControlInboxState): () => void {
...
export default function registerFanoutChildSubagentExtension(dm: ExtensionAPI): void {
	...
	if (registeredApis.has(dm)) return;
	registeredApis.add(dm);
	...
	listenerCleanups.set(routeKey, { state: inboxState, cleanup: startNestedControlInboxListener(dm, state, route, inboxState) });
}
