...
function deliverControlNotice(input: {
	dm: Pick<ExtensionAPI, "sendMessage">;
...
export function handleSubagentControlNotice(input: {
	...
	if (input.details.source === "foreground") {
		// A foreground tool blocks DM from displaying this message. The run can
		// finish before DM flushes it, and queued messages cannot be withdrawn.
