...
/** Versioned public contract exported through `dm-subagents/project-panes`. */
...
export interface CloseProjectPaneOptions {
	cwd: string;
	/** Fail closed unless Herdr explicitly reports the owning DM pane as idle. */
	requireIdle?: boolean;
	signal?: AbortSignal;
}
...
export async function handleHerdrProjectPaneAction(action: HerdrProjectPaneAction, params: ProjectPaneParams, deps: ProjectPaneDeps): Promise<AgentToolResult<Details>> {
	...
	return toolResult(`Opened Herdr project pane ${opened.data.binding.paneId} for ${opened.data.projectRoot}. The pane runs its own DM session; subagents launched there belong to that project.`);
}
