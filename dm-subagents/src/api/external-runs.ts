...
export const EXTERNAL_RUN_REGISTRY_KEY = "dm-subagents.external-runs.v2";
...
export const EXTERNAL_RUN_LIMITS = {
	...
	/**
	 * A DM session id is the session file path, which routinely exceeds a short
...
/** Register one current-session external job. dm-subagents never controls the job. */
...
/** Read a bounded cached snapshot for one DM session. This never invokes third-party code. */
