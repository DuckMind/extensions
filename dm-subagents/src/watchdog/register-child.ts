...
export function registerChildWatchdog(dm: ExtensionAPI, rawConfig = process.env[CHILD_WATCHDOG_CONFIG_ENV]): MainWatchdogRuntime | undefined {
	...
	const runtime = new MainWatchdogRuntime({
		...
		review: createMainWatchdogReview(() => currentContext, { getThinkingLevel: () => dm.getThinkingLevel() }),
		...
		displayWarning: (details) => {
			...
			dm.sendMessage(createWatchdogWarningMessage(childDetails, { display: true, details: childDetails }));
		},
	});
