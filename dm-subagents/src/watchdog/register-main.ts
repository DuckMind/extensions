...
import { Text } from "@duckmind/dm-tui";
...
function sendSlashText(dm: ExtensionAPI, text: string): void {
...
async function handleWatchdogCommand(
	...
	if (!input || input === "status") {
		sendSlashText(dm, buildWatchdogStatus(runtime.getSnapshot(ctx.cwd), ctx));
		return;
	}
	if (input === "recommend-model") {
		try {
			sendSlashText(dm, buildRecommendationText(ctx));
		} catch (error) {
			sendSlashText(dm, `Subagent watchdog recommended model\n\n${messageFromError(error)}`);
		}
		return;
	}
	if (input === "check") {
		try {
			sendSlashText(dm, buildCheckText(runtime, ctx));
		} catch (error) {
			sendSlashText(dm, `Subagent watchdog config check\n\n${messageFromError(error)}`);
		}
		return;
	}
	...
	if (input.startsWith("session model ")) {
		...
		try {
			...
			sendSlashText(dm, `Subagent watchdog session model\n\n${messageFromError(error)}`);
		}
		return;
	}
	if (input.startsWith("model ")) {
		...
		try {
			...
			sendSlashText(dm, `Subagent watchdog model\n\n${messageFromError(error)}\nNo settings files were changed.`);
		}
		return;
	}
	if (input.startsWith("thinking ")) {
		...
		try {
			...
			sendSlashText(dm, `Subagent watchdog thinking\n\n${messageFromError(error)}\nNo settings files were changed.`);
		}
		return;
	}
	...
	if (test) {
		...
		dm.sendMessage(createWatchdogWarningMessage(details, { display: true, details }));
		return;
	}
...
export function registerMainWatchdog(dm: ExtensionAPI, options: RegisterMainWatchdogOptions = {}): MainWatchdogRuntime {
	let currentContext: ExtensionContext | undefined;
	const rememberContext = (ctx: ExtensionContext) => {
		currentContext = ctx;
	};
	const runtime = options.runtime ?? new MainWatchdogRuntime({
		review: options.review ?? createMainWatchdogReview(() => currentContext, { getThinkingLevel: () => dm.getThinkingLevel() }),
		reviewDescription: options.review ? "injected seam" : "real model review",
		reviewChangesOnly: true,
		displayWarning: (details, delivery) => {
			dm.sendMessage(createWatchdogWarningMessage(details, { display: true, details }), delivery?.deliverAs === "steer" ? { deliverAs: "steer" } : undefined);
		},
		sendUserMessage: (message) => dm.sendUserMessage(message),
	});

	dm.registerMessageRenderer<WatchdogWarningDetails>(SUBAGENT_WATCHDOG_WARNING_TYPE, (message, renderOptions, theme) => {
	...
	dm.registerCommand("subagents-watchdog", {
		description: "Show or toggle the default-off subagent watchdog",
		handler: (args, ctx) => {
			rememberContext(ctx);
			return handleWatchdogCommand(dm, runtime, args, ctx);
		},
	});

	dm.on("session_start", (_event, ctx) => {
		rememberContext(ctx);
		runtime.bindSession(ctx);
	});
	dm.on("before_agent_start", (event, ctx) => {
		rememberContext(ctx);
		runtime.handleBeforeAgentStart(event, ctx);
	});
	dm.on("turn_end", (event, ctx) => {
		rememberContext(ctx);
		runtime.handleTurnEnd(event, ctx);
	});
	dm.on("tool_result", (_event, ctx) => {
		rememberContext(ctx);
		runtime.handleToolResult(ctx);
	});
	dm.on("agent_end", (event, ctx) => {
		rememberContext(ctx);
		return runtime.handleAgentEnd(event, ctx);
	});
	dm.on("session_before_switch", () => runtime.reset("session switch", { clearReviewInputSignature: true, clearLspLedger: true, clearScope: true, resetAutoFollow: true }));
	dm.on("session_before_fork", () => runtime.reset("session fork", { clearReviewInputSignature: true, clearLspLedger: true, clearScope: true, resetAutoFollow: true }));
	dm.on("session_compact", () => runtime.reset("session compact", { clearScope: true }));
