import { Agent, type AgentTool, type StreamFn, type ThinkingLevel } from "@duckmind/dm-agent-core";
import { createReadOnlyTools, convertToLlm, type ExtensionContext } from "@duckmind/dm-coding-agent";
import { streamSimple } from "@duckmind/dm-ai/compat";
import type { Model, ProviderHeaders } from "@duckmind/dm-ai";
...
export async function resolveWatchdogReviewModel(
	...
	if (!currentModel) {
		throw new Error("Main watchdog review cannot run because the current DM session model is unavailable and subagents.watchdog.main.model is not configured.");
	}
	return {
		model: currentModel,
		thinkingLevel: resolveReviewThinking({
...
function buildWatchdogSystemPrompt(ctx: ExtensionContext, options: { hasScope?: boolean } = {}): string {
	return [
		"You are the main-session subagent watchdog for DM.",
...
export function createMainWatchdogReview(provider: WatchdogContextProvider, options: CreateMainWatchdogReviewOptions = {}): WatchdogReviewFunction {
	return async (request) => {
		...
		if (!ctx) throw new Error("Main watchdog review cannot run without an active DM extension context.");
