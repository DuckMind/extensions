...
function maxHashFileBytes(): number {
	return positiveEnvNumber("DM_SUBAGENTS_MAX_HASH_FILE_BYTES", DEFAULT_MAX_HASH_FILE_BYTES);
}

function maxHashTotalBytes(): number {
	return positiveEnvNumber("DM_SUBAGENTS_MAX_HASH_TOTAL_BYTES", DEFAULT_MAX_HASH_TOTAL_BYTES);
}

function maxHashEntries(): number {
	return positiveEnvNumber("DM_SUBAGENTS_MAX_HASH_ENTRIES", DEFAULT_MAX_HASH_ENTRIES);
}
	...
	if (stat.size > maxHashFileBytes() || budget.bytes + stat.size > budget.maxBytes) {
		...
		try {
			...
			if (code !== "ERR_FS_FILE_TOO_LARGE") {
				console.warn("[dm-subagents] watchdog hashFile fell back to metadata for", normalized + ":", (error as Error)?.message);
			}
		}
	}
...
export function computeWatchdogRepoChangeSignature(cwd: string): WatchdogRepoChangeSignature | undefined {
	...
	try {
		...
		console.warn("[dm-subagents] watchdog repo change signature failed:", (error as Error)?.message);
		return undefined;
	}
}
