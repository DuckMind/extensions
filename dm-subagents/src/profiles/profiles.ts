...
import { getPiSpawnCommand } from "../runs/shared/dm-spawn.ts";
...
async function probeModel(
	dm: Pick<ExtensionAPI, "exec"> | { exec?: ExtensionAPI["exec"] },
	fullId: string,
): Promise<{ status: ProbeStatus; message?: string }> {
	if (typeof dm.exec !== "function") {
		return { status: "skipped", message: "dm.exec is unavailable in this runtime." };
	}
	const spawnSpec = getPiSpawnCommand(["-p", "--model", fullId, "--no-tools", 'Reply with exactly "OK".']);
	const result = await dm.exec(spawnSpec.command, spawnSpec.args, {
		cwd: os.tmpdir(),
		timeout: 45_000,
...
export function getSubagentProfilesRootDir(): string {
	return path.join(getAgentDir(), "profiles", "dm-subagents");
}
...
export async function generateProfilesForProvider(
	...
	const normalizedProvider = normalizeProviderName(provider);
	const { filePath: catalogPath, catalog, heuristicFallbackCount } = await refreshProviderModelCatalog(dm, ctx, normalizedProvider, {
...
export async function checkSubagentProfile(
	...
	for (const entry of entries) {
		...
		if (!probe) {
			probe = await probeModel(dm, probeModelId);
