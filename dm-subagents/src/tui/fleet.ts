...
import { getMarkdownTheme, type ExtensionContext } from "@duckmind/dm-coding-agent";
import { matchesKey, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component, type MarkdownTheme } from "@duckmind/dm-tui";
...
export async function openSubagentFleet(ctx: ExtensionContext, state: SubagentState, options: FleetViewOptions = {}): Promise<void> {
	...
	const copyText = options.copyText ?? (async (text: string) => {
		...
		if (!copyToClipboard) throw new Error("Clipboard is unavailable in this DM version.");
