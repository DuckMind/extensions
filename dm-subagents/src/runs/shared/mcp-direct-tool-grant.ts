...
export function formatUnresolvedMcpDirectToolSelectors(selectors: readonly string[]): string {
	return `Unresolved MCP direct-tool selectors: ${selectors.join(", ")}. Direct MCP tools require a matching configured server and fresh metadata cache; runtime-registered servers require a host/dm-mcp-adapter handoff before child launch.`;
}
