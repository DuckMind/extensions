...
function resolveRequiredSubagentModelCandidate(
	...
	throw new Error(
		`Unknown subagent model '${model}' in the active DM model registry.${suggestion ? ` Did you mean '${suggestion}'?` : ""}`,
	);
}
...
function defaultScopeWarn(violation: ModelScopeViolation): void {
	console.warn(`[dm-subagents] ${violation.message}`);
}
 ...
 * `~/.dm/agent/settings.json` default, which is shared across every open PI
...
export function buildModelCandidates(
	...
	for (let index = 0; index < rawCandidates.length; index++) {
		...
		if (!normalized) {
			console.warn(`[dm-subagents] Skipping fallback model '${model}' because it is unavailable in this environment.`);
			continue;
		}
	...
	return filterFallbackCandidates(candidates, {
		onExcluded(candidate, exclusion) {
			const reason = redactSecretValues((exclusion.reason ?? "runtime-failure").replace(/[\u0000-\u001f\u007f]+/g, " ")).slice(0, 240);
			console.warn(`[dm-subagents] Skipping model '${candidate}' due to a cached exclusion (reason: ${reason}; expires: ${new Date(exclusion.expiresAt).toISOString()}).`);
		},
	});
}
