...
export const PERMISSION_POLICY_ENV = "DM_SUBAGENT_PERMISSION_POLICY";
export const PERMISSION_AUDIT_PATH_ENV = "DM_SUBAGENT_PERMISSION_AUDIT_PATH";
...
export function validatePermissionRules(value: unknown, label: string): PermissionRules | undefined {
	...
	for (const [tool, decision] of Object.entries(value)) {
		...
		if (tool === "bash") throw new Error(`${label}.bash is unsupported; dm-subagents leaves bash policy to dm-guard.`);
