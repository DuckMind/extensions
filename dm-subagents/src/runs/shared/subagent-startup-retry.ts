 ...
 * launch failures while allowing concurrent DM startup locks to clear.
 */
...
export function formatSubagentExtensionConflictError(
	...
	return `${error}\n\nSubagent startup loaded conflicting ambient DM extensions. Retry this run after disabling ambient extension discovery for agent ${JSON.stringify(input.agent)} in project .dm/settings.json: ${settings}. Replace the empty array with only the required extension entry paths when this agent needs extension tools.`;
}
...
export function formatSubagentStartupRetryExhaustedError(input: {
	...
	return `Subagent failed to start after ${input.attempts} attempts on ${input.model}; no model, tool, output, usage, or diagnostic activity was observed. This may be a concurrent DM startup race. Retry the run or temporarily lower subagent concurrency.`;
}
