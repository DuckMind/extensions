...
export const REQUIRED_CHILD_TOOLS_ENV = "DM_SUBAGENT_REQUIRED_TOOLS";
export const MCP_DIRECT_CHILD_TOOLS_ENV = "DM_SUBAGENT_MCP_DIRECT_TOOLS";
export const CHILD_TOOL_DIAGNOSTIC_PATH_ENV = "DM_SUBAGENT_TOOL_DIAGNOSTIC_PATH";
...
export function formatChildToolDiagnostic(diagnostic: ChildToolDiagnostic): string {
	...
	return [
		...
		...(diagnostic.missingMcpDirectTools?.length
			? [`Resolved MCP direct tools missing from the child registry: ${diagnostic.missingMcpDirectTools.join(", ")}. This indicates a host/dm-mcp-adapter registration problem, not a tool-call failure.`]
			: []),
		...
		"For MCP tools, verify the MCP adapter configuration and selected tool names. For builtin tools, verify the name against the installed DM version.",
	].join("\n");
}
