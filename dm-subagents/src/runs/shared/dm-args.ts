...
import { resolvePiPackageRoot } from "./dm-spawn.ts";
import { encodeExtensionBindings, DM_SUBAGENT_EXTENSION_BINDINGS_ENV, type ExtensionBindings } from "./extension-bindings.ts";
 ...
 * of argv entirely.
 */
export const SUBAGENT_TASK_DELIVERY_ENV = "DM_SUBAGENT_TASK_DELIVERY";
...
export const SUBAGENT_CHILD_ENV = "DM_SUBAGENT_CHILD";
...
export const SUBAGENT_SUPERVISOR_CHANNEL_DIR_ENV =
	"DM_SUBAGENT_SUPERVISOR_CHANNEL_DIR";
export const SUBAGENT_RUN_ID_ENV = "DM_SUBAGENT_RUN_ID";
export const SUBAGENT_CHILD_AGENT_ENV = "DM_SUBAGENT_CHILD_AGENT";
export const SUBAGENT_CHILD_INDEX_ENV = "DM_SUBAGENT_CHILD_INDEX";
export const SUBAGENT_FANOUT_CHILD_ENV = "DM_SUBAGENT_FANOUT_CHILD";
export const SUBAGENT_PARENT_EVENT_SINK_ENV = "DM_SUBAGENT_PARENT_EVENT_SINK";
export const SUBAGENT_PARENT_CONTROL_INBOX_ENV =
	"DM_SUBAGENT_PARENT_CONTROL_INBOX";
export const SUBAGENT_PARENT_ROOT_RUN_ID_ENV = "DM_SUBAGENT_PARENT_ROOT_RUN_ID";
export const SUBAGENT_PARENT_RUN_ID_ENV = "DM_SUBAGENT_PARENT_RUN_ID";
export const SUBAGENT_PARENT_CHILD_INDEX_ENV = "DM_SUBAGENT_PARENT_CHILD_INDEX";
export const SUBAGENT_PARENT_DEPTH_ENV = "DM_SUBAGENT_PARENT_DEPTH";
export const SUBAGENT_PARENT_PATH_ENV = "DM_SUBAGENT_PARENT_PATH";
export const SUBAGENT_PARENT_CAPABILITY_TOKEN_ENV =
	"DM_SUBAGENT_PARENT_CAPABILITY_TOKEN";
export const SUBAGENT_PARENT_SESSION_ENV = "DM_SUBAGENT_PARENT_SESSION";
export const SUBAGENT_FORK_CACHE_KEY_ENV = "DM_SUBAGENT_FORK_CACHE_KEY";
export const SUBAGENT_STEER_INBOX_ENV = "DM_SUBAGENT_STEER_INBOX";
export const SUBAGENT_STEER_CAPABILITY_ENV = "DM_SUBAGENT_STEER_CAPABILITY";
export const SUBAGENT_STEER_ACK_DIR_ENV = "DM_SUBAGENT_STEER_ACK_DIR";
...
export const SUBAGENT_INHERIT_GLOBAL_CONTEXT_ENV = "DM_SUBAGENT_INHERIT_GLOBAL_CONTEXT";
...
export function deriveForkPromptCacheKey(parentSessionId: string | undefined): string | undefined {
	...
	const digest = createHash("sha256").update(parent).digest("hex").slice(0, OPENAI_PROMPT_CACHE_KEY_MAX_LENGTH - "dm-fork:".length);
	return `dm-fork:${digest}`;
}
...
export interface BuildPiArgsInput {
	 ...
	 *  derived by the caller). Passed to the child as DM_SUBAGENT_SESSION_NAME;
	 *  the prompt runtime applies it via dm.setSessionName unless an intercom
...
export function resolvePermissionSystemExtension(): string | undefined {
	...
	const candidates = [
		...
		path.join(
			...
			"node_modules",
			"@gotgenes",
			"dm-permission-system",
		),
		// direct extension directory (some layouts)
		path.join(agentDir, "extensions", "dm-permission-system"),
	];
	const errors: Error[] = [];
...
export function resolvePiLaunchToolPlan(
	...
	// the native supervisor channel (or dm-intercom). The pre-0.50 bridge always
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	if (toolPlan.mcpConfig && toolPlan.runtimeServerNames?.length) {
		if (!tempDir)
			tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagent-"));
	...
	if (input.systemPrompt !== undefined && input.systemPrompt !== null) {
		...
		// Inject <active_agent> tag so @gotgenes/dm-permission-system can
	...
	env[DM_SUBAGENT_EXTENSION_BINDINGS_ENV] = encodeExtensionBindings(input.extensionBindings);
	const piPackageRoot =
		process.env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] ?? resolvePiPackageRoot();
	if (piPackageRoot) env[DM_CODING_AGENT_PACKAGE_ROOT_ENV] = piPackageRoot;
	if (!tempDir)
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-subagent-"));
	...
	env[RUN_FANOUT_BUDGET_ENV] = toolPlan.fanoutAuthorized
		...
		: undefined;
	env.DM_SUBAGENT_INHERIT_PROJECT_CONTEXT = input.inheritProjectContext
		? "1"
		: "0";
	env[SUBAGENT_INHERIT_GLOBAL_CONTEXT_ENV] = input.inheritGlobalContext ? "1" : "0";
	env.DM_SUBAGENT_INHERIT_SKILLS = input.inheritSkills ? "1" : "0";
	env[PI_INTERCOM_STABLE_ID_ENV] = input.intercomSessionName || undefined;
	env[PI_INTERCOM_SESSION_ID_ENV] = undefined;
	if (input.intercomSessionName) {
		env.DM_SUBAGENT_INTERCOM_SESSION_NAME = input.intercomSessionName;
	}
	if (input.sessionName?.trim()) {
		env.DM_SUBAGENT_SESSION_NAME = input.sessionName.trim();
	}
