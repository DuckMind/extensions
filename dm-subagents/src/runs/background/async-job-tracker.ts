...
export function createAsyncJobTracker(dm: Pick<ExtensionAPI, "events">, state: SubagentState, asyncDirRoot: string, options: AsyncJobTrackerOptions = {}): {
	ensurePoller: () => void;
	...
	const externalJobBridgeEligibility = (steps: AsyncJobState["steps"]): "required" | "not-required" | "unknown" => {
		...
		for (const step of steps) {
			...
			if (runnerType !== "dm" && runnerType !== "external-cli") return "unknown";
		}
		return "not-required";
	};
		...
		try {
			...
			const handleLine = (line: string) => {
				...
				if ((parsed as { type?: unknown }).type === "subagent.child-status") {
					...
					dm.events.emit(SUBAGENT_CHILD_STATUS_EVENT, {
				...
				if ((parsed as { type?: unknown }).type === "subagent.steering.notice") {
					...
					dm.events.emit(SUBAGENT_STEERING_NOTICE_EVENT, { ...notice, source: "async", asyncDir: job.asyncDir, noticeText: notice.message });
					return;
				}
