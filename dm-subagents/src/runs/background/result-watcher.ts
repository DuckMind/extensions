	...
	const handleResult = async (file: string, triggerTurn: boolean) => {
		...
		try {
			...
			if (deliverIntercomResults && intercomTarget && triggerTurn) {
				...
				intercomDelivered = await deliverSubagentResultIntercomEvent(dm.events, buildSubagentResultIntercomPayload({
					to: intercomTarget,
					runId,
					mode,
					source: "async",
			...
			markSeenWithTtl(state.completionSeen, completionKey, Date.now(), completionTtlMs);
			try {
				dm.events.emit(SUBAGENT_ASYNC_COMPLETE_EVENT, {
					...data,
					runId,
					triggerTurn,
					intercomDelivered,
