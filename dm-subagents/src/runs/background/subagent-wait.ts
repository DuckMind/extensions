 ...
 * cannot work at all non-interactively (`dm -p ...`), where the run is a single
 ...
 * `needs_attention` (the same signal DM shows as a control notice and,
 ...
 *
 * Wake mechanism: when given DM's event bus (`deps.events`), `bg_wait`
...
/** Minimal event-bus surface wait subscribes to (matches dm.events). */
...
export interface SubagentWaitDeps {
	state: SubagentState;
	/** Stream live wait status into DM's pending tool row. */
	...
	/**
	 * Optional event bus (dm.events). When provided, wait wakes immediately on a
...
export async function waitForSubagents(
	...
	if (deps.enabled === false) {
		return result("bg_wait is disabled by config.waitTool or DM_SUBAGENT_WAIT_TOOL_ENABLED; returning immediately without blocking background work. Active work keeps going, and you can inspect subagents with subagent({ action: \"status\" }) or rely on completion notifications.");
	}
