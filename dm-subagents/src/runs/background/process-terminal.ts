...
function validProcessInstance(value: unknown, kind?: "runner" | "dm-writer"): value is ProcessInstanceExitV1 {
	if (!isRecord(value)) return false;
	if (typeof value.processInstanceId !== "string" || value.processInstanceId.length === 0) return false;
	if (kind ? value.kind !== kind : (value.kind !== "runner" && value.kind !== "dm-writer")) return false;
...
function validInstance(value: unknown): value is ProcessInstanceExitV1 {
	return validProcessInstance(value, "dm-writer");
}
...
export function finalizeProcessTerminal(
	...
	try {
		...
		if (!candidate) proof = unknownProof(runId, runnerClose.processInstanceId, "runner-candidate-missing");
			...
			} else if (allWriters.some((writer) => writer.kind === "dm-writer" && writer.processTree.state !== "observed")) {
