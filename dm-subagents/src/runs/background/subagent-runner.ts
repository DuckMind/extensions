...
import { DM_CODING_AGENT_PACKAGE, getPiSpawnCommand, resolveInstalledPiPackageRoot } from "../shared/dm-spawn.ts";
...
import { applyThinkingSuffix, buildPiArgs, cleanupTempDir, deriveForkPromptCacheKey, projectLaunchResolvedChildExtensions, resolvePiLaunchToolPlan, type SubagentTaskDelivery } from "../shared/dm-args.ts";
	...
	return new Promise((resolve) => {
		...
		if (typeof child.pid === "number") {
			...
			try {
				...
				writerRegistrationError = `Failed to record revived DM writer ownership: ${writerError instanceof Error ? writerError.message : String(writerError)}`;
...
function resolvePiPackageRootFallback(): string {
	...
	if (root) return root;
	throw new Error(`Could not resolve ${DM_CODING_AGENT_PACKAGE} package root`);
}
...
async function runSingleStepInner(
	...
	modelAttemptsLoop: while (modelIndex < candidates.length) {
		...
		if (!launchWarningsEmitted && warnings.length > 0) {
			for (const warning of warnings) console.warn(`[dm-subagents] ${warning}`);
...
async function runSubagent(
	...
	const deliverSteerRequest = (request: SteerRequest): void => {
		...
		const targetStates = targets.map((index) => {
			...
			if (steeringCapabilities.get(index)?.supported === false) return { index, state: "failed" as const, reason: "child DM session does not support steering" };
		...
		onSteerCapability: (capability) => {
			...
			if (!capability.supported) {
				...
				for (const request of lifecycle.recent) {
					...
					updateSteeringLifecycleTarget(request.id, capability.index, "failed", now, { reason: "child DM session does not support steering" });
					emitSteeringEvent("subagent.steer.failed", { type: "steer", id: request.id, ts: request.requestedAt, message: "child DM session does not support steering" }, capability.index, { reason: "child DM session does not support steering" });
