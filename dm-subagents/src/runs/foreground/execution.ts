...
import { getPiSpawnCommand } from "../shared/dm-spawn.ts";
...
async function runSingleAttempt(
	...
	// DM_SUBAGENT_SESSION_NAME and echoed back on the result payload so hosts
...
async function runSyncCompletionInner(
	...
	if (skillNames.some((skill) => skill.trim() === "dm-subagents") && missingSkills.includes("dm-subagents")) {
		return redactResultPrompt(withRunContext({
			index: options.index ?? 0,
			agent: agentName,
			task,
			...
			messages: [],
			usage: emptyUsage(),
			error: "Skills not found: dm-subagents",
		}, options.context));
	}
