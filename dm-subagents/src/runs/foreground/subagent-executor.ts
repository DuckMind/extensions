...
function emitAdvisoryControlEvent(dm: ExtensionAPI, channel: string, payload: unknown): void {
	try {
		dm.events.emit(channel, payload);
	} catch (error) {
...
function emitControlNotification(input: {
	...
	if (input.controlConfig.notifyChannels.includes("event")) {
		emitAdvisoryControlEvent(input.dm, SUBAGENT_CONTROL_EVENT, payload);
	}
	if (input.event.type !== "active_long_running" && input.controlConfig.notifyChannels.includes("intercom") && input.intercomBridge.active && input.intercomBridge.orchestratorTarget) {
		emitAdvisoryControlEvent(input.dm, SUBAGENT_CONTROL_INTERCOM_EVENT, {
			...payload,
			to: input.intercomBridge.orchestratorTarget,
...
async function steerNestedRun(input: { target: ResolvedSubagentRunId & { kind: "nested" }; message: string; mode?: SteerDeliveryMode; index?: number; signal?: AbortSignal }): Promise<AgentToolResult<Details>> {
	...
	if (direct) return direct;
	return { content: [{ type: "text", text: `Nested run ${run.id} is not a live async DM child session with a steering inbox. action='steer' cannot target foreground nested runs.` }], isError: true, details: { mode: "management", results: [] } };
}
...
function externalRunnerControlError(asyncDir: string, action: "steer" | "resume"): AgentToolResult<Details> | undefined {
	...
	const message = externalCli
		...
		: action === "steer"
			...
			: "External runners do not persist DM sessions and cannot be resumed.";
	return { content: [{ type: "text", text: message }], isError: true, details: { mode: "management", results: [] } };
}
...
async function resumeAsyncRun(input: {
	...
	const sessionName = resolveIntercomSessionTarget(input.deps.dm.getSessionName(), input.ctx.sessionManager.getSessionId());
	...
	if (attachChain) {
		...
		if (!isAsyncAvailable()) {
			return {
				content: [{ type: "text", text: "Async mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the dm-subagents package dependencies are installed." }],
				isError: true,
...
function createForegroundControlNotifier(data: Pick<ExecutionContextData, "controlConfig" | "contextPolicy" | "intercomBridge" | "params">, deps: Pick<ExecutorDeps, "dm" | "state">): (event: ControlEvent) => void {
	return (event) => {
...
async function emitForegroundResultIntercom(input: {
	...
	const delivered = await deliverSubagentResultIntercomEvent(input.dm.events, payload);
...
function withAggregatedToolUsage(result: AgentToolResult<Details>): AgentToolResult<Details> {
	...
	return usage.input !== 0 || usage.output !== 0 || usage.cacheRead !== 0 || usage.cacheWrite !== 0 || usage.cost !== 0 || usage.turns !== 0
		? { ...result, details: { ...result.details, totalChildUsage: usage } }
		: result;
}
		...
		const launched = await runSync(ctx.cwd, agents, params.agent!, task, compactOptional<Parameters<typeof runSync>[4]>({
			...
			onDetachedExit: (result) => {
				...
				try {
					...
					try {
						updateRememberedForegroundChild(deps.state, { runId, mode: "single", cwd: singleCwd, sessionId: data.parentSessionId, index: 0, result, events: deps.dm.events, notify: true });
					} catch {
...
function recordMissionWorkflowChild(
	...
	try {
		...
		if (error instanceof MissionNotFoundError) {
			...
			console.warn(`[dm-subagents] Mission '${binding.missionId}' is no longer in the mission store; stopped recording its workflow children. Terminal-mission retention can prune a mission while its run is still active.`);
			return;
		}
		console.warn(`[dm-subagents] Failed to record mission workflow child '${key}': ${error instanceof Error ? error.message : String(error)}`);
	}
}
	...
	const execute = async (
		...
		if (requestParams.workflowScript !== undefined && normalizedAction === undefined) {
			...
			if (workflowArtifactConfig.dir === "project" && !warnedArtifactPackageDirs.has(workflowCwd)) {
				...
				if (warning) console.warn(`[dm-subagents] ${warning}`);
			}
						...
						const result = await runMissionWorkflowChild(missionBinding, _id, key, childPhase, () => {
							...
							if (delegatedWorkflowPermit) {
								...
								const childAgent = typeof childRequest.agent === "string"
									...
									: undefined;
								if (childAgent?.runner?.type === "external-cli" || childAgent?.runner?.type === "external-job") throw new Error("Workflow child permit supports native DM children only.");
		...
		if (action) {
			...
			if (action === "doctor") {
				...
				try {
					orchestratorTarget = resolveIntercomSessionTarget(deps.dm.getSessionName(), ctx.sessionManager.getSessionId());
				} catch (error) {
		...
		const sessionName = resolveIntercomSessionTarget(deps.dm.getSessionName(), ctx.sessionManager.getSessionId());
		const intercomBridge = resolveIntercomBridge({
		...
		if (foregroundControl) {
			foregroundControl.promptAuditRedo = async (index, guidance) => {
				...
				void launch.then((result) => {
					if (result.isError) console.warn(`[dm-subagents] Prompt redo ${newRunId} failed: ${result.content.find((item) => item.type === "text")?.text ?? "unknown error"}`);
				}).catch((error) => {
					console.warn(`[dm-subagents] Prompt redo ${newRunId} failed: ${error instanceof Error ? error.message : String(error)}`);
				});
			...
			try {
				writeNestedEvent(inheritedNestedRoute, compactOptional<Parameters<typeof writeNestedEvent>[1]>({
					...
					child: compactOptional<NestedRunSummary>({
						...
						ownerIntercomTarget: process.env.DM_SUBAGENT_INTERCOM_SESSION_NAME,
						leafIntercomTarget,
