...
function applyAgentConfig(target: AgentConfig, cfg: Record<string, unknown>): string | undefined {
	...
	if (hasKey(cfg, "runner")) {
		if (cfg.runner === false || cfg.runner === "") delete target.runner;
			...
			if (runner.type === "dm" && Object.keys(runner).every((key) => key === "type")) target.runner = { type: "dm" };
			...
			} else return `config.runner must be { type: 'dm' }, { type: 'external-cli', adapter?: ${CODE_OWNED_EXTERNAL_CLI_ADAPTER_LABEL}, command: string, args?: string[], promptDelivery?: 'stdin' }, or { type: 'external-job', provider: string, options?: object }.`;
	...
	if (target.runner?.type === "external-cli" || target.runner?.type === "external-job") {
		...
		if (unsupported.length > 0) return `config.runner type '${target.runner.type}' does not support DM-only fields: ${unsupported.join(", ")}.`;
	}
	return undefined;
}
...
const PI_AGENT_RUNNER = { type: "dm" } as const;
...
function agentCapabilityRunner(agent: AgentConfig, providerNames: Set<string> | undefined): AgentCapabilityRow["runner"] {
	...
	if (!runner || runner.type === "dm") return PI_AGENT_RUNNER;
...
function handleDisable(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	if (scope === "project" && d.projectSettingsPath === null) {
		return result("Project override is not available here: no project config root (.dm or .agents) was found above the cwd. Use agentScope: 'user' or run from inside a project.", true);
	}
