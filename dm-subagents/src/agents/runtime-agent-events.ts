...
export const RUNTIME_AGENT_REGISTER_EVENT = "dm-subagents:runtime-agent-register:v1";
...
/** Register through the installed dm-subagents owner in this DM process. */
export function registerAgentViaEvents(input: RegisterRuntimeAgentViaEventsInput): RuntimeAgentRegistration {
	...
	input.dm.events.emit(RUNTIME_AGENT_REGISTER_EVENT, request);
	const result = request.result as unknown;
	if (result === undefined) {
		throw new Error("dm-subagents is not installed, not ready, or does not support runtime agent event registration.");
	}
	...
	throw new Error("dm-subagents returned a malformed runtime agent registration result.");
}

/** Install the process-local registration listener for the owning dm-subagents runtime. */
export function registerRuntimeAgentEventListener(dm: ExtensionAPI): () => void {
	return dm.events.on(RUNTIME_AGENT_REGISTER_EVENT, (rawRequest) => {
