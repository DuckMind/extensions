...
const CURRENT_FENCE = "dm-subagents-refinement-current";
const SNAPSHOTS_FENCE = "dm-subagents-refinement-snapshots-json";
...
export function parseRefinementFile(markdown: string, label = "refinement file"): ParsedRefinementFile {
	const metadataMatch = markdown.match(/^<!-- dm-subagents-refinement:v1\n([\s\S]*?)\n-->\n/);
...
function serializeRefinementFile(parsed: ParsedRefinementFile): string {
	...
	return [
		`<!-- dm-subagents-refinement:v${REFINEMENT_FORMAT_VERSION}`,
		metadata,
		"-->",
		"",
...
export function appendAgentRefinementOverlay(systemPrompt: string, input: { cwd: string; agentName: string }): string {
	...
	const overlay = [
		`<dm-subagents-refinement agent=${JSON.stringify(input.agentName)} source=${JSON.stringify(path.relative(input.cwd, filePath))}>`,
...
export function validateRefinementProposal(proposal: unknown, evidenceIds: string[]): { ok: true; proposal: RefinementProposal } | { ok: false; error: string } {
	...
	const blocked = new RegExp(`\\b(all agents|every agent|global|(?:disable|ignore|bypass|skip|override)\\s+(?:the\\s+)?${protectedInstruction}(?:\\s+instructions?)?|${protectedInstruction}\\s+(?:instructions?|overrides?)|tool safety|review gates|rewrite base|base agent file|settings\\.json|\\.dm/agent|agents/.*\\.md)\\b`, "i");
	...
	for (const [index, editValue] of editsValue.entries()) {
		...
		if (guidance.includes("```") || guidance.includes("</dm-subagents-refinement>") || blocked.test(guidance)) return { ok: false, error: `Refinement proposal edit ${index} contains disallowed guidance.` };
