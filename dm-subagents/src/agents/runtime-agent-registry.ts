...
export const RUNTIME_AGENT_REGISTRY_KEY = "dm-subagents.runtime-agents.v1";
...
function validatePi(value: unknown): ExtensionAPI {
	if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Runtime agent dm must be an ExtensionAPI object.");
	const dm = value as Partial<ExtensionAPI>;
	if (typeof dm.on !== "function" || typeof dm.registerTool !== "function") throw new Error("Runtime agent dm must be an ExtensionAPI object.");
...
function validateRunner(value: unknown): AgentRunnerConfig | undefined {
	...
	if (runner.type === "dm") {
		if (Object.keys(runner).some((key) => key !== "type")) throw new Error("Runtime agent definition DM runner supports only 'type'.");
		return { type: "dm" };
	}
	...
	if (runner.type !== "external-cli") throw new Error("Runtime agent definition runner.type must be 'dm', 'external-cli', or 'external-job'.");
...
export function registerRuntimeAgent(input: RegisterRuntimeAgentInput): RuntimeAgentRegistration {
	...
	const records = current.byPi.get(dm) ?? [];
	if (records.length >= MAX_RUNTIME_AGENTS_PER_PI) throw new Error(`Runtime agent registry supports at most ${MAX_RUNTIME_AGENTS_PER_PI} agents per DM runtime.`);
	assertNoRuntimeCollision(agent, records.map((record) => record.agent));
	const record: RuntimeAgentRecord = { dm, agent };
	records.push(record);
	current.byPi.set(dm, records);
	let disposed = false;
	return {
		dispose() {
			if (disposed) return;
			disposed = true;
			const latest = current.byPi.get(dm);
			if (!latest) return;
			const next = latest.filter((entry) => entry !== record);
			if (next.length > 0) current.byPi.set(dm, next);
			else current.byPi.delete(dm);
		},
	};
}

export function clearRuntimeAgentsForPi(dm: RuntimeAgentOwner): void {
	registry().byPi.delete(dm);
}

export function listRuntimeAgentConfigs(dm: RuntimeAgentOwner): AgentConfig[] {
	return (registry().byPi.get(dm) ?? []).map((record) => ({ ...record.agent, aliases: record.agent.aliases ? [...record.agent.aliases] : undefined }));
}
...
export function mergeRuntimeAgents<T extends { agents: AgentConfig[]; agentDiagnostics?: AgentDiscoveryDiagnostic[] }>(dm: RuntimeAgentOwner, discovered: T, configuredAgents: readonly AgentConfig[] = discovered.agents): T {
	const runtimeAgents = listRuntimeAgentConfigs(dm).filter((agent) => agent.disabled !== true);
