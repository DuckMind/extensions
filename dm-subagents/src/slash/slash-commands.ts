...
import { keyText, type ExtensionAPI, type ExtensionContext } from "@duckmind/dm-coding-agent";
import { Key, matchesKey, truncateToWidth, type Component, type KeyId, type TUI } from "@duckmind/dm-tui";
...
import { SUBAGENT_FANOUT_CHILD_ENV } from "../runs/shared/dm-args.ts";
...
function discoverSlashAgents(dm: ExtensionAPI, cwd: string, scope: AgentScope): { agents: AgentConfig[]; agentDiagnostics?: AgentDiscoveryDiagnostic[]; unknownAgentDiagnosticContext: UnknownAgentDiagnosticContext } {
...
const makeAgentCompletions = (dm: ExtensionAPI, state: SubagentState) => (prefix: string) => {
	if (!state.baseCwd || prefix.includes(" ")) return null;
	return discoverSlashAgents(dm, state.baseCwd, "both").agents
...
export function registerSlashCommands(
	...
	if (options.foregroundDetachShortcut) {
		dm.registerShortcut(options.foregroundDetachShortcut as KeyId, {
	...
	return {
		dispose: () => {
			...
			// resume a command against the old DM context.
