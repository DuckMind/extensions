...
import type { ThinkingLevel, UserMessage } from "@duckmind/dm-ai";
...
import { completeSimple, retrySkillGenerationCompat } from "./dm-ai-compat.js";
...
export async function generateSkillDraft(ctx: ExtensionContext, answers: SkillCreationAnswers, options?: SkillGenerationOptions, deps: SkillCreationDeps = {}): Promise<string> {
	...
	const userMessage: UserMessage = {
		...
		content: [{
			...
			text: [
				...
				"",
				"Make the description specific enough for DM's skill trigger list. Keep the body compact and execution-oriented.",
			].join("\n"),
		}],
		timestamp: Date.now(),
	};
	...
	const generated = response.content.filter((c: unknown): c is { type: "text"; text: string } => typeof c === "object" && c !== null && (c as { type?: unknown }).type === "text").map((c: { type: "text"; text: string }) => c.text).join("\n").trim();
