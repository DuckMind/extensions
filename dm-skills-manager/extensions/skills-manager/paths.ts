...
export function userDmDir(): string {
	return resolve(expandHome(process.env.PI_CODING_AGENT_DIR?.trim() || "~/.dm/agent"));
}

export function findProjectDmDir(cwd: string): string {
	...
	while (true) {
		...
		if (parent === current) return join(resolve(cwd), ".dm");
		current = parent;
	}
}
...
export function detectExtensionInstallScope(cwd: string): ExtensionInstallScope {
	try {
		...
		if (isWithin(extensionFile, findProjectDmDir(cwd))) return "project";
