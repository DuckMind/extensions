...
function projectSettingsPath(cwd: string): string {
	...
	while (true) {
		...
		if (existsSync(join(current, ".dm")) || existsSync(join(current, ".git")) || existsSync(join(current, ".kendex-lock.json"))) return candidate;
		const parent = dirname(current);
		if (parent === current) return join(resolve(cwd), ".dm", "settings.json");
		current = parent;
	}
}
...
function dmSettingsPaths(cwd = process.cwd()): string[] {
