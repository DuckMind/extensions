...
import { retrySkillGenerationCompat } from "../extensions/skills-manager/dm-ai-compat.ts";
import { resolveSkillDraft } from "../extensions/skills-manager/creation-fallback.ts";

test("uses DM retry defaults for skill generation", async () => {
...
test("falls back to direct generation on older DM versions", async () => {
	let calls = 0;
