# dm-lens ast-grep Rule Reference
...
⚠ `-js` twins: remember there are TWO execution surfaces.
   ...
   - the in-process NAPI fallback (`ast-grep-napi.ts` — dm-lens source checkout
...
✅ Test through the REAL runner from the dm-lens source checkout's repo root — it loads the actual shipped
   rules from rules/ast-grep-rules/rules. Assert on diagnostic `rule` ids:
     const res = await runner.run(ctx);  // ctx.filePath = temp .ts, cwd = repo  (dm-lens source checkout only — not present in the installed package)
