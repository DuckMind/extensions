---
name: dm-lens-write-ast-grep-rule
description: Use when writing a new dm-lens ast-grep rule YAML file — covers schema, drop path, gotchas, and NAPI runner constraints
---

# Writing a dm-lens ast-grep Rule
...
1. **A multi-corpus false-positive census**, not a single-tree count. Run the
   rule over dm-lens's `clients/`/`tests/` plus at least one real external
...
2. **Structural narrowing before exemption.** Suppress a legitimate idiom
   ...
   (`b3e1fd79`).
3. **Self-scan wiring.** Tag the rule `metadata.category: dm-lens-self-scan`
   so `npm run astgrep:self-scan` holds this tree at zero in CI. An `error`
   rule that never runs against dm-lens's own source is an unaudited claim.
...
Rust, so it has no dm-lens corpus to census against, and its `# Safety`
