...
import {
	loadDmLensGlobalConfig,
	resolveDmLensFlag,
	resolveDmLensFlagWithSource,
...
import { loadDmLensProjectConfig } from "./clients/project-lens-config.js";
...
import {
	...
	registerTestRunnerMessageRenderer,
 ...
 */
const LOOP_BLOCK_IDENTITY = "<dm-lens>";
import {
	isFreshSessionStart,
	planToolSet,
...
const DM_LENS_LOAD_MS = markDmLensLoaded();
const DM_LENS_LOADED_AT_MS = getDmLensLoadedAtMs();
const DM_LENS_EVAL_MS = getDmLensEvalMs() ?? 0;
 ...
 * `ctx.mode`, not from dm-lens guessing. In "print"/"json" the message is
		...
		notify: {
			user(message, level) {
				...
				if (mode === "print" || mode === "json") {
					...
					// operators can see them in dmlens_health.
					recordDegradation({
						kind: "mode-suppression",
...
// Log how long pi took to load dm-lens — the jiti transpile of every module is
...
dbg(
	`dm-lens loaded: ${DM_LENS_LOAD_MS}ms after process start (from ${DM_LENS_LOADED_FROM})`,
);
for (const record of buildStartupTimingRecords({
	loadMs: DM_LENS_LOAD_MS,
	evalMs: DM_LENS_EVAL_MS,
})) {
	logLatency(record);
}
 ...
 * is `{ type, reason }`
 * (`@duckmind/dm-coding-agent/dist/core/agent-session.js:152`,
 ...
 * with `id` and `provider` (`@duckmind/dm-ai/dist/types.d.ts:661-667`).
 *
...
function activateExtension(hostPi: ExtensionAPI) {
	// #1434: every dm-lens entry point registered through this API runs inside a
	...
	// loaded module desyncs its screen model. dm-lens's own sites are migrated
	...
	// under test mode and under `DM_LENS_CONSOLE_GUARD=0`.
	installConsoleGuard();
	initI18n(pi);
	...
	const globalConfig = loadDmLensGlobalConfig();
	function getLensFlag(
		name: string,
		editedFilePath?: string,
	): boolean | string | undefined {
		const projectConfig = loadDmLensProjectConfig(runtime.projectRoot);
		return resolveDmLensFlag(
			name,
			pi.getFlag(name),
			globalConfig,
			projectConfig,
	...
	function getLensFlagSource(
		name: string,
		editedFilePath?: string,
	): ReturnType<typeof resolveDmLensFlagWithSource>["source"] {
		const projectConfig = loadDmLensProjectConfig(runtime.projectRoot);
		return resolveDmLensFlagWithSource(
			name,
			pi.getFlag(name),
			globalConfig,
			projectConfig,
	...
	// An in-process producer that writes a file outside dm-lens's tool-event
	...
	// from the registry's DM_LENS_NO_CONTEXT_INJECTION env binding (#166).
	...
	registerTestRunnerMessageRenderer(pi);

	// --- Commands ---

	pi.registerCommand("lens-toggle", {
		description:
			"Toggle dm-lens on/off for the current session. Usage: /lens-toggle",
		handler: async (_args, ctx) => {
			lensEnabled = !lensEnabled;
			notifyUi(
				ctx,
				lensEnabled
					? "dm-lens enabled for this session."
					: "dm-lens disabled for this session. Run /lens-toggle again to resume.",
		...
		handler: async (_args, ctx) => {
			...
			notifyUi(
				...
				contextInjectionEnabled
					? "dm-lens context injection enabled — findings will be added to the next turn."
					: "dm-lens context injection disabled — findings are still cached (lens_diagnostics, /lens-health) but not added to model context.",
	...
	pi.registerCommand("lens-widget-toggle", {
		description:
			"Show or hide the dm-lens diagnostics widget below the editor. Usage: /lens-widget-toggle",
		handler: async (_args, ctx) => {
			...
			if (nextVisible && !supportsTuiWidget(mode)) {
				notifyUi(
					ctx,
					`dm-lens widget needs an interactive TUI — unavailable in "${mode}" mode.`,
					"warning",
				);
				return;
			}
			...
			if (!changed) {
				notifyUi(
					ctx,
					"dm-lens widget is not supported by this pi version.",
					"warning",
				);
				return;
			}
			...
			notifyUi(
				ctx,
				lensWidgetVisible
					? "dm-lens widget shown. Run /lens-widget-toggle to hide it."
					: "dm-lens widget hidden. Run /lens-widget-toggle to show it.",
				"info",
			);
		},
	});
	...
	pi.registerCommand("lens-health", {
		description:
			"Show dm-lens runtime health: pipeline crashes, slow runners, and last dispatch latency. Usage: /lens-health",
		handler: async (_args, ctx) => {
			...
			// this captures WHICH objects retain them. Gated behind DM_LENS_DEBUG_HEAP
	...
	pi.registerCommand("lens-perf", {
		...
		handler: async (_args, ctx) => {
			try {
				...
				// Shared with the MCP `dmlens_health` path (#2515 review, S3): a
	...
	pi.registerCommand("lens-tools", {
		description:
			"Show dm-lens tool installation status: globally installed, auto-installed, or npx fallback. Usage: /lens-tools",
		handler: async (_args, ctx) => {
			...
			const bySource = {
				...
				"dm-lens-auto": statuses.filter((s) => s.source === "dm-lens-auto"),
			...
			// dm-lens auto-installed
			if (bySource["dm-lens-auto"].length > 0) {
				lines.push(
					"",
					`🤖 Auto-installed (${bySource["dm-lens-auto"].length}):`,
				);
				for (const tool of bySource["dm-lens-auto"]) {
					lines.push(`  ✓ ${tool.name}`);
				}
			}
	...
	// activates the ones it needs via `dm_lens_activate_tools`. On hosts without
	// that API this whole tier is simply left statically active, matching
	// dm-lens's behavior before this feature existed.
	const lazyTools = [
	...
	// substantive dm-lens tool — see tools/render-compact.ts) are wrapped;
	...
	// start inactive and the model must call `dm_lens_activate_tools` to bring
	...
	// `rememberOwnEventCtx` would otherwise store the DEAD ctx as dm-lens's
		...
		wrapSessionEventHandler(
			...
			async (event, ctx) => {
				...
				try {
					...
					// below can auto-install a tool or spawn an LSP server. dm-lens is a
					...
					// via dm_lens_activate_tools. session_start is the correct lifecycle
					...
					// reason arrives with all registered dm-lens tools active, because the
					...
					// genuinely preserves the model's activations, because dm-lens's own
					...
					// every host the broad `@duckmind/dm-coding-agent` peer
					...
					// dm-lens-managed binary names and only acts on ones that are BOTH
					...
					// OTHER dm-lens instances (pid-excluded) within the 15-minute
					...
					await handleSessionStart({
						...
						extensionLoadedAt: DM_LENS_LOADED_AT_MS,
						emitHostReadyDelay,
						sessionReason,
						handlerEnteredAt,
					...
					// session swap as a dm-lens crash (#1929, same shape as the
	...
	const onToolResult = async (event: any, ctx: any) => {
		...
		// upstream, before dm-lens even received the event) leaves NO trace — that
		// is exactly why a wedged-LSP edit hang was invisible in latency.log. This
		// row means "dm-lens received this edit"; if it is present but nothing
		...
		// "dm-lens received this edit" row, which is precisely the trace the
	...
	const onTurnStart = (_event: any, ctx: any) => {
		...
		// subagent returns and its dm-lens has autoformatted on top of the
	...
	// see node_modules/@duckmind/dm-coding-agent/dist/core/extensions/types.d.ts).
	...
	const onAgentSettled = async (_event: unknown, ctx: DeferredDrainCtx) => {
		...
		try {
			...
			try {
				...
				// The drain just wrote formatted/autofixed bytes to files dm-lens
			...
			// already in flight. No-op unless DM_LENS_DEBUG_HANDLES=1.
	...
	(pi as any).on("session_shutdown", (_event: unknown, ctx: unknown) => {
		...
		// No-op unless DM_LENS_DEBUG_HANDLES=1.
	...
	// Placement (#1016): splice the ephemeral dm-lens findings in IMMEDIATELY BEFORE
	...
	(pi as any).on(
		...
		wrapSessionEventHandlerWithResult(
			...
			{
				dbg,
				// #1929: a dead ctx means dm-lens contributes NOTHING to this
...
// dm-lens's own entry points opens its own window. This must stay the last
