/**
 * Publishes `dmlens:diagnostics` on pi's shared `pi.events` bus (#502).
 *
 * Sibling to `clients/bus-publish.ts` (the #482 `dmlens:files:touched`
 * producer) rather than a new export crammed into that file: the two events
 * share the emit plumbing (`wireBusEmitter`) and the `DM_LENS_BUS_PUBLISH`
 ...
 * 4. **`dmlens:files:touched` (#482) is an INVALIDATION HINT, not new data.**
 ...
 *    disk but dm-lens hasn't re-analyzed it yet. Consumers that want to
 ...
 *    dmlens:diagnostics event mentions it (at any seq).
 *
 ...
 * request/replay). #478's future `dmlens:rpc:diagnostics` pull API reuses
...
export const BUS_DIAGNOSTICS_EVENT = "dmlens:diagnostics";
...
/**
 * Versioned payload for `dmlens:diagnostics` (#502). This schema is shared
 * VERBATIM with #478's future `dmlens:rpc:diagnostics` pull response — push
...
export interface PublishDiagnosticsArgs {
	...
	/** Loop guard, mirrors #482: set when triggered by an ingested bus event. Always false in practice (dm-lens consumes nothing today); exists so a future consumer can't wire a publish -> react -> publish cycle. */
	origin?: "bus";
	dbg?: (msg: string) => void;
}
...
/**
 * Publish one `dmlens:diagnostics` event for a write batch's final
...
export function publishDiagnostics(args: PublishDiagnosticsArgs): void {
	...
	try {
		...
		if (!hasLoggedFailure) {
			...
			args.dbg?.(
				`diagnostics-publish: dmlens:diagnostics emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
