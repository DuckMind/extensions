 ...
 * and — later — writes by other extensions once dm-lens consumes bus events)
 * is invisible to the model. This module closes that gap: it subscribes to
 * the `dmlens:files:touched` bus event dm-lens itself publishes (#482,
 ...
 * finds files it did not knowingly modify, because dm-lens autoformatted
 ...
 * One feed (`dmlens:files:touched`), three deliveries.
 *
 ...
 * node_modules/@duckmind/dm-coding-agent/dist/core/event-bus.d.ts;
...
const BUS_FILES_TOUCHED_EVENT = "dmlens:files:touched";
 ...
 * written by ANOTHER dm-lens instance (a parent or a subagent child).
 *
...
/** `DM_LENS_AGENT_NUDGE=0` disables accumulation and injection outright. */
export function isAgentNudgeEnabled(): boolean {
	if (_enabledCache === undefined) {
		_enabledCache = process.env.DM_LENS_AGENT_NUDGE !== "0";
	}
...
/**
 * Record a `dmlens:files:touched` event into the accumulator, filtered to
...
/**
 * Subscribe to `dmlens:files:touched` on pi's shared event bus. Called once
 ...
 * message, e.g.:
 *   "dm-lens: 2 file(s) were autoformatted after your last turn: a.ts, b.ts —
 *    working-tree changes to these are expected; re-read before editing."
 * The provenance framing ("dm-lens ... expected") matters: the primary pain
 ...
 * did not knowingly make. Naming dm-lens as the source lets the agent act
 ...
 * used to read "by another dm-lens instance (e.g. a subagent's)", which sent
...
export function consumeAgentNudge(
	...
	try {
		...
		const message = `dm-lens: ${filesTotal} file(s) were ${verbLabel} ${attribution}: ${nameList} — working-tree changes to these are expected; re-read before editing.`;

		return {
			messages: [
				{
					role: "user",
					content: `[dm-lens automated context — not a user request] ${message}`,
				},
			],
		};
	} catch (err) {
