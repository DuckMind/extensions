import type { EditToolInput } from "@duckmind/dm-coding-agent";
...
function resolveOldTextEdits(
	...
	for (let i = 0; i < edits.length; i++) {
		...
		if (occurrenceLines.length === 0) {
			...
			// anyway? hostWouldApply=true => this block is a false-block (dm-lens
...
export function getTouchedLinesForGuard(
	...
	if (mutation.kind === "edit") {
		...
		// remaining keys are dm-lens's own extensions for native-ranged + hashline
