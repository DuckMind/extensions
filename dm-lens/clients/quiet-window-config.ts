...
/** `DM_LENS_QUIET_WINDOW=0` disables the whole scheduler (no-op, no logging). */
export function isQuietWindowEnabled(): boolean {
	if (_enabledCache !== undefined) return _enabledCache;
	_enabledCache = process.env.DM_LENS_QUIET_WINDOW !== "0";
...
export function quietWindowWaitMs(): number {
	const raw = toPositiveFinite(process.env.DM_LENS_QUIET_WINDOW_WAIT_MS);
