 ...
 *
 * `DM_LENS_DEBUG_HANDLES=1` (read ONCE at module load — i.e. must be set
 * before dm-lens starts, not toggled mid-session):
 ...
 *     `~/.dm-lens/debug-handles.log` via the standard `createNdjsonLogger`
 *     family (size-bounded, log-cleanup registered — the same rotation
 *     bounds every other dm-lens log gets, #1101-era registry via
...
const DEBUG_HANDLES_ENABLED = process.env.DM_LENS_DEBUG_HANDLES === "1";
...
/** True iff `DM_LENS_DEBUG_HANDLES=1` was set when this module first loaded. */
 ...
 * when `DM_LENS_DEBUG_HANDLES` was unset at startup.
 */
