...
/**
 * Remove orphaned generic atomic-write staging files from dm-lens-owned
...
/**
 * #658: known dm-lens-managed LSP/scanner binary names (clients/lsp/server.ts
 ...
 * dm-lens, but still part of the tree we manage). `yaml-language-server` and
 ...
 * dm-lens-managed LSP/scanner processes the registry-driven `sweepOrphans`
...
async function acquireBackstopLock(): Promise<(() => Promise<void>) | null> {
	try {
		return await acquireQuarantinePidFileLock(
			path.join(getGlobalDmLensDir(), "orphan-backstop.lock"),
			{
				waitMs: 0,
				retryMs: 50,
 ...
 *  machine-scoped (every dm-lens process shares one OS process table), and
 *  process-lifetime state cannot express "30 minutes have passed". */
function backstopStampPath(): string {
	return path.join(getGlobalDmLensDir(), "orphan-backstop.json");
}
...
async function writeBackstopStamp(at: number): Promise<void> {
	try {
		await fs.promises.mkdir(getGlobalDmLensDir(), { recursive: true });
