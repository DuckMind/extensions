 ...
 * `@duckmind/dm-coding-agent`). An event already queued when the swap
 * happens still reaches dm-lens, carrying the dead ctx, and the first
 ...
 * surfaces as an extension error report against dm-lens. The report names the
 * wrong cause. It reads as a dm-lens handler bug when the real event is a
...
export interface SessionEventGuardOptions {
	/** dm-lens's debug sink, so a skip is also visible in a dogfood trace. */
