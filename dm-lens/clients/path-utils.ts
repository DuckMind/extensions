 ...
 * user's legitimate GLOBAL config, and reads it itself regardless of dm-lens
 * — the ceiling didn't stop dm-lens from seeing an unrelated ancestor
 * config, it stopped dm-lens from seeing the SAME config the tool was about
 * to read on its own, so dm-lens silently fell back to (or, for typos,
 ...
 *
 * `~/.dm-lens/config.json` says everything an operator needs about which file
 * won, and `C:/Users/jane.doe/.dm-lens/config.json` says that plus the
 ...
 * from `@duckmind/dm-coding-agent/dist/utils/paths.js:6`
 * (`UNICODE_SPACES`, source `src/utils/paths.ts`). Widening or narrowing this
 * set makes dm-lens resolve a different file than pi does.
 */
const HOST_UNICODE_SPACES = /[\u00A0\u2000-\u200A\u202F\u205F\u3000]/g;

/**
 * Mirror pi's `normalizeWindowsShellPath`
 * (`@duckmind/dm-coding-agent/dist/utils/paths.js:47-56`): convert Git
 ...
 * (`@duckmind/dm-coding-agent/dist/utils/paths.js:57-79`, source
 ...
 * (AGENTS.md shape 2): dm-lens runs in pi's own process, so mirroring the
...
export function normalizeHostToolPath(
	...
	if (normalized.startsWith("file://")) {
		try {
			...
			// pi lets a malformed file: URL throw out of normalizePath, but dm-lens
 ...
 * (`@duckmind/dm-coding-agent/dist/core/tools/path-utils.js:42-44`,
 ...
 * (`@duckmind/dm-coding-agent/dist/core/tools/path-utils.js:45-70`,
 ...
 * dm-lens keyed its read guard, LSP touch, and dispatch off the naive form —
 * silently doing nothing for exactly those files.
 *
 * Order matters: it is pi's, so dm-lens picks the same file pi did when more
...
export function resolveHostPathVariants(
	...
	for (const { variant, candidate } of candidates) {
		// pi skips a candidate identical to the resolved path, so dm-lens does
