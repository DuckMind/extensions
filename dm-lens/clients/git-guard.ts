...
export interface TurnEndFindingsCache {
	...
	/** Content fingerprints catch edits made outside dm-lens between turns. */
...
function unknown(
	...
	return {
		block: true,
		unknown: true,
		reason: `🔴 COMMIT BLOCKED (--lens-guard): blocker state is unknown (${reasonCategory}). Re-run dm-lens checks or start a fresh session, then retry.`,
	};
}
