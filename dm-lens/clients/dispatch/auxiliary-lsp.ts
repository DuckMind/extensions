 ...
 *   - how to turn its raw LSP diagnostics into dm-lens diagnostics (tool name +
...
export interface AuxiliaryLspProfile {
	...
	serverId: string;
	/** dm-lens tool id its diagnostics are tagged with. */
	tool: string;
	 ...
	 *  the finding. Distinct from dm-lens's own `# dm-lens-ignore` — this honors the
...
export const AUXILIARY_LSP_PROFILES: readonly AuxiliaryLspProfile[] = [
	...
	},
	{
		...
		// dm-lens's shipped baseline sgconfig. In both cases the rule severity is
...
export function enabledAuxiliaryLspServerIds(getFlag: GetFlag): string[] {
	...
	// the same cwd. DM_LENS_SUBAGENT_FULL=1 restores full behavior via
