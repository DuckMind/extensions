/**
 * Writes the `dm-lens-ignore` suppression comment that inline-suppressions.ts
...
/**
 * Insert a `dm-lens-ignore: <rule>` comment on the line immediately above
 * `line` (1-based), matching inline-suppressions.ts's "same line or line
 * above" read semantics. If the line above already carries a dm-lens-ignore
...
export function insertSuppressComment(
	...
	const suppressRe = /((?:\/\/|#)\s*dm-lens-ignore:\s*)(.+)$/;
	const match =
	...
	lines.splice(line - 1, 0, `${indent}${prefix} dm-lens-ignore: ${rule}`);
	return lines.join("\n");
}
