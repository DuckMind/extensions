/**
 * Shared runner utilities for dm-lens dispatch system
 *
...
/**
 * Test-only override for dm-lens's OWN install-tree root (#2544 round 4 F1).
...
// Managed tools directory (~/.dm-lens/tools) — where ensureTool() installs binaries
const _managedToolsDir = path.join(getGlobalDmLensDir(), "tools");
 ...
 *
 * The dm-lens dir is read per call, never memoized at module load, so tests that
 * point `getGlobalDmLensDir` at a temp home still see their own tree.
 */
...
/**
 * The managed shim for a Node CLI tool (`~/.dm-lens/tools/node_modules/.bin/<tool>`),
...
export function createVenvFinder(
	...
	return async (cwd: string): Promise<string> => {
		...
		// Managed-dir install (~/.dm-lens/tools/node_modules/.bin/<command>) — the
...
function buildSgLocalBins(): SgLocalBinCandidate[] {
	...
	// project's own dependency. dm-lens's OWN install-tree walk (from
	// `_thisDir`) is exempt (`ceiling: false`, #2544 round 4 F1): `pi install
	// npm:dm-lens` installs into the HOME-level manifest
	// (`~/package.json` → `~/node_modules/dm-lens`), so dm-lens's bundled
	...
	// cover this location either). `_managedToolsDir` (`~/.dm-lens/tools`) is
