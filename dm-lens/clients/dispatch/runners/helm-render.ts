 ...
 *
 * `.dm-lens.json` is a tracked file, so a cloned repository can arrive with the
 ...
 * in `.dm-lens.json` (the loader walks up, so `~/.dm-lens.json` enables it for
...
export function isHelmRenderEnabled(workspaceRoot: string): boolean {
	try {
		const config = loadDmLensProjectConfig(workspaceRoot);
		const helmConfig = (
			config.raw as
...
/**
 * The flags dm-lens itself puts on the command line. If helm complains about one
...
async function renderAndValidate(
	...
	const outputDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "dm-lens-helm-render-"),
	);
	try {
		const render = await safeSpawnAsync(
			helmCmd,
			["template", "dm-lens-render", chartRoot, "--output-dir", outputDir],
			{
				cwd,
		...
		if (render.status !== 0) {
			...
			if (ourFlag) {
				...
				return failedResult(
					"invocation_rejected",
					`helm rejected dm-lens's own invocation (${ourFlag}); this is a helm/dm-lens compatibility problem, not a chart defect: ${renderOutput.trim().slice(0, 160)}`,
				);
			}
...
const helmRenderRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		// Gate 2 — project trust. `.dm-lens.json` is a TRACKED file, so consent
