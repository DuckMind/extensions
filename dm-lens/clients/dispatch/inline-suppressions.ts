/**
 * Inline `dm-lens-ignore` suppression — shared between the per-edit dispatch
 ...
 *
 * Syntax: `// dm-lens-ignore: rule-id` (JS/TS) or `# dm-lens-ignore: rule-id`
...
const SUPPRESS_RE = /(?:\/\/|#)\s*dm-lens-ignore:\s*(.+)/;

/**
 * Normalize a rule id to the form a user writes in a `dm-lens-ignore` comment.
...
/**
 * Drop diagnostics suppressed by an inline `dm-lens-ignore: <rule[,rule2]>`
...
export function applyInlineSuppressions<T extends SuppressibleDiagnostic>(
	...
	for (let i = 0; i < lines.length; i++) {
		...
		for (const ruleId of rules) {
			...
			// the raw comment token made `// dm-lens-ignore: no-eval-js` fail to
