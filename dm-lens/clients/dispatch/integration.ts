...
/**
 * Load a project's `.dm-lens.json` config.
 *
 ...
 */
export function applyProjectLensConfig(cwd: string): DmLensProjectConfig {
	return loadDmLensProjectConfig(cwd);
}
 ...
 * `dm-lens-ignore`. Two guards, matching the other dispatch callers:
 ...
 *
 * Pass `cwd` to also re-apply the project's `.dm-lens.json` rule thresholds
...
function reverseDepsIdleEvictMs(): number {
	const value = Number.parseInt(
		process.env.DM_LENS_REVERSE_DEPS_IDLE_EVICT_MS ?? "",
		10,
	);
...
function reverseDepsReuseEnabled(): boolean {
	const raw = process.env.DM_LENS_REVERSE_DEPS_REUSE;
...
const CASCADE_TRANSITIVE_DEPTH = Math.max(
	1,
	Number.parseInt(process.env.DM_LENS_CASCADE_TRANSITIVE_DEPTH ?? "2", 10) || 2,
);
 ...
 * (`.dm-lens.json` / `.gitignore` / global `~/.dm-lens/config.json`), via the
 ...
 * (e.g. a `*.test.ts` glob in `.dm-lens.json`) must not be re-surfaced here just
...
export async function computeCascadeForFile(
	...
	try {
		...
		if (CASCADE_GRAPH_KINDS.has(fileKind)) {
			...
			// DM_LENS_REVIEW_GRAPH_MAX_FILES (`too_many_files`) or the root is unsafe
 ...
 * @param pi - DM agent API (for flags)
