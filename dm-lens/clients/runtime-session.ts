...
interface SessionStartDeps {
	 ...
	 * wins — but only when `DM_LENS_STARTUP_MODE` is NOT explicitly set in the
...
function resolveStartupMode(): StartupMode {
	const envMode = (process.env.DM_LENS_STARTUP_MODE ?? "").trim().toLowerCase();
...
// `DM_LENS_WARMUP_DELAY_MS` precedent above) so a regression can use a tiny
// budget instead of racing a real 250ms wall-clock wait.
function sequenceReadBudgetMs(): number {
	const raw = Number(process.env.DM_LENS_SEQUENCE_READ_BUDGET_MS ?? 250);
...
function scheduleManagedToolRefresh(dbg: SessionStartDeps["dbg"]): void {
	const delayMs = Number(
		process.env.DM_LENS_TOOL_REFRESH_DELAY_MS ?? MANAGED_TOOL_REFRESH_DELAY_MS,
	);
...
function scheduleStartupScansWithClients(
	...
	// Explicit opt-in: `trivy.enabled: true` in .dm-lens.json AND a dependency
...
export const SESSION_START_GUIDANCE: string[] = [
	"📌 dm-lens active — automated checks run on every edit/write; blocking errors (including pre-existing) show inline and must be fixed.\n" +
		...
		"• Situational (activate via dm_lens_activate_tools): lsp_navigation, ast_grep_search, ast_grep_replace, ast_grep_dump.",
];
...
export async function handleSessionStart(
	...
	//
	// Opt-out: DM_LENS_COLD_START_QUICK=0 disables this behaviour.
	// Override: DM_LENS_STARTUP_MODE explicitly set wins (we honour it).
	...
	//   entirely — but only when DM_LENS_STARTUP_MODE is unset in the env
	//   (an explicit env var still takes highest precedence).
	// Tunable: DM_LENS_WARMUP_DELAY_MS adjusts the warmup delay.
	let startupMode = resolveStartupMode();
	// SAFETY: these two flags are process-lifetime state dm-lens stashes on
	...
	if (
		...
		process.env.DM_LENS_COLD_START_QUICK !== "0" &&
		!process.env.DM_LENS_STARTUP_MODE
	) {
	...
	if (
		...
		const warmupDelayMs = Number(process.env.DM_LENS_WARMUP_DELAY_MS ?? 2000);
	...
	const globalDir = getGlobalDmLensDir();
	...
	// reason on its own, since dm-lens never wrote anything while
	...
	if (slowFsVerdict.slow) {
		notify(
			`🐢 Slow filesystem detected (median ${slowFsVerdict.medianStatMicros.toFixed(0)}µs/stat) — reduced-scan mode engaged (set DM_LENS_ALLOW_SLOW_FS_SCAN=1 to override).`,
			"warning",
		);
	}
	...
	if (!allowBootstrapTasks) {
		...
		// dm-lens just isn't scanning anything. Fires ONCE per session (this
		...
		if (
			...
			const overrideHint =
				startupScan.reason === "too-many-entries"
					? ` (set DM_LENS_STARTUP_SCAN_MAX_ENTRIES=<n> to override the ${getStartupScanMaxEntries()}-entry cap)`
					: "";
			notify(
