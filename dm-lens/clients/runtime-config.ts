...
import { loadDmLensGlobalConfig } from "./lens-config.js";
 ...
 *   1. `dispatch.runnerTimeoutFloorMs` in `~/.dm-lens/config.json`
 *   2. `DM_LENS_RUNNER_TIMEOUT_FLOOR_MS` environment variable
 ...
 * @example env var
 * ```bash
 * DM_LENS_RUNNER_TIMEOUT_FLOOR_MS=180000 pi
 * ```
 */
export function getRunnerTimeoutFloorMs(): number {
	...
	const envFloor = toPositiveFinite(
		process.env.DM_LENS_RUNNER_TIMEOUT_FLOOR_MS,
	);
