/**
 * Formatter Definitions for dm-lens
 *
...
export const styluaFormatter: FormatterInfo = {
	...
	extensions: [".lua"],
	async resolveCommand(filePath, cwd) {
		// Project binary first (#1731, discipline B): stylua has no dm-lens
...
export async function getFormattersForFile(
	...
	if (formatterPolicy) {
		...
		for (const formatter of candidateFormatters) {
			...
			candidateOutcomes.push(outcome);
			if (error !== undefined) {
				// dm-lens-ignore: missing-error-propagation — optional formatter detection, skip on failure
				logExtension({
					subsystem: "format",
