 ...
 *
 * `scripts/analyze-dm-lens-logs.mjs` (`npm run logs:smells`) already detects
 ...
 *
 * COST BOUND (the constraint that shapes the design): `~/.dm-lens/*.log`
 * files are size-rotated at `DM_LENS_MAX_LOG_SIZE_MB` (default 10MB,
...
export function countRecentSmells(
	root: string = getGlobalDmLensLogDir(),
	sessionStartMs?: number,
): SmellsRollupCounts {
...
export function checkSmellsAndNoteOnce(counts: SmellsRollupCounts): string[] {
	...
	for (const key of SMELL_KEYS) {
		...
		if (counts[key] >= SMELLS_THRESHOLDS[key]) {
			...
			notes.push(
				`dm-lens smell: ${SMELL_LABELS[key]} x${counts[key]} (recent tail-scan) — run \`npm run logs:smells\` for detail`,
			);
		}
	}
	return notes;
