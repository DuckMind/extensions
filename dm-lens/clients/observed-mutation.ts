 ...
 * a tool dm-lens has never met is dropped before the first bookkeeping call —
 ...
 *   background write during a `read`-shaped call taught dm-lens that `read`
 ...
 * diff advanced the clean latch, and two of those stopped dm-lens watching a
 * tool that mutates on every call.
 *
 * ## What layer 3 cannot see, stated rather than hidden
 *
 * The sweep compares against a ledger seeded from files dm-lens has ALREADY
 ...
 * the `tool_result` path, between a tool finishing and dm-lens dispatching its
...
/**
 * Path keys the "dm-lens already recorded this" set may hold.
 *
 ...
 * next sweep read dm-lens's own bytes as third-party drift), the drop emits
 ...
 * read-guard's stored per-line hashes (#505) — dm-lens already paid for those
...
export function noteMutationHandled(filePath: string): void {
	try {
		...
		for (const oldest of evicted) {
			...
			// those bytes were dm-lens's own has just been thrown away, so the
			...
			emitBounded(
				...
				},
				{
					...
					reason: `the handled set is full at ${OBSERVED_HANDLED_MAX}; the oldest dm-lens-authored file lost its mark, so its next drift is reported as third-party`,
...
export async function settleObservedMutation(
	...
	if (changed.length === 0) {
		...
		// teach dm-lens to stop watching a tool it never finished watching, and
...
async function scanTrackedIncrementally(
	...
	if (options.report) {
		...
		// The post-drain refresh's job is to re-baseline dm-lens's OWN drain
	 ...
	 *
	 * Per FILE, not all-or-nothing. The mark exists to stop dm-lens's own drain
...
/**
 * Re-baseline dm-lens's OWN writes AFTER the deferred drain.
 *
 * The drain is dm-lens formatting and autofixing files it already knows about,
 ...
 * in `handled`: that set exists exactly to name "dm-lens wrote these bytes
