 ...
 *
 * Together these close the "stale-process trap": commit → `dmlens_rebuild` →
 * `dmlens_analyze mode=fresh` measures the just-built code first-hand.
 */
 ...
 * stray tsconfig.
 */
export function canRebuildDmLens(repoRoot: string): boolean {
...
export const REBUILD_UNAVAILABLE_MESSAGE =
	"dmlens_rebuild is unavailable in an installed dm-lens package. Rebuilding is only safe from a source checkout containing tsconfig.dist.json; reinstall or update dm-lens through your package manager instead.";

/**
 * Run `<pm> run <script>` in the dm-lens repo, where `<pm>` is resolved from the
 ...
 * `dmlens_project_scan` tool returns a compact, scannable summary instead of
