 ...
 *
 * dm-lens's dispatch core only couples to the host through a single-method
 * interface — `PiAgentAPI.getFlag`. When dm-lens runs *inside pi*, that flag
...
import { loadDmLensGlobalConfig, resolveDmLensFlag } from "../lens-config.js";
...
export function createMcpHost(
	...
	const config = loadDmLensGlobalConfig();
	const projectConfig = loadDmLensProjectConfig(projectRoot);
	return {
