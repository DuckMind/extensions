 ...
 * dispatch pipeline dm-lens runs inside pi (`dispatchLintWithResult`) on a file,
...
function idleEvictMs(): number {
	return positiveEnv(
		"DM_LENS_WORD_INDEX_IDLE_EVICT_MS",
...
function maxWarmRoots(): number {
	return positiveEnv(
		"DM_LENS_WORD_INDEX_MAX_WARM_ROOTS",
 ...
 * following a warm `dmlens_analyze` call in the SAME process would read a
...
export interface AnalyzeFileOptions {
	 ...
	 * query tools (`dmlens_diagnostics`, `dmlens_health`) reflect this analysis.
	 ...
	 * `dmlens_turn_end` (knip/jscpd/dep/tests/cascade) picks it up with no
	 * explicit file list. Default false — only the edit-detection paths (the
	 * warm `dmlens_analyze` and the PostToolUse-hook bin) enable it, so `fresh`
	 ...
	 * `dmlens_module_report`'s usedBy/blastRadius and `dmlens_symbol_search`'s
...
export async function analyzeFile(
	...
	if (options.record !== false) {
		// Mirror pipeline.ts's recording so dmlens_diagnostics (mode=all) and
		// dmlens_health see what this analysis found.
	...
	if (options.updateGraph && !result.hasBlockers) {
		...
		// (`scheduleWordIndexPersist`, `DM_LENS_WORD_INDEX_PERSIST_DEBOUNCE_MS`)
