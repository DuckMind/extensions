...
export function ipcPathForCwd(cwd: string): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\dm-lens-mcp-${hash}`;
	}
	return path.join(os.tmpdir(), `dm-lens-mcp-${hash}.sock`);
}
...
/**
 * Ask the warm server to run dm-lens's real turn-end pass. Execution and
...
// The hook records its outcome to a small per-workspace file; `dmlens_health`
...
export function turnEndStatusPathForCwd(cwd: string): string {
	return path.join(os.tmpdir(), `dm-lens-turn-end-${workspaceHash(cwd)}.json`);
}
