...
interface NdjsonWriterState {
	 ...
	 * reaches `dmlens_health` without this module ever performing I/O about
	 ...
	 * same global log (another dm-lens process sharing ~/.dm-lens) self-heals.
	 */
...
const NDJSON_GLOBAL_STATE_SCHEMA = "dm-lens.ndjson-logger.state";
...
const NDJSON_GLOBAL_STATE_KEY = Symbol.for("dm-lens.ndjson-logger.state");
...
// concurrent writer to the same global log (another dm-lens process sharing
// ~/.dm-lens) can drift `knownSize` from the real file before the next
 ...
 * visible in `dmlens_health` instead of silent (#2505 review F2).
 */
