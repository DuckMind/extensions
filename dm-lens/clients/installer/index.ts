/**
 * Auto-Installation System for dm-lens
 *
 ...
 * - pip packages
 * - GitHub releases (platform-specific binaries → ~/.dm-lens/bin/)
 */
...
import { getGlobalDmLensDir } from "../file-utils.js";
import {
...
// Global installation directory for dm-lens tools
const TOOLS_DIR = path.join(getGlobalDmLensDir(), "tools");
 ...
 * registry is this module's business — don't re-derive `<dm-lens home>/tools`).
 */
...
async function acquireInstallLock(): Promise<{
	...
	// (DM_LENS_INSTALL_TIMEOUT_MS, default 120s) — a 30s waiter gave up on a
	// legitimate slow install and reported the tool unavailable for the whole
	// session even though it arrived seconds later.
	const timeoutMs =
		Number(process.env.DM_LENS_INSTALL_LOCK_TIMEOUT_MS) || 150_000;
	...
	while (Date.now() < deadline) {
		try {
			...
			try {
				...
				const maxAgeMs =
					(Number(process.env.DM_LENS_INSTALL_TIMEOUT_MS) || 120_000) + 60_000;
				const expired =
				...
				try {
					...
					const maxAgeMs =
						(Number(process.env.DM_LENS_INSTALL_TIMEOUT_MS) || 120_000) +
						60_000;
...
const GITHUB_BIN_DIR = path.join(getGlobalDmLensDir(), "bin");

// Debug flag - set via DM_LENS_DEBUG=1 or --debug
const DEBUG =
	process.env.DM_LENS_DEBUG === "1" || process.argv.includes("--debug");

/** Test-only platform seam for Windows resource-layout coverage on Linux CI. */
function installerPlatform(): NodeJS.Platform {
	const override = process.env.DM_LENS_TEST_PLATFORM;
...
/**
 * The subset of a GitHub `releases/latest` response dm-lens reads: the tag that
...
export interface ArchiveSpec {
	 ...
	 * (`~/.dm-lens/tools/<id>`) rather than a shim. The consuming server then
 ...
 * map, written by the `DM_LENS_DISABLE_TOOL_INSTALL` branches and the install-lock
...
const PROBE_CACHE_PATH = path.join(getGlobalDmLensDir(), "probe-cache.json");
...
/**
 * Version reported by the last successful `--version` probe of a dm-lens
...
export async function getAllToolStatuses(): Promise<ToolStatus[]> {
	...
	for (const tool of TOOLS) {
		...
		// 4. Check managed bin (~/.dm-lens/bin/) — github releases + maven/archive launchers
		if (
		...
		// 5. Check dm-lens auto-install (~/.dm-lens/tools/)
...
async function getToolPathResolved(
	...
	// For github/maven tools, prefer the managed install (~/.dm-lens/bin/) over
	// PATH. Managed installs are known-good binaries/launchers dm-lens downloaded
...
function httpsGetWithMeta(
	...
	return new Promise((resolve, reject) => {
		https
			.get(
				url,
				{ headers: { "User-Agent": "dm-lens/1.0", ...headers } },
				(res) => {
					if (
						res.statusCode &&
						res.statusCode >= 300 &&
...
/**
 * Is this tool present in a location dm-lens itself installs to?
 *
 ...
 *
 * A tool dm-lens has never installed is never refreshed: `pip install -U` or a
...
export async function isManagedToolPresent(toolId: string): Promise<boolean> {
	...
	switch (tool.installStrategy) {
		...
		case "pip":
		case "gem": {
			// pip/gem installs are not namespaced to dm-lens: they land in the
			// user's site/gem dirs, so there is no dm-lens-owned directory to look
 ...
 * no manifest dm-lens can read, so the binary itself is the only source. Costs
 ...
 * `TOOLS_DIR`: the `DM_LENS_DISABLE_TOOL_INSTALL` kill-switch, the
...
export async function acquireManagedInstallGate(
	...
	if (process.env.DM_LENS_DISABLE_TOOL_INSTALL === "1") {
		return {
			ok: false,
			reason: "installation disabled by DM_LENS_DISABLE_TOOL_INSTALL=1",
		};
	}
 ...
 * `DM_LENS_DISABLE_TOOL_INSTALL`, the `assertInstallAllowed` project-trust
...
/**
 * `pip` and `gem`: re-run dm-lens's own install command in upgrade form.
 *
 ...
 * dm-lens can read — so the tool's own `--version` output before and after is
 ...
 * their coordinate cannot move upstream — it moves when dm-lens bumps it.
 *
 ...
 *
 * A tool with no recorded coordinate is reinstalled once. dm-lens cannot read
...
async function refreshPinnedManagedTool(
	...
	return {
		...
		// not change, dm-lens simply had no record of it. Reporting it as a move
 ...
 * ~/.dm-lens/tools/<id>/, then writes a thin launcher shim into the managed bin
...
async function installNpmTool(
	...
	try {
		...
		try {
			...
			await writeFileAtomicAsync(
				packageJsonPath,
				JSON.stringify({ name: "dm-lens-tools", version: "1.0.0" }, null, 2),
				{ bestEffort: false },
			);
		}
		...
		const testNpmScript =
			process.env.DM_LENS_TEST_MODE === "1"
				? process.env.DM_LENS_TEST_NPM_SCRIPT
				: undefined;
...
export async function installTool(toolId: string): Promise<boolean> {
	if (process.env.DM_LENS_DISABLE_TOOL_INSTALL === "1") {
		installFailureReasons.set(
			toolId,
			"installation disabled by DM_LENS_DISABLE_TOOL_INSTALL=1",
		);
		noteInstallAttempt(
			toolId,
			"declined",
		...
		logSessionStart(
			`auto-install ${toolId}: refused — DM_LENS_DISABLE_TOOL_INSTALL=1`,
		);
		return false;
	}
	const tool = TOOLS.find((t) => t.id === toolId);
...
async function ensureToolResolved(
	...
	if (opts?.forceReinstall) {
		...
		if (process.env.DM_LENS_DISABLE_TOOL_INSTALL === "1") {
			...
			logSessionStart(
				`auto-install ensure ${toolId}: refused — DM_LENS_DISABLE_TOOL_INSTALL=1`,
			);
			return undefined;
		}
	...
	const ensurePromise = (async () => {
		...
		// caller forbids installs (allowInstall:false, e.g. DM_LENS_DISABLE_LSP_INSTALL=1)
		...
		if (process.env.DM_LENS_DISABLE_TOOL_INSTALL === "1") {
			...
			logSessionStart(
				`auto-install ensure ${toolId}: refused — DM_LENS_DISABLE_TOOL_INSTALL=1 (${Date.now() - ensureStartMs}ms)`,
			);
			return undefined;
		}
