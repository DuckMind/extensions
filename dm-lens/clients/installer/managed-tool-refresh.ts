/**
 * Periodic version refresh for dm-lens's managed tools (#1730, #1747).
 *
 ...
 *
 * `~/.dm-lens/tools/package.json` records ranges, not versions — `npm install
...
function refreshIntervalMs(): number {
	return numericEnv("DM_LENS_TOOL_REFRESH_INTERVAL_MS", DEFAULT_INTERVAL_MS);
}

function retryIntervalMs(): number {
	return numericEnv("DM_LENS_TOOL_REFRESH_RETRY_MS", DEFAULT_RETRY_INTERVAL_MS);
}

function maxPerSession(): number {
	return numericEnv(
		"DM_LENS_TOOL_REFRESH_MAX_PER_SESSION",
		DEFAULT_MAX_PER_SESSION,
	);
}

function refreshTimeoutMs(): number {
	return numericEnv("DM_LENS_INSTALL_TIMEOUT_MS", DEFAULT_TIMEOUT_MS);
}
...
export interface ManagedToolRefreshEntry {
	 ...
	 *     (there is no manifest dm-lens can read for either).
 ...
 * whatever a sibling dm-lens process wrote since this process read.
 *
 ...
 * dm-lens installs. An entry dm-lens has never installed has no version to
...
async function executeManagedToolRefresh(
	...
	if (process.env.DM_LENS_DISABLE_TOOL_REFRESH === "1") {
