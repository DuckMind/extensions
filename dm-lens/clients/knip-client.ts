 ...
 * when the project installs no knip and dm-lens's managed shim is all there is.
 *
 ...
 * Preferring dm-lens's managed shim instead makes the lens report a DIFFERENT
 ...
 * elsewhere on the user's machine, or dm-lens's own managed/PATH fallback.
 */
...
export class KnipClient {
	...
	private async runAnalyze(targetDir: string): Promise<KnipResult> {
		// Cache dir is routed through dm-lens's project-data-dir convention (NOT
		...
		// covered by the existing `.dm-lens/` gitignore entry.
		//
		...
		//    verdict dm-lens then renders as fresh. Fixed upstream in knip 6.28.0
