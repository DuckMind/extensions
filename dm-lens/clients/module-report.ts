...
export async function moduleReport(
	...
	if (graphFileCap !== undefined) {
		warnings.push(
			`who-uses-this is unavailable: review graph disabled because the project ` +
				...
				"maxProjectFiles in .dm-lens.json or set " +
				"DM_LENS_REVIEW_GRAPH_MAX_FILES; for CI/cron, run npx dm-lens " +
	...
	// the second case actionable: a rebuild (dmlens_rebuild) would populate it.
	if (graph && !hasGraphNode) {
		warnings.push(
			"who-uses-this is unavailable for this file: the cached review graph " +
				...
				"was last built). Run dmlens_rebuild to refresh it.",
		);
	}
