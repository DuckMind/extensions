...
const NAV_CLIENT_WAIT_TIMEOUT_MS = Math.max(
	0,
	Number.parseInt(process.env.DM_LENS_LSP_NAV_CLIENT_WAIT_MS ?? "1500", 10) ||
		1500,
);
const TOUCH_DEBOUNCE_MS = Math.max(
	0,
	Number.parseInt(process.env.DM_LENS_LSP_TOUCH_DEBOUNCE_MS ?? "1500", 10) ||
		1500,
);
...
const RENAME_NOTIFY_TIMEOUT_MS = Math.max(
	50,
	Number.parseInt(
		process.env.DM_LENS_LSP_RENAME_NOTIFY_TIMEOUT_MS ?? "1500",
		10,
	) || 1500,
);
...
export function getTypeScriptIdleEvictMs(): number {
	const parsed = Number.parseInt(
		process.env.DM_LENS_TS_IDLE_EVICT_MS ?? "",
		10,
	);
...
export function getLspClientCeiling(): number {
	const parsed = Number.parseInt(
		process.env.DM_LENS_LSP_CLIENT_CEILING ?? "",
		10,
	);
	return Number.isSafeInteger(parsed) && parsed > 0
		? parsed
...
function warmupTimeoutMs(): number {
	const raw = Number.parseInt(
		process.env.DM_LENS_LSP_WARMUP_TIMEOUT_MS ?? "",
		10,
	);
...
function warmupRetryBackoffMs(): number {
	const raw = Number.parseInt(
		process.env.DM_LENS_LSP_WARMUP_RETRY_BACKOFF_MS ?? "",
		10,
	);
...
/**
 * Read the `DM_LENS_LSP_DIAGNOSTICS_MAX_WAIT_MS` env override at call time
...
function readEnvDiagnosticsWaitMs(): number | undefined {
	const raw = process.env.DM_LENS_LSP_DIAGNOSTICS_MAX_WAIT_MS;
...
function readTsserverSyncGraceMs(): number {
	const raw = process.env.DM_LENS_TSSERVER_SYNC_GRACE_MS;
...
/**
 * Read the `DM_LENS_AUX_GRACE_MS` env override at call time (not module
...
function readEnvAuxGraceMs(): number | undefined {
	const raw = process.env.DM_LENS_AUX_GRACE_MS;
	if (raw === undefined) return undefined;
	const parsed = Number.parseInt(raw, 10);
	if (!Number.isFinite(parsed) || parsed < 0) return undefined;
	return parsed;
...
const DIAGNOSTICS_SEMANTIC_SETTLE_THRESHOLD_MS = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_LSP_DIAGNOSTICS_SEMANTIC_THRESHOLD_MS ?? "250",
		10,
	) || 250,
);
const DIAGNOSTICS_SEMANTIC_SETTLE_WAIT_MS = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_LSP_DIAGNOSTICS_SEMANTIC_SETTLE_MS ?? "400",
		10,
	) || 400,
);
...
const EARLY_UNBLOCK_GRACE_MS = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_LSP_EARLY_UNBLOCK_GRACE_MS ?? "400",
		10,
	) || 400,
);
...
export interface LSPTouchFileOptions {
	 ...
	 *   1. `DM_LENS_LSP_DIAGNOSTICS_MAX_WAIT_MS` env var (user override)
	 *   2. this option
...
const WORKSPACE_SWEEP_PREOPEN_CHUNK_SIZE = (() => {
	const raw = Number(process.env.DM_LENS_LSP_WORKSPACE_PREOPEN_CHUNK);
...
function notifyWriteBudgetMs(): number {
	const raw = Number(process.env.DM_LENS_LSP_NOTIFY_BUDGET_MS);
...
function notifyWedgedCapMs(): number {
	const raw = Number(process.env.DM_LENS_LSP_NOTIFY_WEDGED_CAP_MS);
...
function notifyStallCpuSampleMs(): number {
	const raw = Number(process.env.DM_LENS_LSP_NOTIFY_STALL_CPU_SAMPLE_MS);
...
function auxNotifyInflightLimit(info: LSPServerInfo): number {
	...
	const raw = Number(process.env.DM_LENS_LSP_AUX_NOTIFY_INFLIGHT);
...
function workspacePullBudgetMs(): number {
	const raw = Number(process.env.DM_LENS_LSP_WORKSPACE_PULL_MS);
...
function getMaxWorkspaceDiagnosticFiles(): number {
	const override = Number.parseInt(
		process.env.DM_LENS_LSP_WORKSPACE_MAX_FILES ?? "",
		10,
	);
 ...
 * project's `.dm-lens.json` / `.gitignore` patterns via `getProjectIgnoreMatcher`.
...
export class LSPService {
	...
	private shouldAllowInstall(serverId: string): boolean {
		...
		return process.env.DM_LENS_DISABLE_LSP_INSTALL !== "1";
	}
		...
		try {
			...
			if (this.isDestroyed) {
				try {
					...
					// dm-lens-ignore: missing-error-propagation — best-effort kill on aborted spawn
				}
				logSessionStart(
		...
		try {
			...
			if (diagnosticsMode !== "none") {
				...
				if (tsserverSyncEligible) {
					...
					// confirm. The grace (default 300ms, DM_LENS_TSSERVER_SYNC_GRACE_MS)
	...
	async getDiagnostics(
		...
		// settles, capped by the DM_LENS_AUX_GRACE_MS global ceiling (default
	 ...
	 * OWN generous budget (`warmupTimeoutMs`/`DM_LENS_LSP_WARMUP_TIMEOUT_MS`
	...
	private async runWorkspaceDiagnosticsSwept(
		...
		const perFileMs = (() => {
			const raw = Number(process.env.DM_LENS_LSP_WORKSPACE_PER_FILE_MS);
		...
		const workspacePullEnabled = process.env.DM_LENS_LSP_WORKSPACE_PULL === "1";
		...
		// for the existing `scripts/analyze-dm-lens-logs.mjs` consumer) — a
