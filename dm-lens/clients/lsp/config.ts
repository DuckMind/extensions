/**
 * LSP Configuration for dm-lens
 *
 * Allows users to define custom LSP servers and override initialization options
 * for built-in servers via configuration.
 *
 * CANONICAL LOCATION (#2426): the `lsp` namespace of `.dm-lens.json` (project)
 * and `~/.dm-lens/config.json` (machine-global). `.dm-lens/lsp.json`,
 * `pi-lsp.json` and `~/.dm-lens/lsp.json` — and the four LSP keys at the ROOT
 ...
 *
 * Example — custom server (canonical spelling, inside `.dm-lens.json`):
 * {
 *   "lsp": {
 *     "servers": {
 *       "my-server": {
...
import { getDmLensGlobalConfigPath } from "../lens-config.js";
...
/**
 * Load LSP configuration — a PROJECTION of the one resolved dm-lens config.
 *
 ...
 * 2. THE CANONICAL FILE WINS. `.dm-lens.json` used to LOSE to a leftover
 *    `.dm-lens/lsp.json` in the same directory, which made the migration users
 ...
 *    is the same nearest-wins-per-field rule `.dm-lens.json`'s `ignore` has had
 *    since #783.
 *
 ...
 * `summarizeConfigResolution`, the same projection `dmlens_effective_config`
 ...
 * analyzer flagged a real session every time under `DM_LENS_STARTUP_MODE=quick`
 ...
 *
 * Written unconditionally — before {@link resolveDmLensConfig} can throw —
...
export async function loadLSPConfig(
	...
	const resolution = resolveDmLensConfig({
		cwd,
		globalDir: getGlobalDmLensDir(),
		// `homeDir` is threaded, not dropped: it is the `$HOME` this call resolves
		// against, and the canonical global config is `$HOME/.dm-lens/config.json`
		// whenever `DM_LENS_CONFIG_PATH` does not override it. Production behavior
		...
		// can exercise a relocated `DM_LENS_HOME` without reaching the real one.
		globalConfigPath: getDmLensGlobalConfigPath(homeDir),
		homeDir,
		// The subsystem comes from the failing DOCUMENT, not from this loader
		// (#2445). This resolution opens `~/.dm-lens/config.json` and
		// `.dm-lens.json` as well as the LSP-scoped files, and reporting all of
		// them as `lsp-config` announced an "invalid LSP config" for a file whose
		// contents are dm-lens settings. An LSP-scoped file still reports here.
	...
	// loader's report with the dm-lens loaders' report of the SAME record into
	// one notice. Filtering here (as round 2 did) silently dropped a dm-lens-
	// owned record from a document only this multi-file resolution discovered.
	if (reporting) reportDmLensConfigRecords(resolution.records);
 ...
 * the gates and run a SECOND `resolveDmLensConfig` for the provenance, at a
	...
	if (config.servers) {
		for (const [id, serverConfig] of Object.entries(config.servers)) {
			try {
				...
				// dm-lens-ignore: missing-error-propagation — per-server registration, skip bad entries
			}
		}
	}
 ...
 * API the moment `dmlens_effective_config` renders it, so a new member arrives
