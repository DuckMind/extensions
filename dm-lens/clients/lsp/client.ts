...
export interface LSPClientInfo {
	 ...
	 *  the full advertised surface (incl. providers dm-lens does not parse). */
...
export const CLIENT_CAPABILITIES = {
	...
	// #974: workDoneProgress is intentionally NOT advertised. dm-lens never
	...
	// progress tokens dm-lens will silently ignore — and opengrep's
...
const NAV_REQUEST_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_NAV_REQUEST_TIMEOUT_MS",
	10_000,
...
const DIAGNOSTICS_WAIT_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_DIAGNOSTICS_WAIT_MS",
	10_000,
);
const PULL_DIAGNOSTICS_RETRY_INTERVAL_MS = positiveIntFromEnv(
	"DM_LENS_LSP_PULL_RETRY_INTERVAL_MS",
	250,
);
...
const PULL_REQUEST_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_PULL_REQUEST_TIMEOUT_MS",
	10_000,
);
...
const SHUTDOWN_REQUEST_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_SHUTDOWN_TIMEOUT_MS",
	1000,
);
...
const EXIT_NOTIFY_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_EXIT_NOTIFY_TIMEOUT_MS",
	1000,
);
...
const LIVENESS_PING_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_LIVENESS_PING_TIMEOUT_MS",
	300,
);
...
const LIVENESS_PING_QUERY = "__dm_lens_liveness_ping__";
...
const EXECUTE_COMMAND_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_EXECUTE_COMMAND_TIMEOUT_MS",
	30_000,
);
...
const PROBE_COMMAND_TIMEOUT_MS = positiveIntFromEnv(
	"DM_LENS_LSP_PROJECT_IDENTITY_PROBE_TIMEOUT_MS",
	2_500,
);
...
export interface LSPClientState {
	 ...
	 *  `{ text }` shape dm-lens has always sent — an absent value never changes
 ...
 * only the clients this MODULE EVALUATION spawned. pi evaluates the dm-lens
 ...
 * whole-document `{ text }` event, the shape dm-lens has always sent.
 *
...
async function pullDiagnosticSource(
	...
	try {
		...
		// a dm-lens-opened document is exactly that last-sent payload.
...
// DM_LENS_LSP_NAV_STALE_DROP=0 disables it if it ever over-drops.
function navStaleDropEnabled(): boolean {
	return process.env.DM_LENS_LSP_NAV_STALE_DROP !== "0";
}
 ...
 * (#1969). dm-lens must not keep pushing an unimplemented method at a server
 ...
 * is still a reply. What changes is that dm-lens no longer PROVOKES that error
 * on every probe.
 */
...
async function safeSendRequest<T>(
	...
	try {
		...
		// empty — correctness-under-edit is dm-lens's whole hot path (#238 Item 2).
...
function fileOperationUriCandidates(uri: string): string[] {
	...
	// dm-lens performs local filesystem renames only. Even when a caller supplies
