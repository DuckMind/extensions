 ...
 *     `DM_LENS_LATE_AUX_REARM_TTL_MS`) after the LAST re-arm, so a scan
 ...
 * forever. Overridable via `DM_LENS_LATE_AUX_REARM_TTL_MS`.
 */
export const DEFAULT_LATE_AUX_REARM_TTL_MS = 5 * 60_000;
export const MAX_LATE_AUX_REARMS = 8;

/**
 * Read the `DM_LENS_LATE_AUX_REARM_TTL_MS` env override at call time (not
...
export function readLateAuxRearmTtlMs(): number {
	const raw = process.env.DM_LENS_LATE_AUX_REARM_TTL_MS;
