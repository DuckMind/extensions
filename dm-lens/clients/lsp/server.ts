/**
 * LSP Server Definitions for dm-lens
 *
...
const DIRECT_LSP_NEGATIVE_TTL_MS = Math.max(
	30_000,
	Number.parseInt(
		process.env.DM_LENS_DIRECT_LSP_NEGATIVE_TTL_MS ?? "600000",
		10,
	) || 600_000,
);
...
const DM_LENS_BIN_DIR = path.join(getGlobalDmLensDir(), "bin");
...
> {
	...
	// #2140: consult the release-managed binary directory (~/.dm-lens/bin)
	...
	if (spec.managedToolId) {
		...
		if (installed) {
			try {
				...
				// the install actually landed in ~/.dm-lens/bin (github/maven/archive
...
export async function tryDotnetToolInstall(tool: string): Promise<boolean> {
	mkdirSync(DM_LENS_BIN_DIR, { recursive: true });
	const result = await safeSpawnAsync(
		"dotnet",
		["tool", "install", "--tool-path", DM_LENS_BIN_DIR, tool],
		{ timeout: 180000, ignoreAmbientSignal: true },
	);
	...
	const updateResult = await safeSpawnAsync(
		"dotnet",
		["tool", "update", "--tool-path", DM_LENS_BIN_DIR, tool],
		{ timeout: 180000, ignoreAmbientSignal: true },
	);
 ...
 * then process.cwd() as a last-resort fallback, then dm-lens managed
...
function dotnetToolCandidates(tool: string): string[] {
	const home = os.homedir();
	return [
		path.join(DM_LENS_BIN_DIR, `${tool}.exe`),
		path.join(DM_LENS_BIN_DIR, tool),
...
/**
 * Try to install a gem to the dm-lens bin dir. Resolves true if the install succeeded.
 */
export async function tryGemInstall(gem: string): Promise<boolean> {
	const { join } = await import("node:path");
	const binDir = join(getGlobalDmLensDir(), "bin");
...
export const PythonServer: LSPServerInfo = {
	...
	async spawn(root, options) {
		...
		// candidates before a bare command can resolve to dm-lens's managed bin or
		...
		// PATH includes dm-lens-managed bins before the inherited global PATH.
...
export const PHPServer: LSPServerInfo = {
	...
	async spawn(root, options) {
		...
		return {
			...result,
			initialization: {
				storagePath: path.join(getGlobalDmLensDir(), "intelephense"),
			},
		};
	},
};
...
function buildPsesArgs(bundleDir: string): string[] {
	...
	// are under ~/.dm-lens, no spaces). -Stdio makes PSES speak LSP over this
...
// launches with `--config <shipped baseline>` so dm-lens's bundled ruleset runs
...
export const AstGrepServer: LSPServerInfo = {
	...
	async spawn(root, options) {
		...
		// cwd). Otherwise point `--config` at dm-lens's shipped baseline ruleset.
