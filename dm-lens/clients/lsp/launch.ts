...
const SESSIONSTART_LOG_DIR = getGlobalDmLensLogDir();
const SESSIONSTART_LOG = path.join(SESSIONSTART_LOG_DIR, "sessionstart.log");
const DM_LENS_BIN_DIR = path.join(getGlobalDmLensDir(), "bin");
const DM_LENS_TOOLS_BIN_DIR = path.join(
	getGlobalDmLensDir(),
	"tools",
	"node_modules",
	".bin",
...
async function buildAugmentedPath(basePath?: string): Promise<string> {
	...
	if (isWindows) {
		...
		candidates.push(DM_LENS_BIN_DIR);
		candidates.push(DM_LENS_TOOLS_BIN_DIR);
