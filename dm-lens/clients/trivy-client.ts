/**
 * Trivy client for dm-lens — dependency CVE detection (Phase 1 of #131).
 *
 ...
 * filesystem walk. dm-lens has zero dependency-CVE coverage otherwise.
 *
 ...
 * `trivy.enabled: true` in `.dm-lens.json` AND declare a scannable dependency
 ...
 * in `.dm-lens.json` (the loader walks up, so a `~/.dm-lens.json` enables it
...
/**
 * Resolve the `--severity` list from the project's `dm-lens.trivy.minSeverity`
...
export class TrivyClient extends SecurityScanClient<TrivyResult> {
		...
		if (!shouldScanTrivy(targetDir)) {
			return {
				...
				summary: hasAnyDependencyManifest(targetDir)
					? "trivy not enabled (set trivy.enabled in .dm-lens.json)"
	...
	private async runScan(cwd: string): Promise<TrivyResult> {
		...
		const outDir = mkdtempSync(path.join(os.tmpdir(), "dm-lens-trivy-"));
		...
		try {
			...
			// ITSELF (unlike knip/jscpd, which dm-lens's own `EXCLUDED_DIRS`-aware
