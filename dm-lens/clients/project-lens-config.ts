/**
 * Project-level `.dm-lens.json` config loader.
 *
 * Reads an optional `.dm-lens.json` (or `dm-lens.json`) at the project root and
 * surfaces the fields the rest of dm-lens honors:
 *
 ...
 *     `DM_LENS_REVIEW_GRAPH_MAX_FILES` env override still wins outright over
 *     both, unchanged).
 *
 *   - `format.enabled`, `autofix.enabled`, and
 *     `actionableWarnings.autoFix.enabled` — project-owned mutation controls.
 *     These can disable dm-lens writes while leaving diagnostics enabled.
 *
 ...
 * `.dm-lens.json` at the repo root work without per-subdir configs).
 *
 ...
 *
 * `findDmLensConfigInDir` / `loadDmLensConfigInDir` are the per-directory
 * (no upward walk) counterparts used by `file-utils.ts`'s
 * `getProjectIgnoreMatcher` to layer NESTED `.dm-lens.json` `ignore` fields
 ...
 * its own config file, so a package-local `.dm-lens.json`'s `ignore`
...
import {
	...
	reportDmLensConfigRecords,
	resolveOneDmLensConfigDocument,
...
import { findDmLensConfigMarkerInDir } from "./workspace-topology.js";
...
export interface DmLensProjectRuleConfig {
	 ...
	 * `.dm-lens.json` deliberately turns off. Output-only filtering (the
...
export interface DmLensProjectMutationConfig {
	/** Whether this mutation path is enabled for the project. */
	enabled?: boolean;
}

export interface DmLensProjectReviewGraphConfig {
	/**
...
export interface DmLensProjectConfig {
	 ...
	 */
	reviewGraph?: DmLensProjectReviewGraphConfig;
...
export const EMPTY_PROJECT_CONFIG: DmLensProjectConfig = {
	ignore: [],
	rules: {},
...
interface DiscoveryCacheEntry {
	info: DmLensProjectConfigFileInfo | undefined;
...
/**
 * The shared body of `loadDmLensProjectConfig` and `loadDmLensConfigInDir`:
...
function loadCachedConfigFile(
	info: DmLensProjectConfigFileInfo,
): DmLensProjectConfig {
 ...
 * global-only dm-lens setting` as an LSP-config problem, which is the opposite
 * of what it says.
 */
function replayConfigNotices(entry: {
	...
	reportDmLensConfigRecords(entry.records);
...
/**
 * Walk up from `startDir` looking for a `.dm-lens.json` or `dm-lens.json`.
 * Returns the parsed config, or an empty config if none was found.
 */
export function loadDmLensProjectConfig(
	startDir: string,
	preloadedInfo?: DmLensProjectConfigFileInfo,
): DmLensProjectConfig {
	...
	// `dm-lens.json` sitting BESIDE or ABOVE the canonical `.dm-lens.json` this
	...
	// (#449, routine) that loader never runs, and the dm-lens-OWNED records for
	// a dm-lens-owned file were then produced by nobody at all. The dm-lens
	...
	reportDmLensConfigRecords(entry.legacyRecords);
...
export interface DmLensProjectConfigFileInfo {
	path: string;
	dir: string;
	mtimeMs: number;
...
/**
 * Look for a `.dm-lens.json`/`dm-lens.json` directly IN `dir` — no upward
 ...
 * file, independent of whatever config `loadDmLensProjectConfig`'s upward
 ...
 */
export function findDmLensConfigInDir(
	dir: string,
): DmLensProjectConfigFileInfo | undefined {
	const marker = findDmLensConfigMarkerInDir(dir);
...
export function findNestedProjectMutationValue(
	...
	for (const dir of walkUpDirs(start)) {
		...
		const config = loadDmLensConfigInDir(dir);
...
/**
 * Load the `.dm-lens.json`/`dm-lens.json` directly IN `dir` (no upward
 * walk) — the per-directory counterpart to `loadDmLensProjectConfig`'s
 ...
 */
export function loadDmLensConfigInDir(dir: string): DmLensProjectConfig {
	const info = findDmLensConfigInDir(dir);
...
function discoveryEntryFor(startDir: string): DiscoveryCacheEntry {
	...
	const discovered = discoverDmLensProjectConfig(cacheKey);
	discoveryCache.set(cacheKey, discovered);
	return discovered;
}

export function findDmLensProjectConfig(
	startDir: string,
): DmLensProjectConfigFileInfo | undefined {
 ...
 *
 * Split out of `findDmLensProjectConfig` so a caller that already holds the
...
function discoveryCacheStillFresh(entry: DiscoveryCacheEntry): boolean {
	...
	// `discoverDmLensProjectConfig`), so a config created anywhere in it is
...
function discoverDmLensProjectConfig(startDir: string): DiscoveryCacheEntry {
	const dirMtimes: Array<{ dir: string; mtimeMs: number }> = [];
	const legacyDocuments: ConfigDocument[] = [];
	const legacySources: FileStamp[] = [];
	let info: DmLensProjectConfigFileInfo | undefined;
	...
	// filesystem root before, so a `.dm-lens.json` in `$HOME` (or at `C:\`) was
 ...
 * count is user input, so one 28-key `dm-lens.json` produced 28 notifications
...
function parseConfigFile(configPath: string): ParsedConfigFile {
	...
	const resolved = resolveOneDmLensConfigDocument({
		tier: "project",
		file: configPath,
	...
	const rules: Record<string, DmLensProjectRuleConfig> = {};
	if (obj.rules && typeof obj.rules === "object" && !Array.isArray(obj.rules)) {
		...
		for (const [ruleId, ruleCfg] of Object.entries(rawRules)) {
			...
			const entry: DmLensProjectRuleConfig = {};
			if (
	...
	let reviewGraph: DmLensProjectReviewGraphConfig | undefined;
	...
	// shared `.dm-lens.json` (e.g. `maxProjectFile`, `lps`) produces a signal
	...
	// under `DM_LENS_CONFIG_PATH` the canonical global config is NOT
	// `~/.dm-lens/config.json`, and a hardcoded string in this notice would name
	...
	const warnUnhonoredKey = (label: string, globalOnly: boolean): void => {
		note(
			globalOnly
				? `"${label}" is a global-only dm-lens setting and is not honored in a project .dm-lens.json (set it in ${globalConfigLabel} or pass the matching CLI flag); ignored`
				: `unknown key "${label}" is not a recognized dm-lens setting (check for a typo); ignored`,
		);
	};
