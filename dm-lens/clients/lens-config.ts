...
export { getDmLensGlobalConfigPath } from "./config-locations.js";
import {
...
export type DmLensFormatMode = "deferred" | "immediate";

/** The `{ enabled?: boolean }` section every registry flag key lives under. */
export interface DmLensToggleConfig {
	enabled?: boolean;
}

export interface DmLensGlobalConfig {
	/**
	 * Gitignore-style patterns excluded from dm-lens scans across ALL projects.
	 * Merged at LOWEST precedence: a project `.gitignore` or `.dm-lens.json`
	...
	dispatch?: {
		 ...
		 * any runner's declared budget. Also overridable via DM_LENS_RUNNER_TIMEOUT_FLOOR_MS.
		 */
	...
	contextInjection?: {
		/**
		 * Whether dm-lens prepends automatic findings (session-start guidance,
...
export function loadDmLensGlobalConfig(
	configPath = getDmLensGlobalConfigPath(),
): DmLensGlobalConfig | undefined {
	const location = globalCanonicalLocation();
	// #2445: this used to be `JSON.parse(fs.readFileSync(...))` inside a bare
	// `catch { return undefined; }`, so a malformed `~/.dm-lens/config.json`
	...
	// `~/.dm-lens/config.json` while the project loader's identical scan of an
	...
	try {
		...
		// `~/.dm-lens/config.json` exactly as they do to `.dm-lens.json`. The
		...
		const resolved = resolveOneDmLensConfigDocument(document);
		// EVERY record this document produced (#2426 review round 3, F1) — not
		// filtered to what this loader "owns". `reportDmLensConfigRecords` derives
		...
		reportDmLensConfigRecords(resolved.records);
		...
		// `.dm-lens.json` — where `PROJECT_FOREIGN_CONFIG_NAMESPACES` has
		// tolerated them since #2426. The global scan never got the same
		// treatment, so `~/.dm-lens/config.json` with a root `warmFiles` both
		...
		for (const key of Object.keys(raw)) {
			if (!knownGlobalConfigKeys.has(key)) {
				warnInvalid(
					`unknown key "${key}" is not a recognized dm-lens setting (check for a typo); ignored`,
				);
			}
		}

		return config as DmLensGlobalConfig;
	} catch (error) {
		...
		// and no notification: dm-lens ran on defaults and said nothing, which is
		...
		reportDmLensConfigRecords(notedRecords());
	}
}

export function getGlobalIgnorePatterns(configPath?: string): string[] {
	return loadDmLensGlobalConfig(configPath)?.ignore ?? [];
}

export function getGlobalWidgetDefaultVisible(configPath?: string): boolean {
	return loadDmLensGlobalConfig(configPath)?.widget?.visible !== false;
}
...
export function getGlobalActionableWarningMaxFixes(
	...
	return loadDmLensGlobalConfig(configPath)?.actionableWarnings?.autoFix
		?.maxFixes;
}

/** Which tier decided a resolved flag's value — for provenance in debug/skip logs (#792). */
export type DmLensFlagSource =
	| "env"
	| "cli"
	| "project"
...
export interface ResolvedDmLensFlag {
	value: boolean | string | undefined;
	source: DmLensFlagSource;
}

/**
 * Resolve a flag AND report which config tier decided it — same precedence
 * as {@link resolveDmLensFlag} (which now delegates here), just also
 ...
 */
export function resolveDmLensFlagWithSource(
	name: string,
	value: boolean | string | undefined,
	config: DmLensGlobalConfig | undefined,
	projectConfig?: DmLensProjectConfig,
...
export function resolveDmLensFlag(
	name: string,
	value: boolean | string | undefined,
	config: DmLensGlobalConfig | undefined,
	projectConfig?: DmLensProjectConfig,
