...
const VALIDATION_SNIPPETS: Record<string, { ext: string; source: string }> = {
	...
	lua: { ext: "lua", source: "local dm_lens_validate = 1\n" },
	php: { ext: "php", source: "<?php $piLensValidate = 1;\n" },
	python: { ext: "py", source: "dm_lens_validate = 1\n" },
	ruby: { ext: "rb", source: "dm_lens_validate = 1\n" },
...
export class AstGrepClient {
	...
	async dumpAst(
		...
		const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-lens-ast-dump-"));
	...
	async validatePattern(
		...
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "dm-lens-sg-validate-"),
		);
	...
	async validateRule(
		...
		const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dm-lens-sg-rule-"));
		try {
			fs.writeFileSync(
	...
	async replace(
		...
		// describes it — dm-lens's own tool was as invisible to the mutation
		...
		// throws, and a missing bridge (dm-lens not activated, guard disabled) is
		// a silent no-op.
