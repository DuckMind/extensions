/**
 * THE canonical dm-lens config schema (#2426).
 *
 ...
 * `docs/schema/dm-lens-config-v1.json`, so there is no compiler between the two
 * for drift to hide in. It is the FIRST real schema through the #2418 harness —
 * `tests/config/dm-lens-config-schema.test.ts` runs `assertSchemaStabilityTiers`
 ...
 *
 * 1. THE ROOT IS OPEN (`additionalProperties: true`). dm-lens's default is
 ...
 * CHARACTER. `tests/config/dm-lens-config-schema.test.ts` asserts every
 ...
 * measurable: with `~/.dm-lens/config.json` saying
 * `lsp.disabledServers: ["typos"]` and a repository's `.dm-lens.json` saying
...
function buildConfigSchema(): ConfigSchemaNode {
	...
	// Every top-level section dm-lens recognizes today, derived from the
...
export const DM_LENS_CONFIG_SCHEMA: ConfigSchemaNode = buildConfigSchema();
