/**
 * Generic mutation-recording bridge for dm-lens (#2423).
 *
 ...
 * to dm-lens's internal state, so the bridge is not a security boundary. What
 ...
 * ## What it is for
 *
 * `clients/mutating-tool.ts` classifies mutations dm-lens SEES as tool events.
 * A producer that writes a file some other way — a co-process extension with
 * its own registered tool, or dm-lens's own `ast_grep_replace` running
 ...
 * ────────────────────────
 *
 *   const bridge = (globalThis as any)[Symbol.for("dm-lens:mutation-bridge")];
 ...
 * recognize is unsupported. Calling before dm-lens is loaded, or when the guard
 * is disabled, is safe: the bridge is absent or the call is dropped.
 *
 * `recordMutation` returns `true` when dm-lens took the record and `false` when
 * it dropped it (malformed payload, out-of-scope path, or a bookkeeping error),
 * so a producer can count its own drops.
 *
 * Protocol (registration side, internal to dm-lens)
 * ──────────────────────────────────────────────────
...
export interface MutationBridge {
	 ...
	 */
	readonly version: 1;
	/** `true` when dm-lens recorded the mutation, `false` when it dropped it. */
...
export function recordMutationThroughSeam(
	...
	try {
		// 1. Staleness stamp: the file changed under dm-lens, so a later edit is
		...
		// (#2465). "dm-lens accounted for this write" and "dm-lens will also
		...
		// still a write dm-lens recorded. Move this inside the guard and every
...
/**
 * The mounted bridge, or `undefined` when dm-lens has not registered one (or
