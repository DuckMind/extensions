/**
 * Tree-sitter Structural Search Client for dm-lens
 *
...
export class TreeSitterClient {
	...
	private queryCacheCap(): number {
		const value = Number.parseInt(
			process.env.DM_LENS_TREE_SITTER_QUERY_CACHE_CAP ?? "",
			10,
		);
	...
	private queryBatchCacheCap(): number {
		const value = Number.parseInt(
			process.env.DM_LENS_TREE_SITTER_QUERY_BATCH_CACHE_CAP ?? "",
			10,
		);
		return Number.isSafeInteger(value) && value > 0
			? value
	...
	/**
	 * The `grammars/` dir bundled inside the dm-lens package (the core grammars
	...
	private reportPoisonedGrammarFile(
		...
		logTreeSitterDiagnostic({
			...
			message:
				...
				`dm-lens will treat the grammar as missing and try to fetch it again.`,
	...
	private reportStaleGrammarVersion(
		...
		logTreeSitterDiagnostic({
			...
			message:
				...
				`moved since this file was downloaded. dm-lens will treat the grammar as ` +
	...
	private findGrammarsDir(deps = this.grammarDirResolutionDeps): string {
		...
		// (it is not a dm-lens dependency — grammars ship bundled / lazy-fetched).
		try {
	...
	private async ensureGrammar(grammarFile: string): Promise<boolean> {
		...
		if (!assertInstallAllowed(`tree-sitter grammar fetch: ${grammarFile}`)) {
			...
			if (!this.trustBlockedGrammarNotifications.has(grammarFile)) {
				...
				notifyUserDegradation(`dm-lens: ${unavailable}`);
			}
			return false;
		}
	...
	private recordGrammarFailure(
		...
		const unavailable = retryable
			? `tree-sitter grammar '${grammarFile}' is unavailable — symbol search, ` +
				...
				`${detail} dm-lens will retry automatically in ${Math.round((retryDelayMs as number) / 1000)}s; ` +
	 ...
	 * across every language while dm-lens reports "no issues found." Fail loud (log
