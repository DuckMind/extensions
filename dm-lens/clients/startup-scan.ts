 ...
 *
 * Prevents dm-lens from scanning $HOME or generic directories at session
 ...
 *
 * Resolution order: `DM_LENS_STARTUP_SCAN_VERDICT_TTL_MS` env var, else the
...
const _ttl = lazyEnvNumber(
	"DM_LENS_STARTUP_SCAN_VERDICT_TTL_MS",
 ...
 *
 * Resolution order: `DM_LENS_STARTUP_SCAN_MAX_ENTRIES` env var, else the
...
const _maxEntries = lazyEnvNumber(
	"DM_LENS_STARTUP_SCAN_MAX_ENTRIES",
 ...
 * `project-snapshot.json` field never fires for them, because dm-lens never
