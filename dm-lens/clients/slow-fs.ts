 ...
 *
 * Escape hatches:
 *   - `DM_LENS_ALLOW_SLOW_FS_SCAN=1` — disable slow-FS mode entirely (full
 *     normal behavior even when the probe would flag the FS as slow).
 *   - `DM_LENS_FORCE_SLOW_FS=1` — force slow-FS mode on regardless of the
 *     probe (testing/CI, or a user who knows their FS is slow but the probe
 *     under-fires).
 *   - `DM_LENS_SLOW_FS_THRESHOLD_US` — override the median-microseconds
...
function resolveThresholdMicros(): number {
	const envValue = toPositiveFinite(process.env.DM_LENS_SLOW_FS_THRESHOLD_US);
 ...
 *   1. `DM_LENS_ALLOW_SLOW_FS_SCAN=1` — kill switch, always false.
 *   2. `DM_LENS_FORCE_SLOW_FS=1` — always true, probe skipped entirely.
 *   3. Measured probe (memoized per cwd for the process lifetime).
 */
export function getSlowFsVerdict(cwd: string): SlowFsProbeResult {
	if (process.env.DM_LENS_ALLOW_SLOW_FS_SCAN === "1") {
		return { slow: false, medianStatMicros: 0, samples: 0 };
	}
	if (process.env.DM_LENS_FORCE_SLOW_FS === "1") {
...
export function slowFsDegradationNotice(): string {
	return "slow filesystem detected (set DM_LENS_ALLOW_SLOW_FS_SCAN=1 to override)";
}
