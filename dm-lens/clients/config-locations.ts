/**
 * WHERE dm-lens config lives, and in what order (#2426).
 *
 ...
 *
 *   `.dm-lens.json`            — project, nearest-package-wins per field
 *   `~/.dm-lens/config.json`   — machine-global
 *
...
export const CANONICAL_PROJECT_CONFIG_FILE = ".dm-lens.json";

/** Basename of the one canonical GLOBAL config file inside `~/.dm-lens/`. */
...
const GLOBAL_SURFACE_PREFIX = "~/.dm-lens/";
...
export interface ConfigLocation {
	 ...
	 * rather than dm-lens config sections — i.e. the whole file is what the
 ...
 * `.dm-lens/lsp.json`, `.dm-lens.json`, `pi-lsp.json` and took the FIRST hit,
 * so a leftover `.dm-lens/lsp.json` silently beat the file the user was being
...
export const PROJECT_CONFIG_LOCATIONS: readonly ConfigLocation[] = [
	legacyLocation("dm-lens.json", "dm-lens.json"),
	legacyLocation("pi-lsp.json", "pi-lsp.json"),
	legacyLocation(path.posix.join(".dm-lens", "lsp.json"), ".dm-lens/lsp.json"),
	{
 ...
 * settings, not the dm-lens config sections these two probes project.
 */
 ...
 * `~/.dm-lens/`, never by walking into `$HOME` and finding it.
 */
...
/**
 * The absolute path of the canonical GLOBAL config — `~/.dm-lens/config.json`,
 * or `DM_LENS_CONFIG_PATH` when it overrides that.
 *
 ...
 * "WHERE dm-lens config lives", and `project-lens-config.ts` needs this exact
 * function — not a re-derivation of `DM_LENS_CONFIG_PATH` — to name the file a
 ...
 */
export function getDmLensGlobalConfigPath(
	homeDir: string = os.homedir(),
): string {
	const override = process.env.DM_LENS_CONFIG_PATH;
	if (override) return path.resolve(override);
	return path.join(homeDir, ".dm-lens", CANONICAL_GLOBAL_CONFIG_FILE);
}
