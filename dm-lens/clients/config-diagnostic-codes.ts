/**
 * The stability registry for dm-lens's PUBLIC config surfaces (#2418).
 *
...
/**
 * The reserved `$schema` URL for the unified dm-lens config envelope. Pinned
...
export const CONFIG_SCHEMA_ID =
	"https://raw.githubusercontent.com/apmantza/dm-lens/master/docs/schema/dm-lens-config-v1.json";
...
export const CONFIG_DIAGNOSTIC_CODES = {
	 ...
	 * `reportDmLensConfigRecords` -> `warnIgnoredConfigOnce`, which renders it as
	 ...
	 * `reportDmLensConfigRecords` -> `warnIgnoredConfigOnce`, which #2426 wires
	 ...
	 * dm-lens config can ever legitimately carry. A user who sees it in their own
...
/**
 * The legacy config surfaces dm-lens still reads. Each row commits to being
 ...
 * `.dm-lens.json` (project) and `~/.dm-lens/config.json` (global). Neither
 * appears here, and that is the point: a CANONICAL FILE IS NOT DEPRECATED
 * BECAUSE SOME OF ITS KEYS ARE. `.dm-lens.json` keeps being read forever; what
...
export const DEPRECATED_CONFIG_SURFACES = [
	...
	{
		...LEGACY_FILE,
		surface: ".dm-lens/lsp.json",
		reason:
			"project LSP settings consolidate into `.dm-lens.json` under the unified envelope (#2426).",
	},
	{
		...LEGACY_FILE,
		surface: "pi-lsp.json",
		reason:
			"pre-rename project location; superseded by `.dm-lens.json` (#2426).",
	},
	{
		...LEGACY_FILE,
		surface: "dm-lens.json",
		reason:
			"undotted project-config basename; superseded by `.dm-lens.json` (#2426).",
	},
	{
		...LEGACY_FILE,
		surface: "~/.dm-lens/lsp.json",
		reason:
			"global LSP settings consolidate into `~/.dm-lens/config.json` (#2426).",
	},
