...
/**
 * True when dm-lens should suppress side-effecting log writes — e.g. inside
 * the vitest test runner, or when callers explicitly set `DM_LENS_TEST_MODE=1`.
 *
 * Resolution:
 *   - `DM_LENS_TEST_MODE === "1"` → true (explicit opt-in)
 *   - `VITEST` set and `DM_LENS_TEST_MODE !== "0"` → true (vitest default, with explicit opt-out)
 *   - otherwise false
 *
 * Replaces the boolean previously duplicated verbatim in ~10 logger modules.
 */
export function isTestMode(): boolean {
	if (process.env.DM_LENS_TEST_MODE === "1") return true;
	if (process.env.VITEST && process.env.DM_LENS_TEST_MODE !== "0") return true;
	return false;
}
 ...
 *   toPositiveFinite(process.env.DM_LENS_KNOB_MS),
 ...
 * @example
 * ```ts
 * const _ttl = lazyEnvNumber("DM_LENS_STARTUP_SCAN_VERDICT_TTL_MS", DEFAULT_TTL_MS);
