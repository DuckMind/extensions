 ...
 *
 * Opt-in, transcript-persistent record of what dm-lens did: diagnostics
 ...
 * Example: "dm-lens: 3 diagnostics (eslint 2, tsserver 1) · 2 autofixed (ruff) · 1 reformatted (prettier)"
 */
export function formatTurnSummaryLine(details: TurnSummaryDetails): string {
	...
	if (parts.length === 0) return "dm-lens: turn summary (empty)";
	return `dm-lens: ${parts.join(" · ")}`;
}

export const TURN_SUMMARY_CUSTOM_TYPE = "dmlens:turn-summary";
