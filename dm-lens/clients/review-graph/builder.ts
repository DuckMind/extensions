...
function reviewGraphIdleEvictMs(): number {
	const value = Number.parseInt(
		process.env.DM_LENS_REVIEW_GRAPH_IDLE_EVICT_MS ?? "",
		10,
	);
...
function seqFastpathEnabled(): boolean {
	const raw = process.env.DM_LENS_GRAPH_SEQ_FASTPATH;
...
function graphMaxInMemoryBytes(): number {
	const raw = Number(process.env.DM_LENS_GRAPH_MAX_IN_MEMORY_BYTES);
 ...
 * the multi-second graph_build spikes during a burst of new files (dm-lens has
...
// #776: `DM_LENS_REVIEW_GRAPH_MAX_FILES` (the existing per-subsystem env
...
export function getReviewGraphMaxFiles(cwd?: string): number {
	const override = Number.parseInt(
		process.env.DM_LENS_REVIEW_GRAPH_MAX_FILES ?? "",
		10,
	);
...
function getReviewGraphMaxFileBytes(): number {
	const override = Number.parseInt(
		process.env.DM_LENS_REVIEW_GRAPH_MAX_FILE_BYTES ?? "",
		10,
	);
	return Number.isFinite(override) && override > 0
		? override
...
// enough that a repo shrink or a `.dm-lens.json`/env cap raise is noticed
// without restarting the process.
const DEFAULT_REVIEW_GRAPH_SIZE_SKIP_TTL_MS = 15 * 60_000;

const _sizeSkipTtl = lazyEnvNumber(
	"DM_LENS_REVIEW_GRAPH_SIZE_SKIP_TTL_MS",
...
export interface ReviewGraphSizeSkipVerdict {
	 ...
	 */
	sourceFileCount: number;
	/** The cap (derived `maxProjectFiles` or `DM_LENS_REVIEW_GRAPH_MAX_FILES`) that was exceeded. */
	maxFileCount: number;
...
function graphPersistDebounceMs(): number {
	const raw = Number(process.env.DM_LENS_GRAPH_PERSIST_DEBOUNCE_MS);
	return Number.isFinite(raw) && raw >= 0
		? raw
...
function graphPersistMaxElements(): number {
	const raw = Number(process.env.DM_LENS_GRAPH_PERSIST_MAX_ELEMENTS);
	return Number.isFinite(raw) && raw > 0
		? raw
	...
	const request: ReviewGraphPersistWorkerRequest = {
		...
		testDelayMs:
			process.env.NODE_ENV === "test"
				? Number(process.env.DM_LENS_TEST_PERSIST_WORKER_DELAY_MS) || undefined
				: undefined,
	};
...
function graphCheckpointEveryFiles(): number {
	const raw = Number(process.env.DM_LENS_GRAPH_CHECKPOINT_EVERY_FILES);
	return Number.isFinite(raw) && raw > 0
		? Math.floor(raw)
...
function graphCheckpointMinIntervalMs(): number {
	const raw = Number(process.env.DM_LENS_GRAPH_CHECKPOINT_MIN_INTERVAL_MS);
	return Number.isFinite(raw) && raw >= 0
		? raw
...
async function _doBuildGraph(
	...
	// through on the assumption it's already a real project root — true when DM
	// is launched inside a repo, false when DM is launched from $HOME itself and
	...
	const testStopAfter = Number(
		process.env.DM_LENS_GRAPH_CHECKPOINT_TEST_STOP_AFTER,
	);
