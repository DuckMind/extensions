/**
 * Read-Before-Edit Guard for dm-lens
 *
...
export interface ReadRecord {
	...
	// What dm-lens *delivered* (after LSP expansion, if any)
...
const OWN_EDIT_STALE_GRACE_MS = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_READ_GUARD_OWN_EDIT_GRACE_MS ?? "120000",
		10,
	) || 120000,
);
...
const READ_HASH_MAX_LINES = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_READ_GUARD_HASH_MAX_LINES ?? "3000",
		10,
	) || 3000,
);
...
const RELOCATION_WINDOW_MIN = Math.max(
	1,
	Number.parseInt(
		process.env.DM_LENS_READ_GUARD_RELOCATION_WINDOW_MIN ?? "40",
		10,
	) || 40,
);
const RELOCATION_WINDOW_PER_EDIT = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_READ_GUARD_RELOCATION_WINDOW_PER_EDIT ?? "20",
		10,
	) || 20,
);
const RELOCATION_WINDOW_MAX = Math.max(
	...
	Number.parseInt(
		process.env.DM_LENS_READ_GUARD_RELOCATION_WINDOW_MAX ?? "400",
		10,
	) || 400,
);
...
export class ReadGuard {
	...
	private idleEvictMs(): number {
		const value = Number.parseInt(
			process.env.DM_LENS_READ_GUARD_IDLE_EVICT_MS ?? "",
			10,
		);
	 ...
	 * read/edit history with nothing in `~/.dm-lens/read-guard.log` to show it
	 * happened.
	 *
	 ...
	 * grow the ledger's tally map without bound and spam `dmlens_health` in
	...
	private touchFile(filePath: string): void {
		...
		const timer = setTimeout(() => {
			...
			// dmlens_health on every healthy session. See evictFile's doc comment.
	...
	recordRead(record: ReadRecord): void {
		...
		// gated behind DM_LENS_READ_GUARD_VERBOSE — off by default. That left a
	...
	/**
	 * Whether dm-lens has any record of this path from a read or write this
	 * session (#1668). Gates external-delete detection: a path dm-lens never
	 ...
	 * the files dm-lens has actually seen this session, which the `agent_settled`
	...
	/**
	 * Drop all record of a path dm-lens confirmed no longer exists on disk
