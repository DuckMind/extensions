 ...
 * `node -e`, a throwaway `.mjs`, a harness script — has no `DM_LENS_HOME` pin,
 * so every logger and ledger it touches used to write into the maintainer's
 * REAL `~/.dm-lens`. Confirmed live on 2026-09-02: two review probes left 42
 * rows of `/p/.dm-lens.json` fixture garbage in real telemetry. When
 * `DM_LENS_HOME` is unset and the process looks like a probe — `PILENS_PROBE=1`,
 ...
 *
 * WHY IT ALL LIVES HERE and not beside its `getGlobalDmLensDir()` sibling in
 ...
 * call `getGlobalDmLensLogDir()` at their OWN module
 ...
 * redirect for every process that has `VITEST` set but no `DM_LENS_HOME` pin —
 ...
 */
const RESOLUTION_SLOT = Symbol.for("dm-lens.probe-home-state.resolution");
...
/**
 * Where dm-lens writes its LOGS, ledger rows and debug dumps: `~/.dm-lens/` by
 * default, exactly like `getGlobalDmLensDir()` in `file-utils.ts` — but
 ...
 *
 * WHY IT LIVES IN THIS MODULE and not next to its `getGlobalDmLensDir()`
 ...
 * cannot.
 *
 * `DM_LENS_HOME` wins over any redirect, which is how
 ...
 */
export function getGlobalDmLensLogDir(): string {
	const override = process.env.DM_LENS_HOME?.trim();
	if (override) return path.resolve(override);
	return resolveProbeHomeDir() ?? path.join(os.homedir(), ".dm-lens");
}
 ...
 * logging path.
 *
 * `undefined` means "no redirect applies"; `getGlobalDmLensLogDir()` above
 * falls back to the real home. This function never consults `DM_LENS_HOME` —
 ...
 * `getGlobalDmLensLogDir()` call re-resolves from scratch. Exercised by
...
function computeProbeHomeDir(cwd: string): string | undefined {
	...
	// set a test-mode marker but no DM_LENS_HOME.
	if (process.env.PILENS_PROBE === "1") {
		return path.join(cwd, ".dm-lens-probe-home");
	}
	const worktreeRoot = findAgentWorktreeRoot(cwd);
	if (worktreeRoot) return path.join(worktreeRoot, ".dm-lens-probe-home");
