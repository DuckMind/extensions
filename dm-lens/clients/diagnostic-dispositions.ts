 ...
 *                     but the mechanism is an inline `dm-lens-ignore` comment
 *                     written into the source (see suppress-writer.ts), not
 *                     just a store entry — portable, git-visible, discoverable
 *                     without dm-lens's own store. The store entry here is an
 ...
 * `dmlens:diagnostic:disposition` bus event (disposition-publish.ts) both
...
export function applyDispositions<T extends DispositionCandidate>(
	...
	return diagnostics.filter((d) => {
		...
		// Belt-and-braces: the inline `dm-lens-ignore` comment is the real
