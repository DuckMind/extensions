...
function computeTrust(
	...
	if (stale) {
		...
		notes.push(
			`Graph is stale: built ${ageMin}m ago. Sections below may miss recent edits — run dmlens_rebuild or re-analyze to refresh.`,
		);
	}
	if (lowCoverage) {
		...
		if (sizeSkip) {
			...
			return {
				...
				hint:
					...
					`(cap ${sizeSkip.maxFileCount}) — raise maxProjectFiles in .dm-lens.json ` +
					"or set DM_LENS_REVIEW_GRAPH_MAX_FILES; for CI/cron, run " +
					"npx dm-lens build-graph after configuring the cap",
