/**
 * Publishes `dmlens:format:queued`, `dmlens:format:start`, and
 * `dmlens:autofix:start` on pi's shared `pi.events` bus (#673, #684).
 *
 * Sibling to `clients/bus-publish.ts` (#482 `dmlens:files:touched`) and
 * `clients/diagnostics-publish.ts` (#502 `dmlens:diagnostics`) rather than a
 * new export crammed into either file: same emit plumbing shape and the same
 * `DM_LENS_BUS_PUBLISH` kill switch (`isBusPublishEnabled`, imported from
 ...
 * ## Motivation (#673)
 *
 * `dmlens:files:touched` only fires AFTER deferred formatting has completed —
 * it tells a same-process listener what dm-lens just mutated, in the past
 ...
 * candidate tree from the live worktree mid-turn, unaware that dm-lens has a
 ...
 *      (`dmlens:format:queued` — one event per file, only on first queue
 ...
 *      (`dmlens:format:start` — one event per batch, only when there is at
 *      least one queued file to format).
 *
 * This is deliberately visibility-only: it lets a listener know dm-lens MIGHT
 ...
 * `dmlens:files:touched` (`reason: "format"`) event; these two do not
 * duplicate it.
 *
 * ## `dmlens:autofix:start` (#684)
 *
 ...
 * It only emitted `dmlens:files:touched` (`reason: "autofix"`) AFTER the fact.
 * `dmlens:autofix:start` fires once per batch, only when the cached
 ...
 * `dmlens:format:start`'s "only when there's genuine work" gate. Note: this is
...
export const BUS_FORMAT_QUEUED_EVENT = "dmlens:format:queued";
export const BUS_FORMAT_QUEUED_VERSION = 1;

export const BUS_FORMAT_START_EVENT = "dmlens:format:start";
export const BUS_FORMAT_START_VERSION = 1;

export const BUS_AUTOFIX_START_EVENT = "dmlens:autofix:start";
...
/**
 * Publish one `dmlens:format:queued` event for a file newly entering the
...
export function publishFormatQueued(args: PublishFormatQueuedArgs): void {
	...
	try {
		...
		if (!hasLoggedQueuedFailure) {
			...
			args.dbg?.(
				`format-events-publish: dmlens:format:queued emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
...
/**
 * Publish one `dmlens:format:start` event when the `agent_end` deferred-
...
export function publishFormatStart(args: PublishFormatStartArgs): void {
	...
	try {
		...
		if (!hasLoggedStartFailure) {
			...
			args.dbg?.(
				`format-events-publish: dmlens:format:start emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
...
/**
 * Publish one `dmlens:autofix:start` event when the `agent_end`
...
export function publishAutofixStart(args: PublishAutofixStartArgs): void {
	...
	try {
		...
		if (!hasLoggedAutofixStartFailure) {
			...
			args.dbg?.(
				`format-events-publish: dmlens:autofix:start emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
