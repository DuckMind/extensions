 ...
 * marker walk (`.dm-lens.json` discovery in `project-lens-config.ts`,
 ...
 */
const DM_LENS_CONFIG_BASENAMES = PROJECT_CONFIG_BASENAMES;

/**
 * Every marker dm-lens subsystems currently probe for, keyed by the basename
...
export interface DirectoryMarkers {
	...
	/** Resolved path of `.dm-lens.json`, preferred over the no-dot fallback. */
...
function topologyIdleEvictMs(): number {
	const value = Number.parseInt(
		process.env.DM_LENS_WORKSPACE_TOPOLOGY_IDLE_EVICT_MS ?? "",
		10,
	);
 ...
 * markers dm-lens subsystems care about, cached and invalidated by the
 * directory's own mtime (so an add/remove/rename inside `dir` — which bumps
 * the directory's mtime on every platform dm-lens supports — drops the
...
export function getDirectoryMarkers(dir: string): DirectoryMarkers {
	...
	const piLensConfigPath =
		resolveMarker(DM_LENS_CONFIG_BASENAMES[0]) ??
		resolveMarker(DM_LENS_CONFIG_BASENAMES[1]);
...
export interface DmLensConfigMarker {
	path: string;
	dir: string;
	mtimeMs: number;
	/**
...
/**
 * `.dm-lens.json`/`dm-lens.json` directly IN `dir` — no upward walk. The
 * shared-index equivalent of `project-lens-config.ts`'s old private
 * `findDmLensConfigInDir` probe loop.
 */
export function findDmLensConfigMarkerInDir(
	dir: string,
): DmLensConfigMarker | undefined {
	const markers = getDirectoryMarkers(dir);
