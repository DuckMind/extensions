 ...
 * so the subagent's `session_start` re-invokes dm-lens's SAME module-scope
 ...
 *
 * Kill switch: `DM_LENS_CONCURRENT_SESSION_GUARD=0` disables the guard
 ...
 * dm-lens module graph up to nine times in one process (source vs compiled
 ...
 *
 * This is the process's answer to "which directory does dm-lens actually
 ...
 * `DM_LENS_CONCURRENT_SESSION_GUARD=0` disables this branch with the rest of
 * the guard.
 */
...
function guardEnabled(): boolean {
	return process.env.DM_LENS_CONCURRENT_SESSION_GUARD !== "0";
}
 ...
 * `@duckmind/dm-coding-agent` SDK dist). `isIdle` was picked over the
 * plain getters (`cwd`, `mode`, `hasUI`) only for readability at call sites
 * that already branch on idle state elsewhere in dm-lens; any of the other
