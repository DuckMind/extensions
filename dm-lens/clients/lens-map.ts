...
export interface AggregateOptions {
	/** Cap on rendered file nodes; default resolved by the caller (env DM_LENS_MAP_MAX_NODES, else 500). */
	maxNodes?: number;
	/**
...
export function aggregateGraphToFiles(
	...
	// dm-lens (and many TS projects) compile in place, so the review graph's
...
<title>dm-lens project map</title>
<style>
  :root {
    color-scheme: light;
...
(function () {
  ...
  if (searchInput) {
    ...
    // the map caps at DM_LENS_MAP_MAX_NODES (500 default) — cheap per input.
...
function resolveMaxNodes(): number {
	const raw = process.env.DM_LENS_MAP_MAX_NODES?.trim();
