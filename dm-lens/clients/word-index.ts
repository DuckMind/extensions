...
// `dmlens_symbol_search` mirror inherit the syntax for free: they already pass
...
//
// `symbol_search` (pi tool) / `dmlens_symbol_search` (MCP) are stateless callers:
...
// is an MCP-only session that never ran dmlens_session_start), the tool must
...
function wordIndexPersistDebounceMs(): number {
	const raw = Number(process.env.DM_LENS_WORD_INDEX_PERSIST_DEBOUNCE_MS);
 ...
 * override via `DM_LENS_WORD_INDEX_PERSIST_DEBOUNCE_MS`, mirroring the review
 * graph's `DM_LENS_GRAPH_PERSIST_DEBOUNCE_MS`). Merges through the same
