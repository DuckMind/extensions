 ...
 *    machinery the runtime uses — `resolveDmLensConfig` for the resolution,
 ...
 * 2. IT REPORTS NOTHING. `resolveConfig` is pure and `reportDmLensConfigRecords`
...
export interface EffectiveConfigView {
	...
	/** How many leaves each tier decided. The shape `dmlens_health` embeds. */
...
/**
 * The resolved model of dm-lens's configuration, with the provenance of every
 ...
 * because a nested `repo/sub/.dm-lens.json` layer (config.ts header point 3)
 ...
 * case is the view `dmlens_health` takes on every call.
 *
...
export async function effectiveConfig(
	...
	const resolveAt = (dir: string): DmLensConfigResolution =>
		resolveDmLensConfig({
			cwd: dir,
	...
	// as: its own ancestor walk can reach a `.dm-lens.json` the workspace never
	...
	// registered ancestor, so a nested `repo/sub/.dm-lens.json` (config.ts
...
async function fileView(
	...
	// resolved list.
	//
	// The legacy root spelling is no longer a fallback: `resolveDmLensConfig`
