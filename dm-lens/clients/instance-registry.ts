 ...
 * a single machine-global file (`~/.dm-lens/instances.json`), every live
 * dm-lens process: its pid, project root, live LSP child servers, RSS, and a
...
function registryPath(): string {
	return path.join(getGlobalDmLensDir(), "instances.json");
}
...
/**
 * `DM_LENS_INSTANCE_REGISTRY=0` disables the registry entirely: every
...
export function isInstanceRegistryEnabled(): boolean {
	...
	_enabledCache = process.env.DM_LENS_INSTANCE_REGISTRY !== "0";
 ...
 * dm-lens process reads on every heartbeat. Eviction drops the OLDEST
...
// scope did not deliver that. pi evaluates the dm-lens module graph up to nine
...
/**
 * Every OTHER live dm-lens process registered against the same project root,
 * oldest first (#2007).
 *
...
export interface InstanceFootprint {
	 ...
	 *  pre-#2130 consumer of `dmlens_health`'s `resourceFootprint` reads it. */
	projectRoot: string;
	/**
	 ...
	 * the scalar above is only the FIRST root, so `dmlens_health` showed a
...
export interface ResourceFootprint {
	 ...
	 *  attributable to dm-lens", not "% of one core". */
...
/**
 * PURE aggregation over a registry snapshot: "how much CPU/RAM is dm-lens
 ...
 * every existing caller (chiefly `dmlens_health`'s headline instance
 ...
 * side of "how much CPU/RAM is dm-lens using right now" (#620). Best-effort
