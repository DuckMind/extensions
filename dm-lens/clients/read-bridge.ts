/**
 * Generic read-recording bridge for dm-lens.
 *
 * ## Trust model — explicitly advisory
 *
 * This bridge is an **advisory, trust-based protocol** for same-process DM
 * extensions. Any code sharing the same Node.js process already has full
 * access to dm-lens's internal state, so the bridge cannot and does not
 * provide a meaningful security boundary. What it *does* provide is:
 *
 * - A stable API surface so extensions do not need to import dm-lens
 *   internals directly.
 ...
 * co-process extension can call to register a file read against dm-lens's
 * read-guard — without either party needing to know about the other.
 *
 * Protocol (producer side)
 * ────────────────────────
 * A co-process DM extension that performs file reads outside dm-lens's
 ...
 *
 *   const bridge = (globalThis as any)[Symbol.for("dm-lens:read-bridge")];
 *   bridge?.recordRead({
 ...
 *
 * Calling before dm-lens is loaded, or when the guard is disabled via
 * `DM_LENS_NO_READ_GUARD`, is safe — the bridge is absent or the call is
 * silently dropped.
 *
...
export const READ_BRIDGE_KEY: unique symbol = Symbol.for("dm-lens:read-bridge");
...
export interface ReadBridgeEntry {
	 ...
	 * dm-lens will treat the effective limit as the full file length.
	 */
