 ...
 * `clients/instance-reaper.ts`) made every concurrent dm-lens process visible
 * to every other one via `~/.dm-lens/instances.json`. This module is the
 ...
 * case. `DM_LENS_LSP_BUDGET_CEILING` overrides it once real-world data (#620)
 * says otherwise.
 *
 * Kill switch: `DM_LENS_CROSS_PROCESS_BUDGET=0` disables this module
...
/** `DM_LENS_CROSS_PROCESS_BUDGET=0` disables the budget check entirely —
 *  lazy env read (house style), never memoized so tests can flip it mid-run. */
export function isCrossProcessBudgetEnabled(): boolean {
	return process.env.DM_LENS_CROSS_PROCESS_BUDGET !== "0";
}

/** `DM_LENS_LSP_BUDGET_CEILING` overrides {@link DEFAULT_LSP_BUDGET_CEILING}.
...
export function getLspBudgetCeiling(): number {
	const raw = Number(process.env.DM_LENS_LSP_BUDGET_CEILING);
...
export function getLspBudgetRssCeilingBytes(): number | undefined {
	const raw = Number(process.env.DM_LENS_LSP_BUDGET_RSS_MB);
	return Number.isFinite(raw) && raw > 0 ? raw * 1024 * 1024 : undefined;
}

export function getLspBudgetIdleTimeoutMs(): number {
	const raw = Number(process.env.DM_LENS_LSP_BUDGET_IDLE_TIMEOUT_MS);
