...
function emitIdleResetReporterWarning(reportErr: unknown): void {
	try {
		process.emitWarning(
			`dm-lens LSP idle reset error reporter failed: ${reportErr}`,
			{ code: "DM_LENS_LSP_IDLE_RESET_REPORTER_FAILED" },
		);
	} catch {
...
export async function handleTurnEnd(deps: TurnEndDeps): Promise<void> {
		...
		if (deliveryCount >= DEPENDENCY_DRIFT_MAX_DELIVERIES) {
			...
			if (capRetired) {
				...
				incrementDegradationCount({
					...
					reason: `capped after ${deliveryCount} deliveries with no re-run; hidden from the dm-lens footer, still listed by lens_diagnostics mode=all — re-run can still confirm`,
				});
			}
		}
	}
	...
	if (files.length === 0 && !runtime.hasCascadeRuns()) {
		...
		// tick in this call path shares the same answer. DM_LENS_SUBAGENT_FULL=1
	...
	// dm-lens's own cache. This read is the single agent-facing consumer of that
		...
		try {
			...
			logActionableWarningsEvent({
				...
				metadata: {
					...
					// scripts/analyze-dm-lens-logs.mjs sums, so an undercount here is
