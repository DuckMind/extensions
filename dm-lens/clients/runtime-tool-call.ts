...
const LSP_TOOLCALL_NAV_TOUCH_BUDGET_MS = Math.max(
	0,
	Number.parseInt(
		process.env.DM_LENS_TOOLCALL_NAV_TOUCH_MS ??
			process.env.DM_LENS_LSP_NAV_CLIENT_WAIT_MS ??
			"1500",
		10,
	) || 1500,
);
const LSP_TOOLCALL_TOUCH_BUDGET_MS = Math.max(
	0,
	Number.parseInt(process.env.DM_LENS_TOOLCALL_TOUCH_MS ?? "750", 10) || 750,
);
 ...
 * (`@duckmind/dm-coding-agent/dist/core/agent-session.js:2026`, source
...
function shouldSkipLspAutoTouch(
	...
	if (normalized.includes("/.dm-lens/")) return true;
 ...
 * (`emitToolResult` at
 * `@duckmind/dm-coding-agent/dist/core/extensions/runner.js:649-707`,
 ...
 *
 * So an unguarded throw anywhere in dm-lens's `tool_call` handler does not
 * degrade dm-lens — it BLOCKS the user's tool call outright. Advisory
 * instrumentation must never be able to do that. Every failure degrades to
 * "dm-lens did nothing for this call" (`undefined`, which pi reads as
 ...
 * a dm-lens bug into a refused user tool call — the exact outcome this guard
 * exists to prevent.
 *
...
export async function handleToolCall(
	...
	try {
		...
		if (result && (result as { block?: boolean }).block) {
			...
			// `undefined` — "dm-lens has no opinion" — and the refused edit would
...
async function handleToolCallImpl(deps: ToolCallDeps): Promise<ToolCallResult> {
	...
	// turn (`@duckmind/dm-coding-agent/dist/core/extensions/runner.js:701-716`
	// passes `event` unchanged through the loop; source
	// `src/core/extensions/types.ts:914-919`), and dm-lens itself rewrites it in
	...
	if (targetMissing) {
		// #1655 item 5: this early return used to be the whole story — dm-lens
							...
							afterWrite: async () => {
								...
								const result = await handleToolResult({
									event: {
										...
										// (`@duckmind/dm-coding-agent/dist/core/agent-session.js:230-234`,
