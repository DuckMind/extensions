...
import { visibleWidth } from "./deps/dm-tui-compat.js";
...
/**
 * A diagnostic is "blocking" when dm-lens classifies it as a hard stop
 ...
 * `clients/lsp/client.ts` in #555: dm-lens allows concurrent pipeline runs
 ...
 * through dm-lens) has no path to correct the footer — a full scan proves
...
/**
 * Drop widget entries whose file changed on disk after dm-lens last recorded
...
/**
 * Return current diagnostics for every file dm-lens has seen this session.
 ...
 * whole-session snapshot. Used by the #502 `dmlens:diagnostics` bus producer
...
export function renderWidget(
	...
	const header = ` ${cyan("dm-lens")}${langStr ? "  " + dim(langStr) : ""}${lspChip}${summary ? "  " + summary : ""}`;
	...
	if (withBlocking.length > 0) {
		...
		// scan of every file dm-lens has touched this session. A past-EOF
...
// turn-summary message renderer, which learned the hard way that dm-tui-compat crashes
