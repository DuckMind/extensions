...
export async function handleAgentEnd({
	...
	// publishFilesTouched call so consumers can tell "dm-lens formatted this"
	// from an agent edit.
	...
	if (inspectActionableReport) {
		...
		if (!actionReport?.data) {
			...
			const enabledFiles = actionReport.data.files.filter((file) => {
				...
				// `.dm-lens.json` `ignore` entry cannot protect a prose file
			...
			if (!freshness.fresh) {
				...
				if (fixSummary.applied > 0) {
					notify(
						`dm-lens applied ${fixSummary.applied} conservative LSP warning quickfix(es)`,
						"info",
					);
				}
			} else {
	...
	if (summary.failed.length > 0) {
		notify(
			`dm-lens deferred format: ${summary.changed.length} changed, ${summary.failed.length} failed`,
			"warning",
		);
		...
		notify(
			`dm-lens deferred format applied to ${summary.changed.length} file(s): ${names}`,
			"info",
		);
	}
