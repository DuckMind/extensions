 ...
 *   DM_LENS_LOG_RETENTION_DAYS - Days to keep logs (default: 7)
 *   DM_LENS_MAX_LOG_SIZE_MB    - Max size before rotation (default: 10)
 *
 * Scope:
 *   - ~/.dm-lens/*.log (every global log — see getManagedLogFiles())
 *   - ~/.dm-lens/logs/*.jsonl (daily diagnostic logs)
 *   - ~/.dm-lens/<name>.<timestamp>.log (rotated backups, and legacy .log.<ts>)
 *
...
const LOG_DIR = getGlobalDmLensLogDir();
const LOGS_SUBDIR = path.join(LOG_DIR, "logs");

/**
 * Every global `.log` dm-lens writes under ~/.dm-lens, resolved dynamically so
 ...
 *   2. A direct `~/.dm-lens/*.log` directory read, excluding rotated-backup
...
/** Rotation threshold for every active `~/.dm-lens/*.log`, in MB. */
export function getMaxLogSizeMB(): number {
	return Math.max(
		1,
		Number.parseInt(process.env.DM_LENS_MAX_LOG_SIZE_MB ?? "10", 10) || 10,
	);
}
...
function getConfig(): LogCleanupConfig {
	return {
		retentionDays: Math.max(
			1,
			Number.parseInt(process.env.DM_LENS_LOG_RETENTION_DAYS ?? "7", 10) || 7,
		),
 ...
 * Cleans up all dm-lens log files based on retention policy
 */
