/**
 * LensEngine — the single internal-facing seam for dm-lens host adapters.
 *
 * The maintainability rule: host adapters (the MCP server today; index.ts can
 * adopt incrementally) talk ONLY to this module, never reaching into dm-lens
...
export function scanTruncationNotice(
	...
	if (snapshot.treeSitterStatus === "wasm_aborted_restart_required") {
		return (
			...
			"restart the dm-lens extension/MCP server to restore structural analysis."
		);
	}
	return (
		`⚠ Scan truncated at ${snapshot.filesScanned} file(s) — results are partial; ` +
		"raise maxProjectFiles in .dm-lens.json to scan fully."
	);
}
...
export function generatedSkipNotice(
	...
	return (
		...
		"pass includeGenerated: true to dmlens_project_scan / lens_diagnostics " +
...
/**
 * #620: total CPU/RAM footprint attributable to dm-lens across every process
 * it owns — every registered instance's host, plus that instance's live LSP
 * children. Reads the machine-global `~/.dm-lens/instances.json` registry, so
 * this answers across ALL concurrent dm-lens sessions/worktrees on the box,
 ...
 * cwd — a warm `dmlens_analyze` call updates that live copy synchronously but
 ...
 * dmlens_analyze yet this process, or #348 phase 2's forward-index isn't
