/**
 * Shared file path utilities for dm-lens
 */
...
import {
	findDmLensConfigInDir,
	findDmLensProjectConfig,
	loadDmLensConfigInDir,
	loadDmLensProjectConfig,
} from "./project-lens-config.js";
import { safeSpawnAsync } from "./safe-spawn.js";

/**
 * Return the directory where dm-lens stores project-specific data
 * (caches, indexes, worklogs, etc.).
 *
 * Default: reuse <project>/.dm-lens if it already exists, otherwise use
 * ~/.dm-lens/projects/<project-slug>
 *
 ...
 *   PILENS_DATA_DIR=~/.dm-lens/projects
 *   → ~/.dm-lens/projects/home-user-myapp/
 *
 * This keeps project folders clean and avoids creating .dm-lens folders
 * inside user projects.
 */
export function getProjectDataDir(cwd: string): string {
	const legacyProjectDir = path.join(cwd, ".dm-lens");
	...
	const base = configuredBase || path.join(getGlobalDmLensDir(), "projects");
 ...
 * hardcoded `.dm-lens/cache/actionable-warnings.json`, but that spelling is
 * only correct in the LEGACY arm of `getProjectDataDir` — a project with no
 * `.dm-lens/` directory (the common case) stores the report under
 * `~/.dm-lens/projects/<slug>/cache/`, and a `PILENS_DATA_DIR` project stores
 * it somewhere else again. Agents followed the advisory literally and got
 * `cat: .dm-lens/cache/actionable-warnings.json: No such file or directory`.
 *
 ...
 *     (`.dm-lens/cache/actionable-warnings.json`), which is what the legacy
 *     arm produces and what an agent can paste straight into a shell.
 *   - anywhere else → absolute, `/`-separated, and `~`-folded via
 *     {@link homeRelativePath} so the default `~/.dm-lens/projects/...` store
...
export function displayProjectDataPath(
	...
	// `~/.dm-lens/projects/...` store does not leak the account name into
...
/**
 * Machine-global dm-lens directory: `~/.dm-lens/` by default.
 *
 * Used for tool binaries (`~/.dm-lens/tools/`, `~/.dm-lens/bin/`), the
 ...
 * intentionally NOT project-scoped, because it spans every project dm-lens
 * has touched.
 *
 * Override: set `DM_LENS_HOME=/some/path` to relocate this ENTIRE root (every
 ...
 * logger writes into the developer's REAL `~/.dm-lens` (dogfooded live: a
 * test-fixture instance survived in the real `instances.json` for 17h).
 *
 * DELIBERATELY cwd-INDEPENDENT (#2506 round 3). Its sibling
 * `getGlobalDmLensLogDir()` (`probe-home-state.ts`) carries the probe-home
 ...
 * dm-lens process on the box reads. Redirecting THIS function (as round 2 of
 ...
 * instead — `DM_LENS_HOME` is the MACHINE-scoped sibling of that override.
 ...
 */
export function getGlobalDmLensDir(): string {
	const override = process.env.DM_LENS_HOME?.trim();
	if (override) return path.resolve(override);
	return path.join(os.homedir(), ".dm-lens");
}
 ...
 * (lowest → highest) is `global` → `gitignore` → `dmlens` — see
 ...
 *   - `dmlens` (`.dm-lens.json`'s `ignore` field) is dm-lens-native user
 *     intent ("don't analyze this"), not a git emulation, so it excludes
 *     regardless of tracked status.
 *
 * `dmlens` patterns can come from the root `.dm-lens.json` (loaded once at
 * matcher-construction time in `createProjectIgnoreMatcher`) OR from a
 * package-local `.dm-lens.json` layered in per ancestor directory by
 * `buildProjectIgnoreMatcher`'s `patternsForDir` (#783) — both are tagged
 * `"dmlens"` and share the same tracked-file-rescue exemption.
 */
export type GitignorePatternLayer = "global" | "gitignore" | "dmlens";
...
function buildProjectIgnoreMatcher(
	...
	const nestedCache = new Map<
		string,
		{
			...
			dmlensMtimeMs: number;
			dmlensSize: number;
			patterns: GitignorePattern[];
		}
	>();
	// #783: layer a NESTED `.dm-lens.json`'s `ignore` field the same way a
	...
	// upward walk; `findDmLensConfigInDir`/`loadDmLensConfigInDir` look only
	...
	// tagged `"dmlens"` so #703's tracked-file-rescue rule still skips them.
	// `loadDmLensConfigInDir` reuses `project-lens-config.ts`'s own
	...
	const patternsForDir = (dir: string): GitignorePattern[] => {
		...
		// #1105: gate on size too. `findDmLensConfigInDir` returns `size` alongside
		...
		const dmlensInfo = findDmLensConfigInDir(dir);
		const dmlensMtime = dmlensInfo?.mtimeMs ?? -1;
		const dmlensSize = dmlensInfo?.size ?? -1;
		if (
			cached?.gitignoreMtimeMs === gitignoreSig.mtimeMs &&
			cached?.gitignoreSize === gitignoreSig.size &&
			cached?.dmlensMtimeMs === dmlensMtime &&
			cached?.dmlensSize === dmlensSize
		) {
		...
		const nestedConfig = loadDmLensConfigInDir(dir);
		const nextPatterns = [
		...
		nestedCache.set(dir, {
			...
			dmlensMtimeMs: dmlensMtime,
			dmlensSize: dmlensSize,
	...
	return {
		...
		isIgnored(filePath: string, isDirectory = false): boolean {
			...
			// never ignored" rule. A winning match from `dmlens` is dm-lens-native
...
export function createProjectIgnoreMatcher(
	...
	// global (lowest) → project .gitignore → project .dm-lens.json (highest),
	...
	// apart from dm-lens-native intent (`dmlens` — excludes regardless).
	const patterns = [
...
const projectIgnoreMatcherCache = new Map<
	string,
	{
		 ...
		 * too, not just `loadDmLensProjectConfig`'s own cache. Size is free from the
		 * stat that already produced `lensConfigMtimeMs`. */
		lensConfigSize: number;
		globalConfigMtimeMs: number;
		/** #1105 second axis for the global `~/.dm-lens/config.json`. */
 ...
 * gate below (root + nested `.gitignore` and `.dm-lens.json`) compares BOTH
...
function globalConfigSignature(): FreshnessSignature {
	return fileFreshnessSignature(getDmLensGlobalConfigPath());
}
 ...
 * `.dm-lens.json` from a parent directory.
 */
function lensConfigInfo(rootDir: string): {
	info: ReturnType<typeof findDmLensProjectConfig>;
	path: string | undefined;
	mtimeMs: number;
	size: number;
} {
	const info = findDmLensProjectConfig(rootDir);
	return info
...
export function getProjectIgnoreMatcher(rootDir: string): ProjectIgnoreMatcher {
	...
	const projectConfig = loadDmLensProjectConfig(resolvedRoot, lensConfig.info);
...
/**
 * "Is this a project-source path dm-lens's bookkeeping should care about" —
