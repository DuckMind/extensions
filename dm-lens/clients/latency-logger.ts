...
const LATENCY_LOG_DIR = getGlobalDmLensLogDir();
 ...
 * identity dm-lens does not otherwise write to disk, and the join only needs
...
export function logLatency(entry: LatencyEntry): void {
	...
	// (dispatcher.ts-derived `ctx.filePath`) AND non-path labels ("<dm-lens>",
