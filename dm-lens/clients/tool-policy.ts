...
// binary) for `.svelte` — dm-lens already resolves oxfmt via `findInNodeModules`/
...
export function hasOxfmtSvelteConfig(cwd: string): boolean {
	...
	for (const dir of walkUpDirs(cwd)) {
		...
		// oxfmt.toml is TOML; dm-lens has no TOML parser dependency, so this is
...
// ktfmt has no native config file format; these are dm-lens opt-in markers plus
...
// (in addition to the legacy `oxlint.json` name dm-lens already recognized).
