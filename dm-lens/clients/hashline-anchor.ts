 ...
 *
 * This module reproduces the extension's own line-hash function so dm-lens can
 ...
 *   dm-lens with `npm install --omit=dev`, so a hashing dep would ship to every
 ...
 * agent sends back are STORE-CARRIED, not freshly computed. dm-lens has no
 ...
 * for it and dm-lens's own recomputed anchor for it USUALLY agree — that is
 ...
 * here: closing it needs either the extension's own store, which dm-lens
 ...
 * which is a report and never a block. dm-lens's uncertainty about a
...
function nextZeroBit(bits: Uint32Array, start: number): number {
	...
	// Upstream throws `[E_FILE_TOO_LARGE]` here. dm-lens is an observer, so it
 ...
 * against a map dm-lens does not have. The real ambiguity — two lines that our
