/**
 * On-demand V8 heap-snapshot writer — the `DM_LENS_DEBUG_HEAP` sibling that
 ...
 *
 * `DM_LENS_DEBUG_HEAP=1` (read ONCE at module load, exactly like
 ...
 *     `~/.dm-lens/heap-<pid>-<iso>.heapsnapshot` and appends ONE ndjson
 ...
 *     bounds every other dm-lens log gets, #1101-era registry). The breadcrumb
 ...
 * (a) strictly opt-in behind `DM_LENS_DEBUG_HEAP`, and (b) only ever invoked on
...
const DEBUG_HEAP_ENABLED = process.env.DM_LENS_DEBUG_HEAP === "1";

/** Newest-N `.heapsnapshot` files kept in the dm-lens dir; older ones are
...
/** True iff `DM_LENS_DEBUG_HEAP=1` was set when this module first loaded. */
...
/**
 * Write one heap snapshot to `~/.dm-lens/`, append a breadcrumb line, and prune
 * to the newest `SNAPSHOT_RETENTION` files. A pure no-op returning `null` — no
 * file, no allocation past the flag check — when `DM_LENS_DEBUG_HEAP` was unset
...
export function writeHeapSnapshotNow(label: string): HeapSnapshotResult | null {
	...
	const dir = getGlobalDmLensLogDir();
	try {
