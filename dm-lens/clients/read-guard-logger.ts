...
const READ_GUARD_LOG_DIR = getGlobalDmLensLogDir();
...
const MAX_LOG_BYTES = Math.max(
	128 * 1024,
	Number.parseInt(process.env.DM_LENS_READ_GUARD_MAX_BYTES ?? "1048576", 10) ||
		1048576,
);
...
const VERBOSE_READ_GUARD_LOG =
	process.env.DM_LENS_READ_GUARD_VERBOSE === "1" ||
	process.env.DM_LENS_READ_GUARD_LOG === "verbose";
const LOG_ALLOWED_EDITS = process.env.DM_LENS_READ_GUARD_LOG_ALLOWS === "1";
const SNAPSHOT_LOG_SETTING = (
	process.env.DM_LENS_READ_GUARD_LOG_SNAPSHOTS ?? "1"
).toLowerCase();
...
export function shouldLogEvent(event: string): boolean {
	...
	return (
		...
		// regression must be visible without DM_LENS_READ_GUARD_VERBOSE=1.
