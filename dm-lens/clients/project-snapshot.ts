...
function projectSnapshotIdleEvictMs(): number {
	const value = Number.parseInt(
		process.env.DM_LENS_PROJECT_SNAPSHOT_IDLE_EVICT_MS ?? "",
		10,
	);
...
function snapshotWorkerEnabled(): boolean {
	...
	const raw = process.env.DM_LENS_SNAPSHOT_PERSIST_SYNC;
	...
	const request: ProjectSnapshotPersistWorkerRequest = {
		...
		testDelayMs:
			process.env.NODE_ENV === "test"
				? Number(process.env.DM_LENS_TEST_SNAPSHOT_PERSIST_WORKER_DELAY_MS) ||
					undefined
				: undefined,
	};
	try {
