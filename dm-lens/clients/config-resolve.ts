 ...
 *
 * Every dm-lens config file — canonical or legacy, project or global — is read,
 ...
 * (`loadLSPConfig`, `loadDmLensGlobalConfig`, `loadDmLensProjectConfig`) keep
 ...
 * into `DmLensProjectConfig.raw` that was never in the user's file. That
 * objection is why the normalization lives HERE and not in
 * `resolveOneDmLensConfigDocument`: `.raw` comes from the single-document path,
...
import { DM_LENS_CONFIG_SCHEMA } from "./config-schema.js";
import {
 ...
 * loader happened to open it. `loadLSPConfig` resolves `~/.dm-lens/config.json`
 * and `.dm-lens.json` too, and reporting those under `lsp-config` told a user
 * their "LSP config" was invalid when the file they had broken was the dm-lens
...
export interface DmLensConfigResolution {
 ...
 * `options.globalConfigPath` (`getDmLensGlobalConfigPath()`, `$HOME`-derived
 * unless `DM_LENS_CONFIG_PATH` overrides it) while the notice was recomputed as
 * `dirname(<legacy file>) + config.json` — and the legacy file lives under
 * `getGlobalDmLensDir()`, which `DM_LENS_HOME` relocates. Set `DM_LENS_HOME`
 * without `DM_LENS_CONFIG_PATH` and the two diverge, so the notice named a file
...
function canonicalGlobalFile(
	options: ResolveDmLensConfigOptions,
): string | undefined {
 ...
 */
export function collectDmLensConfigDocuments(
	options: ResolveDmLensConfigOptions,
): ConfigDocument[] {
	const documents: ConfigDocument[] = [];
 ...
 * `dmlens_effective_config` view and `loadLSPConfig` writes it to the
 ...
 * `dmlens_effective_config` applies, spelled once rather than twice (rule 3 of
...
export function summarizeConfigResolution(
	resolution: DmLensConfigResolution,
	homeDir?: string,
...
export function resolveDmLensConfig(
	options: ResolveDmLensConfigOptions,
): DmLensConfigResolution {
	const documents = collectDmLensConfigDocuments(options);
	const sources = configSources(documents);
	const resolution = resolveConfig<Record<string, unknown>>({
		sources,
		schema: DM_LENS_CONFIG_SCHEMA,
	});
	return {
 ...
 * namespace the way `resolveDmLensConfig` does (#2427 review round 2, F1).
 * This path's two callers project the dm-lens sections of ONE file and hand
 * the resolved value out as `DmLensProjectConfig.raw`, which is documented and
 ...
 * notice.
 */
export function resolveOneDmLensConfigDocument(
 ...
 * `collectDmLensConfigDocuments`. Falls back to the canonical location so an
 ...
 * (no production caller does; `resolveOneDmLensConfigDocument`'s two callers
 ...
 * (`pi-lsp.json`, `.dm-lens/lsp.json`) is wholly what a canonical file now
 ...
 * An unrecognized key is left alone in both cases — it is not a dm-lens
...
/**
 * The top-level keys a dm-lens config document may legitimately carry — READ
...
const CANONICAL_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(
	schemaPropertyKeys(DM_LENS_CONFIG_SCHEMA),
);

/**
 * The same, for an LSP-SCOPED legacy file (`pi-lsp.json`, `.dm-lens/lsp.json`,
 * `~/.dm-lens/lsp.json`): that whole file is what a canonical file now holds
 * under `lsp`, so its recognized ROOT keys are the `lsp` namespace's own.
 */
const LSP_DOCUMENT_KEYS: ReadonlySet<string> = new Set(
	schemaPropertyKeys(
		(DM_LENS_CONFIG_SCHEMA as { properties?: Record<string, unknown> })
 ...
 * dm-lens actually RECOGNIZES (#2426 review round 4, F2/F5).
 *
 ...
 * not a dm-lens setting at all, which is advice that cannot be followed. A key
			...
			if (unrecognized > 0) {
				...
				records.push({
					...
					reason:
						...
						(unrecognized === 1
							? "1 of its top-level keys is not a recognized dm-lens setting"
							: `${unrecognized} of its top-level keys are not recognized dm-lens settings`) +
...
/**
 * Which SUBSYSTEM a record belongs to — the LSP loader, or the two dm-lens
 ...
 */
export type ConfigRecordOwner = "lsp" | "dm-lens";
...
/** The dm-lens-config subsystem a GLOBAL vs. PROJECT tier reports under. */
 ...
 * dm-lens-owned record, the TIER it was read at — rather than from which
 * loader is calling. Before this, a caller filtered to the records it "owned"
 * with `recordsOwnedBy` before reporting, which meant a dm-lens-owned record
 * from a document only the LSP loader's multi-file resolution actually
 * discovered (a legacy sibling file `loadDmLensProjectConfig`'s single-nearest-
 ...
 * LSP loader is the one home for every LSP setting. A `dm-lens` owner reports
 * under the tier-appropriate dm-lens loader.
 *
...
function configRecordSubsystems(
	...
	if (owner === "dm-lens") return piLensSubsystems;
 ...
 * opens the dm-lens global and project configs too, and labelling their parse
 * failures "ignoring invalid LSP config" told a user to look at an LSP setting
 * when what they had broken was `~/.dm-lens/config.json`. An LSP-SCOPED file
 ...
 * with overlapping records (the LSP loader and a dm-lens loader resolving the
 ...
 */
export function reportDmLensConfigRecords(
