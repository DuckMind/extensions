 ...
 *
 * Before this seam, every producer in dm-lens decided "is this event a file
 ...
 *
 * An edit-shaped tool dm-lens does not otherwise know defaults to `edit`, the
 * safe timing.
 */
...
export type MutationProvenance =
	...
	| "builtin"
	/** A synthetic write dm-lens derived from a bash command (#168, #2000). */
	| "bash-derived"
	/** A shape adapter recognized the input of a tool dm-lens does not name. */
	| "declared"
	 ...
	 * file's content had drifted from the last baseline dm-lens took.
	 */
	| "settled-sweep";
 ...
 * hash dm-lens cannot verify is exactly the guessing that finding F1 removed.
 ...
 * dm-lens's disagreement with a third-party tool would be wrong.
 *
...
/**
 * dm-lens's own marker on a `tool_result` it SYNTHESIZED from a bash command
 ...
 */
export const DM_LENS_SYNTHETIC_MUTATION_FIELD = "piLensSyntheticMutation";

/**
 * Classify an inbound `tool_call` or `tool_result` event.
 *
 * Returns `undefined` for every event that is not a file mutation dm-lens
...
export function classifyMutatingTool(
	...
	for (const adapter of MUTATION_SHAPE_ADAPTERS) {
		...
		return {
			...
			// name dm-lens does not know is a declared third-party shape.
...
export interface BridgeMutationEntry {
	 ...
	 * "an extension told us" about something dm-lens INFERRED is a lie about
