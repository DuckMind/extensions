...
export interface ActionableWarningsReport {
	...
	summary: {
		 ...
		 * can find a file written by a dm-lens build that predates the field.
 ...
 * hardcoded `.dm-lens/cache/actionable-warnings.json` — correct only for a
 * project that already had a legacy `.dm-lens/` directory — so in every other
 * project the agent's `cat` of the advised path failed while the report sat
 * unread under `~/.dm-lens/projects/<slug>/cache/`. An OPTIONAL `cwd` would
