/**
 * THE single declarative source of truth for every dm-lens toggle (#166).
 *
 ...
 *   - `clients/lens-config.ts` — parsing the key out of `~/.dm-lens/config.json`
 ...
 * `~/.dm-lens/config.json`. Adding a toggle means adding one entry here; there
...
export interface LensFlagSpec {
	...
	description: string;
	/**
	 * Dotted path to this flag's boolean in `~/.dm-lens/config.json` (and, for
	 * `scope: "project"` entries, the identical path in `.dm-lens.json`).
	 */
	configKey: string;
	/**
	...
	default: boolean;
	/**
	 * `"project"` flags also read `.dm-lens.json` and nested per-directory
...
export const LENS_FLAGS: readonly LensFlagSpec[] = [
	{
		name: "no-lens",
		description:
			"Start dm-lens disabled for this session. Re-enable with /lens-toggle. Also via lens.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-lsp",
		description:
			"Disable unified LSP diagnostics and use language-specific fallbacks (for example pyright). Also via lsp.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-tests",
		description:
			"Disable test runner on write. Also via tests.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-delta",
		description:
			"Disable delta mode (show all diagnostics, not just new ones). Also via delta.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "lens-guard",
		description:
			"Experimental: block git commit/push when unresolved dm-lens blockers exist. Also via guard.enabled=true in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Experimental: decline git commands that rewrite the working tree when another live dm-lens session shares this dirty checkout. Also via guard.sharedCheckout=true in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-opengrep",
		description:
			"Disable the Opengrep security scanner (a default-on auxiliary LSP; auto-installs, uses repo rules if present else the login-free 'auto' ruleset). Also via opengrep.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-read-guard",
		description:
			"Disable read-before-edit behavior monitor. Also via readGuard.enabled=false in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Disable automatic context injection (session-start guidance, turn-end & test findings) while keeping tools, LSP, read-guard, and formatting active. Toggle with /lens-context-toggle. Also via contextInjection.enabled=false in config or DM_LENS_NO_CONTEXT_INJECTION=1.",
		configKey: "contextInjection.enabled",
		negated: true,
		default: false,
		scope: "global",
		env: "DM_LENS_NO_CONTEXT_INJECTION",
	},
	{
		name: "lens-turn-summary",
		description:
			"Opt-in: persist a per-turn transcript entry summarizing diagnostics found, autofixes applied, and autoformats applied this turn (#484). Collapsed one-line, expandable in place. Default off. Also via turnSummary.enabled=true in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Write turn-delta fixable warning reports and inject a short advisory. Also via actionableWarnings.enabled=true in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Enrich actionable-warning reports with LSP code-action titles (requires an active language server). Also via actionableWarnings.includeLspCodeActions=true in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Report every actionable warning, not just those introduced this turn. Also via actionableWarnings.deltaOnly=false in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Opt-in (#1327): collapse a dm-lens tool's call+result rows into ONE theme-aware line (status glyph + name + summary) instead of two. Preserves expand-to-view-full-output. Default off. Also via ui.compactToolLine=true in ~/.dm-lens/config.json.",
	...
	},
	{
		name: "no-lazy-tools",
		description:
			"Keep all dm-lens tools active to avoid tool-list cache changes. Also via tools.lazy=false in ~/.dm-lens/config.json.",
	...
	},
	{
		...
		description:
			"Run the per-turn-end madge circular-dependency check on import-changed files. Off by default: the pass only writes debug output, and user-facing madge diagnostics come from the session-start scan cache + lens_diagnostics. Also via turnEnd.madge.enabled=true in ~/.dm-lens/config.json.",
...
/** The `scope: "project"` subset — the flags a `.dm-lens.json` may also set. */
export const PROJECT_SCOPED_LENS_FLAGS: readonly LensFlagSpec[] =
	LENS_FLAGS.filter((spec) => spec.scope === "project");

/**
 * Recognized TOP-LEVEL sections of `~/.dm-lens/config.json` that are NOT
...
/**
 * Recognized TOP-LEVEL sections of a project `.dm-lens.json` that are NOT
 ...
 * namespaces another dm-lens runner reads off `DmLensProjectConfig.raw`
 ...
 * `.dm-lens.json` but belong to a DIFFERENT loader: the LSP config loader
 ...
 * the namespace can hold: which sub-keys a `.dm-lens.json` may set.
 *
