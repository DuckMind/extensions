 ...
 * session: an ignored config means dm-lens ran on defaults instead of what the
...
export function warnIgnoredConfigOnce(options: WarnIgnoredConfigOptions): void {
	...
	// recognized dm-lens setting`), so "hand-authored" does not mean
	...
	notifyUserDegradation(`dm-lens: ${message}`, "warning", { code });
}
