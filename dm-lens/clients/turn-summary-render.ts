 ...
 * (`dmlens:turn-summary`). Collapsed = one tool-grouped line (dm-lens brand
 ...
 * just `@duckmind/dm-tui`'s `Component` interface —
 * `{ render(width: number): string[] }` — no framework object graph needed.
 * `dm-tui-compat` is already a devDependency (package.json `@duckmind/dm-tui`).
 */
...
import type { Theme } from "@duckmind/dm-coding-agent";
...
function buildExpandedLines(
	...
	const lines: string[] = [];
	lines.push(theme.fg("accent", theme.bold("dm-lens turn summary")));
...
export function renderTurnSummaryMessage(
	...
	// dm-tui-compat HARD-CRASHES the host on any rendered line wider than the
