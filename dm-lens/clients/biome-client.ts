...
export class BiomeClient {
	...
	// (lives under ~/.dm-lens/tools), so it's stored separately from the
	 ...
	 * Falls back to ~/.dm-lens/tools, then npx.
	 *
	...
	private async getBiomeBinary(
		...
		// Also check ~/.dm-lens/tools (where ensureTool("biome") auto-installs),
