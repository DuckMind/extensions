 ...
 * keeps `DM_LENS_CASCADE_SETTLE_WAIT_MS` from doubling as a neighbour-count
...
export function cascadeSettleWaitMs(): number {
	const raw = Number(process.env.DM_LENS_CASCADE_SETTLE_WAIT_MS);
...
export const CASCADE_NEIGHBOUR_BUDGET = Math.max(
	...
	Number.parseInt(process.env.DM_LENS_CASCADE_NEIGHBOUR_BUDGET ?? "40", 10) ||
		DEFAULT_NEIGHBOUR_BUDGET,
);

/**
 * True when `DM_LENS_CASCADE_NEIGHBOUR_BUDGET` moved the ceiling away from the
...
const NEIGHBOUR_BUDGET_OVERRIDDEN =
	process.env.DM_LENS_CASCADE_NEIGHBOUR_BUDGET !== undefined &&
...
function neighbourCostMs(): number {
	return (
		toPositiveFinite(process.env.DM_LENS_CASCADE_NEIGHBOUR_COST_MS) ||
...
function neighbourFloor(): number {
	return (
		toPositiveFinite(process.env.DM_LENS_CASCADE_NEIGHBOUR_FLOOR) ||
		MIN_NEIGHBOUR_BUDGET
	);
}
...
export interface CascadeBudgetDecision {
	 ...
	 *
	 * Counts the drain only when it will actually run: `DM_LENS_QUIET_WINDOW=0`
	...
	if (onTimeMs <= 0 || onTimeMs < ceiling * perNeighbourMs) {
		...
		if (options.ceiling === undefined && NEIGHBOUR_BUDGET_OVERRIDDEN) {
			recordDegradationOnce({
				...
				reason:
					`DM_LENS_CASCADE_NEIGHBOUR_BUDGET=${CASCADE_NEIGHBOUR_BUDGET} needs ` +
