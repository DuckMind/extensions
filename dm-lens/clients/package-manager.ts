 ...
 * Supports npm, pnpm, yarn and bun so dm-lens works on whatever manager the
 ...
 * into dm-lens's own managed tools directory and `dmlens_rebuild` on a
 * dm-lens source checkout, never a user project's lockfile (#1496). Each
 ...
 * day, start a fresh session, dm-lens still reports it missing until a
 ...
 * satisfies the range makes that a no-op: dm-lens's managed tools tree stayed
 ...
 * global installs, but dm-lens's manager resolution prefers npm/pnpm first, so
...
export interface LocalBinWalkOptions {
	 ...
	 * dm-lens's OWN install directory (`runner-helpers.ts`), never a project
	 * cwd. `pi install npm:dm-lens` installs into the HOME-level manifest
	 * (`~/package.json` → `~/node_modules/dm-lens`), so the bundled
	 ...
	 * PROJECT's own dependency"); it says nothing about where dm-lens's own
 ...
 * the project walk and — with `ceiling: false` — dm-lens's own install-tree
