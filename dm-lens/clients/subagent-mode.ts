 ...
 *
 * Every child currently loads dm-lens fully by default, so a fan-out of N
 ...
 *
 * Escape hatch: `DM_LENS_SUBAGENT_FULL=1` forces full (non-light) behavior
...
/** True iff dm-lens is running inside a detected subagent child `pi` process
...
export function isSubagentSession(): boolean {
	if (process.env.DM_LENS_SUBAGENT_FULL === "1") return false;
...
export function subagentLightModeNotice(): string {
	return "subagent session — skipped background code-quality scans (set DM_LENS_SUBAGENT_FULL=1 to override)";
}
