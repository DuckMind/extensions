	...
	const tasks: Promise<void>[] = [
		...
		task("trivy", async () => {
			if (!TrivyClient.shouldScan(analysisRoot)) {
				markCold(
					"trivy",
					"not opted in (.dm-lens.json trivy.enabled) or no dependency manifest found",
				);
				return;
			}
		...
		//
		// No double-count / honesty gap (#533): explicit `dmlens_turn_end`
		// delivery may call `consumeTestFindings` (`runtime-context.ts`) to
		// read-and-clear this SAME cache key once. Automatic DM delivery uses a
