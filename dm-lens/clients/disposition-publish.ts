/**
 * Publishes `dmlens:diagnostic:disposition` on pi's shared `pi.events` bus
 * (#690).
 *
 ...
 * and the `DM_LENS_BUS_PUBLISH` kill switch (`isBusPublishEnabled`, imported
...
export const BUS_DISPOSITION_EVENT = "dmlens:diagnostic:disposition";
...
/**
 * Publish one `dmlens:diagnostic:disposition` event for a mark (one event per
...
export function publishDisposition(args: PublishDispositionArgs): void {
	...
	try {
		...
		if (!hasLoggedFailure) {
			...
			args.dbg?.(
				`disposition-publish: dmlens:diagnostic:disposition emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
