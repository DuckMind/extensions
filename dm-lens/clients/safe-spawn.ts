...
const lifetimeStateKey = Symbol.for("dm-lens.safe-spawn.lifetime-state");
 ...
 * no-op for space-free commands, so this is safe for the npm/.dm-lens tool paths
...
export function resolveWindowsCommandForEnvironment(
	...
	if (cached) {
		...
		// dm-lens-managed installs also reset the cache immediately on success.
