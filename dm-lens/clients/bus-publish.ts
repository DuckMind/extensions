/**
 * Publishes `dmlens:files:touched` on pi's shared `pi.events` bus (#482).
 *
 * This is dm-lens's FIRST `pi.events` broadcast surface: it exists so other
 * extensions in the same session can observe files dm-lens writes
...
export const BUS_FILES_TOUCHED_EVENT = "dmlens:files:touched";
 ...
 * dm-lens-mechanical hunk (autofix/format) from an agent edit when rendering
...
/**
 * Lazy env read (house style) so tests can flip `DM_LENS_BUS_PUBLISH` at
...
export function isBusPublishEnabled(): boolean {
	if (_envCache === undefined) {
		_envCache = process.env.DM_LENS_BUS_PUBLISH !== "0";
	}
	return _envCache;
}
...
export interface PublishFilesTouchedArgs {
	 ...
	 * an INGESTED bus event (something dm-lens consumed from `pi.events`).
	 * dm-lens does not consume any events today (see #482 non-goals), so this
...
/**
 * Publish one `dmlens:files:touched` event for a logical write batch (one
...
export function publishFilesTouched(args: PublishFilesTouchedArgs): void {
	...
	try {
		...
		if (!hasLoggedFailure) {
			...
			args.dbg?.(
				`bus-publish: dmlens:files:touched emit failed (further failures suppressed): ${err}`,
			);
		}
	}
}
