...
import {
	...
	DM_LENS_SYNTHETIC_MUTATION_FIELD,
...
import type { DmLensFlagSource } from "./lens-config.js";
import type { EditToolDetails } from "@duckmind/dm-coding-agent";
...
/**
 * The `tool_result` payload dm-lens actually receives.
 *
 ...
 * (`@duckmind/dm-coding-agent/dist/core/agent-session.js:243-256`, source
...
interface ToolResultEvent {
	...
	/** Host tool_result status; distinct from dm-lens PipelineResult.isError. */
	isError?: boolean;
	input: unknown;
	details?: unknown;
...
interface ToolResultDeps {
	...
	getFlagSource?: (name: string, filePath?: string) => DmLensFlagSource;
	dbg: (msg: string) => void;
...
function getDebounceMs(): number {
	const raw = Number(process.env.DM_LENS_TOOL_RESULT_DEBOUNCE_MS);
...
async function dispatchPipelineAnalysis(args: {
	 ...
	 * dm-lens' own immediate format/autofix rewrote the bytes (#2464 review
...
> {
	...
	// #2402: dm-lens' own immediate format/autofix may have rewritten the file
	...
	// The model's write/edit and dm-lens' own immediate format/autofix are now
...
export async function handleToolResult(deps: ToolResultDeps): Promise<{
	...
	if (
		...
		for (const wp of written) {
			...
			const syntheticEvent = {
				...
				input: { path: wp },
				isError: false,
				// #2423: dm-lens SYNTHESIZED this write from a bash command, so the
				// seam reports `provenance: "bash-derived"` and a consumer can tell
				// it apart from the host's own write tool.
				[DM_LENS_SYNTHETIC_MUTATION_FIELD]: "bash",
			};
		...
		// act on paths dm-lens already knows about (a read or a write this
		// session) so an `rm` on something dm-lens never touched is not treated
	...
	// `type`/`toolName`/`toolCallId`/`input`/`content`/`details`/`isError`/`usage`
	// (`@duckmind/dm-coding-agent/dist/core/agent-session.js:243-256`,
	...
	if (
		...
		if (isNewlyQueued || autofixNewlyQueued) {
			publishFormatQueued({
				filePath,
				cwd: dispatchCwd,
				// The v1 `dmlens:format:queued` payload declares
	...
	for (const changedFile of result.changedFiles ?? []) {
		...
		try {
			...
			dbg(
				`tool_result: tracking dm-lens side-effect change for ${resolvedChanged}`,
			);
		} catch (err) {
			dbg(
	...
	const attachmentText = postMutation
		? `dm-lens applied autofix to ${postMutation.filePath}. The following full content is authoritative for subsequent edits:\n\n${postMutation.content}`
		: "";
