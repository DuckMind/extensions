/**
 * Single "project scale" knob deriving dm-lens's five independent
 ...
 * env override (e.g. `DM_LENS_REVIEW_GRAPH_MAX_FILES`,
 * `DM_LENS_STARTUP_SCAN_MAX_ENTRIES`) is still checked at the call site
 * BEFORE falling back to this module's derived value, so it always wins:
 *
 *   1. `maxProjectFiles` in the project's `.dm-lens.json` (per-project — see
 *      `clients/project-lens-config.ts`). Requires a `cwd`; a call site with
 *      no cwd in hand skips straight to (2).
 *   2. `DM_LENS_MAX_PROJECT_FILES` environment variable.
...
import { loadDmLensProjectConfig } from "./project-lens-config.js";
...
const _envBase = lazyEnvNumber(
	"DM_LENS_MAX_PROJECT_FILES",
	DEFAULT_PROJECT_SCALE_BASE,
);

/** Test-only: clears the memoized `DM_LENS_MAX_PROJECT_FILES` read so a
 ...
 * project's `.dm-lens.json#reviewGraph.maxFiles` knob (#775 R2, an explicit
 ...
 * PRE-EXISTING `DM_LENS_REVIEW_GRAPH_MAX_FILES` env override BEFORE calling
