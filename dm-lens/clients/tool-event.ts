 ...
 *
 * Why this exists instead of importing from `@duckmind/dm-coding-agent`:
 ...
 * every `@duckmind/dm-coding-agent` import type-only (types compile away).
 ...
 * `isGrepToolResult`, `isFindToolResult`, `isLsToolResult`. dm-lens does NOT
 ...
 *    at dm-lens's `tool_call` sites. Adding seven fixed-name aliases of a
 *    generic we already have would be indirection, not consolidation.
 * 3. **The seven cover strictly fewer tools than dm-lens intercepts.** They
 *    enumerate the built-ins only; dm-lens's `tool_call`/`tool_result` paths
 *    also key off `lsp_navigation` and dm-lens's own registered tools, which no
 *    host discriminator names.
 * 4. **The host union is narrower than the events dm-lens handles.** The host
 *    `ToolResultEvent` declares `toolCallId`/`input`/`content`/`isError`/
 *    `details`/`usage`; dm-lens's local `ToolResultEvent`
 ...
 *    code. The standing reason not to narrow is point 3: dm-lens also
 ...
 * types rather than re-declared ad hoc. Genuinely dm-lens-specific `details`
 ...
 * why dm-lens can't import the SDK's own type at runtime), but
