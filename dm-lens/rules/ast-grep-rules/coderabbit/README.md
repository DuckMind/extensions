...
The rules are included in dm-lens's synthesized baseline `sgconfig` when a target repository does not provide its own `sgconfig.yml` / `sgconfig.yaml`. If a repository supplies its own sgconfig, dm-lens respects that project config instead.
