#!/usr/bin/env node
/**
 * dm-lens MCP server — exposes dm-lens's analysis to any MCP client (Claude Code).
 *
 ...
 * same dispatch/LSP/latency machinery dm-lens runs inside pi — which is what
...
function resolveDefaultCwd(): string {
	const fromArg = process.argv
		...
		?.slice("--cwd=".length);
	return fromArg ?? process.env.DM_LENS_MCP_CWD ?? process.cwd();
}
...
// build layout as the server) and the dm-lens repo root (for rebuilds).
...
//
// `DM_LENS_MCP_STALENESS_STAT_PATH` (test-only override): points the stamp at
...
const STALENESS_STAT_PATH =
	process.env.DM_LENS_MCP_STALENESS_STAT_PATH ?? SERVER_FILE;
const BUILD_STAMP = computeBuildStamp(STALENESS_STAT_PATH);

// `DM_LENS_MCP_STALENESS_INTERVAL_MS` (test-only override): shrinks the
...
const stalenessIntervalOverride = ((): number | undefined => {
	const raw = process.env.DM_LENS_MCP_STALENESS_INTERVAL_MS;
 ...
 * call. Disabled entirely by `DM_LENS_WARM_STALENESS_CHECK=0` (escape hatch).
 */
	...
	for (let depth = 0; depth < 6; depth++) {
		...
		if (fs.existsSync(pkgPath)) {
			try {
				...
				if (pkg.name === "dm-lens") return dir;
			} catch {
				// keep walking up
			}
		}
...
const REPO_ROOT = process.env.DM_LENS_MCP_REPO_ROOT
	? path.resolve(process.env.DM_LENS_MCP_REPO_ROOT)
...
async function ensureReady(cwd: string): Promise<void> {
	...
	try {
		...
		// dm-lens-ignore: missing-error-propagation
	} catch (err) {
		console.error(
			`[dm-lens-mcp] initLSPConfig failed for ${normalized}: ${err}`,
		);
	}
...
// Gated by DM_LENS_MCP_AUTO_SESSION=1 because the full session_start runs project
...
// tracked so `dmlens_health` can report it, and `tools/call` self-heals by
...
function maybeAutoSessionStart(): void {
	if (process.env.DM_LENS_MCP_AUTO_SESSION !== "1") return;
	...
	autoSessionInFlight = ensureReady(DEFAULT_CWD)
		...
		.then(() => {
			...
			console.error("[dm-lens-mcp] auto session_start complete");
		})
		.catch((err) => {
			autoSessionState = { ...autoSessionState, error: String(err) };
			console.error(`[dm-lens-mcp] auto session_start failed: ${err}`);
		})
		.finally(() => {
...
/**
 * `dmlens_health`-facing view of auto-session state. Returns `null` when
 * `DM_LENS_MCP_AUTO_SESSION` isn't set at all, so "the feature is off" is
 * distinguishable from "attempted and failed".
 */
function getAutoSessionStatus(): AutoSessionState | null {
	if (process.env.DM_LENS_MCP_AUTO_SESSION !== "1") return null;
...
function rejectForeignTurnCwd(route: string, cwd: string): string {
	...
	console.error(`[dm-lens-mcp] rejected ${message}`);
	return message;
}
...
function startIpcServer(): void {
			...
			createWarmIpcLineReader((line) => {
				void requestQueue.enqueue(async () => {
					try {
						if (isWarmBuildStale()) {
							console.error(
								"[dm-lens-mcp] warm request: build stale, replying with an error",
							);
						...
						if (parsed.route === "turn-end") {
							...
							if (socket.destroyed) {
								console.error(
									`[dm-lens-mcp] warm turn-end: client gone before the pass started (${turnCwd})`,
								);
								return;
							}
							console.error(`[dm-lens-mcp] warm turn-end: ${turnCwd}`);
						...
						console.error(`[dm-lens-mcp] warm analyze: ${req.file}`);
	...
	ipc.on("error", (err) => {
		...
		console.error(`[dm-lens-mcp] IPC listener unavailable: ${err}`);
	});

	ipc.listen(IPC_PATH, () => {
		console.error(`[dm-lens-mcp] warm side-channel listening at ${IPC_PATH}`);
	});
 ...
 * worth flagging even if the workspace has had zero dmlens_analyze calls yet. */
...
function graphStalenessNote(
	...
	return (
		`${label} last updated ${formatRelativeAge(ageMs)}; run dmlens_analyze ` +
		"on recently-changed files, dmlens_session_start, or dmlens_rebuild to refresh it."
	);
}
...
// (() => flushDebouncedToolResults()) before reading dmlens_diagnostics. Verdict:
...
// this MCP process never calls (dmlens_analyze routes through the independent
...
// whatever `.dm-lens.json` happens to sit there, never the caller's project.
...
const ALL_TOOLS = [
	{
		name: "dmlens_analyze",
		description:
			"Run dm-lens's per-edit dispatch pipeline (LSP + linters + structural " +
		...
		inputSchema: {
			...
			properties: {
				...
				mode: {
					...
					enum: ["warm", "fresh"],
					description:
						"warm (default): run in this server process — fast, warm LSP, but reflects the code the server was started with. fresh: fork a worker that loads the freshly-built code from disk — slower, but reflects the latest commit (the honest review loop; pair with dmlens_rebuild).",
				},
				flags: {
					type: "object",
					description:
						'Optional dm-lens flag overrides for this run, e.g. {"no-lsp": true} to bench the non-LSP path.',
				},
			},
			required: ["file"],
		},
	...
	{
		name: "dmlens_diagnostics",
		description:
			"Query dm-lens's diagnostic state across ALL runners (not just LSP). " +
	...
	},
	{
		name: "dmlens_rebuild",
		description:
			"Rebuild dm-lens so subsequent `dmlens_analyze mode=fresh` runs reflect " +
			...
			"commit → dmlens_rebuild → dmlens_analyze mode=fresh.",
		inputSchema: { type: "object", properties: {} },
	},
	{
		name: "dmlens_project_scan",
		description:
			"Cheap project-wide scan (tree-sitter + fact rules) across source files, " +
			"returning structural/quality diagnostics. Complements dmlens_diagnostics " +
	...
	},
	{
		...
		description:
			...
			"symbol_search finds candidate files, dmlens_module_report explains one, " +
			"dmlens_read_symbol reads a body. Complements grep (raw substrings) and " +
			...
			"use dmlens_module_report on `file` for the real outline. Returns " +
			"`available: false` with a retry hint if the index isn't built yet for this " +
			"workspace (dmlens_session_start builds it, or it self-builds in the background " +
			"on first query).",
		inputSchema: {
			type: "object",
			properties: {
				...
				paths: {
					...
					items: { type: "string" },
					description:
						"Glob array scoping hits to matching files — same shape/semantics as dmlens_ast_grep_search's `paths` (a bare directory/file entry scopes its whole subtree). Filters before ranking, so scores within the scoped set are unaffected.",
				},
				lang: {
					type: "string",
					description:
						"Restrict hits to one language, using the same identifiers as dmlens_ast_grep_search's `lang` param (e.g. 'typescript', 'python', 'go').",
				},
			},
			required: ["query"],
		},
	},
	{
		...
		description:
			...
			"recommendedReads. To read a symbol's body: call dmlens_read_symbol (or " +
			...
			"read; then use dmlens_read_symbol for the exact body. Single mode: " +
	...
	},
	{
		...
		description:
			...
			"discovery funnel: dmlens_project_report orients, dmlens_module_report " +
			"explains a file, dmlens_read_symbol reads a body. Six capped, ranked " +
	...
	{
		name: "dmlens_read_enclosing",
		description:
			"Return the verbatim source for the smallest useful symbol/callback " +
			"enclosing a line in a file. Use after dmlens_ast_grep_search, " +
			"dmlens_diagnostics, or dmlens_lsp_navigation locations when you need " +
	...
	},
	{
		name: "dmlens_health",
		description:
			"dm-lens runtime health for THIS server: alive LSP servers, last dispatch " +
			...
			"dm-lens across every process it owns (host + LSP children) machine-wide, " +
			"from the shared instance registry.",
		inputSchema: { type: "object", properties: {} },
	},
	{
		name: "dmlens_effective_config",
		description:
			"The resolved dm-lens configuration with the provenance of every " +
	...
	{
		name: "dmlens_session_start",
		description:
			"Run dm-lens's real session_start lifecycle: warm the dominant-language " +
			"LSP (so subsequent dmlens_analyze is LSP-complete), establish the " +
			...
			"dmlens_diagnostics afterwards). Run once per workspace before reviewing.",
		inputSchema: {
			type: "object",
	...
	{
		name: "dmlens_turn_end",
		description:
			"Run dm-lens's real turn_end lifecycle over the files changed this turn: " +
			...
			"findings. `files` is OPTIONAL — dmlens_analyze (and the PostToolUse hook) " +
	...
	{
		name: "dmlens_lsp_diagnostics",
		description:
			"Pure LSP diagnostics for a file, directory, or batch of files (type " +
			"errors only — narrower than dmlens_diagnostics, which spans all runners).",
...
const TOOLS = canRebuildDmLens(REPO_ROOT)
	? ALL_TOOLS
	: ALL_TOOLS.filter((tool) => tool.name !== "dmlens_rebuild");
...
/** The per-tier and per-code counts `dmlens_health` reports for a resolution. */
...
/**
 * The `Config:` line of `dmlens_health` (#2427).
 *
...
/** One `dmlens_effective_config` server row, with the tier that decided it. */
...
/** The `File:` heading of `dmlens_effective_config`. */
...
/** One `dmlens_effective_config` config-document row. */
...
async function callTool(
	...
	if (name === "dmlens_analyze") {
		const file = args.file;
		if (typeof file !== "string" || file.length === 0) {
			return {
				...toolText("dmlens_analyze requires a 'file' string."),
				isError: true,
			};
		}
		...
		// Warm = an edit-detection path: register the file so dmlens_turn_end picks
		// it up without an explicit file list, and maintain the review graph
		// (#536) so dmlens_module_report/dmlens_symbol_search reflect files
	...
	if (name === "dmlens_project_scan") {
		...
		if (snapshot.unsafeRoot) {
			return toolText(
				`Refused to scan: the working directory (${cwd}) resolves at or above the home directory, so a project scan would walk every unrelated tree under it. Run dmlens_project_scan from inside a project directory. This is NOT a clean result — nothing was scanned.`,
	...
	if (name === "dmlens_symbol_search") {
		...
		if (!available) {
			return toolText(
				hint ??
					"No word index for this workspace yet — run dmlens_session_start first.",
			...
			lines.join("\n"),
			{
				...
				results: results.map((result) => {
					...
					return {
						...
						suggestedNext: { tool: "dmlens_module_report", path: relFile },
					};
				}),
				...(stalenessNote ? { staleness: stalenessNote } : {}),
			},
	...
	if (name === "dmlens_read_symbol") {
		...
		if (!result.found) {
			const suggestionSuffix = result.suggestions?.length
				...
				: " Use dmlens_module_report to list symbols.";
			return {
				...toolText(
	...
	if (name === "dmlens_read_enclosing") {
		...
		// Same #512 convention as dmlens_read_symbol: the header line already
	...
	if (name === "dmlens_health") {
		...
		// of dmlens_health's (much older, more load-bearing) reporting.
		...
		const lines = [
			...
			autoSession
				...
				: "Auto session_start: disabled (DM_LENS_MCP_AUTO_SESSION not set)",
			turnEnd
			...
			footprint
				? `Resource footprint: ${footprint.instanceCount} dm-lens instance(s) · ` +
	...
	if (name === "dmlens_effective_config") {
		const cwd = typeof args.cwd === "string" ? args.cwd : DEFAULT_CWD;
	...
	if (name === "dmlens_session_start") {
		...
		const lines = [
			...
			"knip/jscpd/type-coverage/dep scans run in background — query dmlens_diagnostics shortly.",
			outcome.guidance ? `\n${outcome.guidance}` : "",
		];
		return toolText(lines.filter(Boolean).join("\n"), outcome);
	}

	if (name === "dmlens_turn_end") {
		const cwd = typeof args.cwd === "string" ? args.cwd : DEFAULT_CWD;
		await ensureReady(cwd);
	...
	if (name === "dmlens_ast_grep_search" || name === "dmlens_ast_grep_replace") {
		const cwd = typeof args.cwd === "string" ? args.cwd : DEFAULT_CWD;
		const tool =
			name === "dmlens_ast_grep_search"
				? astGrepSearchTool
	...
	if (name === "dmlens_lsp_navigation" || name === "dmlens_lsp_diagnostics") {
		const cwd = typeof args.cwd === "string" ? args.cwd : DEFAULT_CWD;
		await ensureReady(cwd);
		const tool =
			name === "dmlens_lsp_navigation" ? lspNavigationTool : lspDiagnosticsTool;
		const out = (await tool.execute(
			"mcp",
			args,
			new AbortController().signal,
...
//
// `dmlens_analyze` is handled specially above (force-routes to the existing
...
//     in-memory review graph built by warm `dmlens_analyze` calls, the warm
...
//
//   dmlens_module_report, dmlens_symbol_search — warm review-graph / word-index
...
//   dmlens_project_scan                        — CacheManager instance is warm-
//     process state; scan results are cache-derived.
//   dmlens_health, dmlens_latency              — these tools report ON the warm
...
//   dmlens_session_start, dmlens_turn_end      — mutate warm LSP/graph state;
//     must run in-process, can't be forked fresh.
//   dmlens_ast_grep_search, dmlens_ast_grep_replace,
//   dmlens_lsp_navigation, dmlens_lsp_diagnostics — depend on the warm LSP
//     fleet / ast-grep client instances; no fresh-fork machinery exists for
//     them today (only dmlens_analyze's worker.ts loads a fresh dispatch
//     graph) and the LSP fleet specifically CANNOT be recreated cheaply per
//     call, so warn is the only honest option.
//   dmlens_read_symbol, dmlens_read_enclosing  — stateless file reads, but no
...
//   dmlens_effective_config                    — derives (never reads) the
...
//
// `dmlens_rebuild` is deliberately excluded: it doesn't answer with analysis
...
const WARN_ONLY_STALE_TOOLS = new Set([
	"dmlens_module_report",
	"dmlens_project_report",
	"dmlens_symbol_search",
	"dmlens_project_scan",
	"dmlens_health",
	"dmlens_latency",
	"dmlens_session_start",
	"dmlens_turn_end",
	"dmlens_ast_grep_search",
	"dmlens_ast_grep_replace",
	"dmlens_lsp_navigation",
	"dmlens_lsp_diagnostics",
	"dmlens_read_symbol",
	"dmlens_read_enclosing",
	"dmlens_effective_config",
]);
...
async function handleRequest(request: JsonRpcRequest): Promise<void> {
	...
	switch (method) {
		...
		case "tools/call": {
			...
			// (DM_LENS_MCP_AUTO_SESSION=1) but never completed successfully — never
			...
			try {
				...
				// #535: dmlens_analyze already self-routes (fresh-fork) when stale —
				...
				sendResult(id ?? null, {
					...toolText(
						`dm-lens tool '${name}' failed: ${(err as Error).message}`,
					),
					isError: true,
				});
			}
...
startIpcServer();
console.error(`[dm-lens-mcp] ready (cwd=${DEFAULT_CWD})`);
