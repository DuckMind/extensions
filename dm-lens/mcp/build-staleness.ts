 ...
 * pre-#517 `dmlens_module_report` shape. That is the "plausible-but-wrong"
 ...
 * cheap, but under a tool-call burst (e.g. a batch of `dmlens_symbol_search`)
...
/** Env kill switch: `DM_LENS_WARM_STALENESS_CHECK=0` disables the whole guard. */
export function stalenessCheckEnabled(
	env: Record<string, string | undefined> = process.env,
): boolean {
	return env.DM_LENS_WARM_STALENESS_CHECK !== "0";
}
...
export const STALE_WARN_ONLY =
	...
	"dmlens_rebuild + a tool that supports mode=fresh if available.";
