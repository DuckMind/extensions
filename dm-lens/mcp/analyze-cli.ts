#!/usr/bin/env node
/**
 * dm-lens-analyze — the *push* half of the mirror, in two modes.
 *
 * Per-edit (default): a Claude Code PostToolUse hook (matched to Edit|Write)
 * that fires dm-lens automatically after every edit, the way pi's per-edit
 ...
 * `dmlens_analyze` against the warm MCP server for the type-check). The fast
...
function formatReport(result: McpAnalyzeResult, cwd: string): string {
	...
	const lines = [
		...
		`🔎 dm-lens: ${rel} — ${result.counts.blockers} blocking, ${result.counts.warnings} warning(s), ${result.counts.advisories} advisory(ies)`,
	];
	...
	if (result.lsp && result.lsp.status === "skipped") {
		lines.push(
			"  (LSP type-check skipped — run dmlens_analyze on the warm MCP server for type errors)",
		);
	}
	return lines.join("\n");
}
...
/**
 * #1272: every skip used to read "no warm dm-lens MCP server", including the
...
const TURN_END_SKIP_REASONS: Record<WarmDiagnosticsFailureReason, string> = {
	"ipc-error":
		"no warm dm-lens MCP server answered for this workspace, or its build is stale — start the MCP server, or run `npm run build` if you have edited dm-lens sources",
	timeout:
		"the warm dm-lens MCP server did not finish the pass within the hook budget — the turn-end findings are kept and will be delivered on a later Stop",
	"schema-mismatch":
		"the warm dm-lens MCP server speaks a different turn-end schema than this hook binary — rebuild and restart the server so both halves match",
	"stale-answer":
		"the warm dm-lens MCP server replied about a superseded state — the findings are kept for a later Stop",
};
...
async function runTurnEndMode(
	...
	if (payload?.hook_event_name === "SubagentStop") {
		process.stderr.write(
			"dm-lens turn-end skipped: SubagentStop (the main agent's Stop runs the pass)\n",
		);
		process.exitCode = 0;
		return;
	}
	...
	if (!outcome.available) {
		...
		const message = `🔎 dm-lens turn-end skipped (${outcome.reason}): ${detail}`;
...
async function main(): Promise<void> {
	...
	if (!result) {
		...
		result = await analyzeFile(file, cwd, {
			...
			record: false,
			// Edit-detection path (PostToolUse) — mark the file for dmlens_turn_end.
...
main().catch((err) => {
	process.stderr.write(`dm-lens-analyze failed: ${(err as Error).message}\n`);
