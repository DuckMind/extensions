...
process.env.DM_LENS_GRAPH_PERSIST_DEBOUNCE_MS = "3600000";
...
function fail(reason: string): never {
	process.stderr.write(`dm-lens build-graph failed: ${reason}\n`);
	process.exitCode = 1;
	throw FAILED;
}
...
async function buildGraph(): Promise<void> {
	...
	if (!persisted.ok) {
		...
		if (!buildInfo.graphChanged && fs.existsSync(snapshotPath)) {
			...
			process.stdout.write(
				`dm-lens build-graph: snapshot already current (mode=${buildInfo.mode}) ` +
	...
	if (coverage?.partial) {
		...
		// fully covered.
		process.stdout.write(
			`dm-lens build-graph: PARTIAL persist (cap=${coverage.cap} exceeded) ` +
				`files=${graph.fileNodes.size} nodes=${graph.nodes.size} ` +
	...
	process.stdout.write(
		`dm-lens build-graph: files=${graph.fileNodes.size} nodes=${graph.nodes.size} ` +
...
async function main(): Promise<void> {
	...
	if (command !== "build-graph") {
		fail("usage: dm-lens build-graph [--cwd <dir>]");
	}
	await buildGraph();
}
...
main().catch((err) => {
	...
	process.stderr.write(
		`dm-lens build-graph failed: ${err instanceof Error ? err.message : String(err)}\n`,
	);
	process.exitCode = 1;
});
