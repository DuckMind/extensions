 ...
 *                     inline `dm-lens-ignore: <rule>` comment into the source
...
export function createLensDiagnosticMarkTool(
	...
	return {
		...
		description:
			...
			"up tagged in a later lens_diagnostics mode=full. suppress additionally writes a `dm-lens-ignore: " +
		...
		async execute(
			...
			if (disposition === "suppress" && !rule) {
				return {
					content: [
						{
							type: "text" as const,
							text: "disposition=suppress requires `rule` — the inline dm-lens-ignore comment names a rule id.",
						},
					],
					isError: true,
					details: {},
			...
			const verb =
				disposition === "suppress"
					? `suppressed (inline dm-lens-ignore comment written above line ${verifiedLine})`
					: disposition === "defer"
