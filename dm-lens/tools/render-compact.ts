...
// dm-lens's own tools and driven off structured `details` rather than blind
...
import { Text } from "../clients/deps/dm-tui-compat.js";
import type { Theme, ThemeColor } from "@duckmind/dm-coding-agent";
...
/** How a rendered line should be styled. `brand` is dm-lens blue (our colour);
 * `error` and `output` defer to the active theme so red/normal stay legible. */
export type CompactStyle = "brand" | "error" | "output";

// dm-lens brand colour: blue characters on whatever background the pi tool shell
...
const DM_LENS_BLUE_FG = "\x1b[1m\x1b[38;2;96;165;250m"; // bold blue
...
export function selectCompactText<D = unknown>(
	...
	// Collapsed summaries render in dm-lens blue; errors stay theme-red.
...
export function paintCompact(
	...
	if (style === "brand") {
		return `${DM_LENS_BLUE_FG}${text}${RESET_FG}`;
	}
