 ...
 *   all             — all known diagnostic counts across every file dm-lens has
...
export function createLensDiagnosticsTool(
	...
	return {
		...
		description:
			"Query dm-lens's diagnostic state. mode=delta/all are cache-only and instant; " +
		...
		parameters: Type.Object({
			...
			maxLspFiles: Type.Optional(
				Type.Number({
					description:
						"mode=full only: cap the number of files routed through the language server for the project-wide LSP sweep. On large projects (e.g. a Next.js app with thousands of source files) the uncapped sweep can take many minutes; set this to bound it. Default is generous (env DM_LENS_LSP_WORKSPACE_MAX_FILES, else 5000).",
				}),
			),
			...
			paths: Type.Optional(
				Type.Array(Type.String(), {
					...
					description:
						...
						"of cached/session state — they can only show findings for files dm-lens " +
						...
						".gitignore/.dm-lens.json; a directory entry's expansion still honors " +
 ...
 * `.dm-lens.json` `rules.<id>.disable`/`select` policy so a project's policy
...
/**
 * Load the rule-policy map from a project's `.dm-lens.json` — same source the
 * per-edit dispatch path uses, so a project's policy applies consistently
 * across every output surface. `loadDmLensProjectConfig` is mtime-cached, and
...
function loadProjectRulePolicyMap(cwd: string) {
	return rulePolicyMapFromConfig(loadDmLensProjectConfig(cwd).rules);
}
...
function formatDeltaMode(
	...
	// Project rule policy (`.dm-lens.json` `rules.<id>.disable` /
...
function pathsScopeCacheOnlyNote(scope: PathsScope | undefined): string {
	if (!scope) return "";
	return (
		"\n\nNote: paths restricts this to cached findings for files dm-lens has " +
 ...
 * `~/.dm-lens/latency.log` by hand — it turned out to be 100% one push-only
...
export function mergeDiagnosticsWithWidgetSummaries(
	...
	for (const summary of widgetSummaries) {
		...
		// practice; inline dm-lens-ignore comments are the primary
...
/**
 * Apply inline `dm-lens-ignore` suppression to the merged mode=full summaries so
 ...
 * suppression.
 *
 * Also applies the project's `.dm-lens.json` `rules.<id>.disable`/`select`
...
async function applyInlineSuppressionsToSummaries(
	...
	return Promise.all(
		summaries.map(async (summary) => {
			...
			// Project rule policy (`.dm-lens.json` `rules.<id>.disable`/`select`).
	...
	if (aborted) {
		...
		const note = timedOut
			? `\n\n⚠ Scan exceeded its ${Math.round((options.wallClockMs ?? 0) / 1000)}s time budget and was stopped — results are partial. ` +
				"Narrow it with maxLspFiles, or raise DM_LENS_LENS_DIAGNOSTICS_FULL_TIMEOUT_MS."
...
function formatAllMode(
	...
	const staleNote =
		...
		(dependencyDemoted > 0
			...
			: "") +
		(footerCapped > 0
			? ` (${footerCapped} demoted finding${footerCapped === 1 ? "" : "s"} capped after ${DEPENDENCY_DRIFT_MAX_DELIVERIES} unconfirmed deliveries — no longer shown in the dm-lens footer, still listed here; re-run to confirm)`
			: "");
	...
	// policy (not before): a finding a mark or a `.dm-lens.json` rule policy
