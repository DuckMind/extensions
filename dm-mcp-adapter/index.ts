import type { AgentToolUpdateCallback, ExtensionAPI, ExtensionContext, ToolInfo } from "@duckmind/dm-coding-agent";
...
export const MCP_RUNTIME_REGISTER_EVENT = "dm-mcp-adapter:runtime-register:v1" as const;
export const MCP_RUNTIME_REGISTER_VERSION = 1 as const;

export const MCP_RUNTIME_SNAPSHOT_EVENT = "dm-mcp-adapter:runtime-snapshot:v1" as const;
...
function installMcpAdapter(pi: ExtensionAPI, options: McpAdapterOptions) {
  ...
  function startInitialization(ctx: ExtensionContext, owner: McpRuntimeOwner, oauthRuntime: McpOAuthRuntime, generation: number, staleReason: string): Promise<void> {
    ...
    return promise.then(async (nextState) => {
      ...
      // after DM's model-facing direct-tool surface reflects live metadata.
  ...
  pi.on("session_shutdown", async () => {
    ...
    // DM context after session shutdown.
  ...
  registerMcpCommand("dm-mcp");
  // Keep the historical alias for existing automation; documentation advertises DM commands only.
...
/**
 * Register an MCP server with the adapter installed for this DM instance.
...
export function registerMcpServer(options: { pi: ExtensionAPI; name: string; definition: ServerEntry }): McpServerRegistration {
  ...
  if (!request.result) {
    throw new Error("dm-mcp-adapter is not installed for this DM instance");
  }
