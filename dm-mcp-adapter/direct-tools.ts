import type { AgentToolResult, AgentToolUpdateCallback, ExtensionContext } from "@duckmind/dm-coding-agent";
...
export function buildProxyDescription(config: McpConfig): string {
  let desc = `MCP gateway — server status, tool search/describe, auth, and single MCP tool calls. When one request needs several MCP calls with logic between them, use mcpScript. Non-MCP DM tools should be called directly, not through mcp.\n`;
