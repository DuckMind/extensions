...
interface PackageManifest {
  name?: unknown;
  dm?: { mcp?: unknown };
  // `pi.mcp` is read only as a compatibility fallback for installed packages.
  pi?: { mcp?: unknown };
}
...
export function loadPackageMcpConfigs(cwd = process.cwd()): McpConfig {
  ...
  for (const packageRoot of getConfiguredPackageRoots(cwd)) {
    ...
    const manifestMcp = manifest.dm?.mcp ?? manifest.pi?.mcp;
    const manifestKey = manifest.dm?.mcp !== undefined ? "dm.mcp" : "pi.mcp";
    const paths = getManifestMcpPaths(manifestMcp, manifest.name, manifestKey);
    if (!paths) continue;
    ...
    for (const path of paths) {
      ...
      if (!configPath) {
        console.warn(`DM package ${manifest.name} skips MCP config ${path}: file must stay inside the package`);
        continue;
      }
      ...
      for (const [serverName, server] of Object.entries(config.mcpServers)) {
        ...
        if (packageServers.has(normalizedName) || seen.has(normalizedName)) {
          console.warn(`DM package ${manifest.name} skips duplicate normalized MCP server ${normalizedName}`);
          continue;
        }
...
function getConfiguredPackageRoots(cwd: string): string[] {
  ...
  for (const [settingsPath, scope] of [
    ...
    const settings = readOptionalJson(settingsPath, `${scope} DM settings`);
    if (settings === undefined) continue;
    if (typeof settings !== "object" || settings === null || Array.isArray(settings)) {
      throw new Error(`${scope} DM settings ${settingsPath} must be a JSON object`);
    }
    const packages = (settings as { packages?: unknown }).packages;
    if (packages === undefined) continue;
    if (!Array.isArray(packages)) throw new Error(`${scope} DM settings ${settingsPath} packages must be an array`);
    for (const entry of packages) {
      const source = typeof entry === "string"
        ...
        : entry && typeof entry === "object" && !Array.isArray(entry) && typeof (entry as PackageSetting).source === "string"
          ...
          : undefined;
      if (!source) throw new Error(`${scope} DM settings ${settingsPath} package entries must be strings or objects with a string source`);
...
function readPackageManifest(packageRoot: string): PackageManifest | null {
  const manifest = readOptionalJson(join(packageRoot, "package.json"), `DM package manifest ${packageRoot}`);
  return manifest && typeof manifest === "object" && !Array.isArray(manifest) ? manifest as PackageManifest : null;
}

function getManifestMcpPaths(value: unknown, packageName: string, manifestKey: string): string[] | null {
  const paths = typeof value === "string" ? [value] : Array.isArray(value) && value.every((path): path is string => typeof path === "string") ? value : null;
  if (value !== undefined && !paths) console.warn(`DM package ${packageName} ignores invalid ${manifestKey} manifest entry`);
  return paths;
}

function readMcpConfig(path: string, packageName: string): McpConfig | null {
  const config = readRequiredJson(path, `DM package ${packageName} MCP config`);
  if (!config || typeof config !== "object" || Array.isArray(config)) {
    throw new Error(`DM package ${packageName} MCP config ${path} must be a JSON object`);
  }
  const servers = (config as { mcpServers?: unknown }).mcpServers;
  if (!servers || typeof servers !== "object" || Array.isArray(servers)) {
    throw new Error(`DM package ${packageName} MCP config ${path} must contain a JSON object mcpServers field`);
  }
  const mcpServers: Record<string, ServerEntry> = {};
  for (const [name, server] of Object.entries(servers)) {
    if (!server || typeof server !== "object" || Array.isArray(server)) {
      throw new Error(`DM package ${packageName} MCP config ${path} server ${name} must be a JSON object`);
    }
