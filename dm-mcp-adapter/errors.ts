...
export function getInputRequiredNeedsUiDetails(
  ...
  const guidance = request.inputMethod === "elicitation/create"
    ? "Run this call in an interactive DM session with elicitation enabled, then retry."
    : "Run this call in an interactive DM session with the required MCP capability enabled, then retry.";
...
export class InputRequiredNeedsUiError extends McpUiError {
  constructor(readonly details: InputRequiredNeedsUiDetails, cause?: Error) {
    super(details.message, {
      ...
      context: { ...details },
      recoveryHint: "Run this call in an interactive DM session with the required MCP input handler enabled, then retry.",
      ...(cause ? { cause } : {}),
    });
