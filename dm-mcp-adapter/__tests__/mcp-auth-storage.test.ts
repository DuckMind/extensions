...
describe("mcp-auth storage paths", () => {
  const originalEnv = {
    ...
    DM_MCP_ADAPTER_TEST_AUTH_STORE: process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE,
    DM_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY: process.env.DM_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY,
    DM_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL: process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL,
    DM_MCP_ADAPTER_KEYRING_RECOVERY_NODE: process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_NODE,
    DM_MCP_ADAPTER_KEYRING_RECOVERY_HELPER: process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_HELPER,
    DM_MCP_ADAPTER_FAKE_KEYRING_STORE: process.env.DM_MCP_ADAPTER_FAKE_KEYRING_STORE,
  };
  let authDir: string;
  ...
  it("uses configured oauthDir as the legacy import source", () => {
    ...
    const options = getAuthStorageOptions(".dm/oauth", project);
    ...
    expect(filePath.startsWith(join(project, ".dm", "oauth"))).toBe(true);
    expect(existsSync(filePath)).toBe(false);
    expect(getAuthEntry("configured", options)?.tokens?.accessToken).toBe("legacy-token");
  ...
  it("does not use configured oauthDir values as secure-store namespaces", () => {
    ...
    const optionsA = getAuthStorageOptions(".dm/oauth", projectA);
    const optionsB = getAuthStorageOptions(".dm/oauth", projectB);
  ...
  it("keeps MCP_OAUTH_DIR as the explicit override over settings.oauthDir", () => {
    ...
    expect(filePath.startsWith(join(project, ".dm", "oauth"))).toBe(false);
    rmSync(project, { recursive: true, force: true });
  });
  ...
  it("persists records that exceed the strictest per-value store limit", () => {
    process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    const accessToken = "x".repeat(5000);
...
const path = process.env.DM_MCP_ADAPTER_FAKE_KEYRING_STORE;
...
`);
    ...
    process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = "keyrevoked";
    process.env.DM_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY = "1";
    process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL = keyctlPath;
    process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_NODE = process.execPath;
    process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_HELPER = helperPath;
    process.env.DM_MCP_ADAPTER_FAKE_KEYRING_STORE = storePath;
  ...
  it("does not use the recovery helper for generic secure-store failures", () => {
    ...
    process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = "unavailable";
    process.env.DM_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY = "1";
    process.env.DM_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL = keyctlPath;
    process.env.DM_MCP_ADAPTER_FAKE_KEYRING_STORE = storePath;
