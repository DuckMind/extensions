...
describe("cli init helper", () => {
  ...
  it("adds detected host imports to the DM config", async () => {
    ...
    const piConfigPath = join(home, ".dm", "agent", "mcp.json");
  ...
  it("detects TOML-only Codex config during dry-run", async () => {
    ...
    expect(logs.join("\n")).toContain("Detected host configs to import into DM: codex");
    expect(existsSync(join(home, ".dm", "agent", "mcp.json"))).toBe(false);
  });

  it("loads existing DM config as JSONC and lists .agents standard paths", async () => {
    ...
    writeFileSync(join(home, ".dm", "agent", "mcp.json"), `{
    ...
    expect(output).toContain("No DM config changes needed.");
  });
  ...
  it("writes detected host imports to DM_CODING_AGENT_DIR when set", async () => {
    const home = mkdtempSync(join(tmpdir(), "pi-mcp-cli-home-"));
  ...
  it("writes init output to the branded host agent directory", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(errors).toEqual([]);
    expect(logs.join("\n")).toContain(`DM global override: ${join(agentDir, "mcp.json")}`);
    expect(logs.join("\n")).toContain(`Project DM override: ${join(process.cwd(), ".arc", "mcp.json")}`);
  ...
  it("runs when invoked through a symlinked bin path", () => {
    ...
    const symlinkPath = join(binDir, "dm-mcp-adapter");
    ...
    const result = spawnSync(process.execPath, [symlinkPath, "init", "--dry-run"], {
      ...
      env: {
        ...process.env,
        HOME: home,
        PI_CODING_AGENT_DIR: join(home, ".dm", "agent"),
      },
      encoding: "utf-8",
    });
    ...
    expect(result.stdout).toContain("No DM config changes needed.");
  });
  ...
  it("explains that install now goes through `pi install`", async () => {
    ...
    expect(errors.join("\n")).toContain("Use `dm install npm:dm-mcp-adapter` instead");
    expect(logs).toEqual([]);
  });
});
...
describe("cli token helper", () => {
  const originalHome = process.env.HOME;
  const originalAuthStore = process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
  const originalCwd = process.cwd();

  beforeEach(() => {
    vi.resetModules();
    process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = "memory";
  });

  afterEach(() => {
    process.env.HOME = originalHome;
    if (originalAuthStore === undefined) {
      delete process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
    } else {
      process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = originalAuthStore;
    }
    process.chdir(originalCwd);
  });
