import { KeybindingsManager, TUI_KEYBINDINGS, visibleWidth } from "@duckmind/dm-tui";
...
describe("mcp-setup-panel custom keybindings", () => {
  ...
  it("shows the actual config precedence including .agents paths", () => {
    ...
    expect(output).toContain("Advanced compatibility and DM-owned layers:");
    ...
    expect(output).toContain("6. .dm/mcp.json");
    panel.dispose();
  });

  it("shows the branded project DM override path in config precedence", () => {
