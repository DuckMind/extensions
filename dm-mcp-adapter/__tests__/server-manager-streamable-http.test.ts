...
async function withAuthStore(value: string, run: () => Promise<void>): Promise<void> {
  const previousStore = process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
  process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = value;
  try {
    await run();
  } finally {
    if (previousStore === undefined) {
      ...
      process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = previousStore;
    }
  }
}
...
describe("McpServerManager StreamableHTTP transport", () => {
  ...
  it("does not multiply a transient initialize 503 inside DM", async () => {
