...
describe("disabled MCP servers", () => {
  ...
  it("writes only a project-local disabled override and removes it cleanly", () => {
    ...
    const filePath = join(cwd, ".dm", "mcp.json");
    mkdirSync(join(cwd, ".dm"));
  ...
  it("writes an explicit enabled override when a lower config disables the server", () => {
    ...
    expect(JSON.parse(readFileSync(join(cwd, ".dm", "mcp.json"), "utf8")).mcpServers.lower).toEqual({ disabled: false });
  ...
  it("enables a server disabled by an import declared in the project override", () => {
    ...
    writeFileSync(join(cwd, ".dm", "mcp.json"), JSON.stringify({
      imports: ["vscode"],
    ...
    expect(JSON.parse(readFileSync(join(cwd, ".dm", "mcp.json"), "utf8")).mcpServers.imported).toEqual({ disabled: false });
