...
const previousAuthStore = process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;

afterEach(() => {
  if (previousAuthStore === undefined) delete process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
  else process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = previousAuthStore;
});
