...
const mocks = vi.hoisted(() => ({
  ...
  openMcpPanel: vi.fn(),
  openMcpSetup: vi.fn(),
  writeProjectServerDisabledOverride: vi.fn(() => ({ path: "/tmp/project/.dm/mcp.json", changed: true })),
...
describe("mcpAdapter session lifecycle", () => {
  ...
  it("waits for keep-alive convergence before DM processes the next input", async () => {
    const config = {
      mcpServers: {
  ...
  it("does not fail load-time tool sync before DM action methods are bound", async () => {
    mocks.loadMcpConfig.mockReturnValue({
      mcpServers: {
        demo: { url: "http://localhost:3999/mcp", lifecycle: "eager" },
      },
