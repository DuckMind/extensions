...
describe("bearer token credential store", () => {
  const originalStore = process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
  ...
  afterEach(() => {
    if (originalStore === undefined) delete process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE;
    else process.env.DM_MCP_ADAPTER_TEST_AUTH_STORE = originalStore;
  });
