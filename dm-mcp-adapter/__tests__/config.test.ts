...
describe("config discovery", () => {
  ...
  it("loads package manifest MCP servers below user config without package settings or imports", async () => {
    ...
    const packageRoot = join(home, ".dm", "agent", "npm", "node_modules", "@acme", "tools");
    writeJson(join(home, ".dm", "agent", "settings.json"), { packages: ["npm:@acme/tools@1.0.0"] });
  ...
  it("fails loudly on malformed package settings JSON", async () => {
    ...
    process.env.HOME = home;
    process.chdir(project);
    writeText(join(home, ".dm", "agent", "settings.json"), "{ not json");

    const { loadMcpConfig } = await import("../config.ts");
    expect(() => loadMcpConfig()).toThrow("user DM settings");
  });
  ...
  it("fails loudly on malformed package MCP JSON", async () => {
    ...
    process.chdir(project);
    const packageRoot = join(home, ".dm", "agent", "npm", "node_modules", "acme-tools");
    writeJson(join(home, ".dm", "agent", "settings.json"), { packages: ["npm:acme-tools"] });
    writeJson(join(packageRoot, "package.json"), { name: "acme-tools", pi: { mcp: "./mcp.json" } });
    writeText(join(packageRoot, "mcp.json"), "{ not json");

    const { loadMcpConfig } = await import("../config.ts");
    expect(() => loadMcpConfig()).toThrow("DM package acme-tools MCP config");
  });
  ...
  it("does not load a package source that escapes DM's managed git directory", async () => {
    ...
    const escapedPackage = join(home, ".dm", "outside-package");
    writeJson(join(home, ".dm", "agent", "settings.json"), { packages: ["git:../../outside-package"] });
  ...
  it("loads raw HTTPS and SSH package sources from DM's managed git directory", async () => {
    ...
    const gitRoot = join(home, ".dm", "agent", "git", "github.com", "acme");
    writeJson(join(home, ".dm", "agent", "settings.json"), {
  ...
  it("gives project package settings precedence over matching user package settings", async () => {
    ...
    const globalPackage = join(home, ".dm", "agent", "global-package");
    const projectPackage = join(project, ".dm", "project-package");
    writeJson(join(home, ".dm", "agent", "settings.json"), { packages: ["./global-package"] });
    writeJson(join(project, ".dm", "settings.json"), { packages: ["./project-package"] });
  ...
  it("loads Agent Plugin MCP servers from configured plugin paths", async () => {
    ...
    const pluginDataDir = join(home, ".dm", "agent", "agent-plugin-data", "acme.tools");
  ...
  it("loads standard MCP files first, then DM overrides", async () => {
    const home = mkdtempSync(join(tmpdir(), "pi-mcp-config-home-"));
    const project = mkdtempSync(join(tmpdir(), "pi-mcp-config-project-"));
    process.env.HOME = home;
    process.chdir(project);
    ...
    writeJson(join(home, ".dm", "agent", "mcp.json"), {
    ...
    writeJson(join(project, ".dm", "mcp.json"), {
      settings: { autoAuth: true, oauthDir: ".dm/oauth", showStatusIcon: false },
      mcpServers: {
  ...
  it("loads the branded project DM override path", async () => {
  ...
  it("loads tool-agnostic .agents global MCP files before DM overrides", async () => {
  ...
  it("loads JSONC MCP config files with comments and trailing commas", async () => {
    ...
    mkdirSync(join(home, ".dm", "agent"), { recursive: true });
    writeFileSync(
      join(home, ".dm", "agent", "mcp.json"),
      `{
  ...
  it("updates project DM overrides that were hand-edited as JSONC", async () => {
    ...
    mkdirSync(join(project, ".dm"), { recursive: true });
    writeFileSync(
      join(project, ".dm", "mcp.json"),
      `{
    ...
    expect(writeProjectServerDisabledOverride(undefined, project, "server", true)).toEqual({
      path: join(project, ".dm", "mcp.json"),
      changed: true,
    });
    expect(JSON.parse(readFileSync(join(project, ".dm", "mcp.json"), "utf-8"))).toEqual({
      "mcp-servers": {
        server: {
          command: "original",
          disabled: true,
  ...
  it("resolves configured oauthDir against the active project cwd", async () => {
    ...
    expect(resolveConfiguredOAuthDir(".dm/oauth", project)).toBe(resolve(project, ".dm/oauth"));
  ...
  it("keeps host discovery opt-in and reports active sources, precedence, conflicts, and provenance", async () => {
    ...
    expect(getServerProvenance().get("hostOnly")).toEqual({
      path: join(home, ".dm", "agent", "mcp.json"),
      kind: "import",
      importKind: "cursor",
    });
  ...
  it("classifies project DM overrides as DM-owned conflict sources", async () => {
    ...
    const projectPiPath = join(process.cwd(), ".dm", "mcp.json");
  ...
  it("maps Codex HTTP authentication fields to adapter fields", async () => {
    ...
    writeJson(join(home, ".dm", "agent", "mcp.json"), { imports: ["codex"], mcpServers: {} });
    mkdirSync(join(home, ".codex"), { recursive: true });
    writeFileSync(
      join(home, ".codex", "config.toml"),
      [
  ...
  it("merges partial DM overrides into shared and imported server definitions", async () => {
  ...
  // Authorization header; a higher-precedence source (~/.dm/agent/mcp.json)
  ...
  it("tracks provenance so project servers write locally and shared/imported servers write to DM config", async () => {
    ...
    expect(provenance.get("projectPiServer")).toEqual({
      path: resolve(realProject, ".dm", "mcp.json"),
      kind: "project",
      importKind: undefined,
    });
  });
  ...
  it("writes imported/global changes to DM config and project changes to the project file", async () => {
  ...
  it("builds real diff previews for compatibility imports and shared server writes", async () => {
    ...
    expect(importsPreview.path).toContain(".dm/agent/mcp.json");
  ...
  it("imports OpenCode servers from the global V1 config", async () => {
    ...
    process.env.HOME = home;
    process.chdir(project);
    writeJson(join(home, ".dm", "agent", "mcp.json"), { imports: ["opencode"], mcpServers: {} });
