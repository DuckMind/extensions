...
import type { OverlayHandle } from "@duckmind/dm-tui";
...
export async function showStatus(state: McpExtensionState, ctx: ExtensionContext): Promise<void> {
  ...
  if (!state.programmaticConfig) {
    lines.push(
      ...
      "DM-owned files hold compatibility imports and adapter-specific overrides.",
      "",
    );
  }
...
export async function manageBearerToken(
  ...
  if (action === "set") {
    const message = `Cannot store bearer token here: DM extension UI has no masked secret input primitive. Run \`dm-mcp-adapter token set ${safeName}\` in a terminal; it reads the token from stdin only.`;
    if (ui) ui.notify(message, "error");
    return { ok: false, message };
  }
...
function buildSharedConfigNoticeLines(configOverridePath: string | undefined, cwd: string): { lines: string[]; fingerprint: string | null } {
  ...
  return {
    lines: [
      ...
      "Use .mcp.json for project/team config or ~/.config/mcp/mcp.json for all projects. DM only writes compatibility imports and adapter-specific overrides into DM-owned files when needed.",
    ],
