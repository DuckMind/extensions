import type { AgentToolResult, ToolInfo } from "@duckmind/dm-coding-agent";
...
export async function executeCall(
  ...
  if (!serverName || !toolMeta) {
    ...
    if (nativeTool) {
      return {
        content: [{ type: "text" as const, text: `"${toolName}" is a native DM tool. Call ${toolName} directly instead of using mcp({ tool: "${toolName}" }).` }],
