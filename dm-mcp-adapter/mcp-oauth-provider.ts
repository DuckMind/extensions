 ...
 *
 * DM registers as its own product so consent screens identify the running
 * client. The historical `pi` app-name value maps to the same DM display name
 * only to preserve existing local registrations.
 */
function defaultClientName(): string {
  const app = getAppName()
  return app === "dm" || app === "pi" ? "DM Coding Agent" : app
}

/**
 * Client homepage advertised during Dynamic Client Registration.
 *
 * RFC 7591 defines client_uri as the home page *of the client*. DM does
 * not invent a homepage. The field is omitted unless a server
 * configuration supplies `oauth.clientUri`, preventing a technical upstream
 * repository from appearing on a user-facing consent screen.
 */
