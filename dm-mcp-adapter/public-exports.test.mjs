...
async function extractPackedPackage(fixtureRoot) {
  ...
  const packageRoot = path.join(fixtureRoot, "node_modules", "dm-mcp-adapter");
...
test("public metadata, config, and type helpers load in plain Node from node_modules", async () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [
      ...
      "--eval",
      [
        'const metadata = await import("dm-mcp-adapter/metadata-cache");',
        'const config = await import("dm-mcp-adapter/config");',
        'const types = await import("dm-mcp-adapter/types");',
...
test("token CLI avoids package-local TypeScript imports under node_modules", async () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [
      ...
      "--eval",
      [
        'const { main } = await import("./node_modules/dm-mcp-adapter/cli.js");',
      ...
      cwd: fixtureRoot,
      env: { ...process.env, DM_MCP_ADAPTER_TEST_AUTH_STORE: "memory" },
      encoding: "utf8"
    });
    assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
  } finally {
