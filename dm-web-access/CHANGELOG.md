...
- Kept an existing legacy `~/.dm/web-search.json` in use when `XDG_CONFIG_HOME` is set but its XDG config file is absent. Thanks to [@hu3rror](https://github.com/hu3rror) for issue #333.
...
- GitHub repository fetching is safer when more than one DM process is running.
...
- Kept Defuddle selector-processing failures out of the DM console and stopped treating the raw page body as a successful extraction. Thanks to [@riabiy](https://github.com/riabiy) for #315.
...
- Kimi Code Plan users can run explicit Kimi web searches through DM's `kimi-coding` login.
- HTML extraction, missing-page guidance, and stored-content lookup are more reliable.

### Added
- Added an optional `proxy` parameter to `web_search`, `source_check`, and `fetch_content`. When set, search APIs, page fetches, and content extraction use `curl` through that proxy. Localhost and `NO_PROXY` hosts still bypass the proxy. A default proxy can also be set with `"proxy"` in `~/.dm/web-search.json`. Thanks to [@mystery4f](https://github.com/mystery4f) for PR #307.
...
- Added explicit-only Kimi Code Plan search using DM's refreshed `kimi-coding` OAuth credentials. Thanks to [@lushangkan](https://github.com/lushangkan) for PR #275.
...
### Highlights
- `auto` search now uses Codex-backed OpenAI search when DM is running a Codex model.
...
### Changed
- Prefer Codex-backed OpenAI search in `auto` mode when the active DM model is `openai-codex`; otherwise prefer Exa before OpenAI.
...
- Added optional `openaiSearchProviders` config to choose which DM model providers fund OpenAI `web_search`, in priority order. Thanks to [@hank-warren](https://github.com/hank-warren) for PR #276.
...
- Made nested summary and rewrite model calls follow DM's model registry routing.
...
### Fixed
- Route nested model calls through DM's model registry. Thanks to [@rany2](https://github.com/rany2) for #263.
...
### Fixed
- Added a tracked npm lockfile for reproducible contributor installs, and updated model-registry auth header typing for current DM peer packages. Thanks `@dougEfresh` for PR #218.
...
- Added explicit-only xAI/Grok search with `xaiApiKey` / `XAI_API_KEY`, DM model-registry auth, and optional `xaiSearchModel`. Thanks `@join3r` for PR #196.
...
### Fixed
- Fixed the `fetch_content` call header showing `fetch (no URL)` when DM supplied `url` together with an empty `urls` array. Thanks `@Vergil824` for issue #192.
...
### Fixed
- Removed the unsupported JSON Schema `uniqueItems` keyword from provider-array tool schemas so Gemini-compatible tool validators can register dm-web-access tools. Thanks `@akmaldira` for PR #191.
...
- Exa MCP rate-limit responses (429) now explain that adding `exaApiKey` to `~/.dm/web-search.json` removes the free-tier limit, instead of surfacing a bare status code. Thanks `@kesku` for PR #187.
...
- Resolve the OpenAI search model from DM’s model registry, with an `openaiSearchModel` override and preserved API-key fallback for partial registries. Thanks `@ahalekelly` for PR #182.
...
- Declared `typebox` as a regular runtime dependency so clean/global installs can load the extension without relying on DM or Feynman to provide that peer. Thanks @DuskyElf, @tonytziorvas, and @53able for reporting #59, #63, and #66.
...
- Registered the web activity widget with DM's supported string-array API to prevent `content is not a function` crashes when toggling non-default shortcuts. Thanks @llllllllqq for reporting #158.
- Registered the web activity widget as a DM component factory instead of passing a `Text` instance directly. Thanks Trey Hoover (@treyhoover) for PR #132.
...
- Resolved `web-search.json` from `DM_CODING_AGENT_DIR`, then `XDG_CONFIG_HOME/pi`, before falling back to `~/.dm`.
...
- Limited curator summary model selection to DM `enabledModels` when configured, and fall back to a deterministic no-billing summary instead of silently calling unrelated catalog models. Thanks @Horace1423 for reporting #73.
...
### Changed
- Updated extension imports for DM 0.80.x, including the `@earendil-works` package namespace, `.ts` local module specifiers, and the `@duckmind/dm-ai/compat` entrypoint for legacy model helpers.
- Declared DM-bundled packages as peer dependencies so DM can provide its runtime aliases when loading the extension.

### Fixed
- Aligned custom message display values and widget clearing with the current DM extension API types.
...
### Added
- Added `summaryModel` config for choosing the default curator summary draft model from `~/.dm/web-search.json`.

### Fixed
- Made Gemini Web browser-cookie access opt-in via `allowBrowserCookies` or `DM_ALLOW_BROWSER_COOKIES=1`, preventing surprise macOS Keychain prompts during provider checks.
...
- Migrated extension tool schemas from `@sinclair/typebox` to DM's bundled `typebox` 1.x import path.

## [0.10.6] - 2026-04-04

### Changed
- Added `promptSnippet` metadata for `web_search`, `code_search`, `fetch_content`, and `get_search_content` so DM 0.59+ includes these tools in the default prompt tool section and improves discoverability of research/fetch flows.

## [0.10.5] - 2026-04-03

### Fixed
- Forward dynamic request `headers` from `ctx.modelRegistry.getApiKeyAndHeaders()` into `complete()` for query rewriting and summary generation, finishing the dm 0.63+ auth migration for providers that require per-request headers.
...
- **`/curator` command.** Toggle or configure the curator workflow at runtime: `/curator on`, `/curator off`, `/curator summary-review`, or `/curator` to toggle. Persists to `~/.dm/web-search.json` and takes effect on the next `web_search` call.
- **Config-based workflow default.** Added `workflow` field to `~/.dm/web-search.json` for persistent curator preference. Per-call `workflow` parameter on `web_search` takes priority, then config, then the built-in default (`summary-review` with UI, `none` without).
...
- Adapted model registry calls to dm SDK changes: `getApiKey()` → `getApiKeyAndHeaders()` in `index.ts` and `summary-review.ts`, and `getAvailable()` from async to sync.
...
- Fixed error-signal downgrade in local-video API fallback (`video-extract.ts`): `tryVideoGeminiApi` now rethrows config parse failures (`Failed to parse ~/.dm/web-search.json`) instead of swallowing them as `null`, preserving actionable root-cause errors.
- Fixed provider config type hardening in search routing (`index.ts`): `normalizeProviderInput` now guards non-string config values before trimming so malformed `provider` entries in `~/.dm/web-search.json` no longer crash runtime provider resolution with `value.trim is not a function`.
...
- Updated curator session timeout defaults in `index.ts`: curator now starts at 20 seconds by default (down from 60) and can be configured via `curatorTimeoutSeconds` in `~/.dm/web-search.json` (capped at 600 seconds).
- Hardened `/websearch` startup error handling in `index.ts`: config/provider bootstrap now runs behind explicit error handling so malformed `~/.dm/web-search.json` no longer throws uncaught command errors before the existing server-start try/catch; users now receive a direct UI error with parse context.
- Hardened extension bootstrap config handling in `index.ts`: shortcut initialization now uses guarded config loading, logging parse errors and falling back to default shortcuts instead of crashing extension registration on malformed `~/.dm/web-search.json`.
...
- **Auto-condense pipeline.** When the countdown expires without manual curation, a single LLM call (Claude Haiku by default) condenses all search results into a deduplicated briefing organized by topic. Preprocessing enriches the prompt with URL overlap, answer similarity, and source quality analysis. Configure via `"autoFilter"` in `~/.dm/web-search.json`. Full uncondensed results stored and retrievable via `get_search_content`.
- **Configurable keyboard shortcuts.** Both shortcuts (curate: Ctrl+Shift+S, activity monitor: Ctrl+Shift+W) can be remapped via `"shortcuts"` in `~/.dm/web-search.json`. Changes take effect on restart.
- **`/websearch` command** — opens the curator directly from dm without an agent round-trip. Accepts optional comma-separated queries or opens empty.
...
- **Provider selection** — global dropdown in the curator UI to switch between Perplexity and Gemini. Persists to `~/.dm/web-search.json`.
...
- `search-filter.ts` — auto-condense pipeline: preprocessing, LLM condensation via dm's model registry, and post-processing (citation verification, source list completion).
...
- `package.json` now includes `repository`, `homepage`, `bugs`, and `description` fields (repo link was missing from dm packages site)
...
- `searchProvider` config option in `~/.dm/web-search.json` for global provider default
...
- YouTube video understanding in `fetch_content` via three-tier fallback chain:
  ...
  - **Gemini API** (secondary): direct REST calls with `GEMINI_API_KEY`. YouTube URLs passed as `file_data.file_uri`. Configure via `GEMINI_API_KEY` env var or `geminiApiKey` in `~/.dm/web-search.json`.
...
- Configurable via `~/.dm/web-search.json` under `youtube` key (`enabled`, `preferredModel`)
...
- Configurable via `~/.dm/web-search.json` under `githubClone` key (enabled, maxRepoSizeMB, cloneTimeoutSeconds, clonePath)
...
### Fixed
- Adapt execute signatures to dm v0.51.0: reorder signal, onUpdate, ctx parameters across all three tools
...
### Changed
- Added `dm` manifest to package.json for dm v0.50.0 package system compliance
...
Initial release. Designed for dm v0.37.3.
...
- Config file support (`~/.dm/web-search.json`)
