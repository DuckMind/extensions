...
async function createHome(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-valyu-serper-"));
...
function runChild(script, env = {}) {
	...
	for (const key of [
		"DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "VALYU_API_KEY", "SERPER_API_KEY", "OPENAI_API_KEY", "BRAVE_API_KEY",
...
test("Serper maps organic results and reads its environment credential", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, SERPER_API_KEY: "serper-test-key" });
	assert.equal(child.status, 0, child.stderr);
	const output = JSON.parse(child.stdout.trim());
...
test("Valyu and Serper can be configured for routing but provider all never calls them", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, VALYU_API_KEY: "valyu-key", SERPER_API_KEY: "serper-key" });
	assert.equal(child.status, 0, child.stderr);
	const output = JSON.parse(child.stdout.trim());
