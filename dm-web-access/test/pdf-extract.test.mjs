...
test("extractPDFToMarkdown uses Gemini before loading unpdf", () => {
	const loaderDir = mkdtempSync(
		join(tmpdir(), "dm-web-access-pdf-gemini-first-"),
	);
...
test("extractPDFToMarkdown passes PDF.js errors-only verbosity", () => {
	const loaderDir = mkdtempSync(join(tmpdir(), "dm-web-access-pdf-loader-"));
...
function buildUnpdfLoader() {
    ...
    export function resolve(specifier, context, nextResolve) {
      if (specifier === "unpdf") {
        return { url: "dm-web-access:test-unpdf", shortCircuit: true };
      }
      return nextResolve(specifier, context);
    }

    export async function load(url, context, nextLoad) {
      if (url === "dm-web-access:test-unpdf") {
        return {
          format: "module",
          shortCircuit: true,
          source:
...
function buildChildScript(
	...
	return `
        ...
        const configDir = await mkdtemp(join(tmpdir(), "dm-web-access-pdf-config-"));
        process.env.DM_CODING_AGENT_DIR = configDir;
        ...
        const outputDir = await mkdtemp(join(tmpdir(), "dm-web-access-pdf-"));
