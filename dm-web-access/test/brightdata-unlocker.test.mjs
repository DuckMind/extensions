...
// as an explicit `$VAR`/`!command` source in the real `~/.dm/web-search.json`
...
const EMPTY_HOME = mkdtempSync(
	join(tmpdir(), "dm-web-access-brightdata-empty-home-"),
);
...
test("Bright Data Web Unlocker is not reached for a blocked target through fetch_content", async () => {
	...
	`,
		{ HOME: home, USERPROFILE: home, DM_CODING_AGENT_DIR: home },
	);
	assert.equal(child.status, 0, child.stderr);
	const output = JSON.parse(child.stdout.trim());
...
test("BRIGHTDATA_UNLOCKER_ZONE takes precedence over brightdataUnlockerZone in the config file", async () => {
	...
	for (const { env, expected } of [
		...
		`,
			{ HOME: home, USERPROFILE: home, DM_CODING_AGENT_DIR: home, ...env },
		);
		assert.equal(child.status, 0, child.stderr);
		assert.equal(
...
test("Bright Data Web Unlocker rejects a malformed zone and a malformed config root", async () => {
	...
	`,
		{ HOME: badZone, USERPROFILE: badZone, DM_CODING_AGENT_DIR: badZone },
	);
	...
	const badRoot = await mkdtemp(join(tmpdir(), "dm-web-access-brightdata-"));
	...
	const badRootChild = runChild(
		...
		console.log(JSON.stringify({ message }));
	`,
		{ HOME: badRoot, USERPROFILE: badRoot, DM_CODING_AGENT_DIR: badRoot },
	);
