...
async function createHome(config = {}) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-jina-search-"));
	await mkdir(join(home, ".dm"), { recursive: true });
	await writeFile(join(home, ".dm", "web-search.json"), JSON.stringify(config) + "\n", "utf8");
	return home;
}
...
test("Jina Search resolves command credentials only when a request starts", async () => {
	const markerDir = await mkdtemp(join(tmpdir(), "dm-web-access-jina-marker-"));
...
test("explicit Jina search maps filters, full content, unique results, and truthful headers", async () => {
	...
	assert.equal(output.captured.headers["user-agent"], "dm-web-access");
...
test("configured routing treats Jina's own timeout as transient rather than caller cancellation", async () => {
	...
	`, {
		HOME: home,
		USERPROFILE: home,
		DM_CODING_AGENT_DIR: join(home, ".dm"),
...
test("configured searchRouting can select Jina Search", async () => {
	...
	`, { HOME: home, USERPROFILE: home, DM_CODING_AGENT_DIR: join(home, ".dm"), JINA_API_KEY: "synthetic-jina-test-key" });
...
test('provider "all" includes configured Jina Search alongside zero-config Exa', async () => {
	...
	`, { HOME: home, USERPROFILE: home, DM_CODING_AGENT_DIR: home, JINA_API_KEY: "synthetic-jina-test-key" });
