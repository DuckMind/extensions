...
async function createHome(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-parallel-mcp-"));
...
function runChild(script, env = {}) {
	...
	for (const key of ["DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "PARALLEL_API_KEY", "OPENAI_API_KEY", "BRAVE_API_KEY", "EXA_API_KEY", "GEMINI_API_KEY", "SEARXNG_BASE_URL"]) delete childEnv[key];
...
test("Parallel MCP web_fetch is available only through explicit hosted fetch routing", async () => {
	...
	`, { DM_CODING_AGENT_DIR: enabledHome });
	...
	`, { DM_CODING_AGENT_DIR: disabledHome });
