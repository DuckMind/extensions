...
async function runScenario(maxInlineContentChars) {
	const agentDir = await mkdtemp(join(tmpdir(), "dm-web-access-inline-content-"));
	...
	const child = spawnSync(process.execPath, ["--input-type=module"], {
		...
		`,
		encoding: "utf8",
		env: { ...process.env, DM_CODING_AGENT_DIR: agentDir, XDG_CONFIG_HOME: undefined },
	});
...
test("stored content schema and execution keep one registered limit", async () => {
	...
	const child = spawnSync(process.execPath, ["--input-type=module"], {
		input: `
			...
			const tools = [];
			initializeExtension({ registerTool(tool) { tools.push(tool); }, registerCommand() {}, registerShortcut() {}, on() {}, appendEntry() {} });
			await writeFile(join(process.env.DM_CODING_AGENT_DIR, "web-search.json"), JSON.stringify({ maxInlineContentChars: 20_000 }) + "\\n", "utf8");
