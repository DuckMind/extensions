...
// Every child starts from a config-less HOME. Deleting DM_CODING_AGENT_DIR and
// XDG_CONFIG_HOME is not enough: getWebSearchConfigDir() then falls through to
// join(homedir(), ".dm"), i.e. the developer's real ~/.dm/web-search.json — which is
...
// `npm test`. Tests that need a config point DM_CODING_AGENT_DIR at a temp dir; the
// two variables below only decide where "no config at all" resolves to.
const EMPTY_HOME = mkdtempSync(join(tmpdir(), "dm-web-access-brightdata-empty-"));

function runChild(script, env = {}) {
	const childEnv = { ...process.env, HOME: EMPTY_HOME, USERPROFILE: EMPTY_HOME };
	for (const key of [
		"DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "BRIGHTDATA_API_KEY", "BRIGHTDATA_SERP_ZONE",
...
async function createRawHome(text) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-brightdata-"));
...
function serpBody() {
	return JSON.stringify({
		organic: [
			{ link: "https://github.com/nicobailon/dm-web-access", title: "Repo", description: "repo   description" },
...
test("the SERP request carries the configured zone, the parsed_light format, and brd_json=1", async () => {
	...
	assert.deepEqual(output.result.results, [
		{ title: "Repo", url: "https://github.com/nicobailon/dm-web-access", snippet: "repo description" },
	...
	assert.match(output.result.answer, /repo description\nSource: Repo \(https:\/\/github\.com\/nicobailon\/dm-web-access\)/);
});
...
test("domain filters are sent to the engine as site: operators and re-applied locally", async () => {
	...
	assert.deepEqual(output.urls, ["https://github.com/nicobailon/dm-web-access"]);
	assert.deepEqual(output.multiUrls, [
		"https://github.com/nicobailon/dm-web-access",
...
test("an invalid zone makes Bright Data unavailable, never fatal for the other providers", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, BRIGHTDATA_SERP_ZONE: "pi serp", BRAVE_API_KEY: "brave-key" });
...
test("an Unlocker zone is never substituted for a missing SERP zone", async () => {
	...
	const child = runChild(`
		...
		try { await searchWithBrightData("q"); } catch (err) { error = err.message; }
		console.log(JSON.stringify({ available, error, fetchCalls }));
	`, { DM_CODING_AGENT_DIR: home, BRIGHTDATA_UNLOCKER_ZONE: "pi_unlocker_env" });
...
test("an HTTP error surfaces the status and never leaks the key", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, BRIGHTDATA_SERP_ZONE: "pi_serp" });
...
test("a quoted upstream body cannot forge our HTTP status for routing", async () => {
	...
	for (const body of [
		...
		`, { DM_CODING_AGENT_DIR: home, BRAVE_API_KEY: "brave-key" });
...
test("the key uses the shared credential-source path, resolved at request time", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-brightdata-cred-"));
	...
	`, { HOME: home, USERPROFILE: home, DM_CODING_AGENT_DIR: agentDir });
...
test("explicit brightdata routing reaches the provider and is attributed", async () => {
	...
	assert.deepEqual(JSON.parse(child.stdout.trim()), {
		...
		first: "https://github.com/nicobailon/dm-web-access",
	});
});
...
test("BRIGHTDATA_SERP_ZONE takes precedence over brightdataSerpZone, valid or not", async () => {
	...
	const wins = runChild(probeScript, { DM_CODING_AGENT_DIR: home, BRIGHTDATA_SERP_ZONE: "env_zone" });
	...
	const malformed = runChild(probeScript, { DM_CODING_AGENT_DIR: home, BRIGHTDATA_SERP_ZONE: "not a zone" });
	...
	const fromConfig = runChild(probeScript, { DM_CODING_AGENT_DIR: home });
...
test("an unparseable config file throws without echoing its contents", async () => {
	...
	`, { DM_CODING_AGENT_DIR: positional });
