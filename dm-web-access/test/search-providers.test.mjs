...
test("Brave search applies domain filters in the query and returned results", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-brave-"));
	const child = runChild(`
		...
		globalThis.fetch = async (url, init) => {
			...
			return new Response(JSON.stringify({
				web: { results: [
					{ title: "GitHub", url: "https://github.com/nicobailon/dm-web-access", description: "repo" },
	...
	assert.deepEqual(output.results.map((result) => result.url), ["https://github.com/nicobailon/dm-web-access"]);
});

test("Perplexity normalizes invalid result counts", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-perplexity-count-"));
	const child = runChild(`
...
test("Tavily search uses bearer auth and maps filters/content", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-tavily-"));
	const child = runChild(`
		let capturedUrl = "";
		let capturedHeaders = null;
...
test("Brave, keyed Exa, and Tavily honor base URL overrides without leaking credentials across origins", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-provider-base-url-"));
...
test("SearXNG search is SSRF-guarded and preferred first when configured", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-"));
	const child = runChild(`
...
test("SearXNG search merges configured custom headers", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-headers-"));
	const child = runChild(`
		const { writeFileSync } = await import("node:fs");
		writeFileSync(${JSON.stringify(home)} + "/web-search.json", JSON.stringify({
			searxngBaseUrl: "http://127.0.0.1:8443/",
	...
	const overrideHome = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-headers-override-"));
	...
	`, {
		...
		DM_CODING_AGENT_DIR: overrideHome,
	});
...
test("SearXNG search strips configured headers on cross-origin redirects", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-headers-redirect-"));
	const child = runChild(`
		const { writeFileSync } = await import("node:fs");
		writeFileSync(${JSON.stringify(home)} + "/web-search.json", JSON.stringify({
			searxngBaseUrl: "http://127.0.0.1:8443/",
...
test("SearXNG redirect validation rejects an unapproved private target", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-redirect-"));
	const child = runChild(`
		const { writeFileSync } = await import("node:fs");
		writeFileSync(${JSON.stringify(home)} + "/web-search.json", JSON.stringify({
			searxngBaseUrl: "http://127.0.0.1:8443/",
...
test("malformed SearXNG SSRF config fails before hosted auto fallback", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-searxng-config-"));
	const child = runChild(`
		const { writeFileSync } = await import("node:fs");
		writeFileSync(${JSON.stringify(home)} + "/web-search.json", JSON.stringify({
...
test("auto provider falls through to Tavily after unavailable earlier providers", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-tavily-auto-"));
	const child = runChild(`
		const calls = [];
		globalThis.fetch = async (url, init = {}) => {
...
test("Exa direct API key ignores full legacy usage counter", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-paid-"));
	const child = runChild(`
	...
	assert.equal(output.integration, "dm-web-access");
...
test("Exa command source is lazy, overrides stale env, and rotates per request", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-command-"));
...
test("failed Exa command source is redacted and blocks MCP or provider fallback", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-command-failure-"));
...
test("Exa provider errors redact the resolved credential", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-redaction-"));
...
test("keyless Exa search sends filters to the advanced MCP tool as parameters", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-mcp-advanced-"));
	const child = runChild(`
		let captured = null;
		globalThis.fetch = async (url, init) => {
...
test("keyless Exa search falls back to the default MCP tool when the advanced tool is missing", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-exa-mcp-fallback-"));
	const child = runChild(`
		const tools = [];
		globalThis.fetch = async (url, init) => {
			const target = String(url);
...
test("failed Gemini command source is redacted and blocks browser fallback", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-command-failure-"));
	const commandPath = join(home, "fail-key.sh");
...
test("OpenAI search requires web_search and maps domain filters", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-"));
	const child = runChild(`
		let capturedUrl = "";
		let capturedHeaders = null;
		let capturedBody = null;
...
test("OpenAI search uses configured Responses endpoint", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-url-"));
	await writeFile(join(home, "web-search.json"), JSON.stringify({
...
test("auto search prefers Codex-backed OpenAI search when the selected model is openai-codex", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-auto-codex-selected-"));
	const child = runChild(`
		let capturedUrl = "";
...
test("auto search uses Exa before OpenAI when the selected model is not openai-codex", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-auto-non-codex-selected-"));
	const child = runChild(`
		let capturedUrl = "";
		globalThis.fetch = async (url) => {
			capturedUrl = String(url);
...
test("OpenAI search falls back to API key when model registry cannot enumerate", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-partial-registry-"));
	const child = runChild(`
		let capturedBody = null;
		globalThis.fetch = async (url, init) => {
			capturedBody = JSON.parse(init.body);
...
test("OpenAI search uses configured model with selected registry auth", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-model-"));
	await writeFile(join(home, "web-search.json"), JSON.stringify({
...
test("OpenAI search honors configured provider priority", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-providers-"));
	await writeFile(join(home, "web-search.json"), JSON.stringify({
...
test("OpenAI search rejects invalid openaiSearchProviders", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-openai-providers-invalid-"));
	await writeFile(join(home, "web-search.json"), JSON.stringify({
...
test("Gemini API search uses its search-only default model", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-default-"));
	const child = runChild(`
		let capturedUrl = "";
		let capturedBody = null;
		let capturedHeaders = null;
...
test("Gemini API search preserves the configured searchModel", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-configured-"));
	const child = runChild(`
		const { writeFile } = await import("node:fs/promises");
		await writeFile(process.env.DM_CODING_AGENT_DIR + "/web-search.json", JSON.stringify({ searchModel: "custom-gemini-model" }));
