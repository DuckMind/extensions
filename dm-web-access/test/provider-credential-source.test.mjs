...
async function createHome(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-credential-source-"));
...
test("previously unsupported providers resolve explicit env and command sources lazily", async () => {
	const tavilyMarker = join(await mkdtemp(join(tmpdir(), "dm-web-access-credential-marker-")), "tavily-ran");
	const openaiMarker = join(await mkdtemp(join(tmpdir(), "dm-web-access-credential-marker-")), "openai-ran");
...
test("Cloudflare gateway response redacts the credential used by the request", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-cloudflare-redaction-"));
