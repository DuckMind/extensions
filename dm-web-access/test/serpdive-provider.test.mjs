...
function runChild(script, env) {
	...
	for (const key of ["DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "SERPDIVE_API_KEY", "KAGI_API_KEY", "OLLAMA_API_KEY", "SERPBASE_API_KEY", "SERPDIVE_MODEL"]) {
		delete childEnv[key];
	}
	...
	return `
		...
		globalThis.fetch = async (url, init) => {
			...
			return new Response(JSON.stringify({
				...
				results: [
					{ url: "https://github.com/nicobailon/dm-web-access", title: "Repo", content: "repo content" },
...
test("le modèle par défaut est krill, et aucune synthèse n'est demandée", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-serpdive-"));
...
test("le filtrage par domaine est appliqué côté client, sur ce qui revient", async () => {
	...
	assert.deepEqual(output.inline, ["https://github.com/nicobailon/dm-web-access"]);
});
...
test("la clé passe par le chemin de credentials partagé, résolue à la requête", async () => {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-serpdive-cred-"));
