...
test("Kimi accepts DM OAuth returned only in the Authorization header", async () => {
...
test("explicit kimi routing dispatches through the Kimi Code search endpoint", async () => {
	const agentDir = await mkdtemp(join(tmpdir(), "dm-web-access-kimi-routing-"));
...
test("configured routing falls back when Kimi hits its provider timeout", async () => {
	const agentDir = await mkdtemp(join(tmpdir(), "dm-web-access-kimi-timeout-routing-"));
	...
	const child = spawnSync(process.execPath, ["--input-type=module"], {
		...
		`,
		encoding: "utf8",
		env: { ...process.env, DM_CODING_AGENT_DIR: agentDir, BRAVE_API_KEY: "brave-routing-key" },
...
test("configured routing does not fall back when the caller cancels Kimi", async () => {
	const agentDir = await mkdtemp(join(tmpdir(), "dm-web-access-kimi-cancel-routing-"));
	await writeFile(join(agentDir, "web-search.json"), JSON.stringify({
		braveApiKey: "brave-routing-key",
		searchRouting: { providers: ["kimi", "brave"], fallbackOn: ["network"] },
	}) + "\n", "utf8");
