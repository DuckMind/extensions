...
function runChild(script, env) {
	...
	for (const key of ["DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "GEMINI_API_KEY", "GOOGLE_GEMINI_BASE_URL", "CLOUDFLARE_API_KEY"]) {
		delete childEnv[key];
	}
...
test("Gemini generate, upload, status, and delete use only shared header auth", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-transport-"));
...
test("Gemini video upload, status, and delete respect a configured geminiBaseUrl relay", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-relay-"));
	const mediaPath = join(root, "synthetic.webm");
	await writeFile(mediaPath, "synthetic media", "utf8");
