...
function runChild(script, env = {}) {
	...
	for (const key of ["DM_CODING_AGENT_DIR", "TAVILY_API_KEY"]) delete childEnv[key];
...
async function createConfig(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-duckduckgo-"));
...
test("DuckDuckGo invalid HTML fails directly and falls through configured invalid-response routing", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, TAVILY_API_KEY: "tavily-test-key" });
	assert.equal(child.status, 0, child.stderr);
	const output = JSON.parse(child.stdout.trim());
