...
function runRegistrationWithConfig(configText) {
	const root = mkdtempSync(join(tmpdir(), "dm-web-access-tool-names-"));
	...
	return spawnSync(process.execPath, ["--input-type=module"], {
		...
		`,
		encoding: "utf8",
		env: { ...process.env, DM_CODING_AGENT_DIR: root, XDG_CONFIG_HOME: "", HOME: join(root, "home"), USERPROFILE: join(root, "home") },
	});
}
...
test("README documents registration gates and toolNames", () => {
	...
	assert.match(readmeSrc, /DM restart is required for tool and command registration changes/);
