...
async function createHome(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-xai-"));
...
function runChild(script, env) {
	...
	for (const key of [
		"DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "XAI_API_KEY", "ANYSEARCH_API_KEY", "OPENAI_API_KEY", "BRAVE_API_KEY",
...
test("explicit xai provider routing works", async () => {
	...
	`, { DM_CODING_AGENT_DIR: home, XAI_API_KEY: "xai-env-key" });
	assert.equal(child.status, 0, child.stderr);
