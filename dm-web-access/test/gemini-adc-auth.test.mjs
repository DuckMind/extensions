...
test("Gemini search uses Vertex AI + ADC bearer auth when geminiAuth is adc", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-"));
	await writeAdc(root);
	await writeFile(
	...
	`, { HOME: root, USERPROFILE: root, DM_CODING_AGENT_DIR: root, GOOGLE_APPLICATION_CREDENTIALS: join(root, "adc.json") });
...
test("Gemini ADC availability requires geminiAuth adc, project, location, and an ADC file", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-avail-"));
...
test("Gemini ADC config parse errors stay visible", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-config-"));
...
test("Gemini ADC credential parse errors include the file path", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-file-"));
	...
	const child = runChild(`
		...
		console.log(JSON.stringify({ error }));
	`, { HOME: root, USERPROFILE: root, DM_CODING_AGENT_DIR: root, GOOGLE_APPLICATION_CREDENTIALS: adcPath });
...
test("Gemini ADC credential shape errors include the file path", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-shape-"));
	const adcPath = join(root, "adc.json");
...
test("Gemini ADC rejects malformed token success responses", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-token-shape-"));
	await writeAdc(root);
	const env = { HOME: root, USERPROFILE: root, DM_CODING_AGENT_DIR: root, GOOGLE_APPLICATION_CREDENTIALS: join(root, "adc.json") };
...
test("Gemini ADC never leaks the access token in errors", async () => {	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-redact-"));
	await writeAdc(root);
	await writeFile(
		join(root, "web-search.json"),
...
test("Gemini ADC classifies only credential rejections as CredentialResolutionError", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-classify-"));
	await writeAdc(root);
	const env = { HOME: root, USERPROFILE: root, DM_CODING_AGENT_DIR: root, GOOGLE_APPLICATION_CREDENTIALS: join(root, "adc.json") };
...
test("Gemini ADC does not advertise the Files-API (YouTube/video) path", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-adc-video-"));
	await writeAdc(root);
	await writeFile(
		join(root, "web-search.json"),
		JSON.stringify({ geminiAuth: "adc", geminiProject: "p", geminiLocation: "l" }) + "\n",
