...
async function createHome(config) {
	const home = await mkdtemp(join(tmpdir(), "dm-web-access-anysearch-"));
...
function runChild(script, env) {
	...
	for (const key of [
		"DM_CODING_AGENT_DIR", "XDG_CONFIG_HOME", "ANYSEARCH_API_KEY", "OPENAI_API_KEY", "BRAVE_API_KEY",
...
test("AnySearch sends keyed auth and resolves command credentials lazily", async () => {
	const marker = join(await mkdtemp(join(tmpdir(), "dm-web-access-anysearch-marker-")), "ran");
