...
test("web-search config path uses DM_CODING_AGENT_DIR before XDG_CONFIG_HOME", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-config-path-"));
	const agentDir = join(root, "agent-dir");
	const xdgDir = join(root, "xdg");
	const home = join(root, "home");
	await mkdir(agentDir, { recursive: true });
	await mkdir(join(home, ".dm"), { recursive: true });
	...
	await writeFile(join(home, ".dm", "web-search.json"), JSON.stringify({ perplexityApiKey: "pplx-from-legacy" }) + "\n", "utf8");
...
test("web-search config path uses XDG_CONFIG_HOME pi directory when agent dir is unset", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-xdg-config-"));
	const home = join(root, "home");
	const xdgDir = join(root, "xdg");
	await mkdir(join(home, ".dm"), { recursive: true });
	await mkdir(join(xdgDir, "pi"), { recursive: true });
...
test("web-search config path falls back to the existing legacy file when XDG file is absent", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-legacy-config-"));
	const home = join(root, "home");
	const xdgDir = join(root, "xdg");
	await mkdir(join(home, ".dm"), { recursive: true });
	await mkdir(join(xdgDir, "pi"), { recursive: true });
	...
	assert.deepEqual(JSON.parse(child.stdout), {
		dir: join(home, ".dm"),
		path: join(home, ".dm", "web-search.json"),
		available: true,
	});
});

test("web-search config path keeps the legacy fallback stable after XDG config is created", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-stable-config-"));
	const home = join(root, "home");
	const xdgDir = join(root, "xdg");
...
test("web-search config path keeps XDG_CONFIG_HOME as the new-config target when both files are absent", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-xdg-new-config-"));
	const home = join(root, "home");
	const xdgDir = join(root, "xdg");
	await mkdir(join(xdgDir, "pi"), { recursive: true });
...
test("Gemini base URL and Cloudflare auth use env before config", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-base-url-"));
	const agentDir = join(root, "agent-dir");
	await mkdir(agentDir, { recursive: true });
...
test("Gemini command source is lazy, overrides stale env, rotates, and uses header auth", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-command-"));
	const agentDir = join(root, "agent-dir");
...
test("Gemini API helper rejects credential query parameters before fetch", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-query-reject-"));
	const child = runChild(`
		let fetchCalls = 0;
...
test("Gemini provider and transport errors redact the resolved credential", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-redaction-"));
...
test("Gemini API requests include role and gateway auth headers", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-gemini-request-"));
	const child = runChild(`
		let capturedUrl = "";
