...
async function createConfig(config = {
	...
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-provider-precedence-"));
...
function runTool(agentDir, provider) {
	...
	const childEnv = { ...process.env, DM_CODING_AGENT_DIR: agentDir, OPENAI_API_KEY: "openai-test-key" };
...
test("malformed config root fails with an explicit object-shape error", async () => {
	const root = await mkdtemp(join(tmpdir(), "dm-web-access-invalid-config-root-"));
	const agentDir = join(root, "agent-dir");
	await mkdir(agentDir, { recursive: true });
	...
	const child = spawnSync(process.execPath, ["--input-type=module"], {
		...
		`,
		encoding: "utf8",
		env: { ...process.env, DM_CODING_AGENT_DIR: agentDir, OPENAI_API_KEY: "openai-test-key" },
	});
