...
const testAgentDir = await mkdtemp(join(tmpdir(), "dm-web-access-summary-deadline-"));
...
process.env.DM_CODING_AGENT_DIR = testAgentDir;
after(() => {
...
test("enabledModels loading uses trusted project settings over global settings", async () => {
	const agentDir = await mkdtemp(join(tmpdir(), "dm-web-access-agent-"));
	const projectDir = await mkdtemp(join(tmpdir(), "dm-web-access-project-"));
	await writeFile(join(agentDir, "settings.json"), JSON.stringify({ enabledModels: ["global/model"] }));
	await mkdir(join(projectDir, ".dm"));
	await writeFile(join(projectDir, ".dm", "settings.json"), JSON.stringify({ enabledModels: ["project/model"] }));

	const previous = process.env.DM_CODING_AGENT_DIR;
	...
	try {
		...
		if (previous === undefined) {
			...
			process.env.DM_CODING_AGENT_DIR = previous;
		}
	}
});
