import type { ExtensionAPI, ExtensionCommandContext } from "@duckmind/dm-coding-agent";
...
function registerServer(dm: ExtensionAPI, server: LLMServer): void {
  dm.registerProvider(toProviderId(server), {
    ...
    models: server.models.map((m) => ({
      ...
      // conventions, which none of our detectors check. dm-ai defaults both
      ...
      // whatever it defaults to internally, no matter what DM's status bar
      // says. Switching it on hands that decision to DM — which is the point,
      ...
      // level DM offers maps onto something the backend really distinguishes.
      compat: {
...
function unregisterServer(dm: ExtensionAPI, server: LLMServer): void {
...
async function showServerMenu(
  ...
  while (true) {
    ...
    if (picked === OPT_REMOVE) {
      if (await removeServer(dm, ctx, server)) return;
      continue;
    }
  }
}
