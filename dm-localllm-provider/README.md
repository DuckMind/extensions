...
A DM extension for wizard-based setup of local LLM servers — MTPLX, oMLX, LM Studio, llama.cpp, Ollama, vLLM, SGLang, ds4, ninfer, or anything else with an OpenAI-compatible API.

- **One command, one place** — `/localllm` is a single TUI menu for adding, inspecting, and managing every local server's integration with DM — no subcommands, no hand-editing `settings.json`.
...
```bash
dm install npm:dm-localllm-provider
```

Start DM, run `/localllm`, select **＋ Add server**, follow the wizard.

Hacking on the extension itself? Skip npm and point DM straight at your checkout:

```bash
mkdir -p ~/.dm/agent/extensions/dm-localllm-provider
cp -r /path/to/dm-localllm-provider/* ~/.dm/agent/extensions/dm-localllm-provider/
```
...
`[oMLX]` is the detected backend, persisted with the server. `✓`/`○` is server-side memory state, unrelated to which model DM has selected via `/model`.
...
Each server registers as its own DM provider — add as many as you like.

## FAQ

**I added a server, but DM's still talking to the old model — what gives?**
...
**Do I have to redo any of this after restarting DM?**
...
Without that map, three of DM's seven levels would 400. If the server can't be reached or answers nothing, no `compat` is recorded at all — an unanswered probe isn't evidence against a convention, so it degrades to the same safe defaults every other backend uses.
...
The `developer` role works, so it is enabled rather than left at the safe default. Reasoning comes back on a separate `reasoning_content` field, which DM reads without help. And the Chat Completions `usage` object carries only `prompt_tokens`, `completion_tokens` and `total_tokens` — no `completion_tokens_details.reasoning_tokens` — so anything wanting a thinking/answer split has to infer it from the stream.

The whole detection, 600 KB probe body included, takes under 400 ms.

### Thinking levels on ds4

Worth understanding before reading anything into the status bar, because the interesting cases are the ones where DM and the server disagree.

While `supportsReasoningEffort` is off — the default for every backend except ds4 — DM never sends `reasoning_effort` at all, so the server keeps using whatever it defaults to internally and the level shown in DM has no effect on it whatsoever. Turning it on is what connects the two for the first time. That can look like a regression: a server quietly running at its own default now follows the session instead. It isn't one — it's the first time the setting was ever wired up.

ds4 recognises only three modes internally (`think_mode_from_enabled` in `ds4_server.c`) — off, high, and max — so DM's seven levels don't map one-to-one. Two of them would land somewhere surprising without help:

- **`off` would not turn thinking off.** DM converts an `off` level into an *omitted* `reasoning_effort` rather than a value, and ds4 reads an omitted one as "use my defaults", which are thinking-enabled at high. The `[ds4]` detector maps `off` to the literal `"none"` — the only string that genuinely disables it.
- **`max` would be unreachable.** DM hides `xhigh` and `max` unless a model explicitly declares them, so ds4's real max mode wouldn't appear in the level list. The detector declares `max`, but deliberately not `xhigh`: ds4 folds `xhigh` into high, so offering it would imply a distinction that doesn't exist.
...
**If the status bar is stuck on `thinking off`,** that's DM's session state, not a detection problem. DM carries the current thinking level across model switches whenever the new model supports thinking, and `off` is a legal level for a reasoning model, so nothing raises it back. A model that reported `reasoning: false` earlier in the session — which is how ds4 looked before it was detected properly — pins the level to `off`, and it stays there through the next **↺ Refresh**. DM deliberately doesn't persist that particular `off` to settings, so set the level once (`dm --thinking high`, or DM's thinking selector) and it sticks from then on.

## Configuration

Stored under the `localllm` key in `~/.dm/agent/settings.json`:
...
DM resolves each turn's `max_tokens` as `options.maxTokens ?? model.maxTokens`, clamped to the context left after the prompt — so this is a real generation cap, and **reasoning and the answer come out of the same budget**.

That matters because no local backend offers a thinking cap worth relying on. DM can send `thinking_token_budget`, but that is vLLM's field and gated behind a compat flag; SGLang's OpenAI surface accepts `max_thinking_tokens` and then silently ignores it. So a reasoning model that thinks its way to the ceiling spends the *entire* budget reasoning and returns no answer at all — which then reads as a truncated turn, gets fed back with a truncation notice, and re-plans into the same wall. Setting the number too low doesn't prevent that; it just reaches it sooner.
...
`apiKey` accepts any DM-resolvable form:
...
Configured servers also show up in DM's `/login` → **Use an API key**, if you'd rather use `auth.json` instead.
