...
export interface DiscoveredModel {
  ...
  // Rewrites DM's thinking levels into the strings this backend actually
  // accepts. Also gates which levels DM offers at all: pi-ai's
  ...
  // Merged into every request by DM. Only set where a detector knows the
...
//
// This is a real generation cap, not just metadata: DM resolves every turn's
...
// max_thinking_tokens, and DM's thinking_token_budget is vLLM-only. Set this
...
// DM's levels in the same order, so "nearest" can be measured by index.
...
export async function probeRequestCompat(
  ...
  if (accepted.length > 0) {
    ...
    // is already the safe one: false just means DM keeps sending "system".
    ...
    // leave it unmapped, which makes DM omit the field and fall back to
...
// serves when the request names no temperature of its own — as DM's do not.
...
// system is. Turning them on is what makes DM's thinking levels reach the
...
// "minimal" and "xhigh" landing on HIGH. Two of DM's seven levels therefore
...
//     without this mapping, selecting "off" in DM leaves the server thinking
 ...
 * file's generic 32768: registering four times the real ceiling would let DM
...
async function ninferModels(
  ...
  // `reasoning_content` field, which DM reads without further help.
