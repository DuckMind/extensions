...
import { nodeRuntimeCommand } from "../src/browser-helpers/node-runtime.mjs";
import {
	existsSync,
	mkdirSync,
	readdirSync,
...
const SOCKET_SCOPE = createHash("sha256")
	.update(GREEDY_CDP_SOCKET_DIR)
	.digest("hex")
	.slice(0, 12);

function sockPath(targetId) {
	// Windows named pipes and Unix sockets share the same scoped key.
	if (platform() === "win32") return `\\\\.\\pipe\\dm-cdp-${SOCKET_SCOPE}-${targetId}`;
	mkdirSync(GREEDY_CDP_SOCKET_DIR, { recursive: true });
	return join(GREEDY_CDP_SOCKET_DIR, `${targetId}.sock`);
}
...
function getWsUrl() {
		// Prefer DM CUA's dedicated profile, never the user's main Chrome session.
	...
	if (profileDir) {
		...
		throw new Error(
				`DM CUA DevToolsActivePort not found at ${p}. Refusing to fall back to the main Chrome session.`,
		);
	}
...
function listDaemonSockets() {
	...
	try {
		return readdirSync(GREEDY_CDP_SOCKET_DIR)
			.filter((f) => f.endsWith(".sock"))
			.map((f) => ({
				targetId: f.slice(0, -5),
				socketPath: join(GREEDY_CDP_SOCKET_DIR, f),
			}));
	} catch {
		return [];
	}
...
async function getOrStartTabDaemon(targetId) {
	...
	child.stderr?.destroy();
	const diagnostic = (daemonStderr || daemonExit || "no startup diagnostic was emitted")
		.replace(/\s+/g, " ")
		.trim();
	throw new Error(
		`Tab daemon did not become ready for ${targetId.slice(0, MIN_TARGET_PREFIX_LEN)}. ${diagnostic}. ` +
			"Keep the dedicated DM browser open and retry; use browser_cua(handoff) if the site needs your interaction.",
	);
}
